...
set EXO_OPTS="-Djavasrc=%JAVA_HOME%\src.zip" "-Djre.lib=%JAVA_HOME%\jre\lib" "-Dtenant.masterhost=localhost" "-Dorganization.application.server.url=http://localhost:8080/userdb" "-Dorg.exoplatform.mimetypes=conf\mimetypes.properties" "-Dorg.exoplatform.ide.server.fs-root-path=${CATALINA_HOME}/bin/git" "-Dorg.exoplatform.ide.git.server=git" "-Dbuilder.timeout=600"
