...
public class FileSystem implements INameEnvironment, SuffixConstants
{
   ...
   */
   ...
   private NameEnvironmentAnswer findClass(String qualifiedTypeName, char[] typeName, boolean asBinaryOnly)
   {
      ...
      System.out.println("FileSystem.findClass()" + qualifiedTypeName + "    []  " + new String(typeName) + " b : " + asBinaryOnly);
      
            String qualifiedBinaryFileName = qualifiedTypeName + SUFFIX_STRING_class;
            String qualifiedPackageName =
               qualifiedTypeName.length() == typeName.length ? Util.EMPTY_STRING : qualifiedTypeName.substring(0,
                  qualifiedTypeName.length() - typeName.length - 1);
            String qp2 =
               File.separatorChar == '/' ? qualifiedPackageName : qualifiedBinaryFileName.replace('/', File.separatorChar);
            ...
            {
//               String qb2 = 
               qualifiedBinaryFileName.replace('/', File.separatorChar);
//               for (int i = 0, length = this.classpaths.length; i < length; i++)
//               {
