   ...
   RelevanceConstants, SuffixConstants
{
    ...
    */
   public CompletionEngine(DummyNameEnvirement nameEnvironment, CompletionRequestor requestor, Map settings,
   // IJavaProject javaProject,
      IProgressMonitor monitor)
   {
      super(settings);
         ...
         next : for (int i = 0; i <= this.expectedTypesPtr; i++)
         {
            ...
            if (this.expectedTypes[i] instanceof ReferenceBinding)
            {
               ...
               if (!inSameUnit || (inSameUnit && refBinding.isMemberType()))
               {
                  ...
                  if (proposeType
                     && (!this.assistNodeIsConstructor || !allowingLongComputationProposals || hasStaticMemberTypes(
