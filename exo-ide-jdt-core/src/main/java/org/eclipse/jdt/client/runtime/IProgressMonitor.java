...
/**
 * The <code>IProgressMonitor</code> interface is implemented by objects that monitor the progress of an activity; the methods in
 ...
 * </p>
 * <p>
 * A request to cancel an operation can be signaled using the <code>setCanceled</code> method. Operations taking a progress
 * monitor are expected to poll the monitor (using <code>isCanceled</code>) periodically and abort at their earliest convenience.
 * Operation can however choose to ignore cancelation requests.
 * </p>
 * <p>
 * Since notification is synchronous with the activity itself, the listener should provide a fast and robust implementation. If
 * the handling of notifications would involve blocking operations, or operations which might throw uncaught exceptions, the
 * notifications should be queued, and the actual processing deferred (or perhaps delegated to a separate thread).
 * </p>
 * <p>
...
public interface IProgressMonitor
{
    ...
    *           <code>UNKNOWN</code> the implementation is free to indicate progress in a way which doesn't require the total
    *           number of work units in advance.
    */
    ...
    * 
    * @param value <code>true</code> indicates that cancelation has been requested (but not necessarily acknowledged);
   ...
   /**
    * Sets the task name to the given value. This method is used to restore the task label after a nested operation was executed.
