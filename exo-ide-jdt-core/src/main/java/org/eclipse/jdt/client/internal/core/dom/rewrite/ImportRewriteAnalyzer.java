...
public final class ImportRewriteAnalyzer
{
   ...
   public ImportRewriteAnalyzer(IDocument document, CompilationUnit root, String[] importOrder, int threshold,
   ...
   private void addExistingImports(CompilationUnit root)
   {
      ...
      for (int i = 1; i < decls.size(); i++)
      {
         ...
         currPackage.add(new ImportDeclEntry(packName.length(), name, isStatic, new Region(currOffset, nextOffset
            - currOffset), rangeBefore, rangeAfter));
         ...
         if (currEndLine < nextOffsetLine)
         {
            ...
            this.packageEntries.add(currPackage);
            currPackage.add(new ImportDeclEntry(packName.length(), null, false, new Region(currOffset, nextOffset
               - currOffset)));
      ...
      currPackage.add(new ImportDeclEntry(packName.length(), name, isStatic,
         new Region(curr.getStartPosition(), length)));
   }
   ...
   public MultiTextEdit getResultingEdits(IProgressMonitor monitor) throws BadLocationException
   {
      if (monitor == null)
      {
   ...
   private void removeAndInsertNew(IDocument document, int contentOffset, int contentEnd, ArrayList stringsToInsert,
   ...
   private int findInBuffer(IDocument document, String str, int start, int end) throws BadLocationException
   {
      int pos = start;
      int len = str.length();
   ...
   private String[] getNewImportStrings(IDocument document, PackageEntry packageEntry, boolean isStatic,
      String lineDelim) throws BadLocationException
   {
   ...
   private final static class PackageEntry
   {
      ...
      /**
       * @param name Name of the package entry. e.g. org.eclipse.jdt.ui, containing imports like org.eclipse.jdt.ui.JavaUI.
       * @param group The index of the preference order entry assigned different group id's will result in spacers between the
       *           entries
       */
