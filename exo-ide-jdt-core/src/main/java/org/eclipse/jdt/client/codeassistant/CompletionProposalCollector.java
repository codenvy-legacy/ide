...
/**
 * Java UI implementation of <code>CompletionRequestor</code>. Produces {@link IJavaCompletionProposal}s from the proposal
...
public class CompletionProposalCollector extends CompletionRequestor
{
    ...
    * 
    * @param length the new replacement length, relative to the code assist offset. Must be equal to or greater than zero.
    */
    ...
    */
   protected final Image getImage(ImageResource descriptor)
   {
   ...
   protected boolean isFiltered(CompletionProposal proposal)
   {
      ...
      {
         String type = new String(declaringType);
         if(type.contains("new ") && type.contains("(){}"))
            return true;
      }
      // TODO
   ...
   private IJavaCompletionProposal createAnnotationAttributeReferenceProposal(CompletionProposal proposal)
   {
      ...
      ImageResource descriptor = fLabelProvider.createMethodImageDescriptor(proposal);
      String completion = String.valueOf(proposal.getCompletion());
      JavaCompletionProposal javaProposal =
         new JavaCompletionProposal(completion, proposal.getReplaceStart(), getLength(proposal), getImage(descriptor),
            displayString, computeRelevance(proposal), false, fInvocationContext);
      // TODO
   ...
   private IJavaCompletionProposal createKeywordProposal(CompletionProposal proposal)
   {
      ...
      int relevance = computeRelevance(proposal);
      return new JavaCompletionProposal(completion, start, length, null, label, relevance, false, fInvocationContext);
   }
   ...
   private IJavaCompletionProposal createLocalVariableProposal(CompletionProposal proposal)
   {
      ...
      final JavaCompletionProposal javaProposal =
         new JavaCompletionProposal(completion, start, length, image, label, relevance, false, fInvocationContext);
      javaProposal.setTriggerCharacters(VAR_TRIGGER);
      return javaProposal;
   }
   ...
   private IJavaCompletionProposal createPackageProposal(CompletionProposal proposal)
   {
      ...
      return new JavaCompletionProposal(completion, start, length, image, label, relevance, false, fInvocationContext);
   }
