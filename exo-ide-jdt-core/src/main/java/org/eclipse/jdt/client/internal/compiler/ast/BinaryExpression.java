...
public class BinaryExpression extends OperatorExpression
{
   ...
   /*
    * Tracking helpers The following are used to elaborate realistic statistics about binary expressions. This must be neutralized
    * in the released code. Search the keyword BE_INSTRUMENTATION to reenable. An external device must install a suitable probe so
    * as to monitor the emission of events and publish the results. public interface Probe { public void ping(int depth); } public
    * int depthTracker; public static Probe probe;
    */
   ...
   // break;
   // case GREATER_EQUAL :
   // generateOptimizedGreaterThanOrEqual(currentScope, codeStream, null, (falseLabel =
   // new BranchLabel(codeStream)), valueRequired);
   // if (valueRequired)
   // {
   // codeStream.iconst_1();
   // if ((this.bits & ASTNode.IsReturnedValue) != 0)
   ...
   // {
   // int pc = codeStream.position;
   // this.left.generateOptimizedStringConcatenation(blockScope, codeStream, this.left.implicitConversion
   ...
   // {
   // int pc = codeStream.position;
   // this.left.generateOptimizedStringConcatenationCreation(blockScope, codeStream, this.left.implicitConversion
   // & TypeIds.COMPILE_TYPE_MASK);
   // codeStream.recordPositionsFrom(pc, this.left.sourceStart);
   // pc = codeStream.position;
   // this.right.generateOptimizedStringConcatenation(blockScope, codeStream, this.right.implicitConversion
   ...
   /**
    * Separates into a reusable method the subpart of {@link #resolveType(BlockScope)} that needs to be executed while climbing up
    * the chain of expressions of this' leftmost branch. For use by {@link CombinedBinaryExpression#resolveType(BlockScope)}.
    * 
