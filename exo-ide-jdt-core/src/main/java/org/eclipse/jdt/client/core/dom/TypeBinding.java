...
class TypeBinding implements ITypeBinding {
	...
	org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding binding;
	private String key;
	...
	public TypeBinding(BindingResolver resolver, org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding binding) {
	...
	public IAnnotationBinding[] getAnnotations() {
		...
		org.eclipse.jdt.client.internal.compiler.lookup.ReferenceBinding refType = null;
		if (this.binding instanceof ParameterizedTypeBinding) {
			...
			refType = (org.eclipse.jdt.client.internal.compiler.lookup.ReferenceBinding) this.binding;
		}
		if (refType != null) {
			org.eclipse.jdt.client.internal.compiler.lookup.AnnotationBinding[] internalAnnotations = refType.getAnnotations();
			int length = internalAnnotations == null ? 0 : internalAnnotations.length;
			if (length != 0) {
				IAnnotationBinding[] tempAnnotations = new IAnnotationBinding[length];
				int convertedAnnotationCount = 0;
				for (int i = 0; i < length; i++) {
					org.eclipse.jdt.client.internal.compiler.lookup.AnnotationBinding internalAnnotation = internalAnnotations[i];
	...
	public String getBinaryName() {
		if (this.binding.isCapture()) {
			...
			org.eclipse.jdt.client.internal.compiler.lookup.Binding declaring = typeVariableBinding.declaringElement;
			StringBuffer binaryName = new StringBuffer();
			switch(declaring.kind()) {
				case org.eclipse.jdt.client.internal.compiler.lookup.Binding.METHOD :
					...
					break;
				default :
					org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding typeBinding = (org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding) declaring;
	...
	public synchronized IMethodBinding[] getDeclaredMethods() {
		...
		try {
			if (isClass() || isInterface() || isEnum()) {
				ReferenceBinding referenceBinding = (ReferenceBinding) this.binding;
				org.eclipse.jdt.client.internal.compiler.lookup.MethodBinding[] internalMethods = referenceBinding.availableMethods(); // be resilient
				int length = internalMethods.length;
				if (length != 0) {
					int convertedMethodCount = 0;
					IMethodBinding[] newMethods = new IMethodBinding[length];
					for (int i = 0; i < length; i++) {
						org.eclipse.jdt.client.internal.compiler.lookup.MethodBinding methodBinding = internalMethods[i];
...
//	}
	...
	public ITypeBinding[] getTypeArguments() {
		...
		if (this.binding.isParameterizedTypeWithActualArguments()) {
			ParameterizedTypeBinding parameterizedTypeBinding = (ParameterizedTypeBinding) this.binding;
			final org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding[] arguments = parameterizedTypeBinding.arguments;
	...
	public ITypeBinding[] getTypeBounds() {
		...
		if (this.binding instanceof TypeVariableBinding) {
			...
			org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding firstClassOrArrayBound = typeVariableBinding.firstBound;
			int boundsLength = 0;
	...
	public boolean isCastCompatible(ITypeBinding type) {
		try {
			...
			if (scope == null) return false;
			if (!(type instanceof TypeBinding)) return false;
			org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding expressionType = ((TypeBinding) type).binding;
	...
	public boolean isEqualTo(IBinding other) {
		...
		org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding otherBinding = ((TypeBinding) other).binding;
		// check return type
	...
	public boolean isFromSource() {
		if (isClass() || isInterface() || isEnum()) {
			...
			if (referenceBinding.isRawType()) {
				...
				org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding erasure = parameterizedTypeBinding.erasure();
			...
			if (declaringElement instanceof MethodBinding) {
				...
				final org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding typeBinding = (org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding) declaringElement;
				if (typeBinding instanceof ReferenceBinding) {
					...
					final org.eclipse.jdt.client.internal.compiler.lookup.TypeBinding leafComponentType = arrayBinding.leafComponentType;
