...
//		this.compilationUnit= cu;
	...
	private int getSpacesBetweenImportGroups() {
		try {
			int num= Integer.parseInt(JavaCore.getOption(DefaultCodeFormatterConstants.FORMATTER_BLANK_LINES_BETWEEN_IMPORT_GROUPS));
			if (num >= 0)
				return num;
	...
	private boolean insertSpaceBeforeSemicolon() {
		return JavaCore.INSERT.equals(JavaCore.getOption(DefaultCodeFormatterConstants.FORMATTER_INSERT_SPACE_BEFORE_SEMICOLON));
	}
...
//			IBuffer buffer= this.compilationUnit.getBuffer();
	...
	private void removeAndInsertNew(IDocument document, int contentOffset, int contentEnd, ArrayList stringsToInsert, MultiTextEdit resEdit) throws BadLocationException {
...
//		new SearchEngine().searchAllTypeNames(allPackages, allTypes, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, monitor);
	...
	private String[] getNewImportStrings(IDocument document, PackageEntry packageEntry, boolean isStatic, String lineDelim) throws BadLocationException {
