...
public class JavaContext extends CompilationUnitContext {
	 ...
	 */
	public JavaContext(TemplateContextType type, IDocument document, int completionOffset, int completionLength, CompilationUnit compilationUnit) {
	 ...
	 * @since 3.2
	 */
	public JavaContext(TemplateContextType type, IDocument document, Position completionPosition, CompilationUnit compilationUnit) {
...
//		IJavaProject project= getJavaProject();
//		if (project != null)
			return StubUtility.getVariableNameSuggestions(NamingConventions.VK_LOCAL, type, dim, Arrays.asList(excludes), true);
...
//	}
	 ...
	 */
	public static String evaluateTemplate(Template template, CompilationUnit compilationUnit, int position) throws CoreException, BadLocationException, TemplateException {
	   ...
	   //TODO
		...
		if (!(contextType instanceof CompilationUnitContextType))
			throw new CoreException(new Status(IStatus.ERROR, "", IStatus.ERROR, "Template file incomplete or has errors.", null));
