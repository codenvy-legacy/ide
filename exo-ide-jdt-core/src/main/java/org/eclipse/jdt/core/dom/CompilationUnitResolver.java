      ...
      Map options, int flags, IProgressMonitor monitor)
   {
      try
      {
         ...
         for (int i = 0; i < unitLength; i++)
         {
            ...
            org.exoplatform.ide.jdt.compiler.batch.CompilationUnit compilationUnit =
               new org.exoplatform.ide.jdt.compiler.batch.CompilationUnit(contents, sourceUnits[i], encoding);
      ...
      FileASTRequestor astRequestor, int apiLevel, Map compilerOptions, int flags)
   {
      ...
      try
      {
         ...
         for (int i = 0; i < length; i++)
         {
            ...
            sourceUnits[count++] =
               new org.exoplatform.ide.jdt.compiler.batch.CompilationUnit(contents, sourceUnitPath, encoding);
         }
         beginToCompile(sourceUnits, bindingKeys);
         // process all units (some more could be injected in the loop by the lookup environment)
         for (int i = 0; i < this.totalUnits; i++)
         {
            ...
            try
            {
               ...
               org.eclipse.jdt.internal.compiler.env.ICompilationUnit source =
                  (org.eclipse.jdt.internal.compiler.env.ICompilationUnit)this.requestedSources.get(fileName);
               if (source != null)
               {
                  // convert AST
                  CompilationResult compilationResult = unit.compilationResult;
