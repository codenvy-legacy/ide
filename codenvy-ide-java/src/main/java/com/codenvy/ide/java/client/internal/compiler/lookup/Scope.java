...
import com.codenvy.ide.java.client.internal.compiler.ast.CaseStatement;
...
import com.codenvy.ide.java.client.internal.compiler.ast.TypeParameter;
...
import com.codenvy.ide.java.client.internal.compiler.util.SimpleLookupTable;
import com.codenvy.ide.java.client.internal.compiler.util.SimpleSet;
                        ...
                        if (methodBinding.isValidBinding())
                        {
                           if (!methodBinding.isStatic() && (insideConstructorCall || insideStaticContext))
                           {
                              ...
                              return new ProblemMethodBinding(methodBinding, // closest match
                                 methodBinding.selector, methodBinding.parameters, insideConstructorCall
                                    ? ProblemReasons.NonStaticReferenceInConstructorInvocation
                                    : ProblemReasons.NonStaticReferenceInStaticContext);
                           }
      ...
      while (currentIndex < length)
      {
         ...
         if (!binding.isValidBinding())
            return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex),
               binding instanceof ReferenceBinding ? (ReferenceBinding)((ReferenceBinding)binding).closestMatch()
   ...
   public final TypeBinding getType(char[] name, PackageBinding packageBinding)
   {
      ...
      if (!binding.isValidBinding())
      {
         return new ProblemReferenceBinding(binding instanceof ReferenceBinding
            ? ((ReferenceBinding)binding).compoundName : CharOperation.arrayConcat(packageBinding.compoundName, name),
         ...
         for (int j = 0; j < classTypeVariablesArity; j++)
         {
            map.put(
               ...
               staticFactory.typeVariables[j] =
                  new TypeVariableBinding(CharOperation.concat(classTypeVariables[j].sourceName, "'".toCharArray()), //$NON-NLS-1$
