...
public class CharUtil {
    ...
    private static native String deLZW(Decoder d, int max) /*-{
        ...
        var current = f(d.@com.codenvy.ide.ext.java.worker.core.util.CharUtil.Decoder::next()());
        var oldPhrase = current, seen = max, phrase, out = [ current ], code;
        for (; ;) {
            if ((code = d.@com.codenvy.ide.ext.java.worker.core.util.CharUtil.Decoder::next()()) < 0) {
                break;
            } else if (code < max) {
                phrase = f(code);
            } else {
