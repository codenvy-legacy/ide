 ...
 */
public class JavaModelUnmarshaller2 {
    ...
    public JavaModelUnmarshaller2(Folder root, JavaProject project, EventBus eventBus, AsyncRequestFactory asyncRequestFactory,
    ...
    public void unmarshal(Array<ItemReference> children, Array<ProjectDescriptor> modules) {
        parseProjectStructure(children, root, root, modules, project);
    }
    ...
    public void unmarshalTree(List<TreeElement> children) {
        for (TreeElement treeElement : children) {
            ...
            if (item.getType().equals(Folder.TYPE) && treeElement.getChildren().size() == 1) {
                unmarshalTree(treeElement.getChildren());
            } else {
                final String path = item.getPath();
                String packageName = path.replace(root.getPath(), "").replaceAll("/", ".");
    ...
    private void parseProjectStructure(Array<ItemReference> children, Folder parentFolder, Folder parentFolderNonModelItems,
            ...
            switch (item.getType()) {
                ...
                case Folder.TYPE:
                    if (isProject(item, modules)) {
                        parseProjectItem(parentFolder, parentFolderNonModelItems, modules, project, item);
                    } else {
...
//                    parseProjectItem(parentFolder, parentFolderNonModelItems, modules, project, item);
    ...
    private void parseProjectItem(Folder parentFolder, Folder parentFolderNonModelItems, Array<ProjectDescriptor> modules, Project project,
                                  ItemReference item) {
