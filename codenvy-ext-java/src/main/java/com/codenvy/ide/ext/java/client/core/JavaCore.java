...
public final class JavaCore {
    ...
    public static final String COMPILER_PB_MISSING_JAVADOC_TAGS_METHOD_TYPE_PARAMETERS = PLUGIN_ID
                                                                                         +
                                                                                         ".compiler.problem.missingJavadocTagsMethodTypeParameters";
    //$NON-NLS-1$
     ...
     * <p>When enabled, the compiler will issue an error or a warning if some non fatal dead code is detected. For instance, <code>if (false)
     * foo();</code>
    ...
    public static HashMap<String, String> getOptions() {
        // get encoding through resource plugin
        if (JavaExtension.get() == null)
            return getDefaultOptions();

        return JavaExtension.get().getOptions();
    }
