...
public class RippleMethodFinder2
{
   ...
   public static IMethod[] getRelatedMethods(IMethod method, boolean excludeBinaries, IProgressMonitor pm,
   ...
   public static IMethod[] getRelatedMethods(IMethod method, IProgressMonitor pm,
      WorkingCopyOwner owner) throws CoreException
   {
   ...
   private IMethod[] findAllRippleMethods(IProgressMonitor pm, WorkingCopyOwner owner) throws CoreException
   {
      ...
      {
         Assert.isTrue(false,
            "Search for method declaration did not find original element: " + fMethod.toString()); //$NON-NLS-1$
      }
   ...
   private ITypeHierarchy getCachedHierarchy(IType type, WorkingCopyOwner owner,
