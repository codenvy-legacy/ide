            ...
            if (resource instanceof IContainer)
            {
               try
               {
                  ...
                  for (int i = 0, max = members.length; i < max; i++)
                  {
                     ...
                     else if (!containsJavaSource && org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(
                        resourceName))
                     {
                        containsJavaSource = true;
                     }
         ...
         else
         {
            ...
            try
            {
               ...
               for (Enumeration entries = zip.entries(); entries.hasMoreElements(); )
               {
                  ZipEntry entry = (ZipEntry)entries.nextElement();
                  String entryName;
                  if (!entry.isDirectory() && org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(
