...
public class DataTreeNode extends AbstractDataTreeNode
{
   ...
   /**
    * @see AbstractDataTreeNode#asBackwardDelta(DeltaDataTree, DeltaDataTree, org.eclipse.core.runtime.IPath)
    */
      ...
      AbstractDataTreeNode[] newNodes, IComparator comparer)
   {
      ...
      while (oldIndex < oldNodes.length && newIndex < newNodes.length)
      {
         ...
         if (compare == 0)
         {
            AbstractDataTreeNode deltaNode = forwardDeltaWithOrNullIfEqual(oldNodes[oldIndex++], newNodes[newIndex++],
               comparer);
            if (deltaNode != null)
            {
         ...
         else if (compare < 0)
         {
            if (numChildDeltas >= childDeltaMax)
            {
               ...
               {
                  System.arraycopy(childDeltas, 0,
                     childDeltas = new AbstractDataTreeNode[childDeltaMax = childDeltaMax * 2 + 1], 0, numChildDeltas);
               }
            }
