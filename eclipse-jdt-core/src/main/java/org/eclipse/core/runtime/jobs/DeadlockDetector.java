...
class DeadlockDetector
{
   ...
   private Thread[] getThreadsOwningLock(ISchedulingRule rule)
   {
      ...
      {
         System.out.println(
            "Lock " + rule + " is involved in deadlock but is not owned by any thread."); //$NON-NLS-1$ //$NON-NLS-2$
      }
      if ((blocking.size() > 1) && (rule instanceof ILock) && (JobManager.DEBUG_LOCKS))
      {
         System.out.println(
            "Lock " + rule + " is owned by more than 1 thread, but it is not a rule."); //$NON-NLS-1$ //$NON-NLS-2$
      }
   ...
   void lockReleased(Thread owner, ISchedulingRule lock)
   {
      ...
      if (lockIndex < 0)
      {
         ...
         {
            System.out.println(
               "[lockReleased] Thread " + owner.getName() + " already released lock " + lock); //$NON-NLS-1$ //$NON-NLS-2$
         }
         return;
      }
      ...
      {
         if ((lock.isConflicting((ISchedulingRule)locks.get(j))) || (!(lock instanceof ILock) && !(locks.get(
            j) instanceof ILock) && (graph[threadIndex][j] > NO_STATE)))
         {
            if (graph[threadIndex][j] == NO_STATE)
            {
               ...
               {
                  System.out.println(
                     "[lockReleased] More releases than acquires for thread " + owner.getName() + " and lock " + lock); //$NON-NLS-1$ //$NON-NLS-2$
               }
            }
            else
            {
   ...
   void lockReleasedCompletely(Thread owner, ISchedulingRule rule)
   {
      ...
      if (ruleIndex < 0)
      {
         ...
         {
            System.out.println(
               "[lockReleasedCompletely] Thread " + owner.getName() + " already released lock " + rule); //$NON-NLS-1$ //$NON-NLS-2$
         }
         return;
      }
      /**
   ...
   void lockWaitStop(Thread owner, ISchedulingRule lock)
   {
      ...
      if (graph[threadIndex][lockIndex] != WAITING_FOR_LOCK)
      {
         ...
         {
            System.out.println(
               "Lock " + lock + " already granted to depth: " + graph[threadIndex][lockIndex]); //$NON-NLS-1$ //$NON-NLS-2$
         }
         return;
      }
      graph[threadIndex][lockIndex] = NO_STATE;
