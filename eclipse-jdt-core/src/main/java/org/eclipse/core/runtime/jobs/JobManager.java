...
public class JobManager implements IJobManager
{
   ...
   private JobManager()
   {
      ...
      pool.setDaemon(false/*JobOSGiUtils.getDefault().useDaemonThreads()*/);
      internalWorker = new InternalWorker(this);
      internalWorker.setDaemon(false/*JobOSGiUtils.getDefault().useDaemonThreads()*/);
      ...
      synchronized (lock)
      {
         synchronized (job.jobStateLock)
         {
            ...
            switch (newState)
            {
               ...
               default:
                  Assert.isLegal(false,
                     "Invalid job state: " + job + ", state: " + newState); //$NON-NLS-1$ //$NON-NLS-2$
            }
         }
      }
   ...
   private void doShutdown()
   {
      ...
      if (toCancel != null)
      {
         for (int i = 0; i < toCancel.length; i++)
         {
            ...
            Util.log(new Status(IStatus.WARNING, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, msg, null));
   ...
   {
      String message = jobCount == 1 ? "jobs_waitFamSubOne" : "JobMessages.jobs_waitFamSub";
      return message + Integer.toString(jobCount);
   }

   private void initDebugOptions()
   {
      DEBUG = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEBUG_JOBS, false);
      DEBUG_BEGIN_END = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEBUG_BEGIN_END, false);
      DEBUG_YIELDING = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEBUG_YIELDING, false);
      DEBUG_YIELDING_DETAILED = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEBUG_YIELDING_DETAILED, false);
      DEBUG_DEADLOCK = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEADLOCK_ERROR, false);
      DEBUG_LOCKS = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_LOCKS, false);
      DEBUG_TIMING = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_DEBUG_JOBS_TIMING, false);
      DEBUG_SHUTDOWN = false;//JobOSGiUtils.getDefault().getBooleanDebugOption(OPTION_SHUTDOWN, false);
   }
    ...
    */
   public void join(final Object family,
      IProgressMonitor monitor) throws InterruptedException, OperationCanceledException
   {
      ...
      if (jobCount == 0)
      {
         ...
         monitor.beginTask("jobs_blocked0", 1);
         monitor.done();
         return;
      }
      ...
      try
      {
         monitor.beginTask("jobs_blocked0", jobCount);
   ...
   private IProgressMonitor monitorFor(IProgressMonitor monitor)
   {
      if (monitor == null || (monitor instanceof NullProgressMonitor))
      {
         if (progressProvider != null)
         {
            ...
            {
               String msg = "meta_pluginProblems " + JobManager.PI_JOBS;
               Util.log(new Status(IStatus.ERROR, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, msg, e));
            }
         }
      }
   ...
   final void reportBlocked(IProgressMonitor monitor, InternalJob blockingJob)
   {
      ...
      {
         reason = new Status(IStatus.INFO, JobManager.PI_JOBS, 1, "jobs_blocked0", null);
      }
      else
      {
         String msg = "jobs_blocked1 " + blockingJob.getName();
   ...
   protected Job yieldRule(InternalJob job, IProgressMonitor monitor)
   {
      ...
      {
         JobManager.debug(
            job + " is waiting for " + unblocked + " to transition from WAITING state"); //$NON-NLS-1$ //$NON-NLS-2$
      }
         ...
         synchronized (lock)
         {
            for (Iterator it = running.iterator(); it.hasNext(); )
            {
               ...
               Assert.isTrue(!other.isConflicting(job),
                  other + " conflicts and ran simultaneously with " + job); //$NON-NLS-1$
            }
         }
   ...
   private boolean shouldRun(Job job)
   {
      ...
      Util.log(new Status(IStatus.ERROR, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR,
