...
public class ProcessorBasedRefactoring extends Refactoring {
     ...
     *
     * @param processor
     *         the refactoring's main processor
    ...
    public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
        ...
        for (Iterator iter = fParticipants.iterator(); iter.hasNext() && !result.hasFatalError(); ) {
            ...
            //			final PerformanceStats stats= PerformanceStats.getStats(PERF_CHECK_CONDITIONS, getName() + ",
            ...
            try {
                ...
                result.merge(RefactoringStatus.createErrorStatus(
                        Messages.format(RefactoringCoreMessages.ProcessorBasedRefactoring_check_condition_participant_failed,
                                        participant.getName())));
                iter.remove();
            }
    ...
    public Change createChange(IProgressMonitor pm) throws CoreException {
        ...
        for (Iterator iter = fParticipants.iterator(); iter.hasNext(); ) {
            ...
            try {
                //				final PerformanceStats stats= PerformanceStats.getStats(PERF_CREATE_CHANGES, getName() + ",
                // " + participant.getName()); //$NON-NLS-1$
                //				stats.startRun();
        ...
        Change postChange = getProcessor().postCreateChange((Change[])changes.toArray(new Change[changes.size()]),
