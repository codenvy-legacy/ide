...
import com.codenvy.eclipse.core.commands.operations.IAdvancedUndoableOperation;
import com.codenvy.eclipse.core.commands.operations.IUndoContext;
import com.codenvy.eclipse.core.commands.operations.IUndoableOperation;
import com.codenvy.eclipse.core.commands.operations.OperationHistoryEvent;
...
import com.codenvy.eclipse.ltk.core.refactoring.IValidationCheckResultQuery;
...
public class UndoableOperation2ChangeAdapter implements IUndoableOperation, IAdvancedUndoableOperation {
    ...
    public IStatus execute(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
        ...
        try {
            ExecuteResult result = executeChange(getQuery(info, RefactoringCoreMessages.Refactoring_execute_label),
                                                 monitor);
    ...
    public IStatus computeUndoableStatus(IProgressMonitor monitor) throws ExecutionException {
        if (fUndoChange == null) {
            return new Status(IStatus.ERROR, RefactoringCorePlugin.getPluginId(), IStatus.ERROR,
                              RefactoringCoreMessages.UndoableOperation2ChangeAdapter_no_undo_available, null);
        }
        try {
            if (monitor == null) {
                monitor = new NullProgressMonitor();
    ...
    public IStatus computeRedoableStatus(IProgressMonitor monitor) throws ExecutionException {
        if (fRedoChange == null) {
            return new Status(IStatus.ERROR, RefactoringCorePlugin.getPluginId(), IStatus.ERROR,
                              RefactoringCoreMessages.UndoableOperation2ChangeAdapter_no_redo_available, null);
        }
        try {
            if (monitor == null) {
                monitor = new NullProgressMonitor();
    ...
    private ExecuteResult executeChange(final IValidationCheckResultQuery query,
    ...
    private IStatus createStatus(ExecuteResult result) {
        if (!result.validationStatus.isOK()) {
            ...
            return new Status(IStatus.ERROR, RefactoringCorePlugin.getPluginId(), IStatus.ERROR,
                              RefactoringCoreMessages.UndoableOperation2ChangeAdapter_error_message, null);
        }
    }
    ...
    private IValidationCheckResultQuery getQuery(IAdaptable info, String title) {
        ...
        IValidationCheckResultQuery result = (IValidationCheckResultQuery)info.getAdapter(
                IValidationCheckResultQuery.class);
        if (result != null) {
            return result;
        }
