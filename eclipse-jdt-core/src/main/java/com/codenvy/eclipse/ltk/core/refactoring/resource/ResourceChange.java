...
public abstract class ResourceChange extends Change {
    ...
    /**
     * The 'not read only' validation method performs the default validations (see {@link #VALIDATE_DEFAULT}) and additionally ensures that
     * the element
     * is not read only.
     */
    ...
    /**
     * The 'not dirty' validation method performs the default validations (see {@link #VALIDATE_DEFAULT}) and additionally ensures that the
     * element
    ...
    /**
     * Sets the validation methods used when the current resource is validated in {@link #isValid(com.codenvy.eclipse.core.runtime
     * .IProgressMonitor)}.
     * <p>
     ...
     *         the validation method used in {@link #isValid(com.codenvy.eclipse.core.runtime.IProgressMonitor)}.
    ...
    /**
     * This implementation of {@link com.codenvy.eclipse.ltk.core.refactoring.Change#isValid(com.codenvy.eclipse.core.runtime
     * .IProgressMonitor)}
    ...
    private static void checkExistence(RefactoringStatus status, IResource element) {
        if (element == null) {
            ...
            status.addFatalError(Messages.format(RefactoringCoreMessages.ResourceChange_error_does_not_exist,
    ...
    private static class ValidationState {
        ...
        public void saveIfDirty(RefactoringStatus status, long stampToMatch, IProgressMonitor pm) throws CoreException {
            if (fDirty) {
                if (fKind == DOCUMENT && fTextFileBuffer != null && stampToMatch == fModificationStamp) {
                    ...
                    status.addFatalError(Messages.format(RefactoringCoreMessages.ResourceChange_error_unsaved,
        ...
        public void checkReadOnly(RefactoringStatus status) {
            if (fReadOnly) {
                status.addFatalError(Messages.format(RefactoringCoreMessages.ResourceChange_error_read_only,
                                                     BasicElementLabels.getPathLabel(fResource.getFullPath(), false)));
            }
        }
        ...
        public void checkSameReadOnly(RefactoringStatus status, boolean valueToMatch) {
            if (fReadOnly != valueToMatch) {
                status.addFatalError(Messages.format(RefactoringCoreMessages.ResourceChange_error_read_only_state_changed,
                                                     BasicElementLabels.getPathLabel(fResource.getFullPath(), false)));
            }
        }
        ...
        public void checkModificationStamp(RefactoringStatus status, long stampToMatch) {
            if (fKind == DOCUMENT) {
                if (stampToMatch != -1 && fModificationStamp != stampToMatch) {
                    status.addFatalError(Messages.format(RefactoringCoreMessages.ResourceChange_error_has_been_modified,
                                                         BasicElementLabels.getPathLabel(fResource.getFullPath(), false)));
                }
            } else {
