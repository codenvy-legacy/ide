...
import com.codenvy.eclipse.ltk.internal.core.refactoring.BufferValidationState;
import com.codenvy.eclipse.ltk.internal.core.refactoring.Changes;
import com.codenvy.eclipse.ltk.internal.core.refactoring.ContentStamps;
import com.codenvy.eclipse.ltk.internal.core.refactoring.Lock;
import com.codenvy.eclipse.ltk.internal.core.refactoring.MultiStateUndoChange;
import com.codenvy.eclipse.ltk.internal.core.refactoring.NonDeletingPositionUpdater;
...
import org.exoplatform.ide.editor.shared.text.edits.TextEditCopier;
import org.exoplatform.ide.editor.shared.text.edits.TextEditGroup;
import org.exoplatform.ide.editor.shared.text.edits.TextEditProcessor;
...
public class MultiStateTextFileChange extends TextEditBasedChange {
     ...
     *         if the document could not successfully be acquired
     */
    ...
    private TextEditProcessor createTextEditProcessor(ComposableBufferChange change, IDocument document, int flags,
                                                      boolean preview) {
        ...
        if (preview) {
            ...
            LocalTextEditProcessor result = new LocalTextEditProcessor(document, change.getEdit(),
                                                                       flags | TextEdit.UPDATE_REGIONS);
    ...
    /*
     * @see org.eclipse.ltk.core.refactoring.TextEditBasedChange#getCurrentContent(org.eclipse.jface.text.IRegion,boolean,int,
     * org.eclipse.core.runtime.IProgressMonitor)
     */
    public final String getCurrentContent(final IRegion region, final boolean expand, final int surround,
    ...
    /*
     * @see org.eclipse.ltk.core.refactoring.TextEditBasedChange#getPreviewContent(org.eclipse.ltk.core.refactoring
     * .TextEditBasedChangeGroup[],org.eclipse.jface.text.IRegion,boolean,int,org.eclipse.core.runtime.IProgressMonitor)
     */
    public final String getPreviewContent(final TextEditBasedChangeGroup[] groups, final IRegion region,
                                          final boolean expand, final int surround, final IProgressMonitor monitor) throws CoreException {
                        ...
                        for (final Iterator inner = group.getCachedEdits().iterator(); inner.hasNext(); ) {
                            ...
                            if (copiedEdit != null) {
                                ...
                                RefactoringCorePlugin.logErrorMessage(
                                        "Could not find a copy for the indexed text edit " + originalEdit.toString()); //$NON-NLS-1$
                            }
                        }
                    }
                    ...
                    final TextEditProcessor processor = new TextEditProcessor(document, copy, TextEdit.NONE) {
                        ...
                        protected final boolean considerEdit(final TextEdit edit) {
                            ...
                            if (originalEdit != null) {
                                ...
                                RefactoringCorePlugin.logErrorMessage(
                                        "Could not find the original of the copied text edit " + edit.toString()); //$NON-NLS-1$
                            }
                            return true;
                        }
                    };
    ...
    public final Change perform(final IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            return new MultiStateUndoChange(getName(), fFile, (UndoEdit[])undoList.toArray(new UndoEdit[undoList.size()]),
                                            fContentStamp, fSaveMode);
     ...
     */
    private void performChanges(final IDocument document, final LinkedList undoList,
                                final boolean preview) throws BadLocationException {
    ...
    private void performChangesInSynchronizationContext(final IDocument document, final LinkedList undoList,
                                                        final boolean preview) throws BadLocationException {
        ...
        try {
            ...
            for (final Iterator iterator = fChanges.iterator(); iterator.hasNext(); ) {
                ...
                final UndoEdit edit = createTextEditProcessor(change, document,
                                                              undoList != null ? TextEdit.CREATE_UNDO : TextEdit.NONE, preview)
                        .performEdits();
                if (undoList != null) {
     ...
     * @throws com.codenvy.eclipse.core.runtime.CoreException
     *         if the document could not successfully be released
     */
