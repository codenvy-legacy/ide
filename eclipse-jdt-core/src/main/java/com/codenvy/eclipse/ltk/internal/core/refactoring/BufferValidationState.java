...
public abstract class BufferValidationState
{
   ...
   public RefactoringStatus isValid(boolean needsSaving, boolean resilientForDerived) throws CoreException
   {
      ...
      if (!fExisted)
      {
         if (fFile.exists())
         {
            return RefactoringStatus.createFatalErrorStatus(
               Messages.format(RefactoringCoreMessages.TextChanges_error_existing,
      ...
      else
      {
         ...
         {
            return RefactoringStatus.createFatalErrorStatus(
               Messages.format(RefactoringCoreMessages.TextChanges_error_not_existing,
                  BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
         }
      }
      if (needsSaving)
      {
         ...
         {
            return RefactoringStatus.createFatalErrorStatus(
               Messages.format(RefactoringCoreMessages.TextChanges_error_read_only,
                  BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
         }
         else if (!fFile.isSynchronized(IResource.DEPTH_ZERO))
         {
            return RefactoringStatus.createFatalErrorStatus(
               Messages.format(RefactoringCoreMessages.TextChanges_error_outOfSync,
                  BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
         }
      }
      if (fEncoding == null)
      {
         return RefactoringStatus.createFatalErrorStatus(
            Messages.format(RefactoringCoreMessages.BufferValidationState_no_character_encoding,
               BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
      }
      else if (!fEncoding.equals(fFile.getCharset(true)))
      {
         return RefactoringStatus.createFatalErrorStatus(
            Messages.format(RefactoringCoreMessages.BufferValidationState_character_encoding_changed,
               BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
      }
      return new RefactoringStatus();
   }
...
class NoStampValidationState extends BufferValidationState
{
   ...
   public RefactoringStatus isValid(boolean needsSaving, boolean resilientForDerived) throws CoreException
   {
      ...
      {
         result.addFatalError(Messages.format(RefactoringCoreMessages.TextChanges_error_content_changed,
            BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
      }
      return result;
   }
...
class ModificationStampValidationState extends BufferValidationState
{
   ...
   public RefactoringStatus isValid(boolean needsSaving, boolean resilientForDerived) throws CoreException
   {
      ...
      if (fModificationStamp.getValue() != currentStamp.getValue()
         ...
         || (fModificationStamp.isFileStamp() && fModificationStamp.getValue() == IResource.NULL_STAMP && !currentStamp.isFileStamp()) || (fModificationStamp.isDocumentStamp() && fModificationStamp.getValue() == -1 && !currentStamp.isDocumentStamp()) || (fModificationStamp.isFileStamp() && currentStamp.isFileStamp() && isDirty(
         fFile)))
      {
         result.addFatalError(Messages.format(RefactoringCoreMessages.TextChanges_error_content_changed,
            BasicElementLabels.getPathLabel(fFile.getFullPath(), false)));
