...
import com.codenvy.eclipse.ltk.core.refactoring.participants.MoveRefactoring;
...
import com.codenvy.eclipse.ltk.internal.core.refactoring.resource.MoveResourcesProcessor;
...
public final class MoveResourcesDescriptor extends RefactoringDescriptor {
    ...
    public MoveResourcesDescriptor() {
        super(ID, null, RefactoringCoreMessages.MoveResourcesDescriptor_unnamed_descriptor, null,
     ...
     *         <code>true</code> if this move will update references
     */
    ...
    public Refactoring createRefactoring(RefactoringStatus status) throws CoreException {
        ...
        if (!(destination instanceof IFolder || destination instanceof IProject) || !destination.exists()) {
            status.addFatalError(
                    Messages.format(RefactoringCoreMessages.MoveResourcesDescriptor_error_destination_not_exists,
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            if (resource == null || !resource.exists()) {
                status.addFatalError(Messages.format(RefactoringCoreMessages.MoveResourcesDescriptor_error_moved_not_exists,
                                                     BasicElementLabels.getPathLabel(path, false)));
                return null;
            }
            if (!(resource instanceof IFile || resource instanceof IFolder)) {
                status.addFatalError(
                        Messages.format(RefactoringCoreMessages.MoveResourcesDescriptor_error_moved_not_file_or_folder,
                                        BasicElementLabels.getPathLabel(path, false)));
                return null;
            }
