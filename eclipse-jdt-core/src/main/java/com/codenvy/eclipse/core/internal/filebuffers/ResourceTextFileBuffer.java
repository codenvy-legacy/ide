...
import com.codenvy.eclipse.core.filebuffers.manipulation.ContainerCreator;
...
public class ResourceTextFileBuffer extends ResourceFileBuffer implements ITextFileBuffer {
    ...
    static final private QualifiedName ENCODING_KEY = new QualifiedName(FileBuffersPlugin.PLUGIN_ID,
                                                                        "encoding");  //$NON-NLS-1$

    /** Constant for representing the error status. This is considered a value object. */
    static final private IStatus STATUS_ERROR = new Status(IStatus.ERROR, FileBuffersPlugin.PLUGIN_ID, IStatus.OK,
    ...
    public IContentType getContentType() throws CoreException {
        ...
        // NLSUtility.format(FileBuffersMessages.FileBuffer_error_queryContentDescription, fFile.getFullPath().toOSString()), x));
        //		}
    }
     ...
     *         if reading of file's content description fails
     */
    ...
    protected void commitFileBufferContent(IProgressMonitor monitor, boolean overwrite) throws CoreException {
        ...
        try {
            ...
            String message = NLSUtility.format(
                    FileBuffersMessages.ResourceTextFileBuffer_error_unsupported_encoding_message_arg, encoding);
            IStatus s = new Status(IStatus.ERROR, FileBuffersPlugin.PLUGIN_ID, IStatus.OK, message, ex);
            throw new CoreException(s);
        } catch (IllegalCharsetNameException ex) {
            String message = NLSUtility.format(
                    FileBuffersMessages.ResourceTextFileBuffer_error_illegal_encoding_message_arg, encoding);
            IStatus s = new Status(IStatus.ERROR, FileBuffersPlugin.PLUGIN_ID, IStatus.OK, message, ex);
            throw new CoreException(s);
        }
        ...
        try {
            ...
            String message = NLSUtility.format(
                    FileBuffersMessages.ResourceTextFileBuffer_error_charset_mapping_failed_message_arg, encoding);
            IStatus s = new Status(IStatus.ERROR, FileBuffersPlugin.PLUGIN_ID,
                                   IFileBufferStatusCodes.CHARSET_MAPPING_FAILED, message, ex);
            throw new CoreException(s);
        }
    ...
    private String computeEncoding() {
        ...
        try {
            ...
            IContentDescription description = null; // Platform.getContentTypeManager().getDescriptionFor(reader, fFile.getName(), options);
            if (description != null) {
     ...
     *
     * @throws CoreException
     *         if the encoding cannot be retrieved from the resource
     * @since 3.1
     */
