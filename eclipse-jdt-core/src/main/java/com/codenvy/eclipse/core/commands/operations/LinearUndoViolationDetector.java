...
public abstract class LinearUndoViolationDetector implements IOperationApprover
{
    ...
    * history.
    *
    * @param operation the operation for which a linear redo violation has been
    *                  detected.
    * @param context   the undo context in which the linear redo violation exists
    * @param history   the operation history containing the operation
   ...
   protected abstract IStatus allowLinearRedoViolation(IUndoableOperation operation, IUndoContext context,
    ...
    * history.
    *
    * @param operation the operation for which a linear undo violation has been
    *                  detected.
    * @param context   the undo context in which the linear undo violation exists
    * @param history   the operation history containing the operation
    * @param info      the IAdaptable (or <code>null</code>) provided by the
    *                  caller in order to supply UI information for prompting the
    *                  user if necessary. When this parameter is not
    ...
    *         severities will not be interpreted by the history.
    */
   protected abstract IStatus allowLinearUndoViolation(IUndoableOperation operation, IUndoContext context,
      IOperationHistory history, IAdaptable info);
