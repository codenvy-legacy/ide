...
import com.codenvy.eclipse.core.internal.dtree.DeltaDataTree;
import com.codenvy.eclipse.core.internal.dtree.NodeComparison;
...
public class ResourceDeltaFactory {
     ...
     *
     * @param markerGeneration
     *         the start generation for which deltas should be computed, or -1
     *         if marker deltas should not be provided.
     */
    public static ResourceDelta computeDelta(WorkspaceResource workspace, ElementTree oldTree, ElementTree newTree,
                                             IPath root, long markerGeneration) {
        //compute the underlying delta tree.
        ResourceComparator comparator =
                markerGeneration >= 0 ? ResourceComparator.getNotificationComparator() : ResourceComparator.getBuildComparator();
        newTree.immutable();
     ...
     * the given path.
     */
    protected static ResourceDelta createDelta(WorkspaceResource workspace, DeltaDataTree delta,
                                               ResourceDeltaInfo deltaInfo, IPath pathInTree, IPath pathInDelta) {
        ...
        if (numChildren == 0) {
            ...
            for (int i = 0; i < numChildren; i++) {
                ...
                IPath newTreePath = pathInTree == pathInDelta ? childKeys[i] : pathInTree.append(
                        childKeys[i].lastSegment());
    ...
    public static IResourceDelta newEmptyDelta(IProject project) {
        ResourceDelta result = new ResourceDelta(project.getFullPath(),
                                                 new ResourceDeltaInfo(((WorkspaceResource)project.getWorkspace()), null,
                                                                       ResourceComparator.getBuildComparator()));
        result.setStatus(0);
        result.setChildren(NO_CHILDREN);
        ResourceInfo info = ((ProjectResource)project).getResourceInfo(true, false);
        result.setOldInfo(info);
        result.setNewInfo(info);
        return result;
    }
