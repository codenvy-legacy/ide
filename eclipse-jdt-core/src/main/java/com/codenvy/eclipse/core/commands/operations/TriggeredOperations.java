 ...
 * @since 3.1
 */
public final class TriggeredOperations extends AbstractOperation
   implements ICompositeOperation, IAdvancedUndoableOperation, IAdvancedUndoableOperation2, IContextReplacingOperation
{
    ...
    * the operation.
    *
    * @param operation the operation that will trigger other operations.
    * @param history   the operation history containing the triggered operations.
    */
   ...
   public void remove(IUndoableOperation operation)
   {
      if (operation == triggeringOperation)
      {
         ...
         history.replaceOperation(this,
            (IUndoableOperation[])childrenToRestore.toArray(new IUndoableOperation[childrenToRestore.size()]));
      }
      else
      {
   ...
   private void recomputeContexts()
   {
      ...
      for (int i = 0; i < children.size(); i++)
      {
         IUndoContext[] contexts = ((IUndoableOperation)children.get(i)).getContexts();
   ...
   {
      IUndoableOperation[] nonTriggers = (IUndoableOperation[])children.toArray(
         new IUndoableOperation[children.size()]);
