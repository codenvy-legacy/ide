...
public class VariablesEvaluator extends Evaluator implements EvaluationConstants {
    ...
    VariablesEvaluator(EvaluationContext context, INameEnvironment environment, Map options, IRequestor requestor,
    ...
    protected void addEvaluationResultForCompilationProblem(Map resultsByIDs, CategorizedProblem problem,
        ...
        for (int i = 0; i < varCount; i++) {
			 ...
			 */
                return;
            } else if ((var.initExpressionStart <= problem.getSourceStart()) &&
    ...
    protected char[] getClassName() {
        return CharOperation.concat(EvaluationConstants.GLOBAL_VARS_CLASS_NAME_PREFIX,
                                    Integer.toString(EvaluationContext.VAR_CLASS_COUNTER + 1).toCharArray());
    }
