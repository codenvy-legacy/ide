...
import com.codenvy.eclipse.jdt.core.util.ClassFileBytesDisassembler;
import com.codenvy.eclipse.jdt.core.util.ClassFormatException;
import com.codenvy.eclipse.jdt.core.util.IAnnotation;
import com.codenvy.eclipse.jdt.core.util.IAnnotationComponent;
import com.codenvy.eclipse.jdt.core.util.IAnnotationComponentValue;
import com.codenvy.eclipse.jdt.core.util.IAnnotationDefaultAttribute;
import com.codenvy.eclipse.jdt.core.util.IAttributeNamesConstants;
import com.codenvy.eclipse.jdt.core.util.IBootstrapMethodsAttribute;
import com.codenvy.eclipse.jdt.core.util.IBootstrapMethodsEntry;
import com.codenvy.eclipse.jdt.core.util.IClassFileAttribute;
import com.codenvy.eclipse.jdt.core.util.IClassFileReader;
import com.codenvy.eclipse.jdt.core.util.ICodeAttribute;
...
import com.codenvy.eclipse.jdt.core.util.IConstantPoolEntry2;
import com.codenvy.eclipse.jdt.core.util.IConstantValueAttribute;
import com.codenvy.eclipse.jdt.core.util.IEnclosingMethodAttribute;
import com.codenvy.eclipse.jdt.core.util.IExceptionAttribute;
import com.codenvy.eclipse.jdt.core.util.IExceptionTableEntry;
import com.codenvy.eclipse.jdt.core.util.IFieldInfo;
import com.codenvy.eclipse.jdt.core.util.IInnerClassesAttribute;
import com.codenvy.eclipse.jdt.core.util.IInnerClassesAttributeEntry;
import com.codenvy.eclipse.jdt.core.util.ILineNumberAttribute;
import com.codenvy.eclipse.jdt.core.util.ILocalVariableAttribute;
import com.codenvy.eclipse.jdt.core.util.ILocalVariableTableEntry;
import com.codenvy.eclipse.jdt.core.util.ILocalVariableTypeTableAttribute;
import com.codenvy.eclipse.jdt.core.util.ILocalVariableTypeTableEntry;
import com.codenvy.eclipse.jdt.core.util.IMethodInfo;
import com.codenvy.eclipse.jdt.core.util.IModifierConstants;
import com.codenvy.eclipse.jdt.core.util.IParameterAnnotation;
import com.codenvy.eclipse.jdt.core.util.IRuntimeInvisibleAnnotationsAttribute;
import com.codenvy.eclipse.jdt.core.util.IRuntimeInvisibleParameterAnnotationsAttribute;
import com.codenvy.eclipse.jdt.core.util.IRuntimeVisibleAnnotationsAttribute;
import com.codenvy.eclipse.jdt.core.util.IRuntimeVisibleParameterAnnotationsAttribute;
import com.codenvy.eclipse.jdt.core.util.ISignatureAttribute;
import com.codenvy.eclipse.jdt.core.util.ISourceAttribute;
import com.codenvy.eclipse.jdt.core.util.IStackMapAttribute;
import com.codenvy.eclipse.jdt.core.util.IStackMapFrame;
import com.codenvy.eclipse.jdt.core.util.IStackMapTableAttribute;
import com.codenvy.eclipse.jdt.core.util.IVerificationTypeInfo;
import com.codenvy.eclipse.jdt.internal.compiler.codegen.AttributeNamesConstants;
...
public class Disassembler extends ClassFileBytesDisassembler
{
      ...
      for (int i = 0, max = checkBits.length; i < max; i++)
      {
         switch (checkBits[i])
         {
            case IModifierConstants.ACC_PUBLIC:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PUBLIC, "public",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_PROTECTED:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PROTECTED, "protected",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_PRIVATE:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PRIVATE, "private",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_ABSTRACT:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_ABSTRACT, "abstract",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_STATIC:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_STATIC, "static",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_FINAL:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_FINAL, "final",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_SYNCHRONIZED:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_SYNCHRONIZED, "synchronized",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_NATIVE:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_NATIVE, "native",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_STRICT:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_STRICT, "strictfp",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_TRANSIENT:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_TRANSIENT, "transient",
                  firstModifier); //$NON-NLS-1$
               break;
            case IModifierConstants.ACC_VOLATILE:
               ...
               if (asBridge)
               {
                  firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_BRIDGE, "bridge",
                     firstModifier); //$NON-NLS-1$
               }
               else
               {
                  firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_VOLATILE, "volatile",
                     firstModifier); //$NON-NLS-1$
               }
               break;
            case IModifierConstants.ACC_ENUM:
               firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_ENUM, "enum",
                  firstModifier); //$NON-NLS-1$
               break;
         }
      }
   ...
   private void decodeModifiersForField(StringBuffer buffer, int accessFlags)
   {
      decodeModifiers(buffer, accessFlags,
         new int[]{IModifierConstants.ACC_PUBLIC, IModifierConstants.ACC_PROTECTED, IModifierConstants.ACC_PRIVATE, IModifierConstants.ACC_STATIC, IModifierConstants.ACC_FINAL, IModifierConstants.ACC_TRANSIENT, IModifierConstants.ACC_VOLATILE, IModifierConstants.ACC_ENUM});
   }

   private void decodeModifiersForFieldForWorkingCopy(StringBuffer buffer, int accessFlags)
   {
      decodeModifiers(buffer, accessFlags,
         new int[]{IModifierConstants.ACC_PUBLIC, IModifierConstants.ACC_PROTECTED, IModifierConstants.ACC_PRIVATE, IModifierConstants.ACC_STATIC, IModifierConstants.ACC_FINAL, IModifierConstants.ACC_TRANSIENT, IModifierConstants.ACC_VOLATILE,});
   }
   ...
   private final void decodeModifiersForInnerClasses(StringBuffer buffer, int accessFlags, boolean printDefault)
   {
      decodeModifiers(buffer, accessFlags, printDefault, false,
         new int[]{IModifierConstants.ACC_PUBLIC, IModifierConstants.ACC_PROTECTED, IModifierConstants.ACC_PRIVATE, IModifierConstants.ACC_ABSTRACT, IModifierConstants.ACC_STATIC, IModifierConstants.ACC_FINAL,});
   }
   ...
   private final void decodeModifiersForMethod(StringBuffer buffer, int accessFlags)
   {
      decodeModifiers(buffer, accessFlags, false, true,
         new int[]{IModifierConstants.ACC_PUBLIC, IModifierConstants.ACC_PROTECTED, IModifierConstants.ACC_PRIVATE, IModifierConstants.ACC_ABSTRACT, IModifierConstants.ACC_STATIC, IModifierConstants.ACC_FINAL, IModifierConstants.ACC_SYNCHRONIZED, IModifierConstants.ACC_NATIVE, IModifierConstants.ACC_STRICT, IModifierConstants.ACC_BRIDGE,});
   }

   private final void decodeModifiersForType(StringBuffer buffer, int accessFlags)
   {
      decodeModifiers(buffer, accessFlags,
         new int[]{IModifierConstants.ACC_PUBLIC, IModifierConstants.ACC_ABSTRACT, IModifierConstants.ACC_FINAL,});
   }
   ...
   /**
    * @see com.codenvy.eclipse.jdt.core.util.ClassFileBytesDisassembler#disassemble(byte[], java.lang.String)
    */
   ...
   /**
    * @see com.codenvy.eclipse.jdt.core.util.ClassFileBytesDisassembler#disassemble(byte[], java.lang.String, int)
    */
   ...
   private void disassemble(IAnnotation annotation, StringBuffer buffer, String lineSeparator, int tabNumber, int mode)
   {
      ...
      buffer.append(Messages.bind(Messages.disassembler_annotationentrystart,
         new String[]{Integer.toString(typeIndex), new String(
            returnClassName(Signature.toCharArray(typeName), '.', mode))}));
      ...
      int tabNumber, int mode)
   {
      ...
      buffer.append(Messages.bind(Messages.disassembler_annotationcomponent,
         new String[]{Integer.toString(annotationComponent.getComponentNameIndex()), new String(
            annotationComponent.getComponentName())}));
      ...
      switch (annotationComponentValue.getTag())
      {
            ...
            switch (constantPoolEntry.getKind())
            {
               ...
               case IConstantPoolConstant.CONSTANT_Integer:
                  ...
                  switch (annotationComponentValue.getTag())
                  {
                     ...
                     case IAnnotationComponentValue.BOOLEAN_TAG:
                        temp.append(
                           constantPoolEntry.getIntegerValue() == 1 ? "true" : "false");//$NON-NLS-1$//$NON-NLS-2$
                        break;
                     case IAnnotationComponentValue.BYTE_TAG:
         ...
         case IAnnotationComponentValue.ENUM_TAG:
            ...
            buffer.append(Messages.bind(Messages.disassembler_annotationenumvalue,
               new String[]{Integer.toString(enumConstantTypeNameIndex), Integer.toString(
                  enumConstantNameIndex), new String(
                  returnClassName(Signature.toCharArray(typeName), '.', mode)), new String(constantName)}));
            break;
         case IAnnotationComponentValue.CLASS_TAG:
            ...
            buffer.append(Messages.bind(Messages.disassembler_annotationclassvalue,
               new String[]{Integer.toString(classIndex), new String(
                  returnClassName(Signature.toCharArray(className), '.', mode))}));
            break;
      ...
      int tabNumber, int mode)
   {
      ...
      buffer.append(Messages.bind(Messages.disassembler_genericattributeheader,
         new String[]{new String(classFileAttribute.getAttributeName()), Long.toString(
      ...
      StringBuffer buffer, String lineSeparator, int tabNumber, int mode)
   {
      ...
      final char[] signature = Signature.toCharArray(methodDescriptor, returnClassName(className, '.', COMPACT),
         getParameterNames(methodDescriptor, codeAttribute, accessFlags), !checkMode(mode, COMPACT), false, isVarArgs);
      ...
      StringBuffer buffer, String lineSeparator, int tabNumber, int mode)
   {
      ...
      if (checkMode(mode, SYSTEM | DETAILED))
      {
         ...
         if (codeAttribute != null)
         {
            buffer.append(Messages.bind(Messages.classfileformat_stacksAndLocals,
               new String[]{Integer.toString(codeAttribute.getMaxStack()), Integer.toString(
      ...
      if (methodInfo.isConstructor())
      {
         if (checkMode(mode, WORKING_COPY) && signatureAttribute != null)
         {
            ...
            buffer.append(' ');
            methodHeader = Signature.toCharArray(signature, returnClassName(className, '.', COMPACT), parameterNames,
      ...
      if (checkMode(mode,
         DETAILED) && (runtimeInvisibleParameterAnnotationsAttribute != null || runtimeVisibleParameterAnnotationsAttribute != null))
      {
            ...
            if (length > 0)
            {
               ...
               if (length < parameterNamesLength)
               {
                  System.arraycopy(invisibleParameterAnnotations, 0,
                     (invisibleParameterAnnotations = new IParameterAnnotation[parameterNamesLength]), 1, length);
            ...
            if (length > 0)
            {
               ...
               if (length < parameterNamesLength)
               {
                  System.arraycopy(visibleParameterAnnotations, 0,
                     (visibleParameterAnnotations = new IParameterAnnotation[parameterNamesLength]), 1, length);
                  length = parameterNamesLength;
               }
            }
         }
      ...
      if (checkMode(mode, SYSTEM))
      {
         ...
         if (runtimeInvisibleParameterAnnotationsAttribute != null)
         {
            disassemble((IRuntimeInvisibleParameterAnnotationsAttribute)runtimeInvisibleParameterAnnotationsAttribute,
               buffer, lineSeparator, tabNumber, mode);
         }
      }
   }
   ...
   /**
    * @see #disassemble(com.codenvy.eclipse.jdt.core.util.IClassFileReader, java.lang.String, int)
    */
   ...
   public String disassemble(IClassFileReader classFileReader, String lineSeparator, int mode)
   {
      ...
      {
         return com.codenvy.eclipse.jdt.internal.compiler.util.Util.EMPTY_STRING;
      }
      ...
      if (checkMode(mode, SYSTEM | DETAILED))
      {
         ...
         buffer.append(Messages.bind(Messages.classfileformat_versiondetails,
            new String[]{versionNumber, Integer.toString(majorVersion), Integer.toString(
               minorVersion), ((accessFlags & IModifierConstants.ACC_SUPER) != 0 ? Messages.classfileformat_superflagisset : Messages.classfileformat_superflagisnotset) + (isDeprecated(
               classFileReader) ? ", deprecated" : com.codenvy.eclipse.jdt.internal.compiler.util.Util.EMPTY_STRING)//$NON-NLS-1$
            }));
      ...
      if (checkMode(mode, SYSTEM | DETAILED))
      {
         ...
         if (checkMode(mode, SYSTEM))
         {
            ...
            if (length != 0)
            {
               ...
               {
                  IClassFileAttribute attribute = attributes[i];
                  if (attribute != innerClassesAttribute && attribute != sourceAttribute && attribute != signatureAttribute && attribute != enclosingMethodAttribute && attribute != runtimeInvisibleAnnotationsAttribute && attribute != runtimeVisibleAnnotationsAttribute && !CharOperation.equals(
                     attribute.getAttributeName(), IAttributeNamesConstants.DEPRECATED) && !CharOperation.equals(
                     attribute.getAttributeName(), IAttributeNamesConstants.SYNTHETIC) && attribute != bootstrapMethods)
                  {
      ...
      boolean isStatic, StringBuffer buffer, String lineSeparator, int tabNumber, int mode)
   {
      ...
      if (exceptionTableLength != 0)
      {
         ...
         for (int i = 0; i < exceptionTableLength; i++)
         {
            ...
            buffer.append(Messages.bind(Messages.classfileformat_exceptiontableentry,
               new String[]{Integer.toString(exceptionTableEntry.getStartPC()), Integer.toString(
                  exceptionTableEntry.getEndPC()), Integer.toString(exceptionTableEntry.getHandlerPC()), new String(
                  catchType),}));
         }
      }
      ...
      if (lineAttributeLength != 0)
      {
         ...
         for (int i = 0; i < lineAttributeLength; i++)
         {
            ...
            buffer.append(Messages.bind(Messages.classfileformat_linenumbertableentry,
               new String[]{Integer.toString(lineattributesEntries[i][0]), Integer.toString(
                  lineattributesEntries[i][1])}));
         }
      }
      ...
      if (localVariableAttributeLength != 0)
      {
         ...
         for (int i = 0; i < localVariableAttributeLength; i++)
         {
            ...
            buffer.append(Messages.bind(Messages.classfileformat_localvariabletableentry,
               new String[]{Integer.toString(startPC), Integer.toString(startPC + length), new String(
                  localVariableTableEntry.getName()), Integer.toString(index), new String(
                  returnClassName(typeName, '.', mode))}));
         }
      }
      final ILocalVariableTypeTableAttribute localVariableTypeAttribute = (ILocalVariableTypeTableAttribute)getAttribute(
         IAttributeNamesConstants.LOCAL_VARIABLE_TYPE_TABLE, codeAttribute);
      ...
      if (localVariableTypeTableLength != 0)
      {
         ...
         for (int i = 0; i < localVariableTypeTableLength; i++)
         {
            ...
            buffer.append(Messages.bind(Messages.classfileformat_localvariabletableentry,
               new String[]{Integer.toString(startPC), Integer.toString(startPC + length), new String(
                  localVariableTypeTableEntry.getName()), Integer.toString(index), new String(
                  returnClassName(typeName, '.', mode))}));
         }
      }
      ...
      if (length != 0)
      {
         ...
         for (int i = 0; i < length; i++)
         {
            ...
            if (CharOperation.equals(attribute.getAttributeName(), IAttributeNamesConstants.STACK_MAP_TABLE))
            {
               ...
               buffer.append(
                  Messages.bind(Messages.disassembler_stackmaptableattributeheader, Integer.toString(numberOfEntries)));
            ...
            else if (CharOperation.equals(attribute.getAttributeName(), IAttributeNamesConstants.STACK_MAP))
            {
               ...
               buffer.append(
                  Messages.bind(Messages.disassembler_stackmapattributeheader, Integer.toString(numberOfEntries)));
               if (numberOfEntries != 0)
               {
            ...
            else if (attribute != lineNumberAttribute && attribute != localVariableAttribute && attribute != localVariableTypeAttribute)
            {
               ...
               buffer.append(Messages.bind(Messages.disassembler_genericattributeheader,
                  new String[]{new String(attribute.getAttributeName()), Long.toString(
   ...
   private void disassemble(IStackMapTableAttribute attribute, StringBuffer buffer, String lineSeparator, int tabNumber,
      int mode)
   {
            ...
            case 255:
               ...
               buffer.append(Messages.bind(Messages.disassembler_frame_full_frame,
                  new String[]{Integer.toString(absolutePC), Integer.toString(frame.getNumberOfLocals()), disassemble(
                     frame.getLocals(), mode), Integer.toString(frame.getNumberOfStackItems()), disassemble(
                     frame.getStackItems(), mode), dumpNewLineWithTabs(lineSeparator, tabNumber + 5)}));
               break;
            default:
               if (type <= 63)
               {
               ...
               else if (type <= 127)
               {
                  ...
                  offsetDelta = type - 64;
                  buffer.append(
                     Messages.bind(Messages.disassembler_frame_same_locals_1_stack_item, Integer.toString(absolutePC),
      ...
      int mode)
   {
      ...
      for (int j = 0; j < numberOfEntries; j++)
      {
         ...
         // disassemble each frame
         buffer.append(Messages.bind(Messages.disassembler_frame_full_frame,
            new String[]{Integer.toString(frame.getOffsetDelta()), Integer.toString(
               frame.getNumberOfLocals()), disassemble(frame.getLocals(), mode), Integer.toString(
               frame.getNumberOfStackItems()), disassemble(frame.getStackItems(), mode), dumpNewLineWithTabs(
         ...
         switch (constantPool.getEntryKind(i))
         {
            case IConstantPoolConstant.CONSTANT_Class:
               buffer.append(Messages.bind(Messages.disassembler_constantpool_class,
                  new String[]{Integer.toString(i), Integer.toString(
                     constantPoolEntry.getClassInfoNameIndex()), new String(constantPoolEntry.getClassInfoName())}));
               break;
            case IConstantPoolConstant.CONSTANT_Double:
            ...
            case IConstantPoolConstant.CONSTANT_Fieldref:
               buffer.append(Messages.bind(Messages.disassembler_constantpool_fieldref,
                  new String[]{Integer.toString(i), Integer.toString(
                     constantPoolEntry.getClassIndex()), Integer.toString(
                     constantPoolEntry.getNameAndTypeIndex()), new String(constantPoolEntry.getClassName()), new String(
                     constantPoolEntry.getFieldName()), new String(constantPoolEntry.getFieldDescriptor())}));
               break;
            case IConstantPoolConstant.CONSTANT_Float:
            ...
            case IConstantPoolConstant.CONSTANT_InterfaceMethodref:
               buffer.append(Messages.bind(Messages.disassembler_constantpool_interfacemethodref,
                  new String[]{Integer.toString(i), Integer.toString(
                     constantPoolEntry.getClassIndex()), Integer.toString(
                     constantPoolEntry.getNameAndTypeIndex()), new String(constantPoolEntry.getClassName()), new String(
                     constantPoolEntry.getMethodName()), new String(constantPoolEntry.getMethodDescriptor())}));
               break;
            case IConstantPoolConstant.CONSTANT_Long:
            ...
            case IConstantPoolConstant.CONSTANT_NameAndType:
               ...
               buffer.append(Messages.bind(Messages.disassembler_constantpool_name_and_type,
                  new String[]{Integer.toString(i), Integer.toString(nameIndex), Integer.toString(
                     typeIndex), String.valueOf(nameValue), String.valueOf(typeValue)}));
               break;
            case IConstantPoolConstant.CONSTANT_String:
               buffer.append(Messages.bind(Messages.disassembler_constantpool_string,
                  new String[]{Integer.toString(i), Integer.toString(
                     constantPoolEntry.getStringIndex()), decodeStringValue(constantPoolEntry.getStringValue())}));
               break;
            case IConstantPoolConstant.CONSTANT_Utf8:
               buffer.append(Messages.bind(Messages.disassembler_constantpool_utf8,
                  new String[]{Integer.toString(i), decodeStringValue(new String(constantPoolEntry.getUtf8Value()))}));
               break;
            case IConstantPoolConstant.CONSTANT_MethodHandle:
               ...
               buffer.append(Messages.bind(Messages.disassembler_constantpool_methodhandle,
                  new String[]{Integer.toString(i), getReferenceKind(entry2.getReferenceKind()), Integer.toString(
                     entry2.getReferenceIndex()),}));
               break;
            case IConstantPoolConstant.CONSTANT_MethodType:
               ...
               buffer.append(Messages.bind(Messages.disassembler_constantpool_methodtype,
                  new String[]{Integer.toString(i), Integer.toString(entry2.getDescriptorIndex()), String.valueOf(
                     entry2.getMethodDescriptor()),}));
               break;
            case IConstantPoolConstant.CONSTANT_InvokeDynamic:
               ...
               buffer.append(Messages.bind(Messages.disassembler_constantpool_invokedynamic,
                  new String[]{Integer.toString(i), Integer.toString(
                     entry2.getBootstrapMethodAttributeIndex()), Integer.toString(
                     entry2.getNameAndTypeIndex()), new String(constantPoolEntry.getMethodName()), new String(
                     constantPoolEntry.getMethodDescriptor())}));
         }
      }
   }
      ...
      String lineSeparator, int tabNumber)
   {
      ...
      buffer.append(Messages.disassembler_constantpoolindex).append(
         enclosingMethodAttribute.getEnclosingClassIndex()).append(" ")//$NON-NLS-1$
         .append(Messages.disassembler_constantpoolindex).append(
         enclosingMethodAttribute.getMethodNameAndTypeIndex()).append(" ")//$NON-NLS-1$
   ...
   private void disassembleEnumConstants(IFieldInfo fieldInfo, StringBuffer buffer, String lineSeparator, int tabNumber,
      char[][] argumentTypes, int mode)
   {
      writeNewLine(buffer, lineSeparator, tabNumber);
   ...
   private void disassemble(IFieldInfo fieldInfo, StringBuffer buffer, String lineSeparator, int tabNumber, int mode)
   {
      ...
      if (constantValueAttribute != null)
      {
         ...
         switch (constantPoolEntry.getKind())
         {
            ...
            case IConstantPoolConstant.CONSTANT_String:
               buffer.append(
                  "\"" + decodeStringValue(constantPoolEntry.getStringValue()) + "\"");//$NON-NLS-1$//$NON-NLS-2$
         }
      }
      ...
      if (checkMode(mode, SYSTEM))
      {
         ...
         if (length != 0)
         {
            ...
            {
               IClassFileAttribute attribute = attributes[i];
               if (attribute != constantValueAttribute && attribute != signatureAttribute && attribute != runtimeInvisibleAnnotationsAttribute && attribute != runtimeVisibleAnnotationsAttribute && !CharOperation.equals(
                  attribute.getAttributeName(), IAttributeNamesConstants.DEPRECATED) && !CharOperation.equals(
                  attribute.getAttributeName(), IAttributeNamesConstants.SYNTHETIC))
               {
                  disassemble(attribute, buffer, lineSeparator, tabNumber, mode);
   ...
   private void disassemble(IInnerClassesAttribute innerClassesAttribute, StringBuffer buffer, String lineSeparator,
      int tabNumber)
   {
      writeNewLine(buffer, lineSeparator, tabNumber);
      ...
      for (int i = 0; i < length; i++)
      {
         ...
         buffer.append(Messages.disassembler_openinnerclassentry).append(
            Messages.disassembler_inner_class_info_name).append(Messages.disassembler_constantpoolindex).append(
            innerClassNameIndex);
         if (innerClassNameIndex != 0)
         {
            buffer.append(Messages.disassembler_space).append(innerClassesAttributeEntry.getInnerClassName());
         }
         buffer.append(Messages.disassembler_comma).append(Messages.disassembler_space).append(
            Messages.disassembler_outer_class_info_name).append(Messages.disassembler_constantpoolindex).append(
            outerClassNameIndex);
         ...
         buffer.append(Messages.disassembler_space);
         buffer.append(Messages.disassembler_inner_name).append(Messages.disassembler_constantpoolindex).append(
            innerNameIndex);
         if (innerNameIndex != 0)
         {
         ...
         buffer.append(Messages.disassembler_comma).append(Messages.disassembler_space).append(
            Messages.disassembler_inner_accessflags).append(accessFlags).append(Messages.disassembler_space);
      ...
      String lineSeparator, int tabNumber)
   {
      ...
      for (int i = 0; i < length; i++)
      {
         ...
         buffer.append(Messages.bind(Messages.disassembler_bootstrapmethodentry,
            new String[]{Integer.toString(i), Integer.toString(entry.getBootstrapMethodReference()), getArguments(
   ...
   private String getArguments(int[] arguments)
   {
      ...
      for (int i = 0, max = arguments.length; i < max; i++)
      {
         ...
         buffer.append(Messages.bind(Messages.disassembler_bootstrapmethodentry_argument,
            new String[]{Integer.toString(arguments[i]),}));
      }
      buffer.append('}');
   ...
   private void disassemble(
      IRuntimeInvisibleParameterAnnotationsAttribute runtimeInvisibleParameterAnnotationsAttribute, StringBuffer buffer,
      String lineSeparator, int tabNumber, int mode)
   {
      writeNewLine(buffer, lineSeparator, tabNumber + 1);
   ...
   private void disassemble(IRuntimeVisibleAnnotationsAttribute runtimeVisibleAnnotationsAttribute, StringBuffer buffer,
      String lineSeparator, int tabNumber, int mode)
   {
      writeNewLine(buffer, lineSeparator, tabNumber + 1);
      ...
      String lineSeparator, int tabNumber, int mode, boolean isEnum)
   {
      ...
      if (isEnum && checkMode(mode, WORKING_COPY))
      {
         ...
         for (; index < fieldsLength; index++)
         {
            if (!foundSyntheticField && CharOperation.equals(TypeConstants.SYNTHETIC_ENUM_VALUES,
         ...
         for (int i = 0, max = methods.length; i < max; i++)
         {
            ...
            else if (CharOperation.equals(methodInfo.getName(), TypeConstants.VALUEOF))
            {
               ...
               if (parameterTypes.length == 1 && CharOperation.equals(parameterTypes[0],
   ...
   /**
    * @see com.codenvy.eclipse.jdt.core.util.ClassFileBytesDisassembler#getDescription()
    */
   ...
   private char[][] getParameterNames(char[] methodDescriptor, ICodeAttribute codeAttribute, int accessFlags)
   {
      ...
      if (codeAttribute != null)
      {
         ...
         if (localVariableAttribute != null)
         {
            ...
            {
               ILocalVariableTableEntry searchedEntry = getEntryFor(getLocalIndex(startingIndex, i, methodDescriptor),
                  entries);
