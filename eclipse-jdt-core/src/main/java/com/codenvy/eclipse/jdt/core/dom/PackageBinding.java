...
class PackageBinding implements IPackageBinding {
    ...
    private com.codenvy.eclipse.jdt.internal.compiler.lookup.PackageBinding binding;
    ...
    PackageBinding(com.codenvy.eclipse.jdt.internal.compiler.lookup.PackageBinding binding, BindingResolver resolver) {
            ...
            for (int i = 0, len = pkgs.length; i < len; i++) {
                ...
                switch (fragType) {
                    ...
                    case IPackageFragmentRoot.K_BINARY:
                        NameEnvironmentAnswer answer = nameEnvironment.findType(TypeConstants.PACKAGE_INFO_NAME,
                                                                                this.binding.compoundName);
                        if (answer != null && answer.isBinaryType()) {
                            IBinaryType type = answer.getBinaryType();
                            char[][][] missingTypeNames = type.getMissingTypeNames();
                            IBinaryAnnotation[] binaryAnnotations = type.getAnnotations();
                            com.codenvy.eclipse.jdt.internal.compiler.lookup.AnnotationBinding[] binaryInstances =
                                    BinaryTypeBinding.createAnnotations(
                                            binaryAnnotations, this.binding.environment, missingTypeNames);
                            com.codenvy.eclipse.jdt.internal.compiler.lookup.AnnotationBinding[] allInstances =
                                    com.codenvy.eclipse.jdt.internal.compiler.lookup.AnnotationBinding.addStandardAnnotations(
                                            binaryInstances, type.getTagBits(), this.binding.environment);
    ...
    public IJavaElement getJavaElement() {
        ...
        if (pkgs.length == 0) {
            ...
            com.codenvy.eclipse.jdt.internal.core.util.Util.log(new Status(IStatus.WARNING, JavaCore.PLUGIN_ID,
                                                                           "Searching for package " + getName() +
                                                                           " returns an empty array")); //$NON-NLS-1$ //$NON-NLS-2$
            return null;
        }
        return pkgs[0];
    }
    ...
    public boolean isEqualTo(IBinding other) {
        ...
        com.codenvy.eclipse.jdt.internal.compiler.lookup.PackageBinding packageBinding2 = ((PackageBinding)other).binding;
