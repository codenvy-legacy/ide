...
public class JavaBuilder {
    ...
    protected IProject[] build(int kind, Map ignored, IProgressMonitor monitor) throws CoreException {
        ...
        try {
                ...
                if (kind == FULL_BUILD) {
                    ...
                    if ((this.lastState = getLastState(this.currentProject)) == null) {
                        ...
                        if (deltas == null) {
                            if (DEBUG) {
                                System.out.println(
                                        "Performing full build since deltas are missing after incremental request"); //$NON-NLS-1$
                            }
                            buildAll();
                        ...
                        if (hasStructuralDelta()) { // double check that a jar file didn't get replaced in a binary project
                            ...
                            if (DEBUG) {
                                System.out.println(
                                        "Nothing to build since there are no source folders and no deltas"); //$NON-NLS-1$
                            }
            ...
            Util.log(e,
                     "JavaBuilder handling CoreException while building: " + this.currentProject.getName()); //$NON-NLS-1$
            createInconsistentBuildMarker(e);
        } catch (ImageBuilderInternalException e) {
            Util.log(e.getThrowable(),
                     "JavaBuilder handling ImageBuilderInternalException while building: " + this.currentProject.getName()); //$NON-NLS-1$
            ...
            marker.setAttributes(new String[]{IMarker.MESSAGE, IMarker.SEVERITY, IMarker.SOURCE_ID},
                                 new Object[]{Messages.bind(Messages.build_missingSourceFile, e.missingSourceFile), new Integer(
                                         IMarker.SEVERITY_ERROR), JavaBuilder.SOURCE_ID});
        } finally {
    ...
    protected void clean(IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            Util.log(e,
                     "JavaBuilder handling CoreException while cleaning: " + this.currentProject.getName()); //$NON-NLS-1$
            createInconsistentBuildMarker(e);
        } finally {
            this.notifier.done();
            cleanup();
    ...
    private void createInconsistentBuildMarker(CoreException coreException) throws CoreException {
        ...
        marker.setAttributes(
                ...
                new Object[]{Messages.bind(Messages.build_inconsistentProject, message), new Integer(
                        IMarker.SEVERITY_ERROR), new Integer(CategorizedProblem.CAT_BUILDPATH), JavaBuilder.SOURCE_ID});
    }
    ...
    */
            ...
            for (int i = 0, l = entries.length; i < l; i++) {
                ...
                switch (entry.getEntryKind()) {
                    case IClasspathEntry.CPE_PROJECT:
                        ...
                        if (((ClasspathEntry)entry).isOptional() && !JavaProject.hasJavaNature(
                                p)) // except if entry is optional
                        {
                            p = null;
                        }
                        break;
                    case IClasspathEntry.CPE_LIBRARY:
                        if (includeBinaryPrerequisites && path.segmentCount() > 0) {
                            // some binary resources on the class path can come from projects that are not included in the project
                            // references
    ...
    private boolean hasClasspathChanged() {
        ...
        for (n = o = 0; n < newLength && o < oldLength; n++, o++) {
            ...
            if (DEBUG) {
                System.out.println(
                        ...
                        oldSourceLocations[o]); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (n = o = 0; n < newLength && o < oldLength; n++, o++) {
            ...
            if (DEBUG) {
                System.out.println(
                        "New location: " + newBinaryLocations[n] + "\n!= old location: " +
                        oldBinaryLocations[o]); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private boolean hasStructuralDelta() {
        ...
        if (delta != null && delta.getKind() != IResourceDelta.NO_CHANGE) {
            ClasspathLocation[] classFoldersAndJars = (ClasspathLocation[])this.binaryLocationsPerProject.get(
                    this.currentProject);
    ...
    private int initializeBuilder(int kind, boolean forBuild) throws CoreException {
        ...
        if (forBuild) {
            ...
            this.participants = JavaModelManager.getJavaModelManager().compilationParticipants.getCompilationParticipants(
                    this.javaProject);
        ...
        if (forBuild) {
            ...
            char[][] filters = filterSequence != null && filterSequence.length() > 0 ? CharOperation.splitAndTrimOn(',',
                                                                                                                    filterSequence
                                                                                                                            .toCharArray())
                                                                                     : null;
            if (filters == null) {
    ...
    private boolean isWorthBuilding() throws CoreException {
        boolean abortBuilds = JavaCore.ABORT.equals(
        ...
        if (isClasspathBroken(this.javaProject.getRawClasspath(), this.currentProject)) {
            if (DEBUG) {
                System.out.println(
                        "Aborted build because project has classpath errors (incomplete or involved in cycle)"); //$NON-NLS-1$
            }
            ...
            marker.setAttributes(
                    new String[]{IMarker.MESSAGE, IMarker.SEVERITY, IJavaModelMarker.CATEGORY_ID, IMarker.SOURCE_ID},
                    new Object[]{Messages.build_abortDueToClasspathProblems, new Integer(IMarker.SEVERITY_ERROR), new Integer(
        ...
        // make sure all prereq projects have valid build states... only when aborting builds since projects in cycles do not have build
        // states
        ...
        for (int i = 0, l = requiredProjects.length; i < l; i++) {
            ...
            if (getLastState(p) == null) {
                // The prereq project has no build state: if this prereq project has a 'warning' cycle marker then allow build (see bug
                // id 23357)
                JavaProject prereq = (JavaProject)JavaCore.create(p);
                if (prereq.hasCycleMarker() && JavaCore.WARNING.equals(
                ...
                marker.setAttributes(
                        new String[]{IMarker.MESSAGE, IMarker.SEVERITY, IJavaModelMarker.CATEGORY_ID, IMarker.SOURCE_ID},
                        new Object[]{isClasspathBroken(prereq.getRawClasspath(), p) ? Messages.bind(
                                Messages.build_prereqProjectHasClasspathProblems, p.getName()) : Messages.bind(
                                Messages.build_prereqProjectMustBeRebuilt, p.getName()), new Integer(
                                IMarker.SEVERITY_ERROR), new Integer(CategorizedProblem.CAT_BUILDPATH), JavaBuilder.SOURCE_ID});
                return false;
            }
        }
    ...
    void mustPropagateStructuralChanges() {
        ...
        this.javaProject.updateCycleParticipants(new ArrayList(), cycleParticipants, this.workspaceRoot, new HashSet(3),
                                                 null);
        ...
        while (i.hasNext()) {
            ...
            if (participantPath != currentPath) {
                ...
                if (hasBeenBuilt(project)) {
                    if (DEBUG) {
                        System.out.println(
                                "Requesting another build iteration since cycle participant " + project.getName() //$NON-NLS-1$
