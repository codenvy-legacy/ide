...
public class RenameNonVirtualMethodProcessor extends RenameMethodProcessor {
    ...
    protected RefactoringStatus doCheckFinalConditions(IProgressMonitor pm,
        ...
        try {
            ...
            IMethod[] hierarchyMethods = hierarchyDeclaresMethodName(new SubProgressMonitor(pm, 1),
                                                                     declaring.newTypeHierarchy(new SubProgressMonitor(pm, 1)), method,
                                                                     name);
                ...
                if (Checks.compareParamTypes(method.getParameterTypes(), hierarchyMethod.getParameterTypes())) {
                    String message = Messages.format(
                            ...
                            new String[]{BasicElementLabels.getJavaElementName(
                                    declaring.getFullyQualifiedName('.')), BasicElementLabels.getJavaElementName(name)});
    ...
    /*
     * @see RenameMethodProcessor#addOccurrences(org.eclipse.jdt.internal.corext.refactoring.util.TextChangeManager,
