...
public final class ImportRewriteAnalyzer
{
   ...
   public ImportRewriteAnalyzer(ICompilationUnit cu, CompilationUnit root, String[] importOrder, int threshold,
   ...
   {
      return JavaCore.INSERT.equals(this.compilationUnit.getJavaProject().getOption(
         DefaultCodeFormatterConstants.FORMATTER_INSERT_SPACE_BEFORE_SEMICOLON, true));
   }
   ...
   private void addExistingImports(CompilationUnit root)
   {
      ...
      for (int i = 1; i < decls.size(); i++)
      {
         ...
         currPackage.add(
            new ImportDeclEntry(packName.length(), name, isStatic, new Region(currOffset, nextLineOffset - currOffset),
               // should not include leading comments of next import, line delimiters, etc.
         ...
         if (currEndLine < nextOffsetLine)
         {
            ...
            this.packageEntries.add(currPackage);
            currPackage.add(new ImportDeclEntry(packName.length(), null, false,
               new Region(nextLineOffset, nextOffset - nextLineOffset)));
      ...
      currPackage.add(
         new ImportDeclEntry(packName.length(), name, isStatic, new Region(curr.getStartPosition(), length),
            leadingComments, allTrailingComments));
   }
            ...
            if ((nextOffsetLine - currEndLine) > 1)
            {
               ...
               if (index < 0)
               {
                  ...
                  for (int j = -(index + 1); j < numberOfComments; j++)
                  {
                     ...
                     if ((commentStartPosition > currentExtendedEnd) && ((commentStartPosition + commentLength - 1) < extendedStart))
                     {
                        if (regions == null)
                        {
                           regions = new ArrayList();
      ...
      private boolean sameMatchLenTest(String currName)
      {
         ...
         if (newChar < currChar)
         {
            ...
            {                                    // n < b  && n < c
               ...
               { // longer match between curr and best
      ...
      try
      {
         ...
         for (int i = 0; i < nPackageEntries; i++)
         {
            ...
            for (int k = 0; k < nImports; k++)
            {
               ...
               if (region == null)
               { // new entry
                  if (!doStarImport || currDecl.isOnDemand() || (onDemandConflicts != null && onDemandConflicts.contains(
                     currDecl.getSimpleName())))
                  {
               ...
               else if (!doStarImport || currDecl.isOnDemand() || onDemandConflicts == null || onDemandConflicts.contains(
                  currDecl.getSimpleName()))
               {
   ...
   private Set evaluateStarImportConflicts(IProgressMonitor monitor) throws JavaModelException
   {
      ...
      IJavaSearchScope scope = SearchEngine.createJavaSearchScope(
         new IJavaElement[]{this.compilationUnit.getJavaProject()});
   ...
   private static final class ImportDeclEntry
   {
      ...
      public ImportDeclEntry(int containerNameLength, String elementName, boolean isStatic, IRegion sourceRange,
         IRegion precedingCommentRange, IRegion trailingCommentRange)
      {
   ...
   private final static class PackageEntry
   {
      ...
      public boolean hasStarImport(int threshold, Set explicitImports)
      {
         ...
         for (int i = 0; i < nImports; i++)
         {
            ...
            {
               count++;
               boolean isExplicit = !curr.isStatic() && (explicitImports != null) && explicitImports.contains(
                  curr.getSimpleName());
