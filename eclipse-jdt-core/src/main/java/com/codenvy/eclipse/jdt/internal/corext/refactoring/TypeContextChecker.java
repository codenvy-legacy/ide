...
import com.codenvy.eclipse.jdt.core.dom.IExtendedModifier;
...
import com.codenvy.eclipse.jdt.internal.corext.dom.ASTFlattener;
...
import com.codenvy.eclipse.jdt.internal.corext.util.TypeNameMatchCollector;
...
public class TypeContextChecker
{
   ...
   public static final String DUMMY_CU_NAME = "$$__$$.java"; //$NON-NLS-1$
   ...
   private static class MethodTypesChecker
   {
      ...
      {
         RefactoringStatus[] results = new MethodTypesSyntaxChecker(fMethod, fParameterInfos,
         ...
         boolean firstPass) throws CoreException
      {
         ...
         try
         {
            ...
            CompilationUnit compilationUnit = new RefactoringASTParser(AST.JLS4).parse(wc, true);
            ASTNode method = NodeFinder.perform(compilationUnit, offsetBeforeMethodName,
      ...
      private int appendMethodDeclaration(StringBuffer cuString, String[] types,
         int parameterCount) throws JavaModelException
      {
      ...
      private static String resolveType(String elementTypeName, RefactoringStatus status, IType declaringType,
         IProgressMonitor pm) throws CoreException
      {
         ...
         else
         {
            ...
            String[] keys = {BasicElementLabels.getJavaElementName(elementTypeName), String.valueOf(
               typeRefsFound.size())};
            String msg = Messages.format(RefactoringCoreMessages.TypeContextChecker_ambiguous, keys);
            status.addError(msg);
            return elementTypeName;
      ...
      {
         IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new IJavaProject[]{contextType.getJavaProject()},
            true);
         ...
         new SearchEngine().searchAllTypeNames(null, matchMode, typeName.toCharArray(), matchMode,
            IJavaSearchConstants.TYPE, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, pm);
   ...
   private static class MethodTypesSyntaxChecker
   {
      ...
      private RefactoringStatus checkReturnTypeSyntax()
      {
         ...
         {
            String[] keys = new String[]{BasicElementLabels.getJavaElementName(
               newTypeName), BasicElementLabels.getJavaElementName(iter.next())};
   ...
   public static RefactoringStatus[] checkAndResolveMethodTypes(IMethod method, StubTypeContext stubTypeContext,
      List<ParameterInfo> parameterInfos, ReturnTypeInfo returnTypeInfo) throws CoreException
   {
   ...
   public static RefactoringStatus checkParameterTypeSyntax(String type, IJavaProject project)
   {
      ...
      for (Iterator<String> iter = problemsCollector.iterator(); iter.hasNext(); )
      {
         String msg = Messages.format(RefactoringCoreMessages.TypeContextChecker_invalid_type_syntax,
            new String[]{BasicElementLabels.getJavaElementName(newTypeName), BasicElementLabels.getJavaElementName(
               iter.next())});
         result.addError(msg);
      }
      return result;
   ...
   public static StubTypeContext createSuperClassStubTypeContext(String typeName, IType enclosingType,
      IPackageFragment packageFragment)
   {
      return createSupertypeStubTypeContext(typeName, false, enclosingType, packageFragment);
   }

   private static StubTypeContext createSupertypeStubTypeContext(String typeName, boolean isInterface,
      ...
      if (enclosingType != null)
      {
         try
         {
            ...
            stubTypeContext = new StubTypeContext(stubTypeContext.getCuHandle(),
               stubTypeContext.getBeforeString() + prolog, epilog + stubTypeContext.getAfterString());
         }
         catch (CoreException e)
         {
            Util.log(e);
