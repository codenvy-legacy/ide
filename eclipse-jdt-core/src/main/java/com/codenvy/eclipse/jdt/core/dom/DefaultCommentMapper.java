...
class DefaultCommentMapper
{
   ...
   int storeLeadingComments(ASTNode node, int previousEnd, int[] parentLineRange)
   {
      ...
      if (startIdx != -1)
      {
         ...
         if (previousEnd < commentStart && previousEndLine != nodeStartLine)
         {
            ...
            while (startIdx < length && lastTokenLine == getLineNumber(this.comments[startIdx].getStartPosition(),
               parentLineRange) && nodeStartLine != lastTokenLine)
            {
               startIdx++;
            }
         }
   ...
   class CommentMapperVisitor extends DefaultASTVisitor
   {
      ...
      protected void endVisitNode(ASTNode node)
      {
         ...
         if (this.topSiblingParent != null /*not a CompilationUnit*/ && this.topSiblingParent == node)
         {
            this.siblingPtr--;
