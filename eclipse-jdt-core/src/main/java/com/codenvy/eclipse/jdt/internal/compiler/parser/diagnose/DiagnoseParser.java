 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.compiler.parser.diagnose;
...
import com.codenvy.eclipse.jdt.internal.compiler.parser.ParserBasicInformation;
...
public class DiagnoseParser implements ParserBasicInformation, TerminalTokens {
        ...
        try {
            ...
            do {
                ...
                while (act > ERROR_ACTION || act < ACCEPT_ACTION) { // SHIFT-REDUCE action or SHIFT action ?
                    ...
                    //				if((tok != TokenNameRBRACE || (forceRecoveryToken != currentToken && (lexStream.flags(currentToken) & LexStream
                    // .LBRACE_MISSING) != 0))
    ...
    private PrimaryRepairInfo checkPrimaryDistance(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        //
        j = parseCheck(stck, stack_top, this.lexStream.kind(this.buffer[repair.bufferPosition + 1]),
                       repair.bufferPosition + 2);
        if (this.lexStream.kind(this.buffer[repair.bufferPosition]) == EOLT_SYMBOL && this.lexStream.afterEol(
    ...
    private void scopeTrialCheck(int stck[], int stack_top, PrimaryRepairInfo repair, int indx) {
        ...
        for (int i = 0; i < SCOPE_SIZE; i++) {
            ...
            if (act != ERROR_ACTION) {
                ...
                for (j = this.tempStackTop + 1; j >= (max_pos + 1) && Parser.in_symbol(
                        this.tempStack[j]) == Parser.scope_rhs[k]; j--) {
                    k++;
                }
                if (j == max_pos) {
                ...
                if (Parser.scope_rhs[k] == 0 && j < marked_pos) { // match?
                    ...
                    for (j = Parser.scope_state_set[i]; stck[stack_position] != Parser.scope_state[j] && Parser.scope_state[j] != 0;
                         j++) {/*empty*/}
                    //
                    ...
                    if (Parser.scope_state[j] != 0) {     // state was found
                        ...
                        int distance = parseCheck(stck, stack_position, Parser.scope_lhs[i] + NT_OFFSET,
                        ...
                        if (this.lexStream.kind(
                                this.buffer[repair.bufferPosition]) == EOFT_SYMBOL && repair.distance == previous_distance) {
                            this.scopeStackTop = indx;
    ...
    private RepairCandidate secondaryPhase(int error_token) {
        ...
        if (this.nextStackTop >= 0) {
            ...
            // index).
            //
            for (next_last_index = MAX_DISTANCE - 1; next_last_index >= 1 && this.lexStream.kind(
                    this.buffer[next_last_index]) == EOFT_SYMBOL; next_last_index--) {/*empty*/}
        ...
        for (last_index = MAX_DISTANCE - 1; last_index >= 1 && this.lexStream.kind(
                this.buffer[last_index]) == EOFT_SYMBOL; last_index--) {/*empty*/}
        last_index++;
        ...
        if (repair.code == SECONDARY_CODE || repair.code == DELETION_CODE) {
            ...
            for (scope_repair.bufferPosition = 2; scope_repair.bufferPosition <= repair.bufferPosition && repair.code != SCOPE_CODE;
        ...
        switch (repair.code) {
            case SCOPE_CODE: {
                if (repair.stackPosition < this.stateStackTop) {
                    reportError(DELETION_CODE, Parser.terminal_index[ERROR_SYMBOL], this.locationStack[repair.stackPosition],
                                this.buffer[1]);
                }
                for (int i = 0; i < this.scopeStackTop; i++) {
