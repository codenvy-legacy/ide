...
public class DeleteModifications extends RefactoringModifications {
    ...
    public RefactoringParticipant[] loadParticipants(RefactoringStatus status, RefactoringProcessor owner,
        ...
        for (Iterator<IJavaElement> iter = fDelete.iterator(); iter.hasNext(); ) {
            result.addAll(Arrays.asList(
                    ParticipantManager.loadDeleteParticipants(status, owner, iter.next(), new DeleteArguments(), natures,
                                                              shared)));
        }
     ...
     *         a collector for IResources to be deleted
     * @throws com.codenvy.eclipse.core.runtime.CoreException
     *
     */
    private void handlePackageFragmentDelete(IPackageFragment pack,
                                             ArrayList<IResource> resourcesCollector) throws CoreException {
