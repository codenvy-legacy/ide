...
import com.codenvy.eclipse.jdt.internal.corext.codemanipulation.GetterSetterUtil;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.participants.ResourceProcessors;
...
import com.codenvy.eclipse.jdt.internal.corext.util.Resources;
...
import com.codenvy.eclipse.ltk.core.refactoring.participants.DeleteProcessor;
...
public final class JavaDeleteProcessor extends DeleteProcessor {
    ...
    private void checkDirtyFile(RefactoringStatus result, IFile file) {
        ...
        if (buffer != null && buffer.isDirty()) {
            if (buffer.isStateValidated() && buffer.isSynchronized()) {
                result.addWarning(Messages.format(RefactoringCoreMessages.JavaDeleteProcessor_unsaved_changes,
                                                  BasicElementLabels.getPathLabel(file.getFullPath(), false)));
            } else {
                result.addFatalError(Messages.format(RefactoringCoreMessages.JavaDeleteProcessor_unsaved_changes,
                                                     BasicElementLabels.getPathLabel(file.getFullPath(), false)));
            }
        }
    }
    ...
    private void addSubPackages() throws JavaModelException {
        ...
        for (int i = 0; i < fJavaElements.length; i++) {
            if (fJavaElements[i] instanceof IPackageFragment) {
                javaElements.addAll(
                        Arrays.asList(JavaElementUtil.getPackageAndSubpackages((IPackageFragment)fJavaElements[i])));
            } else {
    ...
    private void addDeletableParentPackagesOnPackageDeletion() throws CoreException {
        ...
        final List<IPackageFragment> initialPackagesToDelete = (List<IPackageFragment>)ReorgUtils.getElementsOfType(
                fJavaElements, IJavaElement.PACKAGE_FRAGMENT);
     ...
     *         should not happen
     */
    private boolean canRemoveCompletely(IPackageFragment pack,
                                        List<IPackageFragment> packagesToDelete) throws JavaModelException {
     ...
     *         the initial packages to delete
     * @param resourcesToDelete
     *         result to add resources to delete
     * @param deletableParentPackages
     *         result ro add deletable parent packages
     * @throws com.codenvy.eclipse.core.runtime.CoreException
     *         should not happen
     */
    private void addDeletableParentPackages(IPackageFragment frag, List<IPackageFragment> initialPackagesToDelete,
                                            Set<IResource> resourcesToDelete, List<IPackageFragment> deletableParentPackages)
            throws CoreException {

        if (frag.getResource().isLinked()) {
            final IConfirmQuery query = fDeleteQueries.createYesNoQuery(
                    RefactoringCoreMessages.JavaDeleteProcessor_confirm_linked_folder_delete, false,
                    IReorgQueries.CONFIRM_DELETE_LINKED_PARENT);
            if (!query.confirm(Messages.format(RefactoringCoreMessages.JavaDeleteProcessor_delete_linked_folder_question,
                                               BasicElementLabels.getResourceName(frag.getResource())))) {
                return;
            }
        }
    ...
    private void removeUnconfirmedReferencedArchiveFiles(
            IConfirmQuery query) throws JavaModelException, OperationCanceledException {
    ...
    private void removeUnconfirmedReferencedPackageFragmentRoots(
            IConfirmQuery query) throws JavaModelException, OperationCanceledException {
        ...
        for (int i = 0; i < fJavaElements.length; i++) {
            ...
            ArrayList<IJavaProject> referencingProjects = new ArrayList<IJavaProject>(
                    Arrays.asList(JavaElementUtil.getReferencingProjects(root)));
    ...
    private static boolean skipDeletingReferencedRoot(IConfirmQuery query, IPackageFragmentRoot root,
                                                      List<IJavaProject> referencingProjects) throws OperationCanceledException {
        ...
        String question = referencingProjects.size() == 1 ? Messages.format(
                RefactoringCoreMessages.DeleteRefactoring_3_singular, label) : Messages.format(
    ...
    private void removeUnconfirmedFoldersThatContainSourceFolders() throws CoreException {
        ...
        IConfirmQuery query = fDeleteQueries.createYesYesToAllNoNoToAllQuery(queryTitle, true,
                                                                             IReorgQueries
                                                                                     .CONFIRM_DELETE_FOLDERS_CONTAINING_SOURCE_FOLDERS);
        ...
        for (int i = 0; i < fResources.length; i++) {
            ...
            if (resource instanceof IFolder) {
                ...
                if (containsSourceFolder(folder)) {
                    String question = Messages.format(RefactoringCoreMessages.DeleteRefactoring_5,
                                                      BasicElementLabels.getResourceName(folder));
    ...
    public Change createChange(IProgressMonitor monitor) throws CoreException {
        try {
            ...
            final String description = fElements.length == 1 ? RefactoringCoreMessages.JavaDeleteProcessor_description_singular
                                                             : RefactoringCoreMessages.JavaDeleteProcessor_description_plural;
            final IProject resource = getSingleProject();
            final String project = resource != null ? resource.getName() : null;
            final String source = project != null ? Messages.format(
                    RefactoringCoreMessages.JavaDeleteProcessor_project_pattern,
                    BasicElementLabels.getJavaElementName(project)) : RefactoringCoreMessages.JavaDeleteProcessor_workspace;
            final String header = fElements.length == 1 ? Messages.format(
                    RefactoringCoreMessages.JavaDeleteProcessor_header_singular, source) : Messages.format(
                    RefactoringCoreMessages.JavaDeleteProcessor_header_plural,
            ...
            final DeleteDescriptor descriptor = RefactoringSignatureDescriptorFactory.createDeleteDescriptor(project,
                                                                                                             description,
                                                                                                             comment.asString(), arguments,
                                                                                                             flags);
            ...
            for (int offset = 0; offset < fResources.length; offset++) {
                arguments.put(JavaRefactoringDescriptorUtil.ATTRIBUTE_ELEMENT + (offset + 1),
                              JavaRefactoringDescriptorUtil.resourceToHandle(project, fResources[offset]));
            }
            ...
            for (int offset = 0; offset < fJavaElements.length; offset++) {
                arguments.put(JavaRefactoringDescriptorUtil.ATTRIBUTE_ELEMENT + (offset + fResources.length + 1),
                              JavaRefactoringDescriptorUtil.elementToHandle(project, fJavaElements[offset]));
            }
            return new DynamicValidationRefactoringChange(descriptor, RefactoringCoreMessages.DeleteRefactoring_7,
    ...
    private List<IMethod> getGettersSettersToDelete(Map<IField, IMethod[]> getterSetterMapping) {
        ...
        IConfirmQuery getterSetterQuery = fDeleteQueries.createYesYesToAllNoNoToAllQuery(queryTitle, true,
                                                                                         IReorgQueries.CONFIRM_DELETE_GETTER_SETTER);
        for (Iterator<IField> iter = getterSetterMapping.keySet().iterator(); iter.hasNext(); ) {
            ...
            String deleteGetterSetter = Messages.format(RefactoringCoreMessages.DeleteRefactoring_9,
                                                        JavaElementUtil.createFieldSignature(field));
    ...
    private RefactoringStatus initialize(JavaRefactoringArguments extended) {
        ...
        if (value != null && !"".equals(value)) {//$NON-NLS-1$
            ...
            return RefactoringStatus.createFatalErrorStatus(
                    Messages.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_RESOURCES));
        }
        ...
        if (value != null && !"".equals(value)) {//$NON-NLS-1$
            ...
            return RefactoringStatus.createFatalErrorStatus(
                    Messages.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_ELEMENTS));
        }
        String handle = null;
        ...
        for (int index = 0; index < resourceCount; index++) {
            ...
            if (handle != null && !"".equals(handle)) { //$NON-NLS-1$
                ...
                if (resource == null || !resource.exists()) {
                    status.merge(JavaRefactoringDescriptorUtil.createInputWarningStatus(resource, getProcessorName(),
        ...
        for (int index = 0; index < elementCount; index++) {
            ...
            if (handle != null && !"".equals(handle)) { //$NON-NLS-1$
                ...
                if (element == null || !element.exists()) {
                    status.merge(JavaRefactoringDescriptorUtil.createInputWarningStatus(element, getProcessorName(),
                                                                                        IJavaRefactorings.DELETE));
                } else {
