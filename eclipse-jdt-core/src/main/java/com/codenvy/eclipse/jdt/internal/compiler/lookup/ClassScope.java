...
public class ClassScope extends Scope {
    ...
    void buildFields() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (field.getKind() == AbstractVariableDeclaration.INITIALIZER) {
                ...
                FieldBinding fieldBinding = new FieldBinding(field, null,
                                                             field.modifiers | ExtraCompilerModifiers.AccUnresolved, sourceType);
        ...
        sourceType.tagBits &=
                ~(TagBits.AreFieldsSorted | TagBits.AreFieldsComplete); // in case some static imports reached already into this type
    ...
    void buildMethods() {
        ...
        sourceType.tagBits &=
                ~(TagBits.AreMethodsSorted | TagBits.AreMethodsComplete); // in case some static imports reached already into this type
    ...
    SourceTypeBinding buildType(SourceTypeBinding enclosingType, PackageBinding packageBinding,
        ...
        if (enclosingType == null) {
            ...
            className[className.length - 1] = CharOperation.concat(className[className.length - 1],
                                                                   this.referenceContext.name, '$');
    ...
    private void checkAndSetModifiers() {
        ...
        if ((realModifiers & ClassFileConstants.AccInterface) != 0) { // interface and annotation type
            ...
            if (!sourceType.isAnonymousType()) {
                checkAbstractEnum:
                {
                    ...
                    // tag this enum as abstract since an abstract method must be implemented AND all enum constants define an anonymous
                    // body
        ...
        if (isMemberType) {
            ...
            if (enclosingType.isInterface()) {
                ...
                int accessorBits =
                        realModifiers & (ClassFileConstants.AccPublic | ClassFileConstants.AccProtected | ClassFileConstants.AccPrivate);
    ...
    */
    private void checkAndSetModifiersForField(FieldBinding fieldBinding, FieldDeclaration fieldDecl) {
        ...
        if (declaringClass.isInterface()) {
            ...
            final int IMPLICIT_MODIFIERS =
                    ClassFileConstants.AccPublic | ClassFileConstants.AccStatic | ClassFileConstants.AccFinal | ClassFileConstants.AccEnum |
    ...
    public void checkParameterizedSuperTypeCollisions() {
        ...
        for (int i = 0, length = interfaces.length; i < length; i++) {
            ...
            if (itsSuperclass != null && hasErasedCandidatesCollisions(itsSuperclass, one, invocations, sourceType,
    ...
    */
    private boolean connectSuperclass() {
        ...
        if (this.referenceContext.superclass == null) {
            if (sourceType.isEnum() && compilerOptions().sourceLevel >=
                                       ClassFileConstants.JDK1_5) // do not connect if source < 1.5 as enum already got flagged as syntax
                                       // error
            {
    ...
    */
    private boolean connectSuperInterfaces() {
        ...
        if (this.referenceContext.superInterfaces == null) {
            if (sourceType.isAnnotationType() && compilerOptions().sourceLevel >=
                                                 ClassFileConstants.JDK1_5) { // do not connect if source < 1.5 as annotation already got flagged as syntax error) {
    ...
    private boolean detectHierarchyCycle(SourceTypeBinding sourceType, ReferenceBinding superType,
        ...
        if (superType.isHierarchyBeingActivelyConnected()) {
            com.codenvy.eclipse.jdt.internal.compiler.ast.TypeReference ref = ((SourceTypeBinding)superType).scope.superTypeReference;
