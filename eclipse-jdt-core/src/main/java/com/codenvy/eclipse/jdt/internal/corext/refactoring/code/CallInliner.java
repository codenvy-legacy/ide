...
import com.codenvy.eclipse.jdt.internal.corext.dom.TypeBindingVisitor;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.util.NoCommentSourceRangeComputer;
...
public class CallInliner {
    ...
    public CallInliner(ICompilationUnit unit, CompilationUnit targetAstRoot,
                       SourceProvider provider) throws CoreException {
        super();
        fCUnit = unit;
    ...
    private void checkMethodDeclaration(RefactoringStatus result, int severity) {
        ...
        if (fSourceProvider.hasSuperMethodInvocation() && fInvocation.getNodeType() == ASTNode.METHOD_INVOCATION) {
            ...
            if (receiver instanceof ThisExpression) {
                result.addEntry(
                        new RefactoringStatusEntry(severity, RefactoringCoreMessages.CallInliner_super_into_this_expression,
                                                   JavaStatusContext.create(fCUnit, fInvocation)));
            }
        }
    }
    ...
    private void addEntry(RefactoringStatus result, String message, int code, int severity) {
        result.addEntry(
                new RefactoringStatusEntry(severity, message, JavaStatusContext.create(fCUnit, fInvocation), "IDE", code,
                                           null));
    }
    ...
    private void computeRealArguments() {
        ...
        for (int i = 0; i < (needsVarargBoxing ? varargIndex : arguments.size()); i++) {
            ...
            if (canInline(expression, parameter) && !canNotInline.contains(expression)) {
                ...
                fLocals.add(createLocalDeclaration(parameter.getTypeBinding(), name,
    ...
    private void computeReceiver() throws BadLocationException {
        ...
        switch (fSourceProvider.getReceiversToBeUpdated()) {
            case 0:
                ...
                // local.
                fLocals.add(createLocalDeclaration(receiver.resolveTypeBinding(), fInvocationScope.createName("r", true),
                                                   //$NON-NLS-1$
    ...
    private boolean needsExplicitCast(RefactoringStatus status) {
        ...
        if (fTargetNode.getLocationInParent() == MethodInvocation.ARGUMENTS_PROPERTY) {
            ...
            TypeBindingVisitor visitor = new AmbiguousMethodAnalyzer(fTypeEnvironment, method,
                                                                     fTypeEnvironment.create(parameters));
    ...
    private void initializeInsertionPoint(int nos) {
        ...
        ASTNode parentStatement = fInvocation instanceof Statement ? fInvocation : ASTNodes.getParent(fInvocation,
                                                                                                      Statement.class);
    ...
    private boolean needsBlockAroundDanglingIf() {
		 ...
		 */
        return fTargetNode.getLocationInParent() == IfStatement.THEN_STATEMENT_PROPERTY && fTargetNode.getParent().getStructuralProperty(
                IfStatement.ELSE_STATEMENT_PROPERTY) != null && fSourceProvider.isDangligIf();
    }
