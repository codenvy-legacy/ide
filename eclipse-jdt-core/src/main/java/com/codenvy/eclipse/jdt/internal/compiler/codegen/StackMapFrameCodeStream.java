...
public class StackMapFrameCodeStream extends CodeStream {
    ...
    public static class StackDepthMarker {
        ...
        public String toString() {
            ...
            if (this.typeBinding != null) {
                buffer.append(',').append(this.typeBinding.qualifiedPackageName()).append(
                        this.typeBinding.qualifiedSourceName());
            }
            buffer.append(')');
            return String.valueOf(buffer);
        }
    ...
    public static class StackMarker {
        ...
        public String toString() {
            StringBuffer buffer = new StringBuffer();
            buffer.append("[copy stack items from ") //$NON-NLS-1$
                    .append(this.pc).append(" to ") //$NON-NLS-1$
    ...
    public void addExceptionMarker(int pc, TypeBinding typeBinding) {
        ...
        if (typeBinding == null) {
            ...
            switch (typeBinding.id) {
                case TypeIds.T_null:
                    this.exceptionMarkers.add(
                            new ExceptionMarker(pc, ConstantPool.JavaLangClassNotFoundExceptionConstantPoolName));
                    break;
                case TypeIds.T_long:
                    this.exceptionMarkers.add(
                            new ExceptionMarker(pc, ConstantPool.JavaLangNoSuchFieldErrorConstantPoolName));
                    break;
                default:
