...
import com.codenvy.eclipse.jdt.internal.corext.dom.ScopeAnalyzer;
...
public class ImportReferencesCollector extends GenericVisitor
{
   ...
   public static void collect(ASTNode node, IJavaProject project, Region rangeLimit,
   ...
   public static void collect(ASTNode node, IJavaProject project, Region rangeLimit, boolean skipMethodBodies,
      Collection<SimpleName> resultingTypeImports, Collection<SimpleName> resultingStaticImports)
   {
      ASTNode root = node.getRoot();
      CompilationUnit astRoot = root instanceof CompilationUnit ? (CompilationUnit)root : null;
      node.accept(new ImportReferencesCollector(project, astRoot, rangeLimit, skipMethodBodies, resultingTypeImports,
   ...
   private ImportReferencesCollector(IJavaProject project, CompilationUnit astRoot, Region rangeLimit,
      boolean skipMethodBodies, Collection<SimpleName> resultingTypeImports,
   ...
   private void possibleStaticImportFound(Name name)
   {
      ...
      if (binding == null || binding instanceof ITypeBinding || !Modifier.isStatic(
         binding.getModifiers()) || simpleName.isDeclaration())
      {
         return;
      }
      ...
      if (binding instanceof IVariableBinding)
      {
         ...
         if (varBinding.isField())
         {
            ...
            {
               if (new ScopeAnalyzer(fASTRoot).isDeclaredInScope(varBinding, simpleName,
      ...
      else if (binding instanceof IMethodBinding)
      {
         ...
         if (declaringClass != null && !declaringClass.isLocal())
         {
            if (new ScopeAnalyzer(fASTRoot).isDeclaredInScope(methodBinding, simpleName,
