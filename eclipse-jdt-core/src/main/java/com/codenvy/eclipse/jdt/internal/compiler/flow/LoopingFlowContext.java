...
public class LoopingFlowContext extends SwitchFlowContext
{
   ...
   static private class EscapingExceptionCatchSite
   {
      ...
      {
         this.catchingContext.recordHandlingException(this.caughtException, flowInfo.unconditionalInits(), null,
            // raised exception, irrelevant here,
   ...
   public void complainOnDeferredNullChecks(BlockScope scope, FlowInfo callerFlowInfo)
   {
      for (int i = 0; i < this.innerFlowContextsCount; i++)
      {
         this.upstreamNullFlowInfo.
            addPotentialNullInfoFrom(this.innerFlowContexts[i].upstreamNullFlowInfo).
         ...
         for (int i = 0; i < this.nullCount; i++)
         {
            ...
            switch (this.nullCheckTypes[i] & ~HIDE_NULL_COMPARISON_WARNING_MASK)
            {
                  ...
                  if (flowInfo.isDefinitelyNull(local))
                  {
                     ...
                     switch (this.nullCheckTypes[i] & CONTEXT_MASK)
                     {
                        case FlowContext.IN_COMPARISON_NULL:
                           if (((this.nullCheckTypes[i] & CHECK_MASK & ~HIDE_NULL_COMPARISON_WARNING_MASK) == CAN_ONLY_NULL) && (expression.implicitConversion & TypeIds.UNBOXING) != 0)
                           { // check for auto-unboxing first and report appropriate warning
               ...
               case ASSIGN_TO_NONNULL:
                  ...
                  {
                     this.parent.recordNullityMismatch(scope, (Expression)location, this.providedExpectedTypes[i][0],
                        this.providedExpectedTypes[i][1], nullStatus);
                  }
                  break;
               case EXIT_RESOURCE:
            ...
            switch (this.nullCheckTypes[i] & ~HIDE_NULL_COMPARISON_WARNING_MASK)
            {
               ...
               case ASSIGN_TO_NONNULL:
                  ...
                  if (nullStatus != FlowInfo.NON_NULL)
                  {
                     ...
                     scope.problemReporter().nullityMismatch((Expression)location, this.providedExpectedTypes[i][0],
                        this.providedExpectedTypes[i][1], nullStatus, annotationName);
                  }
                  break;
               case EXIT_RESOURCE:
   ...
   public void recordContinueFrom(FlowContext innerFlowContext, FlowInfo flowInfo)
   {
      if ((flowInfo.tagBits & FlowInfo.UNREACHABLE_OR_DEAD) == 0)
      {
         ...
         {
            this.upstreamNullFlowInfo.
               addPotentialNullInfoFrom(flowInfo.unconditionalInitsWithoutSideEffect());
         }
         else
         {
            int length = 0;
   ...
   protected void recordNullReference(LocalVariableBinding local, ASTNode expression, int status)
   {
      if (this.nullCount == 0)
      {
      ...
      FlowInfo flowInfo)
   {
      ...
      switch (checkTypeWithoutHideNullWarning)
      {
         ...
         case CAN_ONLY_NULL_NON_NULL | IN_COMPARISON_NON_NULL:
            ...
            else if (this.upstreamNullFlowInfo.isDefinitelyNonNull(local) && !flowInfo.isPotentiallyNull(
               local) && !flowInfo.isPotentiallyUnknown(local))
            {
            ...
            else
            {
               ...
               else if (flowInfo.isPotentiallyNull(local))
               {
                  ...
                  recordNullReference(local, reference,
                     CAN_ONLY_NULL | checkType & (CONTEXT_MASK | HIDE_NULL_COMPARISON_WARNING_MASK));
               }
               else
               {
                  recordNullReference(local, reference, checkType);
         ...
         case CAN_ONLY_NULL | IN_INSTANCEOF:
            ...
            if (flowInfo.isPotentiallyNonNull(local) || flowInfo.isPotentiallyUnknown(
               local) || flowInfo.isProtectedNonNull(local))
            {
