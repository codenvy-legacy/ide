...
public class CorrectionEngine {
     ...
     *         the given correction requestor
     * @throws IllegalArgumentException
     *         if <code>targetUnit</code> or <code>requestor</code> is <code>null</code>
     * @throws JavaModelException
     *         currently this exception is never thrown, but the opportunity to thrown an exception
     *         when the correction failed is kept for later.
     * @since 2.0
                    ...
                    if (!(Flags.isEnum(flags) || Flags.isAnnotation(flags))) {
                        if ((CorrectionEngine.this.filter & (CLASSES | INTERFACES)) != 0) {
                            ...
                            CorrectionEngine.this.correctionRequestor.acceptClass(proposal.getDeclarationSignature(),
                                                                                  ...
                                                                                  CharOperation.subarray(completionName,
                                                                                                         CorrectionEngine.this.prefixLength,
                                                                                                         completionName.length),
                                                                                  proposal.getFlags(),
                            ...
                            CorrectionEngine.this.correctionRequestor.acceptClass(packageName, className,
                                                                                  CharOperation.subarray(fullName,
                                                                                                         CorrectionEngine.this.prefixLength,
                                                                                                         fullName.length),
                                                                                  proposal.getFlags(),
                                                                                  CorrectionEngine.this.correctionStart,
                ...
                case CompletionProposal.PACKAGE_REF:
                    if ((CorrectionEngine.this.filter & (CLASSES | INTERFACES | IMPORT)) != 0) {
                        ...
                        CorrectionEngine.this.correctionRequestor.acceptPackage(packageName,
                                                                                CharOperation.subarray(packageName,
                                                                                                       CorrectionEngine.this.prefixLength,
                                                                                                       packageName.length),
                                                                                CorrectionEngine.this.correctionStart,
                                                                                CorrectionEngine.this.correctionEnd);
