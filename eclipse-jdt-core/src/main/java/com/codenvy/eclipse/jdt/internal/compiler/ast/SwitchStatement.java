...
import com.codenvy.eclipse.jdt.internal.compiler.codegen.CaseLabel;
...
import com.codenvy.eclipse.jdt.internal.compiler.flow.SwitchFlowContext;
        ...
        try {
            ...
            if (this.statements != null) {
                int initialComplaintLevel =
                        (flowInfo.reachMode() & FlowInfo.UNREACHABLE) != 0 ? Statement.COMPLAINED_FAKE_REACHABLE : Statement.NOT_COMPLAINED;
    ...
    public void generateCodeForStringSwitch(BlockScope currentScope, CodeStream codeStream) {
        ...
        try {
            ...
            for (int i = 0, max = this.caseCount; i < max; i++) {
                this.cases[i].targetLabel = (sourceCaseLabels[i] = new BranchLabel(
                        codeStream));  // A branch label, not a case label.
            ...
            if (uniqHashCount != this.caseCount) { // multiple keys hashed to the same value.
                ...
                System.arraycopy(hashCodeCaseLabels, 0, hashCodeCaseLabels = new CaseLabel[uniqHashCount], 0,
                                 uniqHashCount);
            }
    ...
    public void resolve(BlockScope upperScope) {
        try {
                ...
                checkType:
                {
                    if (!expressionType.isValidBinding()) {
                        ...
                        if (compilerOptions.complianceLevel < ClassFileConstants.JDK1_5) {
                            upperScope.problemReporter().incorrectSwitchType(this.expression,
                                                                             expressionType); // https://bugs.eclipse.org/bugs/show_bug
                                                                             // .cgi?id=360317
                        }
                        break checkType;
                        ...
                        break checkType;
                    } else if (compilerOptions.complianceLevel >= ClassFileConstants.JDK1_7 &&
            ...
            if (isStringSwitch) {
                ...
                this.dispatchStringCopy = new LocalVariableBinding(SecretStringVariableName, upperScope.getJavaLangString(),
