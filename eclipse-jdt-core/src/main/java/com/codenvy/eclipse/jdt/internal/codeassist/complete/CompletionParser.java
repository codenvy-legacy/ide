 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.codeassist.complete;
...
import com.codenvy.eclipse.jdt.internal.compiler.parser.JavadocParser;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredBlock;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredElement;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredField;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredInitializer;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredLocalVariable;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredMethod;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredType;
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveredUnit;
...
public class CompletionParser extends AssistParser {
            ...
            if ((!isInsideMethod() && !isInsideFieldInitialization())) {
                if (this.genericsPtr > -1 && this.genericsLengthPtr > -1 && this.genericsIdentifiersLengthPtr > -1) {
                    ...
                    if (this.assistNodeParent != null && this.assistNodeParent instanceof TypeReference) {
                        if (this.currentElement instanceof RecoveredType) {
                            this.currentElement =
                                    this.currentElement.add(new CompletionOnFieldType((TypeReference)this.assistNodeParent, false), 0);
                        } else {
        ...
        if (kind != 0) {
            ...
            switch (kind) {
                ...
                case K_BETWEEN_IF_AND_RIGHT_PAREN:
                    IfStatement ifStatement = new IfStatement(expression, new EmptyStatement(expression.sourceEnd, expression.sourceEnd),
                            ...
                            if (this.currentElement instanceof RecoveredType) {
                                ...
                                if (prevKind == K_BETWEEN_NEW_AND_LEFT_BRACKET) {
                                    ...
                                    if (this.expressionPtr > -1 && this.expressionStack[this.expressionPtr] instanceof AllocationExpression
                                        && this.invocationType ==
                                           QUALIFIED_ALLOCATION) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=361963
    ...
    private boolean checkClassLiteralAccess() {
        if (this.identifierLengthPtr >= 1 &&
            ...
            if ((length = this.identifierLengthStack[this.identifierLengthPtr - 1]) < 0) {
                ...
                //typeRef.sourceEnd = typeRef.sourceStart + typeRef.token.length; // NB: It's ok to use the length of the token since it
                // doesn't contain any unicode
    ...
    private boolean checkInvocation() {
        ...
        if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER
            && ((isEmptyNameCompletion = topExpression == this.assistNode && isEmptyNameCompletion())
                // e.g. it is something like "this.fred([cursor]" but it is not something like "this.fred(1 + [cursor]"
                || (isEmptyAssistIdentifier = this.indexOfAssistIdentifier() >= 0 && this.identifierStack[this.identifierPtr].length ==
                                                                                     0))) { // e.g. it is something like "this.fred(1
                                                                                     // [cursor]"
        ...
        if (isInsideMethod() || isInsideFieldInitialization()) {
            ...
            CompletionOnBranchStatementLabel statementLabel =
                    new CompletionOnBranchStatementLabel(
                            kind == K_INSIDE_BREAK_STATEMENT ? CompletionOnBranchStatementLabel.BREAK
		...
		/* check if on line with an error already - to avoid completing inside
            ...
            if (this.lastErrorEndPosition <= this.cursorLocation
                && Util.getLineNumber(this.lastErrorEndPosition, this.scanner.lineEnds, 0, this.scanner.linePtr)
                   == Util.getLineNumber(((CompletionScanner)this.scanner).completedIdentifierStart, this.scanner.lineEnds, 0,
 ...
 */
    ...
    protected void consumeCastExpressionWithPrimitiveType() {
        ...
        this.expressionPtr--;
        this.expressionLengthPtr--;
        this.expressionStack[this.expressionPtr] = cast = new CastExpression(exp = this.expressionStack[this.expressionPtr + 1], castType =
 ...
 */
        ...
        if (isInsideMethod() || isInsideFieldInitialization() || isInsideAttributeValue()) {
            switch (token) {
                ...
                case TokenNamenew:
                    ...
                    this.qualifier =
                            this.expressionPtr; // NB: even if there is no qualification, set it to the expression ptr so that the number
                            // of arguments are correctly computed
                        ...
                        case TokenNameUNSIGNED_RIGHT_SHIFT: //or Fred<X<X<X>>>[(]1, 2)
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                                ...
                                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BINARY_OPERATOR &&
                                    ((info = topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER, 1)) == GREATER || info == RIGHT_SHIFT ||
    ...
    public TypeReference createParameterizedQualifiedAssistTypeReference(char[][] previousIdentifiers, TypeReference[][] typeArguments,
                                                                         char[] assistName, TypeReference[] assistTypeArguments,
                                                                         long[] positions) {
