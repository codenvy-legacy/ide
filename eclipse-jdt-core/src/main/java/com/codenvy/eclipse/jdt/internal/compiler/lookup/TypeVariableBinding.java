...
public class TypeVariableBinding extends ReferenceBinding {
    ...
    public int boundCheck(Substitution substitution, TypeBinding argumentType) {
            ...
            switch (wildcard.boundKind) {
                case Wildcard.EXTENDS:
                    ...
                    if (!wildcardBound.isInterface()) {
                        TypeBinding substitutedSuperType = hasSubstitution ? Scope.substitute(substitution,
                ...
                case Wildcard.SUPER:
                    // if the wildcard is lower-bounded by a type variable that has no relevant upper bound there's nothing to check here
                    // (bug 282152):
        ...
        for (int i = 0, length = this.superInterfaces.length; i < length; i++) {
            TypeBinding substitutedSuperType = hasSubstitution ? Scope.substitute(substitution,
                                                                                  this.superInterfaces[i]) : this.superInterfaces[i];
            if (substitutedSuperType != argumentType) {
                if (!argumentType.isCompatibleWith(substitutedSuperType)) {
                    return TypeConstants.MISMATCH;
                }
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.lookup.ReferenceBinding#readableName() */
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.lookup.ReferenceBinding#shortReadableName() */
