...
public abstract class PatternLocator implements IIndexConstants {
    ...
    protected void matchReportReference(ASTNode reference, IJavaElement element, Binding elementBinding, int accuracy, MatchLocator locator)
        ...
        switch (referenceType) {
            ...
            case IJavaElement.TYPE:
                this.match = locator.newTypeReferenceMatch(element, elementBinding, accuracy, offset, reference.sourceEnd - offset + 1,
                                                           reference);
                break;
            case IJavaElement.FIELD:
                this.match =
                        locator.newFieldReferenceMatch(element, null, elementBinding, accuracy, offset, reference.sourceEnd - offset + 1,
                                                       reference);
                break;
     ...
     *
     * @param flavors
     *         Bits mask specifying the flavors to be accepted or
    ...
    protected void updateMatch(ParameterizedTypeBinding parameterizedBinding, char[][][] patternTypeArguments,
        ...
        boolean isRaw =
                parameterizedBinding.isRawType() || (argumentsBindings == null && parameterizedBinding.genericType().isGenericType());
...
//	return resolveLevelForType(qualifiedPattern(simpleNamePattern, qualificationPattern), type);
    ...
    protected int resolveLevelForType(char[] simpleNamePattern,
        ...
        if (enclosingType != null && enclosingType.isParameterizedType() && depth < patternTypeArguments.length &&
            ...
            int enclosingLevel =
                    resolveLevelForType(enclosingSimpleNamePattern, enclosingQualificationPattern, patternTypeArguments, depth + 1,
                                        enclosingType);
