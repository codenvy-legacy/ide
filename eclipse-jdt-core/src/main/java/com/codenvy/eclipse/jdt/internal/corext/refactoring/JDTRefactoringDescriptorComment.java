...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.rename.RenamingNameSuggestor;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.reorg.IReorgPolicy;
...
public final class JDTRefactoringDescriptorComment {
    ...
    public String asString() {
        ...
        for (final Iterator<String> iterator = fSettings.iterator(); iterator.hasNext(); ) {
            ...
            buffer.append(LINE_DELIMITER);
            buffer.append(
                    Messages.format(RefactoringCoreMessages.JavaRefactoringDescriptor_inferred_setting_pattern, setting));
        }
        return buffer.toString();
    }
    ...
    private void initializeInferredSettings(final Object object) {
        if (object instanceof INameUpdating) {
            ...
            fSettings.add(Messages.format(RefactoringCoreMessages.JavaRefactoringDescriptor_original_element_pattern,
                                          JavaElementLabels
                                                  .getTextLabel(updating.getElements()[0], JavaElementLabels.ALL_FULLY_QUALIFIED)));
            try {
                ...
                if (element != null) {
                    ...
                    fSettings.add(
                            Messages.format(RefactoringCoreMessages.JavaRefactoringDescriptor_renamed_element_pattern, newLabel));
                }
                ...
                if (elements.length == 1 && elements[0] != null) {
                    ...
                    for (int index = 0; index < elements.length; index++) {
                        if (elements[index] != null) {
                            ...
                            buffer.append(ELEMENT_DELIMITER);
                            buffer.append(
                                    JavaElementLabels.getTextLabel(elements[index], JavaElementLabels.ALL_FULLY_QUALIFIED));
                        } else {
                            buffer.append(LINE_DELIMITER);
                            buffer.append(ELEMENT_DELIMITER);
                            buffer.append(RefactoringCoreMessages.JavaRefactoringDescriptor_not_available);
            ...
            if (destination != null) {
                ...
                if (destination != null) {
                    fSettings.add(
                            Messages.format(RefactoringCoreMessages.JavaRefactoringDescriptorComment_destination_pattern,
                                            JavaElementLabels.getTextLabel(destination, JavaElementLabels.ALL_FULLY_QUALIFIED)));
                }
            }
