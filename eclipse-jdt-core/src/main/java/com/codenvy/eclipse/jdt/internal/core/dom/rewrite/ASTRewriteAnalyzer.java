...
import com.codenvy.eclipse.jdt.core.dom.rewrite.TargetSourceRangeComputer;
import com.codenvy.eclipse.jdt.core.dom.rewrite.TargetSourceRangeComputer.SourceRange;
...
import com.codenvy.eclipse.jdt.internal.compiler.parser.RecoveryScannerData;
...
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.ASTRewriteFormatter.BlockContext;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.ASTRewriteFormatter.NodeMarker;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.ASTRewriteFormatter.Prefix;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.NodeInfoStore.CopyPlaceholderData;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.NodeInfoStore.StringPlaceholderData;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.RewriteEventStore.CopySourceInfo;

import org.exoplatform.ide.editor.shared.text.edits.CopySourceEdit;
import org.exoplatform.ide.editor.shared.text.edits.CopyTargetEdit;
...
import org.exoplatform.ide.editor.shared.text.edits.MoveSourceEdit;
import org.exoplatform.ide.editor.shared.text.edits.MoveTargetEdit;
...
public final class ASTRewriteAnalyzer extends ASTVisitor {
     ...
     * @param comments
     *         list of comments of the compilation unit to rewrite (elements of type <code>Comment</code>) or <code>null</code>.
     * @param options
        ...
        if (this.tokenScanner == null) {
            ...
            if (this.recoveryScannerData == null) {
                ...
                scanner = new RecoveryScanner(false,/*tokenizeWhiteSpace*/
                                              ...
                                              compilerOptions.sourceLevel, compilerOptions.complianceLevel, null/*taskTags*/,
                                              null/*taskPriorities*/,
                                              true/*taskCaseSensitive*/, this.recoveryScannerData);
            }
        ...
        if (event != null) {
            switch (event.getChangeKind()) {
                ...
                case RewriteEvent.REPLACED: {
                    ...
                    if (insertNewLine) {
                        insertedPrefix = getLineDelimiter() + this.formatter.createIndentString(
                                indent) + insertedPrefix.trim() + ' ';
                    }
            ...
            switch (event.getChangeKind()) {
                ...
                case RewriteEvent.REMOVED: {
                    try {
                        ASTNode node = (ASTNode)event.getOriginalValue();
                        TextEditGroup editGroup = getEditGroup(event);
                        int dotEnd = getScanner().getTokenEndOffset(TerminalTokens.TokenNameDOT,
    ...
    final void doTextInsert(int insertOffset, ASTNode node, int initialIndentLevel, boolean removeLeadingIndent,
        ...
        for (int i = 0; i < markers.size(); i++) { // markers.size can change!
            ...
            if (data instanceof TextEditGroup) { // tracking a node
                ...
                String destIndentString = (lineOffset == 0) ? this.formatter.createIndentString(
                        initialIndentLevel) : this.formatter.getIndentString(formatted.substring(lineOffset, offset));
    ...
    private int rewriteModifiers2(ASTNode node, ChildListPropertyDescriptor property, int pos) {
        ...
        int endPos = new ModifierRewriter(formatterPrefix).rewriteList(node, property, pos, Util.EMPTY_STRING,
                                                                       " "); //$NON-NLS-1$ 
    ...
    private void rewriteReturnType(MethodDeclaration node, boolean isConstructor, boolean isConstructorChange) {
        ChildPropertyDescriptor property = (node.getAST().apiLevel() == JLS2_INTERNAL) ? MethodDeclaration.RETURN_TYPE_PROPERTY
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        if (node.getAST().apiLevel() == JLS2_INTERNAL) {
            ...
            pos = rewriteOptionalTypeParameters(node, MethodDeclaration.TYPE_PARAMETERS_PROPERTY, pos, " ", true,
                                                pos != node.getStartPosition()); //$NON-NLS-1$
        }
        ...
        try {
            ...
            pos = rewriteNodeList(node, MethodDeclaration.THROWN_EXCEPTIONS_PROPERTY, pos, " throws ",
                                  ", "); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public boolean visit(ReturnStatement node) {
        try {
            this.beforeRequiredSpaceIndex = getScanner().getTokenEndOffset(TerminalTokens.TokenNamereturn,
    ...
    public boolean visit(BreakStatement node) {
        ...
        try {
            ...
            rewriteNode(node, BreakStatement.LABEL_PROPERTY, offset,
        ...
        if (node.getAST().apiLevel() == JLS2_INTERNAL) {
            ...
            if (isChanged(node, ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY)) {
                try {
                    ...
                    rewriteOptionalTypeParameters(node, ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY, pos, " ", true,
                                                  true); //$NON-NLS-1$
                } catch (CoreException e) {
                    handleException(e);
                }
    ...
    public boolean visit(ContinueStatement node) {
        ...
        try {
            ...
            rewriteNode(node, ContinueStatement.LABEL_PROPERTY, offset,
                        ASTRewriteFormatter.SPACE); // space between continue and label
        } catch (CoreException e) {
            handleException(e);
        }
        return false;
        ...
        if (thenEvent != null && thenEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
            try {
                ...
                if (elseStatement != null) {
                    ...
                    endPos = getScanner().getTokenStartOffset(TerminalTokens.TokenNameelse,
    ...
    class SwitchListRewriter extends ParagraphListRewriter {
        ...
        public SwitchListRewriter(int initialIndent) {
            ...
            this.indentSwitchStatementsCompareToCases = DefaultCodeFormatterConstants.TRUE.equals(
        ...
        if (getChangeKind(node, property) != RewriteEvent.UNCHANGED) {
            try {
                ...
                if (DefaultCodeFormatterConstants.TRUE.equals(
                        this.options.get(DefaultCodeFormatterConstants.FORMATTER_INDENT_SWITCHSTATEMENTS_COMPARE_TO_SWITCH))) {
                    insertIndent++;
                }
    ...
    public boolean visit(ThrowStatement node) {
        try {
            this.beforeRequiredSpaceIndex = getScanner().getTokenEndOffset(TerminalTokens.TokenNamethrow,
                                                                           node.getStartPosition());
    ...
    public boolean visit(TryStatement node) {
        ...
        if (node.getAST().apiLevel() >= AST.JLS4) {
            if (isChanged(node, TryStatement.RESOURCES_PROPERTY)) {
                ...
                pos = rewriteNodeList(node, TryStatement.RESOURCES_PROPERTY, getPosAfterTry(pos), newParen, ")",
    ...
    public boolean visit(EnumConstantDeclaration node) {
        ...
        if (isChanged(node, EnumConstantDeclaration.ANONYMOUS_CLASS_DECLARATION_PROPERTY)) {
            ...
            rewriteNode(node, EnumConstantDeclaration.ANONYMOUS_CLASS_DECLARATION_PROPERTY, pos,
                        ASTRewriteFormatter.SPACE);
        }
        return false;
    }
