    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        switch ((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) {
                ...
                switch (this.bits & ASTNode.ReturnTypeIDMASK) {
                    case T_int:
                        ...
                        if ((this.left.constant != Constant.NotAConstant) && (this.left.constant.typeID() == TypeIds.T_int) &&
                            ...
                            // x & 0
                            if ((this.right.constant != Constant.NotAConstant) && (this.right.constant.typeID() == TypeIds.T_int) &&
                    ...
                    case T_long:
                        ...
                        if ((this.left.constant != Constant.NotAConstant) && (this.left.constant.typeID() == TypeIds.T_long) &&
                            ...
                            // x & 0
                            if ((this.right.constant != Constant.NotAConstant) && (this.right.constant.typeID() == TypeIds.T_long) &&
            ...
            case LESS:
                ...
                break;
            case LESS_EQUAL:
                generateOptimizedLessThanOrEqual(currentScope, codeStream, null, (falseLabel = new BranchLabel(codeStream)),
                                                 valueRequired);
                if (valueRequired) {
                    codeStream.iconst_1();
                    if ((this.bits & ASTNode.IsReturnedValue) != 0) {
   ...
   /* In the case trying to make a string concatenation, there is no need to create a new
    ...
    void nonRecursiveResolveTypeUpwards(BlockScope scope) {
        ...
        if ((leftIsCast = (this.left instanceof CastExpression)) == true || rightIsCast) {
            CastExpression.checkNeedForArgumentCasts(scope, operator, operatorSignature, this.left, leftTypeID, leftIsCast,
                                                     this.right, rightTypeID, rightIsCast);
        }
        // compute the constant when valid
