   ...
   protected void populateArgumentMap()
   {
      ...
      for (int i = 0; i < parameters.length; i++)
      {
         ...
         JavaRefactoringDescriptorUtil.setBoolean(fArguments,
            JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_CREATE_FIELD, i), param.isCreateField());
         if (param.isCreateField())
         {
            JavaRefactoringDescriptorUtil.setString(fArguments,
               JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_FIELD_NAME, i), param.getFieldName());
         }
      }
      ...
      if (fParameters != null)
      {
         ...
         for (int i = 0; i < fParameters.length; i++)
         {
            ...
            if (parameter.isCreateField())
            {
               ...
               {
                  result.addError(
                     "The parameter " + parameter.getIndex() + " is marked for field creation but does not have a field name"); //$NON-NLS-1$ //$NON-NLS-2$
               }
               else
               {
