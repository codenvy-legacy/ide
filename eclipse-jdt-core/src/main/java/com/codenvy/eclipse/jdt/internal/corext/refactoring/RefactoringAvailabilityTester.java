...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.reorg.ReorgPolicyFactory;
...
public final class RefactoringAvailabilityTester
{
   ...
   {
      return Checks.isAvailable(
         type) && !type.isBinary() && !type.isReadOnly() && !type.isAnnotation() && !type.isAnonymous();
   }
   ...
   //				&& !selection.resolveInAnnotation()
   //				&& (Checks.isExtractableExpression(nodes,
   //         selection.resolveCoveringNode()) || (nodes != null && nodes.length == 1 && nodes[0] instanceof ExpressionStatement));
   //	}
   ...
   {
      return Checks.isAvailable(field) && JdtFlags.isStatic(field) && JdtFlags.isFinal(field) && !JdtFlags.isEnum(
         field);
   }
   ...
   {
      return Checks.isAvailable(type) && !Checks.isAnonymous(type) && !JavaElementUtil.isMainType(
         type) && !Checks.isInsideLocalType(type);
   }
   ...
   {
      return method.exists() && !method.isConstructor() && !method.isBinary() && !method.getDeclaringType().isInterface() && !method.isReadOnly() && !JdtFlags.isStatic(
         method);
   }
   ...
   public static boolean isRenameNonVirtualMethodAvailable(
      final IMethod method) throws JavaModelException, CoreException
   {
