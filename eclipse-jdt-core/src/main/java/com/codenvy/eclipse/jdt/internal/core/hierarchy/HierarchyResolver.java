...
public class HierarchyResolver implements ITypeRequestor
{
            ...
            else if ((superBinding.id == TypeIds.T_JavaLangObject))
            {
               ...
               { // check whether subclass of Object due to broken hierarchy (as opposed to explicitly extending it)
                  ...
                  char[] simpleName = lastSeparator == -1 ? superclassName : CharOperation.subarray(superclassName,
                     lastSeparator + 1, superclassName.length);
   ...
   private IType[] findSuperInterfaces(IGenericType type, ReferenceBinding typeBinding)
   {
      ...
      else if (type instanceof ISourceType)
      {
         ...
         if (sourceType.getName().length == 0)
         { // if anonymous type
      ...
      else if (type instanceof HierarchyType)
      {
         ...
         if (hierarchyType.name.length == 0)
         { // if anonymous type
            if (typeBinding.superInterfaces() != null && typeBinding.superInterfaces().length > 0)
            {
      ...
      try
      {
         if (suppliedType.isBinaryType())
         {
            BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment.cacheBinaryType((IBinaryType)suppliedType,
               false/*don't need field and method (bug 125067)*/, null /*no access restriction*/);
