    ...
    public static final String generateTableTestCase() {
      ...
      org.eclipse.jdt.internal.compiler.ast.
		...
		*/
        ...
        int[] operators =
                new int[]{AND, AND_AND, DIVIDE, GREATER, GREATER_EQUAL, LEFT_SHIFT, LESS, LESS_EQUAL, MINUS, MULTIPLY, OR, OR_OR, PLUS,
        ...
        for (int i = 0; i < operators.length; i++) {
            ...
            for (int left = 0; left < 16; left++) {
                for (int right = 0; right < 16; right++) {
                    ...
                    {
                        s += "\t\t" + decode.type(result) + "0" + " = " + decode.type(
                                left); //$NON-NLS-1$ //$NON-NLS-3$ //$NON-NLS-2$
                        s += " " + decode.operator(operator) + " " + decode.type(
