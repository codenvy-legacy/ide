...
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.ASTRewriteAnalyzer;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.LineInformation;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.NodeInfoStore;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.NodeRewriteEvent;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.RewriteEventStore;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.RewriteEventStore.CopySourceInfo;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.RewriteEventStore.PropertyLocation;
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.TrackedNodePosition;
 ...
 * (see {@link com.codenvy.eclipse.jdt.core.dom.CompilationUnit#rewrite(IDocument, Map)}).
 * <p>
...
public class ASTRewrite {
     ...
     *         A {@link JavaModelException} is thrown when
	...
	*/
     ...
     *
     * @param node
     *         the node being removed. The node can either be an original node in the AST
     ...
     *         or (since 3.4) a new node already inserted or used as replacement in this AST rewriter.
     * @param replacement
     *         the replacement node, or <code>null</code> if no
     *         replacement
     * @param editGroup
     *         the edit group in which to collect the corresponding
     *         text edits, or <code>null</code> if ungrouped
    ...
    private ASTNode createTargetNode(ASTNode node, boolean isMove) {
        ...
        validateIsCorrectAST(node);
        CopySourceInfo info = getRewriteEventStore().markAsCopySource(node.getParent(), node.getLocationInParent(), node,
                                                                      isMove);
