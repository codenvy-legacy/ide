 ...
 *     Benjamin Muskalla <bmuskalla@eclipsesource.com> - [extract method] Name ambiguous return value in error message - https://bugs
 *     .eclipse.org/bugs/show_bug.cgi?id=50607
 *******************************************************************************/
...
import com.codenvy.eclipse.jdt.internal.corext.dom.LocalVariableIndex;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.code.flow.FlowContext;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.code.flow.FlowInfo;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.code.flow.InOutFlowAnalyzer;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.code.flow.InputFlowAnalyzer;
...
/* package */ class ExtractMethodAnalyzer extends CodeAnalyzer {
    ...
    private void initReturnType(ImportRewrite rewriter) {
        ...
        switch (fReturnKind) {
            case ACCESS_TO_LOCAL:
                ...
                fReturnType = ASTNodeFactory.newType(ast, declaration, rewriter,
                ...
                if (fExpressionBinding != null) {
                    if (fExpressionBinding.isNullType()) {
                        ...
                        if (normalizedBinding != null) {
                            ImportRewriteContext context = new ContextSensitiveImportRewriteContext(fEnclosingBodyDeclaration,
                                                                                                    rewriter);
    ...
    private Statement getParentLoopBody(ASTNode node) {
        Statement stmt = null;
        ASTNode start = node;
        while (start != null && !(start instanceof ForStatement) && !(start instanceof DoStatement) && !(start instanceof WhileStatement) &&
               !(start instanceof EnhancedForStatement) && !(start instanceof SwitchStatement)) {
    ...
    private void computeOutput(RefactoringStatus status) {
        ...
        switch (localReads.size()) {
            ...
            default:
                ...
                for (int i = 0; i < localReads.size(); i++) {
                    ...
                    String bindingName = BindingLabelProvider.getBindingLabel(binding,
                ...
                String message = String.format(RefactoringCoreMessages.ExtractMethodAnalyzer_assignments_to_local,
        ...
        if (fReturnValue != null && getSelection().covers(
                ASTNodes.findDeclaration(fReturnValue, fEnclosingBodyDeclaration))) {
    ...
    public void endVisit(CompilationUnit node) {
        ...
        superCall:
        {
            ...
            if (!hasSelectedNodes()) {
                ...
                if (coveringNode instanceof Block && coveringNode.getParent() instanceof MethodDeclaration) {
                    ...
                    if (messages.length > 0) {
                        status.addFatalError(Messages.format(RefactoringCoreMessages.ExtractMethodAnalyzer_compile_errors,
            ...
            if (fEnclosingBodyDeclaration == null || (fEnclosingBodyDeclaration.getNodeType() != ASTNode.METHOD_DECLARATION &&
            ...
            if (isExpressionSelected()) {
                ...
                if (expression instanceof Name) {
                    ...
                    if (name.resolveBinding() instanceof IVariableBinding) {
                        ...
                        if (locationInParent == QualifiedName.NAME_PROPERTY || (locationInParent == FieldAccess.NAME_PROPERTY &&
                ...
                fForceStatic = ASTNodes.getParent(expression,
                                                  ASTNode.SUPER_CONSTRUCTOR_INVOCATION) != null || ASTNodes.getParent(expression,
                                                                                                                      ASTNode
    ...
    public boolean visit(Assignment node) {
        boolean result = super.visit(node);
        if ((getSelection().covers(node.getLeftHandSide()) && !getSelection().covers(
                node.getRightHandSide())) || getSelection().coveredBy(node.getLeftHandSide())) {
    ...
    public boolean visit(VariableDeclarationFragment node) {
        boolean result = super.visit(node);
        if (isFirstSelectedNode(node)) {
            invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_variable_declaration_fragment,
                             JavaStatusContext.create(fCUnit, node));
            return false;
        }
        return result;
