...
public class CodeStream
{
   ...
   public void generateSyntheticBodyForMethodAccess(SyntheticMethodBinding accessMethod)
   {
      ...
      else
      {
         ...
         {
            // target method declaring class may not be accessible (247953);
            TypeBinding declaringClass = accessMethod.purpose == SyntheticMethodBinding.SuperMethodAccess ? accessMethod.declaringClass.superclass() : accessMethod.declaringClass;
   ...
   public void generateSyntheticBodyForSwitchTable(SyntheticMethodBinding methodBinding)
   {
      ...
      LocalVariableBinding localVariableBinding = new LocalVariableBinding(" tab".toCharArray(),
         scope.createArrayType(TypeBinding.INT, 1), 0, false); //$NON-NLS-1$
      ...
      Expression enclosingInstance, ASTNode invocationSite)
   {
      ...
      if ((syntheticArgumentTypes = targetType.syntheticEnclosingInstanceTypes()) != null)
      {
         ...
         for (int i = 0, max = syntheticArgumentTypes.length; i < max; i++)
         {
            ...
            {
               Object[] emulationPath = currentScope.getEmulationPath(syntheticArgType, false /*not only exact match (that is, allow compatible)*/,
 ...
 */
      ...
      TypeBinding actualReceiverType, boolean isImplicitThisReceiver)
   {
      ...
      if (constantPoolDeclaringClass != actualReceiverType.erasure() && !actualReceiverType.isArrayType() && constantPoolDeclaringClass != null // array.length
         && codegenBinding.constant() == Constant.NotAConstant)
      {
         CompilerOptions options = currentScope.compilerOptions();
         if ((options.targetJDK >= ClassFileConstants.JDK1_2 && (options.complianceLevel >= ClassFileConstants.JDK1_4 || !(isImplicitThisReceiver && codegenBinding.isStatic())) && constantPoolDeclaringClass.id != TypeIds.T_JavaLangObject) // no change for Object fields
      ...
      TypeBinding actualReceiverType, boolean isImplicitThisReceiver)
   {
      ...
      else
      {
         ...
         {
            CompilerOptions options = currentScope.compilerOptions();
            if ((options.targetJDK >= ClassFileConstants.JDK1_2 && (options.complianceLevel >= ClassFileConstants.JDK1_4 || !(isImplicitThisReceiver && codegenBinding.isStatic())) && codegenBinding.declaringClass.id != TypeIds.T_JavaLangObject) // no change for Object methods
   ...
   public void goto_(BranchLabel label)
   {
	...
	returns. Also see 114894
      ...
      if (chained && this.lastAbruptCompletion == this.position)
      {
         ...
         { // ensure existing forward references are updated
