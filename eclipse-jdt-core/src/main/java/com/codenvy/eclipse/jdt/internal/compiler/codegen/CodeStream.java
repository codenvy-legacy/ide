...
import com.codenvy.eclipse.jdt.internal.compiler.flow.UnconditionalFlowInfo;
...
import com.codenvy.eclipse.jdt.internal.compiler.lookup.NestedTypeBinding;
...
import com.codenvy.eclipse.jdt.internal.compiler.problem.AbortMethod;
...
public class CodeStream {
    ...
    public static final CompilationResult RESTART_CODE_GEN_FOR_UNUSED_LOCALS_MODE = new CompilationResult((char[])null,
                                                                                                          0, 0, 0);
    ...
    public CodeStream(ClassFile givenClassFile) {
        ...
        if ((givenClassFile.produceAttributes & ClassFileConstants.ATTR_LINES) != 0) {
            this.lineSeparatorPositions =
                    givenClassFile.referenceBinding.scope.referenceCompilationUnit().compilationResult.getLineSeparatorPositions();
        }
    }
    ...
    public static final void sort(int[] tab, int lo0, int hi0, int[] result) {
        ...
        int hi = hi0;
        int mid;
        if (hi0 > lo0) {      /* Arbitrarily establishing partition element as the midpoint of
          * the array.
		  */
    ...
    public void addDefinitelyAssignedVariables(Scope scope, int initStateIndex) {
        ...
        if ((this.generateAttributes &
             (ClassFileConstants.ATTR_VARS | ClassFileConstants.ATTR_STACK_MAP_TABLE | ClassFileConstants.ATTR_STACK_MAP)) == 0) {
            return;
        }
        ...
        switch (unboxedTypeID) {
            case TypeIds.T_byte:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangByteConstantPoolName, ConstantPool.Init, ConstantPool.ByteConstrSignature);
                }
                break;
            case TypeIds.T_short:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangShortConstantPoolName, ConstantPool.Init, ConstantPool.ShortConstrSignature);
                }
                break;
            case TypeIds.T_char:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangCharacterConstantPoolName, ConstantPool.Init, ConstantPool.CharConstrSignature);
                }
                break;
            case TypeIds.T_int:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokestatic, 1, // receiverAndArgsSize
                           1, // return type size
                           ConstantPool.JavaLangIntegerConstantPoolName, ConstantPool.ValueOf, ConstantPool.IntIntegerSignature);
                } else {
                    // new Integer(int)
                    newWrapperFor(unboxedTypeID);
                    dup_x1();
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangIntegerConstantPoolName, ConstantPool.Init, ConstantPool.IntConstrSignature);
                }
                break;
            case TypeIds.T_long:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 3, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangLongConstantPoolName, ConstantPool.Init, ConstantPool.LongConstrSignature);
                }
                break;
            case TypeIds.T_float:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangFloatConstantPoolName, ConstantPool.Init, ConstantPool.FloatConstrSignature);
                }
                break;
            case TypeIds.T_double:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 3, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangDoubleConstantPoolName, ConstantPool.Init, ConstantPool.DoubleConstrSignature);
                }
            ...
            case TypeIds.T_boolean:
                if (this.targetLevel >= ClassFileConstants.JDK1_5) {
                    ...
                    invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
                           0, // return type size
                           ConstantPool.JavaLangBooleanConstantPoolName, ConstantPool.Init, ConstantPool.BooleanConstrSignature);
                }
        }
    }
    ...
    public void generateClassLiteralAccessForType(TypeBinding accessedType, FieldBinding syntheticFieldBinding) {
		...
		we have no way to get a hand on the field cache to do better. */
            ...
            this.ldc(accessedType == TypeBinding.NULL ? "java.lang.Object" : String.valueOf(
                    accessedType.constantPoolName()).replace('/', '.')); //$NON-NLS-1$
            invokeClassForName();
        ...
        if (mappingSequence == null) {
            if (target instanceof LocalVariableBinding) {
                scope.problemReporter().needImplementation(
                        invocationSite); //TODO (philippe) should improve local emulation failure reporting
            } else {
    ...
    /**
     * @param accessBinding
     *         the access method binding to generate
     */
    ...
    public void generateSyntheticBodyForFieldReadAccess(SyntheticMethodBinding accessMethod) {
        ...
        TypeBinding declaringClass =
                accessMethod.purpose == SyntheticMethodBinding.SuperFieldReadAccess ? accessMethod.declaringClass.superclass()
    ...
    public void generateSyntheticBodyForFieldWriteAccess(SyntheticMethodBinding accessMethod) {
        ...
        // target method declaring class may not be accessible (247953);
        TypeBinding declaringClass =
                accessMethod.purpose == SyntheticMethodBinding.SuperFieldWriteAccess ? accessMethod.declaringClass.superclass()
                                                                                     : accessMethod.declaringClass;
        if (fieldBinding.isStatic()) {
    ...
    public void generateSyntheticBodyForMethodAccess(SyntheticMethodBinding accessMethod) {
        ...
        if (targetMethod.isStatic()) {
            invoke(Opcodes.OPC_invokestatic, targetMethod,
                   accessMethod.declaringClass); // target method declaring class may not be accessible (128563)
        } else {
            if (targetMethod.isConstructor() || targetMethod.isPrivate()
                ...
                // target method declaring class may not be accessible (247953);
                TypeBinding declaringClass =
                        accessMethod.purpose == SyntheticMethodBinding.SuperMethodAccess ? accessMethod.declaringClass.superclass()
                                                                                         : accessMethod.declaringClass;
                ...
                if (targetMethod.declaringClass.isInterface()) { // interface or annotation type
                    ...
                    invoke(Opcodes.OPC_invokevirtual, targetMethod,
                           accessMethod.declaringClass); // target method declaring class may not be accessible (128563)
                }
            }
        }
    ...
    public void generateSyntheticBodyForSwitchTable(SyntheticMethodBinding methodBinding) {
        ...
        astore_0();
        LocalVariableBinding localVariableBinding = new LocalVariableBinding(" tab".toCharArray(),
                                                                             scope.createArrayType(TypeBinding.INT, 1), 0,
                                                                             false); //$NON-NLS-1$
    ...
    public void generateSyntheticEnclosingInstanceValues(BlockScope currentScope, ReferenceBinding targetType,
        ...
        if ((syntheticArgumentTypes = targetType.syntheticEnclosingInstanceTypes()) != null) {
            ...
            if (compliance <= ClassFileConstants.JDK1_3) {
                ...
                denyEnclosingArgInConstructorCall = invocationSite instanceof AllocationExpression ||
                                                    invocationSite instanceof ExplicitConstructorCall &&
            ...
            for (int i = 0, max = syntheticArgumentTypes.length; i < max; i++) {
                ...
                if (hasExtraEnclosingInstance && syntheticArgType == targetEnclosingType) {
                    ...
                    Object[] emulationPath =
                            currentScope.getEmulationPath(syntheticArgType, false /*not only exact match (that is, allow compatible)*/,
            ...
            if (hasExtraEnclosingInstance) {
                currentScope.problemReporter().unnecessaryEnclosingInstanceSpecification(enclosingInstance,
                                                                                         checkedTargetType);
            }
        }
    }
 ...
 */
        ...
        switch (baseTypeID) {
            case TypeIds.T_byte:
                ...
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangByteConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_short:
                // getstatic: java.lang.Short.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangShortConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_char:
                // getstatic: java.lang.Character.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangCharacterConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_int:
                // getstatic: java.lang.Integer.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangIntegerConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_long:
                // getstatic: java.lang.Long.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangLongConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_float:
                // getstatic: java.lang.Float.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangFloatConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_double:
                // getstatic: java.lang.Double.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangDoubleConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_boolean:
                // getstatic: java.lang.Boolean.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangBooleanConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
            case TypeIds.T_void:
                // getstatic: java.lang.Void.TYPE
                fieldAccess(Opcodes.OPC_getstatic, 1, // return type size
                            ConstantPool.JavaLangVoidConstantPoolName, ConstantPool.TYPE, ConstantPool.JavaLangClassSignature);
                break;
        }
    }
    ...
    public void invoke(byte opcode, MethodBinding methodBinding, TypeBinding declaringClass) {
        ...
        invoke(opcode, receiverAndArgsSize, returnTypeSize, declaringClass.constantPoolName(), methodBinding.selector,
    ...
    public void invokeEnumOrdinal(char[] enumTypeConstantPoolName) {
        ...
        invoke(Opcodes.OPC_invokevirtual, 1, // receiverAndArgsSize
               1, // return type size
               enumTypeConstantPoolName, ConstantPool.Ordinal, ConstantPool.OrdinalSignature);
    }
    ...
    public void invokeIterableIterator(TypeBinding iterableReceiverType) {
        ...
        invoke(iterableReceiverType.isInterface() ? Opcodes.OPC_invokeinterface : Opcodes.OPC_invokevirtual, 1,
               // receiverAndArgsSize
               1, // returnTypeSize
    ...
    public void invokeAutoCloseableClose(TypeBinding resourceType) {
        ...
        invoke(resourceType.erasure().isInterface() ? Opcodes.OPC_invokeinterface : Opcodes.OPC_invokevirtual, 1,
               // receiverAndArgsSize
               0, // returnTypeSize
               resourceType.constantPoolName(), ConstantPool.Close, ConstantPool.CloseSignature);
    }
    ...
    public void invokeJavaLangAssertionErrorConstructor(int typeBindingID) {
        ...
        invoke(Opcodes.OPC_invokespecial, receiverAndArgsSize, 0, // return type size
               ConstantPool.JavaLangAssertionErrorConstantPoolName, ConstantPool.Init, signature);
    }
    ...
    public void invokeJavaLangEnumValues(TypeBinding enumBinding, ArrayBinding arrayBinding) {
        ...
        invoke(Opcodes.OPC_invokestatic, 0,  // receiverAndArgsSize
               1,  // return type size
               enumBinding.constantPoolName(), TypeConstants.VALUES, signature);
    }
    ...
    protected void invokeJavaLangReflectFieldGetter(int typeID) {
        ...
        invoke(Opcodes.OPC_invokevirtual, 2, // receiverAndArgsSize
               ...
               ConstantPool.JAVALANGREFLECTFIELD_CONSTANTPOOLNAME, selector, signature);
    }
    ...
    public void invokeStringConcatenationAppendForType(int typeID) {
        ...
        invoke(Opcodes.OPC_invokevirtual, receiverAndArgsSize, 1, // return type size
               declaringClass, selector, signature);
    }
    ...
    public void invokeStringConcatenationDefaultConstructor() {
        ...
        invoke(Opcodes.OPC_invokespecial, 1, // receiverAndArgsSize
               0, // return type size
               declaringClass, ConstantPool.Init, ConstantPool.DefaultConstructorSignature);
    }
    ...
    public void invokeStringConcatenationStringConstructor() {
        ...
        invoke(Opcodes.OPC_invokespecial, 2, // receiverAndArgsSize
               0, // return type size
               declaringClass, ConstantPool.Init, ConstantPool.StringConstructorSignature);
    }
    ...
    public void invokeStringValueOf(int typeID) {
        ...
        invoke(Opcodes.OPC_invokestatic, receiverAndArgsSize, // receiverAndArgsSize
               1, // return type size
               ConstantPool.JavaLangStringConstantPoolName, ConstantPool.ValueOf, signature);
    }
    ...
    public void invokeSystemArraycopy() {
        ...
        invoke(Opcodes.OPC_invokestatic, 5, // receiverAndArgsSize
               0, // return type size
               ConstantPool.JavaLangSystemConstantPoolName, ConstantPool.ArrayCopy, ConstantPool.ArrayCopySignature);
    }
        ...
        if (index > 0) {
            ...
            while ((length < 65532) && (i < constantLength)) {
                ...
                if (length + 3 > (utf8encodingLength = utf8encoding.length)) {
                    System.arraycopy(utf8encoding, 0, utf8encoding = new byte[Math.min(utf8encodingLength + 100, 65535)], 0,
                                     length);
                }
            ...
            while (i < constantLength) {
                ...
                while ((length < 65532) && (i < constantLength)) {
                    ...
                    // we resize the byte array immediately if necessary
                    if (length + 3 > (utf8encodingLength = utf8encoding.length)) {
                        System.arraycopy(utf8encoding, 0, utf8encoding = new byte[Math.min(utf8encodingLength + 100, 65535)],
                                         0, length);
                    }
                    if ((current >= 0x0001) && (current <= 0x007F)) {
                        // we only need one byte: ASCII table
    ...
    public void record(LocalVariableBinding local) {
        ...
        if (this.allLocalsCounter == this.locals.length) {
            ...
            System.arraycopy(this.locals, 0,
                             this.locals = new LocalVariableBinding[this.allLocalsCounter + LOCALS_INCREMENT], 0, this.allLocalsCounter);
        }
	 ...
	 */
        if ((this.generateAttributes & ClassFileConstants.ATTR_LINES) == 0 || sourcePos == 0 || (startPC == this.position && !widen)) {
            return;
        }
                ...
                if (previousLineNumber == 1) {
					...
					*/
                                ...
                                if (!((insertionIndex > 1) && (this.pcToSourceMap[insertionIndex - 1] == lineNumber))) {
                                    if ((this.pcToSourceMapSize > 4) && (this.pcToSourceMap[this.pcToSourceMapSize - 4] > startPC)) {
                                        System.arraycopy(this.pcToSourceMap, insertionIndex, this.pcToSourceMap,
                                                         insertionIndex + 2, this.pcToSourceMapSize - 2 - insertionIndex);
                        ...
                        if (startPC <= this.lastEntryPC) {
							...
							*/
                                if (existingEntryIndex != -1) {
                                    ...
                                    System.arraycopy(this.pcToSourceMap, insertionIndex, this.pcToSourceMap, insertionIndex + 2,
                                                     this.pcToSourceMapSize - insertionIndex);
                                    this.pcToSourceMap[insertionIndex++] = startPC;
                                    this.pcToSourceMap[insertionIndex] = lineNumber;
                            ...
                            // we can safely add the new entry. The endPC of the previous entry is not in conflit with the startPC of the
                            // new entry.
                    ...
                    if (lineSeparatorPositions2[previousLineNumber - 2] < sourcePos) {
						...
						/* the last recorded entry is on the same line. But it could be relevant to widen this entry.
						   we want to extend this entry forward in case we generated some bytecode before the last entry that are not related to any
						   statement
						*/
                            if (startPC < this.pcToSourceMap[this.pcToSourceMapSize - 2]) {
                                int insertionIndex = insertionIndex(this.pcToSourceMap, this.pcToSourceMapSize, startPC);
                            ...
                            if (startPC <= this.lastEntryPC) {
								...
								*/
                                ...
                                // we can safely add the new entry. The endPC of the previous entry is not in conflit with the startPC of
                                // the new entry.
                                this.pcToSourceMap[this.pcToSourceMapSize++] = startPC;
                                this.pcToSourceMap[this.pcToSourceMapSize++] = lineNumber;
                            }
                    ...
                    if (startPC <= this.lastEntryPC) {
						...
						*/
                        ...
                        // we can safely add the new entry. The endPC of the previous entry is not in conflit with the startPC of the new
                        // entry.
                        this.pcToSourceMap[this.pcToSourceMapSize++] = startPC;
                        this.pcToSourceMap[this.pcToSourceMapSize++] = lineNumber;
                    }
                ...
                if (startPC < this.pcToSourceMap[this.pcToSourceMapSize - 2]) {
					 ...
					 */
                        if (!((insertionIndex > 1) && (this.pcToSourceMap[insertionIndex - 1] == lineNumber))) {
                            if ((this.pcToSourceMapSize > 4) && (this.pcToSourceMap[this.pcToSourceMapSize - 4] > startPC)) {
                                System.arraycopy(this.pcToSourceMap, insertionIndex, this.pcToSourceMap, insertionIndex + 2,
                                                 this.pcToSourceMapSize - 2 - insertionIndex);
                                this.pcToSourceMap[insertionIndex++] = startPC;
                                this.pcToSourceMap[insertionIndex] = lineNumber;
                            } else {
                                this.pcToSourceMap[this.pcToSourceMapSize - 2] = startPC;
    ...
    public void registerExceptionHandler(ExceptionLabel anExceptionLabel) {
        ...
        if (this.exceptionLabelsCounter == (length = this.exceptionLabels.length)) {
            ...
            System.arraycopy(this.exceptionLabels, 0, this.exceptionLabels = new ExceptionLabel[length + LABELS_INCREMENT],
                             0, length);
        }
    ...
    public void removeNotDefinitelyAssignedVariables(Scope scope, int initStateIndex) {
        ...
        for (int i = 0; i < this.visibleLocalsCount; i++) {
            LocalVariableBinding localBinding = this.visibleLocals[i];
            if (localBinding != null && !isDefinitelyAssigned(scope, initStateIndex,
                                                              localBinding) && localBinding.initializationCount > 0) {
    ...
    /**
     * @param targetClassFile
     *         The given classfile to reset the code stream
     */
