...
public class ClassFile extends Openable implements IClassFile, SuffixConstants {
    ...
    public void codeComplete(int offset, ICompletionRequestor requestor,
        ...
        codeComplete(offset, new com.codenvy.eclipse.jdt.internal.codeassist.CompletionRequestorWrapper(requestor), owner);
    }
    ...
    public boolean existsUsingJarTypeCache() {
        if (getPackageFragmentRoot().isArchive()) {
            ...
            try {
                info = getJarBinaryTypeInfo((PackageFragment)getParent(), true/*fully initialize so as to not keep a reference to the
                byte array*/);
    ...
    public byte[] getBytes() throws JavaModelException {
        ...
        if (pkg instanceof JarPackageFragment) {
            ...
            try {
                ...
                if (ze != null) {
                    return com.codenvy.eclipse.jdt.internal.compiler.util.Util.getZipEntryByteContent(ze, zip);
                }
    ...
    private IBinaryType getJarBinaryTypeInfo(PackageFragment pkg,
                                             boolean fullyInitialize) throws CoreException, IOException, ClassFormatException {
        JarPackageFragmentRoot root = (JarPackageFragmentRoot)pkg.getParent();
        ZipFile zip = null;
        try {
            zip = root.getJar();
            ...
            ZipEntry ze = zip.getEntry(entryName);
            if (ze != null) {
                byte contents[] = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getZipEntryByteContent(ze, zip);
    ...
    public IBuffer getBuffer() throws JavaModelException {
        ...
        if (status.isOK()) {
            ...
            switch (status.getCode()) {
                case IJavaModelStatusConstants.ELEMENT_NOT_ON_CLASSPATH: // don't throw a JavaModelException to be able to open .class
                // file outside the classpath (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=138507 )
                case IJavaModelStatusConstants.INVALID_ELEMENT_TYPES: // don't throw a JavaModelException to be able to open .class file
                // in proj==src case without source (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=221904 )
                    return null;
                default:
    ...
    public IJavaElement getWorkingCopy(IProgressMonitor monitor,
                                       com.codenvy.eclipse.jdt.core.IBufferFactory factory) throws JavaModelException {
    ...
    protected IStatus validateExistence(IResource underlyingResource) {
        ...
        if (underlyingResource != null) {
            ...
            if ((underlyingResource instanceof IFolder) &&
                (root = getPackageFragmentRoot()).isArchive()) { // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=204652
