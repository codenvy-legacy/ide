...
import com.codenvy.eclipse.jdt.internal.compiler.ast.SubRoutineStatement;
...
import com.codenvy.eclipse.jdt.internal.compiler.lookup.CatchParameterBinding;
...
public class FlowContext implements TypeConstants {
     ...
     *         This is for checking exception handlers for exceptions raised during the
            ...
            if (traversedContext instanceof ExceptionHandlingFlowContext) {
                ...
                if ((caughtExceptions = exceptionContext.handledExceptions) != Binding.NO_EXCEPTIONS) {
                    ...
                    for (int caughtIndex = 0, caughtCount = caughtExceptions.length; caughtIndex < caughtCount; caughtIndex++) {
                        ...
                        int state = caughtException == null ? Scope.EQUAL_OR_MORE_SPECIFIC /* any exception */ : Scope.compareTypes(
                                raisedException, caughtException);
    ...
    public FlowInfo getInitsForFinalBlankInitializationCheck(TypeBinding declaringType, FlowInfo flowInfo) {
        ...
        do {
            if (current instanceof InitializationFlowContext) {
                ...
                current = exceptionContext.initializationParent == null ? exceptionContext.getLocalParent()
     ...
     *         enclosing scope
     * @param flowInfo
     *         flowInfo at the point of the early exit
     * @param trackingVar
     *         representation of the resource
     * @param reference
     *         the return or throw statement marking the early exit
    ...
    protected void recordProvidedExpectedTypes(TypeBinding providedType, TypeBinding expectedType, int nullCount) {
        if (nullCount == 0) {
            ...
            System.arraycopy(this.providedExpectedTypes, 0, this.providedExpectedTypes = new TypeBinding[nullCount * 2][],
                             0, oldLen);
        }
     ...
     * @param location
     *         the location triggering the analysis, for normal null dereference
     *         this is an expression resolving to 'local', for resource leaks it is an
     *         early exit statement.
     * @param status
     *         the status against which the check must be performed; one of
     ...
     *         early exit statement.
     * @param checkType
     *         the status against which the check must be performed; one
    ...
    public void recordNullityMismatch(BlockScope currentScope, Expression expression, TypeBinding providedType,
        ...
        if (expression.localVariableBinding() != null) { // flowContext cannot yet handle non-localvar expressions (e.g., fields)
            ...
            while (currentContext != null) {
                ...
                if (currentContext.internalRecordNullityMismatch(expression, providedType, nullStatus, expectedType,
                                                                 ASSIGN_TO_NONNULL | isInsideAssert)) {
                    return;
                }
    ...
    protected boolean internalRecordNullityMismatch(Expression expression, TypeBinding providedType, int nullStatus,
                                                    TypeBinding expectedType, int checkType) {
