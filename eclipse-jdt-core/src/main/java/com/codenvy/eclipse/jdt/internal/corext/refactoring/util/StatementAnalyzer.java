...
import com.codenvy.eclipse.jdt.internal.corext.dom.TokenScanner;
...
public class StatementAnalyzer extends SelectionAnalyzer {
    ...
    public StatementAnalyzer(ICompilationUnit cunit, Selection selection,
                             boolean traverseSelectedNode) throws CoreException {
    ...
    protected void checkSelectedNodes() {
        ...
        invalidSelection(RefactoringCoreMessages.StatementAnalyzer_beginning_of_selection,
                         JavaStatusContext.create(fCUnit, range));
    }
    ...
    public void endVisit(CompilationUnit node) {
        ...
        if (node != selectedNode) {
            ...
            fStatus.merge(
                    CommentAnalyzer.perform(selection, fScanner.getScanner(), parent.getStartPosition(), parent.getLength()));
        }
    ...
    private boolean doAfterValidation(ASTNode node, ASTNode[] selectedNodes) {
        return selectedNodes.length > 0 && node == selectedNodes[0].getParent() && getSelection().getEndVisitSelectionMode(
                node) == Selection.AFTER;
    }
