...
public class ReturnStatement extends Statement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext,
        ...
        if (saveValueNeeded) {
            ...
            if (((this.bits & ASTNode.IsSynchronized) == 0) && this.expression != null &&
                ...
                if (noAutoCloseables) { // can't abruptly return in the presence of autocloseables. See https://bugs.eclipse
                // .org/bugs/show_bug.cgi?id=367566
    ...
    void checkAgainstNullAnnotation(BlockScope scope, FlowContext flowContext, int nullStatus) {
        if (nullStatus != FlowInfo.NON_NULL) {
            ...
            if ((tagBits & TagBits.AnnotationNonNull) != 0) {
                flowContext.recordNullityMismatch(scope, this.expression, this.expression.resolvedType,
                                                  methodBinding.returnType, nullStatus);
            }
        }
    }
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        if (needValueStore()) {
            ...
            this.expression.generateCode(currentScope, codeStream,
                                         needValue()); // no value needed if non-returning subroutine
        ...
        if (this.subroutines != null) {
            ...
            for (int i = 0, max = this.subroutines.length; i < max; i++) {
                ...
                boolean didEscape = sub.generateSubRoutineInvocation(currentScope, codeStream, reusableJSRTarget,
                                                                     this.initStateIndex, this.saveValueVariable);
                if (didEscape) {
    ...
    public void resolve(BlockScope scope) {
        ...
        TypeBinding methodType = (methodScope.referenceContext instanceof AbstractMethodDeclaration) ? (
                (methodBinding = ((AbstractMethodDeclaration)methodScope.referenceContext).binding) == null ? null
        ...
        if (this.expression.isConstantValueOfTypeAssignableToType(expressionType,
                                                                  methodType) || expressionType.isCompatibleWith(methodType)) {
