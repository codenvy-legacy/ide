...
public class ConstantPool implements ClassFileConstants, TypeIds {
    ...
    public int literalIndex(char[] utf8Constant) {
        ...
        if ((index = this.UTF8Cache.putIfAbsent(utf8Constant, this.currentIndex)) < 0) {
            ...
            if (length >= 65535) {
                ...
                this.classFile.referenceBinding.scope.problemReporter().noMoreAvailableSpaceForConstant(
                        this.classFile.referenceBinding.scope.referenceType());
            }
            if (index > 0xFFFF) {
                this.classFile.referenceBinding.scope.problemReporter().noMoreAvailableSpaceInConstantPool(
    ...
    /**
     * @param key1
     *         the given declaring class name
     * @param key2
     *         the given field name or method selector
     * @param key3
     *         the given signature
     * @param value
     *         the new index
     ...
     *
     * @see com.codenvy.eclipse.jdt.internal.compiler.ast.TypeDeclaration#addClinit()
     */
     ...
     *
     * @param value
     *         <CODE>int</CODE> The value to write into the byte array
     */
