...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.util.JavaConventionsUtil;
...
public abstract class RenameMethodProcessor extends JavaRenameProcessor implements IReferenceUpdating, IDelegateUpdating
{
   ...
   public final RefactoringStatus checkNewElementName(String newName)
   {
      ...
      {
         status = RefactoringStatus.createWarningStatus(
            fIsComposite ? Messages.format(RefactoringCoreMessages.Checks_method_names_lowercase2,
               new String[]{BasicElementLabels.getJavaElementName(
                  newName), getDeclaringTypeLabel()}) : RefactoringCoreMessages.Checks_method_names_lowercase);
      }

      if (Checks.isAlreadyNamed(fMethod, newName))
      {
         status.addFatalError(fIsComposite ? Messages.format(RefactoringCoreMessages.RenameMethodRefactoring_same_name2,
            new String[]{BasicElementLabels.getJavaElementName(
               newName), getDeclaringTypeLabel()}) : RefactoringCoreMessages.RenameMethodRefactoring_same_name,
   ...
   private void initializeMethodsToRename(IProgressMonitor pm,
   ...
   private RefactoringStatus checkRelatedMethods() throws CoreException
   {
      ...
      for (Iterator<IMethod> iter = fMethodsToRename.iterator(); iter.hasNext(); )
      {
         ...
         result.merge(
            Checks.checkIfConstructorName(method, getNewElementName(), method.getDeclaringType().getElementName()));

         String[] msgData = new String[]{BasicElementLabels.getJavaElementName(
            method.getElementName()), BasicElementLabels.getJavaElementName(
   ...
   private RefactoringStatus analyzeRenameChanges(IProgressMonitor pm) throws CoreException
   {
      ...
      try
      {
         ...
         newDeclarationWCs = RenameAnalyzeUtil.createNewWorkingCopies(declarationCUs, fChangeManager, fWorkingCopyOwner,
         ...
         result.merge(
            RenameAnalyzeUtil.analyzeRenameChanges2(fChangeManager, fOccurrences, newOccurrences, getNewElementName()));
         return result;
      }
      finally
      {
      ...
      String newName) throws CoreException
   {
      try
      {
         ...
         IMethod[] foundInImplementingClasses = classesDeclareMethodName(hierarchy, Arrays.asList(implementingClasses),
            method, newName);
   ...
   public Change createChange(IProgressMonitor monitor) throws CoreException
   {
      try
      {
         ...
         final String header = Messages.format(RefactoringCoreMessages.RenameMethodProcessor_descriptor_description,
            new String[]{JavaElementLabels.getTextLabel(fMethod,
               JavaElementLabels.ALL_FULLY_QUALIFIED), BasicElementLabels.getJavaElementName(getNewElementName())});
         ...
         if (declarationsInThisCu.size() > 0)
         {
            ...
            if (fDelegateUpdating)
            {
               ...
               for (Iterator<SearchMatch> iter = declarationsInThisCu.iterator(); iter.hasNext(); )
               {
                  ...
                  MethodDeclaration method = ASTNodeSearchUtil.getMethodDeclarationNode((IMethod)element.getElement(),
                     rewrite.getRoot());
   ...
   protected final ReplaceEdit createReplaceEdit(SearchMatch searchResult, ICompilationUnit cu)
   {
      ...
      { // handle Annotation Element references, see bug 94062
         StringBuffer sb = new StringBuffer(getNewElementName());
         if (JavaCore.INSERT.equals(cu.getJavaProject().getOption(
