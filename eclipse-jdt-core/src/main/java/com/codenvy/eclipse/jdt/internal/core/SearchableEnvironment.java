...
import com.codenvy.eclipse.jdt.internal.core.search.IRestrictedAccessConstructorRequestor;
...
public class SearchableEnvironment implements INameEnvironment, IJavaSearchConstants
{
   ...
   protected com.codenvy.eclipse.jdt.core.ICompilationUnit[] workingCopies;
   ...
   public SearchableEnvironment(JavaProject project,
      com.codenvy.eclipse.jdt.core.ICompilationUnit[] workingCopies) throws JavaModelException
   {
      this.project = project;
      this.checkAccessRestrictions = !JavaCore.IGNORE.equals(
         project.getOption(JavaCore.COMPILER_PB_FORBIDDEN_REFERENCE, true)) || !JavaCore.IGNORE.equals(
         ...
         { //SourceType
            try
            {
               ...
               for (int i = 0, index = 1; i < length; i++)
               {
                  ...
                  if (!otherType.equals(
                     topLevelType) && index < length) // check that the index is in bounds (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=62861)
                  {
   ...
   public void findExactTypes(char[] name, final boolean findMembers, int searchFor, final ISearchRequestor storage)
   {
      ...
      try
      {
         ...
         try
         {
            new BasicSearchEngine(this.workingCopies).searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH, name,
               SearchPattern.R_EXACT_MATCH, searchFor, getSearchScope(), typeRequestor, CANCEL_IF_NOT_READY_TO_SEARCH,
               progressMonitor);
         }
   ...
   /**
    * @see com.codenvy.eclipse.jdt.internal.compiler.env.INameEnvironment#findType(char[][])
    */
   ...
   /**
    * @see com.codenvy.eclipse.jdt.internal.compiler.env.INameEnvironment#findType(char[], char[][])
    */
      ...
      try
      {
         ...
         if (monitor != null)
         {
            ...
            if (indexManager.awaitingJobsCount() == 0)
            {
               ...
               new BasicSearchEngine(this.workingCopies).searchAllTypeNames(qualification, SearchPattern.R_EXACT_MATCH,
                  simpleName, matchRule, // not case sensitive
         ...
         else
         {
            try
            {
               new BasicSearchEngine(this.workingCopies).searchAllTypeNames(qualification, SearchPattern.R_EXACT_MATCH,
                  simpleName, matchRule, // not case sensitive
                  searchFor, getSearchScope(), typeRequestor, CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
            }
            catch (OperationCanceledException e)
            {
               findTypes(new String(prefix), storage, convertSearchFilterToModelFilter(searchFor));
      ...
      IProgressMonitor monitor)
   {
      try
      {
         ...
         if (monitor != null)
         {
            ...
            new BasicSearchEngine(this.workingCopies).searchAllConstructorDeclarations(qualification, simpleName,
               matchRule, getSearchScope(), constructorRequestor, FORCE_IMMEDIATE_SEARCH, progressMonitor);
         }
         else
         {
            try
            {
               new BasicSearchEngine(this.workingCopies).searchAllConstructorDeclarations(qualification, simpleName,
                  matchRule, getSearchScope(), constructorRequestor, CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
            }
            catch (OperationCanceledException e)
            {
               // Do nothing
   ...
   /**
    * @see com.codenvy.eclipse.jdt.internal.compiler.env.INameEnvironment#isPackage(char[][], char[])
    */
