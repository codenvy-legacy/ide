...
public class ASTNodes {
     ...
     * @param extendedRange
     *         if set, the extended ranges of the nodes should ne used
     * @param removeIndent
     *         if set, the indentation is removed.
            ...
            try {
                if (typeRoot != null && typeRoot.getBuffer() != null) {
                    ...
                    if (removeIndent) {
                        ...
                        str = Strings.changeIndent(str, indent, project, new String(),
    ...
    public static boolean isControlStatementBody(StructuralPropertyDescriptor locationInParent) {
        return locationInParent == IfStatement.THEN_STATEMENT_PROPERTY || locationInParent == IfStatement.ELSE_STATEMENT_PROPERTY ||
               locationInParent == ForStatement.BODY_PROPERTY || locationInParent == EnhancedForStatement.BODY_PROPERTY ||
               locationInParent == WhileStatement.BODY_PROPERTY || locationInParent == DoStatement.BODY_PROPERTY;
    }
    ...
    private static boolean needsExplicitBoxing(Expression expression) {
        ...
        if (locationInParent == ClassInstanceCreation.EXPRESSION_PROPERTY || locationInParent == FieldAccess.EXPRESSION_PROPERTY ||
     ...
     *         the node
     * @param parentClass
     *         the class of the sought ancestor node
    ...
    public static ASTNode getNormalizedNode(ASTNode node) {
        ...
        // normalize type
        if (QualifiedType.NAME_PROPERTY.equals(current.getLocationInParent()) || SimpleType.NAME_PROPERTY.equals(
                current.getLocationInParent())) {
            current = current.getParent();
        }
     ...
     *
     * @param invocation
     *         method invocation to resolve type of
    ...
    public static ITypeBinding getTypeBinding(CompilationUnit root, IType type) throws JavaModelException {
        if (type.isAnonymous()) {
            ...
            final AbstractTypeDeclaration declaration = (AbstractTypeDeclaration)getParent(
    ...
    public static String getEscapedCharacterLiteral(char ch) {
        CharacterLiteral characterLiteral = AST.newAST(AST.JLS4).newCharacterLiteral();
