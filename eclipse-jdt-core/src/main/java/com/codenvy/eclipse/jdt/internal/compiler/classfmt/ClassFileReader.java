 ...
 *     Stephan Herrmann - Contribution for bug 365992 - [builder] [null] Change of nullness for a parameter doesn't trigger a build for
 *     the files that call the method
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.compiler.classfmt;
...
public class ClassFileReader extends ClassFileStruct implements IBinaryType {
    ...
    public static ClassFileReader read(InputStream stream, String fileName,
                                       boolean fullyInitialize) throws ClassFormatException, IOException {
    ...
    public static ClassFileReader read(java.util.zip.ZipFile zip,
                                       String filename) throws ClassFormatException, java.io.IOException {
        return read(zip, filename, false);
    }

    public static ClassFileReader read(java.util.zip.ZipFile zip, String filename,
                                       boolean fullyInitialize) throws ClassFormatException, java.io.IOException {
        ...
        try {
            ...
            if (this.methodsCount != 0) {
                ...
                for (int i = 0; i < this.methodsCount; i++) {
                    this.methods[i] = isAnnotationType ? AnnotationMethodInfo.createAnnotationMethod(this.reference,
                                                                                                     this.constantPoolOffsets, readOffset)
                                                       : MethodInfo.createMethod(this.reference,
                                                                                 this.constantPoolOffsets, readOffset);
                        ...
                        if (CharOperation.equals(attributeName, AttributeNamesConstants.InnerClassName)) {
                            ...
                            if (number_of_classes != 0) {
                                ...
                                for (int j = 0; j < number_of_classes; j++) {
                                    this.innerInfos[j] = new InnerClassInfo(this.reference, this.constantPoolOffsets,
                                                                            innerOffset);
    ...
    private void decodeAnnotations(int offset, boolean runtimeVisible) {
        ...
        if (numberOfAnnotations > 0) {
            ...
            for (int i = 0; i < numberOfAnnotations; i++) {
                ...
                AnnotationInfo newInfo = new AnnotationInfo(this.reference, this.constantPoolOffsets, readOffset,
    ...
    */
        ...
        try {
            ...
            if (currentMemberTypes != otherMemberTypes) { // TypeConstants.NoMemberTypes
                ...
                for (int i = 0; i < currentMemberTypeLength; i++) {
                    if (!CharOperation.equals(currentMemberTypes[i].getName(),
                                              otherMemberTypes[i].getName()) ||
    ...
    private boolean hasStructuralMethodChanges(MethodInfo currentMethodInfo, MethodInfo otherMethodInfo) {
        ...
        for (int i = 0; i < currentAnnotatedParamsCount; i++) {
            if (hasStructuralAnnotationChanges(currentMethodInfo.getParameterAnnotations(i),
                                               otherMethodInfo.getParameterAnnotations(i))) {
                return true;
            }
        }
    ...
    public String toString() {
        ...
        print.println(" this.superclassName: " + (getSuperclassName() == null ? "null" : new String(
                getSuperclassName()))); //$NON-NLS-2$ //$NON-NLS-1$
        print.println(" access_flags: " + printTypeModifiers(
                accessFlags()) + "(" + accessFlags() + ")"); //$NON-NLS-1$ //$NON-NLS-3$ //$NON-NLS-2$
        print.flush();
        return out.toString();
    }
}
