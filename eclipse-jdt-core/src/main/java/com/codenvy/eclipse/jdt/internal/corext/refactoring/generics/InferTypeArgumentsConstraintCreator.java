...
package com.codenvy.eclipse.jdt.internal.corext.refactoring.generics;
...
import com.codenvy.eclipse.jdt.internal.corext.dom.HierarchicalASTVisitor;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.rename.MethodChecks;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.GenericType;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.ParameterizedType;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.TType;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.TypeVariable;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.WildcardType;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ArrayElementVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.CollectionElementVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ConstraintVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ImmutableTypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.IndependentTypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ParameterTypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ParameterizedTypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ReturnTypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.TTypes;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.TypeVariable2;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.VariableVariable2;
...
public class InferTypeArgumentsConstraintCreator extends HierarchicalASTVisitor
{
            ...
            if (receiver != null)
            {
               ...
               if (receiverCv != null)
               {
                  ...
                  fTCModel.createTypeVariablesEqualityConstraints(receiverCv,
                     Collections.<String, IndependentTypeVariable2>emptyMap(), returnTypeCv, declaredVariableTType);
                  return;
               }
            }
   ...
   public void endVisit(Assignment node)
   {
      ...
      if (op == Assignment.Operator.PLUS_ASSIGN && (lhs.resolveTypeBinding() == node.getAST().resolveWellKnownType(
         "java.lang.String")))
      { //$NON-NLS-1$
   ...
   public void endVisit(CastExpression node)
   {
      ...
      if (!(expressionCv instanceof TypeVariable2 || expressionCv instanceof IndependentTypeVariable2 || expressionCv instanceof CollectionElementVariable2) && fTCModel.getElementVariables(
         expressionCv).size() == 0 && fTCModel.getArrayElementVariable(expressionCv) == null)
      {
         return;
      }
   ...
   private void addConstraintsForOverriding(IMethodBinding methodBinding, ConstraintVariable2 returnTypeCv,
   ...
   public void endVisit(MethodInvocation node)
   {
      ...
      { //$NON-NLS-1$
         ...
         setConstraintVariable(node, expressionCv);
         ConstraintVariable2 classTypeVariable = fTCModel.getElementVariable(expressionCv,
            returnTypeDeclaration.getTypeParameters()[0]);
   ...
   private void doVisitMethodInvocationReturnType(MethodInvocation node, IMethodBinding methodBinding,
      Expression receiver, Map<String, IndependentTypeVariable2> methodTypeVariables)
   {
      ...
      {
         ConstraintVariable2 returnTypeCv = fTCModel.makeParameterizedTypeVariable(
            declaredReturnType.getTypeDeclaration());
         setConstraintVariable(node, returnTypeCv);
         ...
         fTCModel.createTypeVariablesEqualityConstraints(receiverCv, methodTypeVariables, returnTypeCv,
            declaredReturnTType);
   ...
   {
      return fAssumeCloneReturnsSameType && "clone".equals(methodBinding.getName()) //$NON-NLS-1$
         && methodBinding.getParameterTypes().length == 0 && receiver != null && receiver.resolveTypeBinding() != methodBinding.getMethodDeclaration().getReturnType();
   }

   private void doVisitMethodInvocationArguments(IMethodBinding methodBinding, List<Expression> arguments,
      Expression receiver, Map<String, IndependentTypeVariable2> methodTypeVariables, Type createdType)
   {
      ...
      for (int i = 0; i < arguments.size(); i++)
      {
         ...
         { // isVararg() && i >= lastParamIdx
            iParam = lastParamIdx;
            declaredParameterType = fTCModel.createTType(declaredParameterTypes[iParam]);
            if (i == lastParamIdx && canAssignToVararg(fTCModel.createTType(arg.resolveTypeBinding()),
               (com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.ArrayType)declaredParameterType))
            {
               //OK: argument will not be packed into an array
            }
            else
            {
               declaredParameterType = ((com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.ArrayType)declaredParameterType).getComponentType();
            }
         }
         ...
         if (declaredParameterType.isTypeVariable())
         {
            ...
            else
            {
               if (createdType != null)
               {
                  ...
                  ConstraintVariable2 elementCv = fTCModel.getElementVariable(createdTypeCv,
                     (TypeVariable)declaredParameterType);
               ...
               if (receiver != null)
               {
                  ...
                  ConstraintVariable2 expressionCv = getConstraintVariable(receiver);
                  ConstraintVariable2 elementCv = fTCModel.getElementVariable(expressionCv,
                     (TypeVariable)declaredParameterType);
               ...
               if (typeArgument.isWildcardType())
               {
                  ...
                  if (bound != null && bound.isTypeVariable())
                  {
                     ...
                     else
                     {
                        ...
                        {
                           ConstraintVariable2 createdTypeCv = getConstraintVariable(createdType);
                           CollectionElementVariable2 elementCv = fTCModel.getElementVariable(createdTypeCv,
                              typeParameters[ta]);
                        ...
                        if (receiver != null)
                        {
                           ...
                           ConstraintVariable2 expressionCv = getConstraintVariable(receiver);
                           CollectionElementVariable2 elementCv = fTCModel.getElementVariable(expressionCv,
                              typeParameters[ta]);
                           createWildcardConstraint(wildcardTypeArgument, argElementCv, elementCv);
                        }
                        else
         ...
         else if (declaredParameterType.isArrayType())
         {
            ...
            TType declaredElementType = ((com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.ArrayType)declaredParameterType).getElementType();
   ...
   private boolean canAssignToVararg(TType rhs,
      com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.ArrayType lhs)
   {
      return TTypes.canAssignTo(rhs.getErasure(), lhs.getErasure());
   }

   private void createWildcardConstraint(WildcardType typeArgument, CollectionElementVariable2 argElementCv,
    ...
    * @return the {@link com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ConstraintVariable2} associated with the node, or <code>null</code>
    */
   ...
   /**
    * @param node               the ASTNode
    * @param constraintVariable the {@link com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints2.ConstraintVariable2} to be associated with node
    */
