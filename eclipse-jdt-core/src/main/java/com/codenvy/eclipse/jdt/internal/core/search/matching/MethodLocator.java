...
//public int match(TypeReference node, MatchingNodeSet nodeSet) - SKIP IT
    ...
    /* (non-Javadoc)
     * @see org.eclipse.jdt.internal.core.search.matching.PatternLocator#matchLevelAndReportImportRef(org.eclipse.jdt.internal.compiler
     * .ast.ImportReference, org.eclipse.jdt.internal.compiler.lookup.Binding, org.eclipse.jdt.internal.core.search.matching.MatchLocator)
    ...
    protected int matchMethod(MethodBinding method, boolean skipImpossibleArg) {
        ...
        if (this.pattern.declaringSimpleName == null) {
            ...
            // this.pattern.returnTypeArguments, 0, method.returnType);
...
//				 newLevel = resolveLevelForType(this.pattern.parameterSimpleNames[i], this.pattern.parameterQualifications[i],
// this.pattern.parametersTypeArguments[i], 0, argType);
            ...
            if (foundTypeVariable) {
                if (!method.isStatic() && !method.isPrivate()) {
                    // https://bugs.eclipse.org/bugs/show_bug.cgi?id=123836, No point in textually comparing type variables,
                    // captures etc with concrete types.
    ...
    /**
     * @see com.codenvy.eclipse.jdt.internal.core.search.matching.PatternLocator#matchReportReference(com.codenvy.eclipse.jdt.internal
     * .compiler.ast.ASTNode,
     *      com.codenvy.eclipse.jdt.core.IJavaElement, Binding, int, com.codenvy.eclipse.jdt.internal.core.search.matching.MatchLocator)
     */
    protected void matchReportReference(ASTNode reference, IJavaElement element, IJavaElement localElement, IJavaElement[] otherElements,
        ...
        MethodBinding methodBinding = (reference instanceof MessageSend) ? ((MessageSend)reference).binding
                                                                         : ((elementBinding instanceof MethodBinding)
                                                                            ? (MethodBinding)elementBinding : null);
    ...
    void matchReportReference(MessageSend messageSend, MatchLocator locator, int accuracy, MethodBinding methodBinding)
        ...
        boolean report = (this.isErasureMatch && this.match.isErasure()) || (this.isEquivalentMatch && this.match.isEquivalent()) ||
                         this.match.isExact();
        if (!report) return;
    ...
    public SearchMatch newDeclarationMatch(ASTNode reference, IJavaElement element, Binding elementBinding, int accuracy, int length,
    ...
    protected void reportDeclaration(MethodBinding methodBinding, MatchLocator locator, SimpleSet knownMethods) throws CoreException {
        ...
        if (type.isBinary()) {
            ...
            IBinaryType info = locator.getBinaryInfo((com.codenvy.eclipse.jdt.internal.core.ClassFile)type.getClassFile(), resource);
        ...
        if (scope != null) {
            ...
            if (methodDecl != null) {
                ...
                this.match = new MethodDeclarationMatch(method, SearchMatch.A_ACCURATE, offset, methodDecl.sourceEnd - offset + 1,
    ...
    public int resolveLevel(Binding binding) {
        ...
        if (subType && this.pattern.declaringQualification != null && method.declaringClass != null &&
            ...
            subType =
                    CharOperation.compareWith(this.pattern.declaringQualification, method.declaringClass.fPackage.shortReadableName()) == 0;
        }
        int declaringLevel = subType
                             ? resolveLevelAsSubtype(this.pattern.declaringSimpleName, this.pattern.declaringQualification,
                                                     method.declaringClass, method.selector, null,
                                                     method.declaringClass.qualifiedPackageName(), method.isDefault())
    ...
    protected int resolveLevel(MessageSend messageSend) {
        ...
        if (isVirtualInvoke(method, messageSend) && (messageSend.actualReceiverType instanceof ReferenceBinding)) {
            ...
            declaringLevel =
                    resolveLevelAsSubtype(this.pattern.declaringSimpleName, this.pattern.declaringQualification, methodReceiverType,
                                          method.selector, method.parameters, methodReceiverType.qualifiedPackageName(),
                                          method.isDefault());
            if (declaringLevel == IMPOSSIBLE_MATCH) {
                if (method.declaringClass == null || this.allSuperDeclaringTypeNames == null) {
                    declaringLevel = INACCURATE_MATCH;
                } else {
                    char[][][] superTypeNames = (method.isDefault() && this.pattern.focus == null) ? this.samePkgSuperDeclaringTypeNames
                                                                                                   : this.allSuperDeclaringTypeNames;
                    if (superTypeNames != null &&
                        resolveLevelAsSuperInvocation(methodReceiverType, method.parameters, superTypeNames, true)) {
    ...
    protected int resolveLevelAsSubtype(char[] simplePattern, char[] qualifiedPattern, ReferenceBinding type, char[] methodName,
                                        TypeBinding[] argumentTypes, char[] packageName, boolean isDefault) {
        ...
        // matches superclass
        if (!type.isInterface() && !CharOperation.equals(type.compoundName, TypeConstants.JAVA_LANG_OBJECT)) {
            level = resolveLevelAsSubtype(simplePattern, qualifiedPattern, type.superclass(), methodName, argumentTypes, packageName,
                                          isDefault);
            if (level != IMPOSSIBLE_MATCH) {
     ...
     */
    private boolean resolveLevelAsSuperInvocation(ReferenceBinding type, TypeBinding[] argumentTypes, char[][][] superTypeNames,
