...
public class NullInfoRegistry extends UnconditionalFlowInfo {
     ...
     *
     * @param upstream
     *         - UnconditionalFlowInfo: the flow info before we enter the
    ...
    public NullInfoRegistry(UnconditionalFlowInfo upstream) {
        ...
        if ((upstream.tagBits & NULL_FLAG_MASK) != 0) {
            ...
            this.nullBit2 = (u1 = upstream.nullBit1) & (u2 = upstream.nullBit2) & (nu3 = ~(u3 = upstream.nullBit3)) &
            ...
            if (upstream.extra != null) {
                ...
                for (int i = 0; i < length; i++) {
                    this.extra[2 + 1][i] =
                            (u1 = upstream.extra[1 + 1][i]) & (u2 = upstream.extra[2 + 1][i]) & (nu3 = ~(u3 = upstream.extra[3 + 1][i])) &
     ...
     *
     * @param flowInfo
     *         - FlowInfo: the flow information that this null info
    ...
    public UnconditionalFlowInfo mitigateNullInfoOf(FlowInfo flowInfo) {
        ...
        if (this.extra != null && source.extra != null) {
            ...
            for (int i = 0; i < length; i++) {
                m1 = (s1 = source.extra[1 + 1][i]) & (s3 = source.extra[3 + 1][i]) & (s4 = source.extra[4 + 1][i]) &
