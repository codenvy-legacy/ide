...
import com.codenvy.eclipse.jdt.internal.compiler.classfmt.FieldInfo;
import com.codenvy.eclipse.jdt.internal.compiler.classfmt.MethodInfo;
...
import com.codenvy.eclipse.jdt.internal.core.ResolvedBinaryField;
import com.codenvy.eclipse.jdt.internal.core.ResolvedBinaryMethod;
import com.codenvy.eclipse.jdt.internal.core.ResolvedBinaryType;
...
public class ClassFileMatchLocator implements IIndexConstants {
                ...
                for (int i = 0; i < aMethodsLength; i++) {
                    ...
                    if (level != PatternLocator.IMPOSSIBLE_MATCH) {
                        IMethod methodHandle = binaryType.getMethod(
                                new String(
                                        method.isConstructor() ? binding.compoundName[binding.compoundName.length - 1] : method.selector),
    ...
    private void matchAnnotations(SearchPattern pattern, MatchLocator locator, ClassFile classFile, IBinaryType binaryType)
        ...
        if (checkAnnotations(typeReferencePattern, annotations, binaryType.getTagBits())) {
            ...
            TypeReferenceMatch match =
                    new TypeReferenceMatch(classFileBinaryType, SearchMatch.A_ACCURATE, -1, 0, false, locator.getParticipant(),
            ...
            for (int i = 0, max = methods.length; i < max; i++) {
                ...
                if (checkAnnotations(typeReferencePattern, method.getAnnotations(), method.getTagBits())) {
                    ...
                    IMethod methodHandle = classFileBinaryType.getMethod(
                            new String(method.isConstructor() ? binaryTypeBinding.compoundName[binaryTypeBinding.compoundName.length - 1]
                                                              : method.getSelector()),
                            CharOperation.toStrings(Signature.getParameterTypes(convertClassFileFormat(method.getMethodDescriptor()))));
                    TypeReferenceMatch match =
                            new TypeReferenceMatch(methodHandle, SearchMatch.A_ACCURATE, -1, 0, false, locator.getParticipant(),
                                                   locator.currentPossibleMatch.resource);
                    // TODO 3.4 M7 (frederic) - bug 209996: see how create the annotation handle from the binary and put it in the local
                    // element
                    match.setLocalElement(null);
                    locator.report(match);
                }
        ...
        if (fields != null) {
            for (int i = 0, max = fields.length; i < max; i++) {
                ...
                if (checkAnnotations(typeReferencePattern, field.getAnnotations(), field.getTagBits())) {
                    ...
                    TypeReferenceMatch match =
                            new TypeReferenceMatch(fieldHandle, SearchMatch.A_ACCURATE, -1, 0, false, locator.getParticipant(),
                                                   locator.currentPossibleMatch.resource);
                    // TODO 3.4 M7 (frederic) - bug 209996: see how create the annotation handle from the binary and put it in the local
                    // element
                    match.setLocalElement(null);
    ...
    boolean matchMethod(MethodPattern pattern, Object binaryInfo, IBinaryType enclosingBinaryType) {
        ...
        boolean checkReturnType =
                pattern.declaringSimpleName == null && (pattern.returnSimpleName != null || pattern.returnQualification != null);
        ...
        if (checkReturnType || checkParameters) {
            ...
            if (checkParameters &&
                !checkParameters(methodDescriptor, pattern.parameterSimpleNames, pattern.parameterQualifications, pattern.isCaseSensitive(),
                                 pattern.isCamelCase()))
                return false;
        }
        return true;
