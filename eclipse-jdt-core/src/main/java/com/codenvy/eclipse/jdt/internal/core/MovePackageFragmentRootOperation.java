...
public class MovePackageFragmentRootOperation extends CopyPackageFragmentRootOperation {
    ...
    protected void renameEntryInClasspath(IPath rootPath, IJavaProject project) throws JavaModelException {
        ...
        for (int i = 0; i < cpLength; i++) {
            ...
            if (rootPath.equals(entryPath)) {
                ...
                if (newExclusionPatterns != null || newInclusionPatterns != null) {
                    ...
                    newClasspath[newCPIndex++] = JavaCore.newSourceEntry(entry.getPath(),
                                                                         newInclusionPatterns == null ? entry.getInclusionPatterns()
                                                                                                      : newInclusionPatterns,
                                                                         newExclusionPatterns == null ? entry.getExclusionPatterns()
        ...
        if (newClasspath != null) {
            ...
            IJavaModelStatus status = JavaConventions.validateClasspath(project, newClasspath,
                                                                        project.getOutputLocation());
            if (status.isOK()) {
                project.setRawClasspath(newClasspath, this.progressMonitor);
            }
            // don't update classpath if status is not ok to avoid JavaModelException (see https://bugs.eclipse.org/bugs/show_bug
            // .cgi?id=129991)
        }
    }
    ...
    public MovePackageFragmentRootOperation(IPackageFragmentRoot root, IPath destination, int updateResourceFlags,
    ...
    protected void moveResource(IPackageFragmentRoot root, IClasspathEntry rootEntry,
        ...
        if (rootEntry.getEntryKind() != IClasspathEntry.CPE_SOURCE || exclusionPatterns == null) {
            try {
                IResource destRes;
                if ((this.updateModelFlags & IPackageFragmentRoot.REPLACE) != 0 && (destRes = workspaceRoot.findMember(
                    ...
                    if (proxy.getType() == IResource.FOLDER) {
                        ...
                        if (prefixesOneOf(path, nestedFolders)) {
                            if (equalsOneOf(path, nestedFolders)) {
                                ...
                                folder.create(MovePackageFragmentRootOperation.this.updateResourceFlags, true,
                                              MovePackageFragmentRootOperation.this.progressMonitor);
                                return true;
                            }
                        } else {
                            // subtree doesn't contain any nested source folders
                            IPath destPath = MovePackageFragmentRootOperation.this.destination.append(
                                    path.removeFirstSegments(sourceSegmentCount));
                            IResource destRes;
                            if ((MovePackageFragmentRootOperation.this.updateModelFlags & IPackageFragmentRoot.REPLACE) != 0 &&
                                (destRes = workspaceRoot.findMember(
                                        destPath)) != null) {
                                destRes.delete(MovePackageFragmentRootOperation.this.updateResourceFlags,
                                               MovePackageFragmentRootOperation.this.progressMonitor);
                            }
                            proxy.requestResource().move(destPath, MovePackageFragmentRootOperation.this.updateResourceFlags,
                                                         MovePackageFragmentRootOperation.this.progressMonitor);
                            return false;
                        }
                    } else {
