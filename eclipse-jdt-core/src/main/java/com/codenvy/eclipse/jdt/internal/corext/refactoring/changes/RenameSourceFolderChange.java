...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.AbstractJavaElementRenameChange;
...
public final class RenameSourceFolderChange extends AbstractJavaElementRenameChange {
    ...
    private static RefactoringStatus checkIfModifiable(IPackageFragmentRoot root) throws CoreException {
        ...
        if (root.isArchive()) {
            result.addFatalError(
                    Messages.format(RefactoringCoreMessages.RenameSourceFolderChange_rename_archive, getRootLabel(root)));
            return result;
        }

        if (root.isExternal()) {
            result.addFatalError(
                    Messages.format(RefactoringCoreMessages.RenameSourceFolderChange_rename_external, getRootLabel(root)));
            return result;
        }
        ...
        if (correspondingResource == null || !correspondingResource.exists()) {
            result.addFatalError(
                    Messages.format(RefactoringCoreMessages.RenameSourceFolderChange_error_underlying_resource_not_existing,
                                    getRootLabel(root)));
            return result;
        }
        ...
        if (correspondingResource.isLinked()) {
            result.addFatalError(
                    Messages.format(RefactoringCoreMessages.RenameSourceFolderChange_rename_linked, getRootLabel(root)));
            return result;
        }
    ...
    private int getJavaModelUpdateFlags() {
        return IPackageFragmentRoot.DESTINATION_PROJECT_CLASSPATH | IPackageFragmentRoot.ORIGINATING_PROJECT_CLASSPATH |
               IPackageFragmentRoot.OTHER_REFERRING_PROJECTS_CLASSPATH | IPackageFragmentRoot.REPLACE;
    }

    @Override
    public String getName() {
        String[] keys = {BasicElementLabels.getJavaElementName(getOldName()), BasicElementLabels.getJavaElementName(
                getNewName())};
