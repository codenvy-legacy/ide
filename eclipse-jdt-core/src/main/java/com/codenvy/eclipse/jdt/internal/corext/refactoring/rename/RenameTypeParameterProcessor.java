...
public class RenameTypeParameterProcessor extends JavaRenameProcessor implements IReferenceUpdating
{
   ...
   private class RenameTypeParameterVisitor extends HierarchicalASTVisitor
   {
      ...
      public boolean visit(SimpleName node)
      {
         ...
         if (fBinding == binding)
         {
            ...
            {
               fRewrite.getASTRewrite().set(node, SimpleName.IDENTIFIER_PROPERTY, getNewElementName(),
      ...
      public boolean visit(AbstractTypeDeclaration node)
      {
         ...
         {
            fStatus.addError(
               Messages.format(RefactoringCoreMessages.RenameTypeParameterRefactoring_type_parameter_inner_class_clash,
                  new String[]{name}),
               JavaStatusContext.create(fTypeParameter.getDeclaringMember().getCompilationUnit(),
   ...
   {
      return 3; // RefactoringSaveHelper.SAVE_NOTHING;
   }

   @Override
   protected RefactoringStatus doCheckFinalConditions(IProgressMonitor monitor,
      ...
      IProgressMonitor monitor) throws CoreException, OperationCanceledException
   {
      ...
      if (!fTypeParameter.exists())
      {
         return RefactoringStatus.createFatalErrorStatus(
            Messages.format(RefactoringCoreMessages.RenameTypeParameterRefactoring_deleted,
   ...
   public RefactoringStatus checkNewElementName(String name) throws CoreException
   {
      ...
      if (member instanceof IType)
      {
         ...
         if (type.getTypeParameter(name).exists())
         {
            result.addFatalError(
               RefactoringCoreMessages.RenameTypeParameterRefactoring_class_type_parameter_already_defined);
         }
      }
      else if (member instanceof IMethod)
      {
         ...
         {
            result.addFatalError(
               RefactoringCoreMessages.RenameTypeParameterRefactoring_method_type_parameter_already_defined);
         }
      }
      else
      {
         Util.log(new Exception("Unexpected sub-type of IMember: " + member.getClass().getName())); //$NON-NLS-1$
         Assert.isTrue(false);
      }
      return result;
   }
   ...
   public Change createChange(IProgressMonitor monitor) throws CoreException, OperationCanceledException
   {
      ...
      try
      {
         ...
         if (change != null)
         {
            ...
            String header = Messages.format(RefactoringCoreMessages.RenameTypeParameterProcessor_descriptor_description,
               new String[]{BasicElementLabels.getJavaElementName(
                  fTypeParameter.getElementName()), JavaElementLabels.getElementLabel(
                  fTypeParameter.getDeclaringMember(),
            ...
            change = new DynamicValidationRefactoringChange(descriptor,
               RefactoringCoreMessages.RenameTypeParameterProcessor_change_name, new Change[]{change});
         }
         return change;
      }
      finally
   ...
   private RefactoringStatus createRenameChanges(IProgressMonitor monitor) throws CoreException
   {
      ...
      try
      {
         ...
         monitor.worked(1);
         RenameTypeParameterVisitor visitor = new RenameTypeParameterVisitor(rewrite, fTypeParameter.getNameRange(),
            status);
         if (declaration != null)
         {
   ...
   private RefactoringStatus initialize(JavaRefactoringArguments extended)
   {
      ...
      {
         return RefactoringStatus.createFatalErrorStatus(
            Messages.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_PARAMETER));
      }
      ...
      if (handle != null)
      {
         ...
         else
         {
            ...
            {
               return JavaRefactoringDescriptorUtil.createInputFatalStatus(fTypeParameter, getProcessorName(),
                  IJavaRefactorings.RENAME_TYPE_PARAMETER);
            }
         }
      }
