...
public class JavaElementDelta extends SimpleDelta implements IJavaElementDelta {
    ...
    protected void addAffectedChild(JavaElementDelta child) {
        ...
        for (int i = 0; i < this.affectedChildren.length; i++) {
            if (equalsAndSameParent(this.affectedChildren[i].getElement(),
                                    child.getElement())) { // handle case of two jars that can be equals but not in the same project
    ...
    protected void addResourceDelta(IResourceDelta child) {
        ...
        if (this.resourceDeltas.length == this.resourceDeltasCounter) {
            ...
            System.arraycopy(this.resourceDeltas, 0,
                             (this.resourceDeltas = new IResourceDelta[this.resourceDeltasCounter * 2]), 0, this.resourceDeltasCounter);
        }
        this.resourceDeltas[this.resourceDeltasCounter++] = child;
    }
    ...
    protected JavaElementDelta createDeltaTree(IJavaElement element, JavaElementDelta delta) {
        ...
        if (ancestors == null) {
            if (equalsAndSameParent(delta.getElement(),
                                    getElement())) { // handle case of two jars that can be equals but not in the same project
    ...
    protected JavaElementDelta getDeltaFor(IJavaElement element) {
        if (equalsAndSameParent(getElement(),
                                element)) // handle case of two jars that can be equals but not in the same project
        {
            return this;
        }
    ...
    public IResourceDelta[] getResourceDeltas() {
        ...
        if (this.resourceDeltas.length != this.resourceDeltasCounter) {
            System.arraycopy(this.resourceDeltas, 0, this.resourceDeltas = new IResourceDelta[this.resourceDeltasCounter],
                             0, this.resourceDeltasCounter);
        }
    ...
    protected boolean toDebugString(StringBuffer buffer, int flags) {
        ...
        if ((flags & IJavaElementDelta.F_MOVED_FROM) != 0) {
            ...
            buffer.append(
                    "MOVED_FROM(" + ((JavaElement)getMovedFromElement()).toStringWithAncestors() + ")"); //$NON-NLS-1$ //$NON-NLS-2$
            prev = true;
        }
        if ((flags & IJavaElementDelta.F_MOVED_TO) != 0) {
            ...
            buffer.append(
                    "MOVED_TO(" + ((JavaElement)getMovedToElement()).toStringWithAncestors() + ")"); //$NON-NLS-1$ //$NON-NLS-2$
            prev = true;
        }
