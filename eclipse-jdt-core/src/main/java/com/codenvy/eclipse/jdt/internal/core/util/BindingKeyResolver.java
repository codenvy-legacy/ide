...
public class BindingKeyResolver extends BindingKeyParser {
    ...
    public void consumeParameterizedGenericMethod() {
        ...
        if (arguments.length != this.methodBinding.typeVariables().length) {
            this.methodBinding = this.environment.createParameterizedGenericMethod(this.methodBinding,
                                                                                   (RawTypeBinding)null);
        } else {
        ...
        for (int i = 0, methodLength = methods.length; i < methodLength; i++) {
            ...
            if (CharOperation.equals(selector, method.selector) || (selector.length == 0 && method.isConstructor())) {
                ...
                if (CharOperation.equals(signature, methodSignature)) {
                    ...
                    PolymorphicMethodBinding polymorphicMethod = this.environment.createPolymorphicMethod(method,
                                                                                                          parameterTypes);
    ...
    public void consumeParameterizedType(char[] simpleTypeName, boolean isRaw) {
        ...
        if (simpleTypeName != null) {
            ...
            if (enclosing != null) {
                enclosing = (ReferenceBinding)this.environment
    ...
    public void consumeTopLevelType() {
        ...
        if (this.parsedUnit != null && this.compiler != null && !this.resolvedUnits.containsKey(
                fileName = this.parsedUnit.getFileName())) {
            this.compiler.process(this.parsedUnit,
                                  this.compiler.totalUnits + 1); // unit is resolved only once thanks to the resolvedUnits protection
        ...
        switch (kind) {
            ...
            case Wildcard.SUPER:
                ...
                if (boundBinding instanceof TypeBinding) {
                    this.typeBinding = this.environment.createWildcard((ReferenceBinding)this.typeBinding, this.wildcardRank,
    ...
    private TypeBinding getTypeBinding(char[] simpleTypeName) {
        ...
        TypeDeclaration[] typeDeclarations =
                this.typeDeclaration == null ? (this.parsedUnit == null ? null : this.parsedUnit.types) : this.typeDeclaration.memberTypes;
