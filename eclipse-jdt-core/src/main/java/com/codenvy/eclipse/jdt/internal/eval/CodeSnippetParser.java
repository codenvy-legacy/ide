...
public class CodeSnippetParser extends Parser implements EvaluationConstants {
    ...
    protected void consumeClassDeclaration() {
        super.consumeClassDeclaration();   /* recovery */
    ...
    protected void consumeMethodDeclaration(boolean isNotAbstract) {
        ...
        if (this.evaluationContext.localVariableNames != null) {
            ...
            for (int i = 0; i < varCount; i++) {
                ...
                LocalDeclaration declaration = new LocalDeclaration(this.evaluationContext.localVariableNames[i], start,
                                                                    end);
            ...
            for (int i = 0; i < varCount; i++) {
                SingleNameReference nameRef = new SingleNameReference(this.evaluationContext.localVariableNames[i],
                                                                      position);
                finallyBlock.statements[i] = new Assignment(new SingleNameReference(
                        CharOperation.concat(LOCAL_VAR_PREFIX, this.evaluationContext.localVariableNames[i]), position), nameRef,
                                                            nameRef.sourceEnd);
            }
    ...
    protected void consumePrimaryNoNewArrayThis() {
        ...
        if (this.scanner.startPosition >= this.codeSnippetStart &&
            ...
            pushOnExpressionStack(
                    new CodeSnippetThisReference(this.intStack[this.intPtr--], this.endPosition, this.evaluationContext,
                                                 false));
        } else {
    ...
    protected void consumeStatementReturn() {
        ...
        // support have to be defined at toplevel only
        if ((this.hasRecoveredOnExpression || (this.scanner.startPosition >= this.codeSnippetStart && this.scanner.startPosition <=
                                                                                                      this.codeSnippetEnd + 1 +
                                                                                                      this.lineSeparatorLength /*
                                                                                                      14838*/)) &&
            this.expressionLengthStack[this.expressionLengthPtr] != 0 && isTopLevelType()) {
    ...
    protected boolean resumeOnSyntaxError() {
        ...
        this.scanner.eofPosition =
                this.codeSnippetEnd < Integer.MAX_VALUE ? this.codeSnippetEnd + 1 : this.codeSnippetEnd; // stop after expression
