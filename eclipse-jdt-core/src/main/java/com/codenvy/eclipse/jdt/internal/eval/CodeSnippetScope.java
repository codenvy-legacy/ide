...
*/
      ...
      InvocationSite invocationSite, Scope scope)
   {
      ...
      {
         if (type.isCapture())
         { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=285002
...
*/
   ...
   public final boolean canBeSeenByForCodeSnippet(ReferenceBinding referenceBinding, ReferenceBinding receiverType)
   {
      ...
      if (referenceBinding.isProtected())
      {
         ...
         return receiverType.fPackage == referenceBinding.fPackage || referenceBinding.isSuperclassOf(
            receiverType) || referenceBinding.enclosingType().isSuperclassOf(
            receiverType); // protected types always have an enclosing one
      }
      ...
      InvocationSite invocationSite)
   {
      ...
      while (keepLooking)
      {
         ...
         if (itsInterfaces != Binding.NO_SUPERINTERFACES)
         {
            ...
            {
               System.arraycopy(interfacesToVisit, 0, interfacesToVisit = new ReferenceBinding[lastPosition * 2][], 0,
                  lastPosition);
            }
      ...
      if (interfacesToVisit != null)
      {
         ...
         com.codenvy.eclipse.jdt.internal.compiler.util.SimpleSet interfacesSeen = new com.codenvy.eclipse.jdt.internal.compiler.util.SimpleSet(
            lastPosition * 2);
         done:
            ...
            for (int j = 0, length = interfaces.length; j < length; j++)
            {
               ...
               if (interfacesSeen.addIfNotIncluded(anInterface) == anInterface)
               {
                  ...
                  if ((field = anInterface.getField(fieldName, true /*resolve*/)) != null)
                  {
                     ...
                     {
                        ambiguous = new ProblemFieldBinding(visibleField, visibleField.declaringClass, fieldName,
                           ProblemReasons.Ambiguous);
                        break done;
                     }
                  }
...
*/
      ...
      InvocationSite invocationSite)
   {
      ...
      {
         return new ProblemMethodBinding(TypeConstants.INIT, argumentTypes,
            ProblemReasons.NotFound); // need a more descriptive error... cannot convert from X to Y
      }
