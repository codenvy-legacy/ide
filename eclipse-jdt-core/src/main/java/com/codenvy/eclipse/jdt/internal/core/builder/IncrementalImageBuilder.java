...
public class IncrementalImageBuilder extends AbstractImageBuilder
{
   ...
   protected IncrementalImageBuilder(JavaBuilder javaBuilder, State buildState)
   {
      ...
      this.makeOutputFolderConsistent = JavaCore.ENABLED.equals(
         javaBuilder.javaProject.getOption(JavaCore.CORE_JAVA_BUILD_RECREATE_MODIFIED_CLASS_FILES_IN_OUTPUT_FOLDER,
            true));
   }
         ...
         else
         {
            ...
            for (int i = 0, l = valueTable.length; i < l; i++)
            {
               ...
               if (delta != null)
               {
                  ...
                  ClasspathLocation[] classFoldersAndJars = (ClasspathLocation[])this.javaBuilder.binaryLocationsPerProject.get(
                     p);
   ...
   protected void buildAfterBatchBuild()
   {
      ...
      {
         System.out.println(
            "INCREMENTAL build after batch build @ " + new Date(System.currentTimeMillis())); //$NON-NLS-1$
      }
      ...
      for (int i = 0, l = valueTable.length; i < l; i++)
      {
         ...
         if (typeLocator != null)
         {
            ...
            if (refs.includes(internedQualifiedNames, internedSimpleNames, internedRootNames))
            {
               ...
               if (this.compiledAllAtOnce && this.previousSourceFiles != null && this.previousSourceFiles.contains(
                  sourceFile))
               {
      ...
      int segmentCount) throws CoreException
   {
      ...
      boolean isExcluded = (md.exclusionPatterns != null || md.inclusionPatterns != null) && Util.isExcluded(resource,
         md.inclusionPatterns, md.exclusionPatterns);
      switch (resource.getType())
      {
         case IResource.FOLDER:
            ...
            return true;
         case IResource.FILE:
            if (!isExcluded && com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resource.getName()))
            {
   ...
   protected void deleteGeneratedFiles(IFile[] deletedGeneratedFiles)
   {
      ...
      try
      {
         for (int j = deletedGeneratedFiles.length; --j >= 0; )
         {
            ...
            IPath typePath = sourceFile.resource.getFullPath().removeFirstSegments(
               mdSegmentCount).removeFileExtension();
         ...
         { // skip unchanged output folder
            ...
            if (p != null)
            {
               ...
               if (binaryDelta != null)
               {
                  ...
                  {
                     structurallyChangedTypes = this.newState.getStructurallyChangedTypes(
      ...
      StringSet structurallyChangedTypes)
   {
      ...
      switch (resource.getType())
      {
         case IResource.FOLDER:
            ...
            return;
         case IResource.FILE:
            if (com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resource.getName()))
            {
               IPath typePath = resource.getFullPath().removeFirstSegments(segmentCount).removeFileExtension();
               switch (binaryDelta.getKind())
               {
   ...
   protected boolean findSourceFiles(IResourceDelta sourceDelta, ClasspathMultiDirectory md,
      int segmentCount) throws CoreException
   {
      // When a package becomes a type or vice versa, expect 2 deltas,
      ...
      switch (resource.getType())
      {
         ...
         case IResource.FILE:
            ...
            if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resourceName))
            {
               IPath typePath = resource.getFullPath().removeFirstSegments(segmentCount).removeFileExtension();
                  ...
                  case IResourceDelta.REMOVED:
                     ...
                     else
                     {
                        ...
                        addDependentsOf(typePath,
                           true); // add dependents of the source file since it may be involved in a name collision
                        if (definedTypeNames.length > 0)
                        { // skip it if it failed to successfully define a type
                           IPath packagePath = typePath.removeLastSegments(1);
            ...
            else if (com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(resourceName))
            {
               // perform full build if a managed class file has been changed
   ...
   /**
    * @see com.codenvy.eclipse.jdt.internal.core.builder.AbstractImageBuilder#writeClassFileContents(com.codenvy.eclipse.jdt.internal.compiler.ClassFile, com.codenvy.eclipse.core.resources.IFile, java.lang.String, boolean, com.codenvy.eclipse.jdt.internal.core.builder.SourceFile)
    */
      ...
      boolean isTopLevelType, SourceFile compilationUnit) throws CoreException
   {
      ...
      if (file.exists())
      {
         if (writeClassFileCheck(file, qualifiedFileName, bytes) || compilationUnit.updateClassFile)
         { // see 46093
            if (JavaBuilder.DEBUG)
            {
