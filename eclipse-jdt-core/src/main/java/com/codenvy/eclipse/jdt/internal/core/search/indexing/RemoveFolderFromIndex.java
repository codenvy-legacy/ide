...
class RemoveFolderFromIndex extends IndexRequest {
    ...
    public RemoveFolderFromIndex(IPath folderPath, char[][] inclusionPatterns, char[][] exclusionPatterns, IProject project,
    ...
    public boolean execute(IProgressMonitor progressMonitor) {
        ...
        try {
            ...
            // all file names belonging to the folder or its subfolders and that are not excluded (see http://bugs.eclipse
            // .org/bugs/show_bug.cgi?id=32607)
            if (paths != null) {
            ...
            if (JobManager.VERBOSE) {
                Util.verbose("-> failed to remove " + this.folderPath + " from index because of the following exception:",
