...
import com.codenvy.eclipse.jdt.internal.compiler.codegen.ObjectCache;
...
public class ExceptionHandlingFlowContext extends FlowContext {
    ...
    // WARNING: This is an array that maps to catch blocks, not caught exceptions (which could be more than catch blocks in a multi-catch
    // block)
    ...
    public ExceptionHandlingFlowContext(FlowContext parent, ASTNode associatedNode, ReferenceBinding[] handledExceptions,
    ...
    public ExceptionHandlingFlowContext(FlowContext parent, ASTNode associatedNode, ReferenceBinding[] handledExceptions,
                                        int[] exceptionToCatchBlockMap, Argument[] catchArguments, FlowContext initializationParent,
                                        BlockScope scope,
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (handledException.isUncheckedException(true)) {
                if (markExceptionsAndThrowableAsReached ||
                    handledException.id != TypeIds.T_JavaLangThrowable && handledException.id != TypeIds.T_JavaLangException) {
    ...
    public void recordHandlingException(ReferenceBinding exceptionType, UnconditionalFlowInfo flowInfo,
        ...
        this.initsOnExceptions[catchBlock] =
                (this.initsOnExceptions[catchBlock].tagBits & FlowInfo.UNREACHABLE) == 0 ? this.initsOnExceptions[catchBlock].mergedWith(
                        flowInfo) : flowInfo.unconditionalCopy();
    }
     ...
     * exception handlers.
     *
     * @see com.codenvy.eclipse.jdt.internal.compiler.flow.FlowContext#subroutine()
     */
