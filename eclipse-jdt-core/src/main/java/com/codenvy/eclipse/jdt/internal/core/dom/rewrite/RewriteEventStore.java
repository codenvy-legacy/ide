...
public final class RewriteEventStore
{
   ...
   private void removeMoveRangePlaceholders()
   {
      for (Iterator iter = this.nodeRangeInfos.entrySet().iterator(); iter.hasNext(); )
      {
         ...
         RewriteEvent[] revertedChildrenArr = (RewriteEvent[])revertedChildren.toArray(
            new RewriteEvent[revertedChildren.size()]);
         addEvent(loc.getParent(), loc.getProperty(),
            new ListRewriteEvent(revertedChildrenArr)); // replace the current edits
      }
   }
      ...
      TargetSourceRangeComputer sourceRangeComputer)
   {
      ...
      for (int k = 0; k < childEvents.length; k++)
      {
         ...
         {
            RewriteEvent[] placeholderChildEvents = (RewriteEvent[])newChildEvents.toArray(
               new RewriteEvent[newChildEvents.size()]);
   ...
   private void validateHasChildProperty(ASTNode parent, StructuralPropertyDescriptor property)
   {
      ...
      {
         String message = Signature.getSimpleName(
            parent.getClass().getName()) + " has no property " + property.getId(); //$NON-NLS-1$
         throw new IllegalArgumentException(message);
      }
   }
