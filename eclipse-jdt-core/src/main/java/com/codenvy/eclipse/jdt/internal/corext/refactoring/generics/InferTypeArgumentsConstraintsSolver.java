...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.types.HierarchyType;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.typesets.SingletonTypeSet;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.typesets.TypeSet;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.typesets.TypeSetEnvironment;
...
public class InferTypeArgumentsConstraintsSolver {
    ...
    public InferTypeArgumentsUpdate solveConstraints(IProgressMonitor pm) {
        ...
        ParametricStructureComputer parametricStructureComputer = new ParametricStructureComputer(allConstraintVariables,
                                                                                                  fTCModel);
        ...
        allConstraintVariables = newAllConstraintVariables.toArray(
     ...
     *
     * @param usedIn
     *         the <code>List</code> of <code>ITypeConstraint2</code>s
     *         to process
     */
    ...
    private void maintainSimpleConstraint(ITypeConstraint2 stc) {
        ...
        if (!rhsSubTypes.containsAll(leftEstimate)) {
            ...
            //				throw new IllegalStateException("Type estimate set is now empty for LHS in " + left + " <= " + right + "; estimates were
            // " + leftEstimate + " <= " + rightEstimate); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        ...
        if (!lhsSuperTypes.containsAll(rightEstimate)) {
            ...
            //			if (xsection.isEmpty())
            //				throw new IllegalStateException("Type estimate set is now empty for RHS in " + left + " <= " + right + "; estimates were
            // " + leftEstimate + " <= " + rightEstimate); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    ...
    private void findCastsToRemove(CastVariable2[] castVariables) {
        for (int i = 0; i < castVariables.length; i++) {
            ...
            if (chosenType != null && TTypes.canAssignTo(chosenType, castType)) {
                ...
                if (chosenArrayElementType != null && TTypes.canAssignTo(chosenArrayElementType,
                                                                         ((ArrayType)castType).getComponentType())) {
                    if (expressionType instanceof ArrayType && chosenArrayElementType.equals(
                            ((ArrayType)expressionType).getComponentType())) {
                        continue; // The type has not changed. Don't remove the cast, since it could be
                    }
