...
class MethodVerifier15 extends MethodVerifier
{
   ...
   boolean areParametersEqual(MethodBinding one, MethodBinding two)
   {
      ...
      for (i = 0; i < length; i++)
      {
         if (!areTypesEqual(oneArgs[i], twoArgs[i]))
         {
            ...
            {
               if (oneArgs[i].dimensions() == twoArgs[i].dimensions() && oneArgs[i].leafComponentType().isEquivalentTo(
                  twoArgs[i].leafComponentType()))
               {
   ...
   void checkConcreteInheritedMethod(MethodBinding concreteMethod, MethodBinding[] abstractMethods)
   {
      ...
      for (int i = 0, l = abstractMethods.length; i < l; i++)
      {
         ...
         {
            if ((concreteMethod.declaringClass == this.type.superclass && this.type.superclass.isParameterizedType() && !areMethodsCompatible(
               concreteMethod, originalInherited)) || this.type.superclass.erasure().findSuperTypeOriginatingFrom(
               originalInherited.declaringClass) == null)
            {
      ...
      MethodBinding[] allInheritedMethods)
   {
      ...
      if (bridge != null)
      {
         ...
         {
            if (allInheritedMethods[i] != null && detectInheritedNameClash(originalInherited,
               allInheritedMethods[i].original()))
            {
               return;
            }
         }
         ...
         for (int i = current.length - 1; i >= 0; --i)
         {
            ...
            if (thisMethod.areParameterErasuresEqual(
               bridge) && thisMethod.returnType.erasure() == bridge.returnType.erasure())
            {
   ...
   void checkForNameClash(MethodBinding currentMethod, MethodBinding inheritedMethod)
   {
      ...
      if (inheritedMethod.isStatic() || currentMethod.isStatic())
      {
         ...
         if (this.type.scope.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_7 && currentMethod.areParameterErasuresEqual(
            original))
         {
      ...
      { // check up the hierarchy for skipped inherited methods
         ...
         for (int i = 0; i < length; i++)
         {
            ...
            {
               if (currentParams[i].isBaseType() != inheritedParams[i].isBaseType() || !inheritedParams[i].isCompatibleWith(
                  currentParams[i]))
               {
         ...
         while (superType != null && superType.isValidBinding())
         {
            ...
            for (int m = 0, n = methods.length; m < n; m++)
            {
               ...
               if (substitute != null && !isSubstituteParameterSubsignature(currentMethod,
                  substitute) && detectNameClash(currentMethod, substitute, true))
               {
                  return;
               }
            }
   ...
   void checkNullSpecInheritance(MethodBinding currentMethod, MethodBinding inheritedMethod)
   {
      ...
      if (inheritedMethod.parameterNonNullness != null)
      {
         ...
         for (int i = 0; i < length; i++)
         {
            ...
            Boolean currentNonNullNess = (currentMethod.parameterNonNullness == null) ? null : currentMethod.parameterNonNullness[i];
            if (inheritedNonNullNess != null)
            {            // super has a null annotation
               if (currentNonNullNess == null)
               {         // current parameter lacks null annotation
                  ...
                  {
                     this.type.scope.problemReporter().parameterLackingNullAnnotation(currentArgument,
                        inheritedMethod.declaringClass, needNonNull, annotationName);
                     continue;
                  }
                  else
                  {
                     this.type.scope.problemReporter().cannotImplementIncompatibleNullness(currentMethod,
                        inheritedMethod);
                     break;
                  }
               }
            ...
            if (inheritedNonNullNess != Boolean.TRUE)
            {      // super parameter is not restricted to @NonNull
               if (currentNonNullNess == Boolean.TRUE)
               {    // current parameter is restricted to @NonNull
                  if (currentArgument != null)
                  {
                     this.type.scope.problemReporter().illegalRedefinitionToNonNullParameter(currentArgument,
      ...
      else if (currentMethod.parameterNonNullness != null)
      {
         ...
         for (int i = 0; i < currentMethod.parameterNonNullness.length; i++)
         {
            ...
            { // tightening from unconstrained to @NonNull
               if (currentArguments != null)
               {
                  this.type.scope.problemReporter().illegalRedefinitionToNonNullParameter(currentArguments[i],
                     inheritedMethod.declaringClass, null);
               }
               else
               {
                  this.type.scope.problemReporter().cannotImplementIncompatibleNullness(currentMethod, inheritedMethod);
         ...
         for (int i = 0, length = current.length; i < length; i++)
         {
            ...
            if ((currentMethod.modifiers & (ExtraCompilerModifiers.AccImplementing | ExtraCompilerModifiers.AccOverriding)) == 0)
            {
               ...
               for (int j = 0, size = currentMethod.parameters.length; j < size; j++)
               {
                  ...
                  if (parameterType.leafComponentType().isRawType() && compilerOptions.getSeverity(
                     CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (arg.type.bits & ASTNode.IgnoreRawTypeCheck) == 0)
                  {
               ...
               if (!methodDecl.isConstructor() && methodDecl instanceof MethodDeclaration)
               {
                  ...
                  if (returnType != null)
                  {
                     if (methodType.leafComponentType().isRawType() && compilerOptions.getSeverity(
                        CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (returnType.bits & ASTNode.IgnoreRawTypeCheck) == 0)
                     {
   ...
   public void reportRawReferences(MethodBinding currentMethod, MethodBinding inheritedMethod)
   {
      ...
      for (int j = 0, size = currentMethod.parameters.length; j < size; j++)
      {
         ...
         if (parameterType.leafComponentType().isRawType())
         {
            ...
            {
               if (compilerOptions.getSeverity(
                  CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (arg.type.bits & ASTNode.IgnoreRawTypeCheck) == 0)
               {
                  methodDecl.scope.problemReporter().rawTypeReference(arg.type, parameterType);
               }
      ...
      if (!methodDecl.isConstructor() && methodDecl instanceof MethodDeclaration && (returnType = ((MethodDeclaration)methodDecl).returnType) != null)
      {
         ...
         if (methodType.leafComponentType().isRawType())
         {
            ...
            {
               if ((returnType.bits & ASTNode.IgnoreRawTypeCheck) == 0 && compilerOptions.getSeverity(
   ...
   void checkMethods()
   {
      ...
      for (int s = methodSelectors.length; --s >= 0; )
      {
         ...
         for (int i = 0; i < inheritedLength; i++)
         {
            ...
            { // current == null case handled already.
               MethodBinding inheritedMethod = inherited[i];
               if (inheritedMethod.isPublic() && !inheritedMethod.declaringClass.isPublic())
               {
                  this.type.addSyntheticBridgeMethod(inheritedMethod.original());
         ...
         for (int i = 0, length = inherited.length; i < length; i++)
         {
            ...
            if (inheritedMethod != null)
            {
               ...
               for (int j = i + 1; j < length; j++)
               {
                  ...
                  otherInheritedMethod = computeSubstituteMethod(otherInheritedMethod, inheritedMethod);
                  if (otherInheritedMethod != null && isSubstituteParameterSubsignature(inheritedMethod,
            ...
            if (index > 0)
            {
               ...
               while (--count > 0)
               {
                  ...
                  if (interfaceMethod != null && implementation != null && !isAsVisible(implementation,
                     interfaceMethod))
                  {
                  ...
                  if (first.declaringClass.isInterface() && match.declaringClass.isInterface() && areReturnTypesCompatible(
                     match, first))
                  {
                     continue;
                  }
   ...
   MethodBinding computeSubstituteMethod(MethodBinding inheritedMethod, MethodBinding currentMethod)
   {
      ...
      ParameterizedGenericMethodBinding substitute = this.environment.createParameterizedGenericMethod(inheritedMethod,
         arguments);
      for (int i = 0; i < inheritedLength; i++)
      {
   ...
   {
      if (!com.codenvy.eclipse.jdt.core.compiler.CharOperation.equals(method.selector, inheritedMethod.selector))
      {
         return false;
      }
   ...
   boolean isUnsafeReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod)
   {
      ...
      if (currentMethod.typeVariables == Binding.NO_TYPE_VARIABLES && inheritedMethod.original().typeVariables != Binding.NO_TYPE_VARIABLES && currentMethod.returnType.erasure().findSuperTypeOriginatingFrom(
         inheritedMethod.returnType.erasure()) != null)
      {
         return true;
      }
      return false;
