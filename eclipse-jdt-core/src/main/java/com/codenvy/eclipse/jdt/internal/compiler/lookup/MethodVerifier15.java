...
class MethodVerifier15 extends MethodVerifier {
    ...
    boolean areParametersEqual(MethodBinding one, MethodBinding two) {
        ...
        for (i++; i < length; i++) {
            if (!areTypesEqual(oneArgs[i], twoArgs[i])) {
                ...
                return false; // no remaining parameter can be a Parameterized type (if one has been converted then all RAW types must be
                // converted)
            }
        }
        return true;
    ...
    void checkInheritedMethods(MethodBinding inheritedMethod, MethodBinding otherInheritedMethod) {
        ...
        if (this.environment.globalOptions.complianceLevel < ClassFileConstants.JDK1_7 && inheritedMethod.declaringClass.isInterface()) {
            return;  // JDK7 checks for name clashes in interface inheritance, while JDK6 and below don't. See https://bugs.eclipse
            // .org/bugs/show_bug.cgi?id=354229
        }
                    ...
                    if (currentNonNullNess == null) {         // current parameter lacks null annotation
                        ...
                        if (currentArgument != null) {
                            this.type.scope.problemReporter().parameterLackingNullAnnotation(currentArgument,
                                                                                             inheritedMethod.declaringClass, needNonNull,
                                                                                             annotationName);
                            continue;
                        } else {
                    ...
                    if (currentNonNullNess == Boolean.TRUE) {    // current parameter is restricted to @NonNull
                        if (currentArgument != null) {
                            this.type.scope.problemReporter().illegalRedefinitionToNonNullParameter(currentArgument,
                                                                                                    ...
                                                                                                    inheritedNonNullNess == null ? null
                                                                                                                                 : this
                                                                                                            .environment
	...
	*/
    ...
    void checkMethods() {
        ...
        boolean skipInheritedMethods = mustImplementAbstractMethods &&
                                       canSkipInheritedMethods(); // have a single concrete superclass so only check overridden methods
    ...
    boolean isSubstituteParameterSubsignature(MethodBinding method, MethodBinding substituteMethod) {
        ...
        if (substituteMethod instanceof ParameterizedGenericMethodBinding) {
            ...
            // since substituteMethod has substituted type variables, method cannot have a generic signature AND no variables -> its a
            // name clash if it does
    ...
    boolean isUnsafeReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        ...
        if (currentMethod.typeVariables == Binding.NO_TYPE_VARIABLES &&
            ...
            currentMethod.returnType.erasure().findSuperTypeOriginatingFrom(
    ...
    void verify() {
        ...
        for (int i = this.type.typeVariables.length; --i >= 0; ) {
            ...
            for (int j = itsInterfaces.length; --j >= 0; ) {
                superInterfaces[j] =
                        itsInterfaces[j].kind() == Binding.TYPE_PARAMETER ? (ReferenceBinding)itsInterfaces[j].erasure() : itsInterfaces[j];
            }
