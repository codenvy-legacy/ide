...
import com.codenvy.eclipse.jdt.internal.core.index.DiskIndex;
...
import com.codenvy.eclipse.jdt.internal.core.search.PatternSearchJob;
...
public class IndexManager extends JobManager implements IIndexConstants {
    ...
    private File participantIndexNamesFile = new File(getSavedIndexesDirectory(),
                                                      "participantsIndexNames.txt"); //$NON-NLS-1$
    ...
    public synchronized IndexLocation computeIndexLocation(IPath containerPath) {
        ...
        if (indexLocation == null) {
            ...
            indexLocation = (IndexLocation)getIndexStates().getKey(
                    new FileIndexLocation(new File(getSavedIndexesDirectory(), fileName)));
    ...
    public SourceElementParser getSourceElementParser(IJavaProject project, ISourceElementRequestor requestor) {
        ...
        SourceElementParser parser = new IndexingParser(requestor, new DefaultProblemFactory(Locale.getDefault()),
                                                        new CompilerOptions(options), true, // index local declarations
        ...
        if (index == null) {
            ...
            if (reuseExistingFile) {
                if (indexLocation.exists()) { // check before creating index so as to avoid creating a new empty index if file is missing
                    try {
                        ...
                        if (currentIndexState != REBUILDING_STATE && currentIndexState !=
                                                                     REUSE_STATE) { // rebuild index if existing file is corrupt,
                                                                     // unless the index is already being rebuilt
                            if (VERBOSE) {
                                Util.verbose(
                                        "-> cannot reuse existing index: " + indexLocation + " path: " +
                                        containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
                            }
                            rebuildIndex(indexLocation, containerPath);
                            return null;
                        }               /*index = null;*/ // will fall thru to createIfMissing & create a empty index for the rebuild all
                        // job to populate
                    }
                }
                ...
                if (currentIndexState == REUSE_STATE) {
                    ...
                    if (VERBOSE) {
                        Util.verbose(
                                "-> cannot reuse given index: " + indexLocation + " path: " +
                                containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
                    }
            ...
            if (createIfMissing) {
                try {
                    ...
                    if (VERBOSE) {
                        Util.verbose(
                                "-> unable to create empty index: " + indexLocation + " path: " +
                                containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                    e.printStackTrace();
                ...
                if (containerPath != null) {// sanity check
                    ...
                    if (index != null && this.javaLikeNamesChanged && !index.isIndexForJar()) {
                        ...
                        if (indexFile.exists()) {
                            if (DEBUG) {
                                Util.verbose(
                                        "Change in javaLikeNames - removing index file for " + containerPath); //$NON-NLS-1$
                            }
                            indexFile.delete();
                        }
    ...
    private IPath getJavaPluginWorkingLocation() {
        ...
        IPath stateLocation = JavaCore.getJavaCore().getStateLocation();
    ...
    private boolean hasJavaLikeNamesChanged() {
        ...
        if (prevNames == null) {
            if (VERBOSE && current != 1) {
                Util.verbose("No Java like names found and there is atleast one non-default javaLikeName",
...
//      }
    ...
    private char[][] readJavaLikeNamesFile() {
        try {
            ...
            char[] javaLikeNames = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getFileCharContent(javaLikeNamesFile, null);
    ...
    private void rebuildIndex(IndexLocation indexLocation, IPath containerPath) {
        ...
        if (VERBOSE) {
            Util.verbose(
                    "-> request to rebuild index: " + indexLocation + " path: " + containerPath); //$NON-NLS-1$ //$NON-NLS-2$
        }
    ...
    public synchronized Index recreateIndex(IPath containerPath) {
        ...
        try {
            ...
            if (VERBOSE) {
                Util.verbose(
                        "-> recreating index: " + indexLocation + " for path: " + containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
            }
            index = new Index(indexLocation, containerPathString, false /*do not reuse index file*/);
            this.indexes.put(indexLocation, index);
    ...
    public synchronized boolean resetIndex(IPath containerPath) {
        ...
        try {
            ...
            if (VERBOSE) {
                Util.verbose(
                        "-> reseting index: " + indexLocation + " for path: " + containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
            }
            if (index == null) {
    ...
    private void readIndexMap() {
        try {
            char[] indexMaps = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getFileCharContent(this.indexNamesMapFile,
                                                                                                      null);
            char[][] names = CharOperation.splitOn('\n', indexMaps);
            if (names.length >= 3) {
    ...
    private char[][] readIndexState(String dirOSString) {
        try {
            char[] savedIndexNames = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getFileCharContent(
    ...
    private void readParticipantsIndexNamesFile() {
        ...
        try {
            char[] participantIndexNames = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getFileCharContent(
