      ...
      if (index == null)
      {
         ...
         if (reuseExistingFile)
         {
            ...
            { // check before creating index so as to avoid creating a new empty index if file is missing
               ...
               catch (IOException e)
               {
                  ...
                  { // rebuild index if existing file is corrupt, unless the index is already being rebuilt
                     ...
                     {
                        Util.verbose(
                           "-> cannot reuse existing index: " + indexLocation + " path: " + containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
                     }
                     rebuildIndex(indexLocation, containerPath);
                     return null;
            ...
            if (currentIndexState == REUSE_STATE)
            {
               ...
               {
                  Util.verbose(
                     "-> cannot reuse given index: " + indexLocation + " path: " + containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
               }
