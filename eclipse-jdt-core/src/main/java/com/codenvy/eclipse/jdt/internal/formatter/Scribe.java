    ...
    * Adapt edits to regions.
            ...
            if (this.lastNumberOfNewLines == 0 || specificEmptyLinesRule || this.formatter.arrayInitializersDepth >= 0) {
                ...
                switch (emptyLinesRules) {
                    ...
                    case PRESERVE_EMPTY_LINES_IN_FORMAT_OPENING_BRACE:
                        useAlignmentBreakIndentation = this.formatter.arrayInitializersDepth <= 1 && this.currentAlignment != null &&
    ...
    private void handleLineTooLongSmartly() {
        ...
        while (targetAlignment != null) {
            boolean couldBreak = targetAlignment.tieBreakRule == Alignment.R_OUTERMOST || (!insideStringConcat &&
                                                                                           insideMessage > 0 && targetAlignment.kind ==
                                                                                                                Alignment
                                                                                                                        ...
                                                                                                                        .MESSAGE_ARGUMENTS &&
                                                                                           (!targetAlignment.wasReset() ||
                                                                                            previousKind != Alignment.MESSAGE_SEND));
                ...
                if (this.lastLineComment.contiguous) {
                    ...
                    if (this.tabLength == 0) {
                        ...
                        similarCommentsIndentation =
                                relativeIndentation == 0 || currentCommentIndentation != 0 && this.lastLineComment.currentIndentation != 0;
                    }
                        ...
                        try {
                            ...
                            switch (token) {
                                ...
                                case TerminalTokens.TokenNameMULTIPLY:
                                    ...
                                    break;
                                default:
                                    lastColumn += (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition) -
    ...
    private void printJavadocBlockReference(FormatJavadocBlock block, FormatJavadocReference reference) {
        ...
        while (!this.scanner.atEnd()) {
            ...
            try {
                token = this.scanner.getNextToken();
                int tokenLength =
                        (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition) - this.scanner.startPosition;
                switch (token) {
                    case TerminalTokens.TokenNameWHITESPACE:
                        ...
                        if (!inlined && spacePosition > 0 && (this.column + tokenLength) > maxColumn) {
                            ...
                            if (newLineString == null) {
                                ...
                                newLineString = this.tempBuffer.substring(0,
                                                                          ...
                                                                          1); // remove last space as buffer will be inserted before a space
                                firstColumn = this.column;
                            } else {
                                this.column = firstColumn;
                            }
    ...
    private int printJavadocHtmlTag(FormatJavadocText text, FormatJavadocBlock block, boolean textOnNewLine) {
        ...
        for (int idx = 0, ptr = 0; idx <= max || (text.htmlNodesPtr != -1 && ptr <= text.htmlNodesPtr); idx++) {
            ...
            if (text.htmlNodesPtr >= 0 && ptr <= text.htmlNodesPtr && end > text.htmlNodes[ptr].sourceStart) {
                ...
                this.needSpace = idx > 1 && (previousEnd + 1) <
                                            nextStart; // There's no space between text and html tag or inline block => do not insert space a the beginning of the text
        ...
        try {
            ...
            while (!this.scanner.atEnd()) {
                    ...
                    case TerminalTokens.TokenNameWHITESPACE:
                        if (tokensBufferLength > 0) {
                            boolean shouldSplit = (this.column + tokensBufferLength) > maxColumn // the max length is reached
                                                  ...
                                                  // allow to split at the beginning only when starting with an identifier or a token with a length > 1
                ...
                if (shouldSplit) {
                    ...
                    if ((tokensBufferLength > 0 || /*(firstColumn+tokenLength) < maxColumn || (insertSpace &&*/
                        ...
                        if (this.useTags && this.editsEnabled) {
                            ...
                            if (foundTaskCount > 0) {
                                ...
                                turnOff = true;
                                                                                                  ...
                                                                                                  this.scanner.source, tokenStartPosition,
                                                                                                  tokenEndPosition + 1)) {
                                this.editsEnabled = false;
                                turnOff = true;
                            }
    ...
    public String toString() {
        ...
        stringBuffer.append(") - (tabSize = " + this.tabLength + ")")//$NON-NLS-1$//$NON-NLS-2$
                ...
                ") - (checkLineWrapping = " + this.checkLineWrapping + ")")   //$NON-NLS-1$	//$NON-NLS-2$	//$NON-NLS-3$	//$NON-NLS-4$
                .append(this.lineSeparator).append(
                "==================================================================================")   //$NON-NLS-1$
