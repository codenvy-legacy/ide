...
import com.codenvy.eclipse.jdt.internal.core.util.RecordedParsingInformation;
...
import com.codenvy.eclipse.jdt.internal.formatter.comment.CommentFormatterUtil;
import com.codenvy.eclipse.jdt.internal.formatter.comment.HTMLEntity2JavaReader;
import com.codenvy.eclipse.jdt.internal.formatter.comment.IJavaDocTagConstants;
import com.codenvy.eclipse.jdt.internal.formatter.comment.Java2HTMLEntityReader;
...
public class Scribe implements IJavaDocTagConstants
{
   ...
   private void adaptRegions()
   {
      ...
      for (int i = 0; i < max; i++)
      {
         ...
         if (index >= 0 && this.commentPositions[index][0] >= 0)
   ...
   private int adaptEdit(OptimizedReplaceEdit[] sortedEdits, int start, int regionStart, int regionEnd)
   {
      ...
      while (bottom <= top)
      {
         ...
         {  // the edit starts after the region's start => no possible overlap of region's start
            top = i - 1;
            if (editStart > regionEnd)
            { // the edit starts after the region's end => no possible overlap of region's end
               topEnd = top;
            }
         }
         else
         {
            ...
            { // the edit ends before the region's start => no possible overlap of region's start
               bottom = i + 1;
            }
            else
            {
      ...
      while (bottom <= topEnd)
      {
         ...
         else if (regionEnd == editStart)
         {   // special case when the edit starts just after the region's end...
         ...
         else if (editEnd <= regionEnd)
         {   // the edit ends before the region's end => no possible overlap of region's end
            bottom = i + 1;
         }
         else
         {
      ...
      if (this.editsIndex > 0)
      {
         ...
         if (endOffsetOfPreviousEdit == offset)
         {
            if (length != 0)
            {
               ...
               {
                  this.edits[this.editsIndex - 1] = new OptimizedReplaceEdit(previousOffset, previousLength + length,
      ...
      int continuationIndent, boolean adjust)
   {
      ...
      if ((this.currentAlignment == null && this.formatter.expressionsDepth >= 0) || (this.currentAlignment != null && this.currentAlignment.kind == Alignment.BINARY_EXPRESSION &&
         ...
         if (this.lastNumberOfNewLines == 0 || specificEmptyLinesRule || this.formatter.arrayInitializersDepth >= 0)
         {
            ...
            switch (emptyLinesRules)
            {
               ...
               case PRESERVE_EMPTY_LINES_IN_FORMAT_OPENING_BRACE:
                  useAlignmentBreakIndentation = this.formatter.arrayInitializersDepth <= 1 && this.currentAlignment != null && this.currentAlignment.kind == Alignment.ARRAY_INITIALIZER;
                  break;
   ...
   public TextEdit getRootEdit()
   {
      ...
      for (int i = 0, max = this.editsIndex; i < max; i++)
      {
         ...
         {
            if (currentEdit.length == 0 || (currentEdit.offset != this.scannerEndPosition && isMeaningfulEdit(
               currentEdit)))
            {
               try
               {
   ...
   private void handleLineTooLongSmartly()
   {
      ...
      while (targetAlignment != null)
      {
         boolean couldBreak = targetAlignment.tieBreakRule == Alignment.R_OUTERMOST || (!insideStringConcat &&
   ...
   {
      return ((this.formatComments & INCLUDE_LINE_COMMENTS) == INCLUDE_LINE_COMMENTS && this.headerEndPosition < this.scanner.currentPosition) || (this.formatter.preferences.comment_format_header && this.headerEndPosition >= this.scanner.currentPosition);
   }
   ...
   private void initializeScanner(long sourceLevel, DefaultCodeFormatterOptions preferences)
   {
      ...
      this.scanner = new Scanner(true, true, false/*nls*/, sourceLevel/*sourceLevel*/, taskTags, null/*taskPriorities*/,
   ...
   private void printCodeSnippet(int startPosition, int endPosition, int linesGap)
   {
      ...
      int lastLine = Util.getLineNumber(endPosition, this.lineEnds, firstLine > 1 ? firstLine - 2 : 0,
         this.maxLines) - 1;
   ...
   private void printLineComment()
   {
      ...
      {
         if (onFirstColumn && ((includesLineComments && !this.formatter.preferences.comment_format_line_comment_starting_on_first_column) || this.formatter.preferences.never_indent_line_comments_on_first_column))
         {
            commentIndentationLevel = this.column - 1;
         }
         else
         ...
         switch (token)
         {
            ...
            case TerminalTokens.TokenNameIdentifier:
               if (previousToken == -1 || previousToken == SKIP_FIRST_WHITESPACE_TOKEN)
               {
                  ...
                  if (CharOperation.equals(identifier, Parser.FALL_THROUGH_TAG, 0,
                     5/*length of string "$FALL"*/) && this.scanner.currentCharacter == '-')
                  {
                     try
                     {
         ...
         else
         {
            ...
            if (inlinedBlock.reference != null)
            {
               ...
               while (!this.scanner.atEnd())
               {
                  ...
                  {
                     case TerminalTokens.TokenNameWHITESPACE:
                        if (previousToken == TerminalTokens.TokenNameCOMMA)
                        { // space between method arguments
                           length++;
                        }
                        break;
                     case TerminalTokens.TokenNameMULTIPLY:
                  ...
                  if (!inlined && spacePosition > 0 && (this.column + tokenLength) > maxColumn)
                  {
                     ...
                     if (newLineString == null)
                     {
                        ...
                        newLineString = this.tempBuffer.substring(0,
                           this.tempBuffer.length() - 1); // remove last space as buffer will be inserted before a space
                        firstColumn = this.column;
                     }
                     else
                     {
         ...
         if (isCode)
         {
            ...
            if (codeEnd > end)
            {
               if (this.formatter.preferences.comment_format_source)
               {
                  ...
                  if (linesGap > 0)
                  {
                     ...
                     { // if code starts at the line, then no leading space is needed
                  ...
                  this.needSpace = false;
                  printJavadocGapLines(end + 1, nextStart - 1, newLines, false/* clear first blank lines inside <pre> tag as done by old formatter */,
                     false, null);
                  // Format the code
                  printCodeSnippet(nextStart, codeEnd, linesGap);
                  // Format the gap lines after the code
                  nextStart = (int)text.separators[max];
                  printJavadocGapLines(codeEnd + 1, nextStart - 1, 1, false/* clear blank lines inside <pre> tag as done by old formatter */,
                     false, null);
                  return 2;
               }
            }
            else
            {
               ...
               if ((nextStart - 1) > (end + 1))
               {
                  ...
                  printJavadocGapLines(end + 1, nextStart - 1, gapLines, false/* never clear blank lines inside <pre> tag*/,
                     false, null);
                  if (gapLines > 0)
                  {
                     textOnNewLine = true;
   ...
   private void printJavadocText(FormatJavadocText text, FormatJavadocBlock block, boolean textOnNewLine)
   {
      ...
      for (int idx = 0, max = text.separatorsPtr; idx <= max; idx++)
      {
         ...
         printJavadocTextLine(this.javadocTextBuffer, nextStart, end, block, idx == 0 || (!joinLines && textOnNewLine)/*first text?*/,
      ...
      printJavadocTextLine(this.javadocTextBuffer, nextStart, text.sourceEnd, block, text.separatorsPtr == -1 /* first text?*/,
         needIndentation, false /*not an html tag*/);
      ...
      try
      {
         ...
         while (!this.scanner.atEnd())
         {
               ...
               case TerminalTokens.TokenNameWHITESPACE:
                  if (tokensBufferLength > 0)
                  {
                     boolean shouldSplit = (this.column + tokensBufferLength) > maxColumn // the max length is reached
                        && !isHtmlTag && (insertSpace || tokensBufferLength > 1) // allow to split at the beginning only when starting with an identifier or a token with a length > 1
            ...
            boolean shouldSplit = lastColumn > maxColumn // the max length is reached
               ...
               && token != TerminalTokens.TokenNameAT && (tokensBufferLength == 0 || this.javadocTokensBuffer.charAt(
               tokensBufferLength - 1) != '@'); // avoid to split just before a '@'
            if (shouldSplit)
            {
               // not enough space on the line
   ...
   public void printNextToken(int[] expectedTokenTypes, boolean considerSpaceIfAny)
   {
      ...
      try
      {
         ...
         if (Arrays.binarySearch(expectedTokenTypes, this.currentToken) < 0)
         {
            ...
            throw new AbortFormatting(
               "unexpected token type, expecting:[" + expectations.toString() + "], actual:" + this.currentToken);//$NON-NLS-1$//$NON-NLS-2$
         }
         print(this.scanner.currentPosition - this.scanner.startPosition, considerSpaceIfAny);
      }
      catch (InvalidInputException e)
   ...
   public String toString()
   {
      ...
      stringBuffer.append(") - (tabSize = " + this.tabLength + ")")//$NON-NLS-1$//$NON-NLS-2$
         .append(this.lineSeparator).append(
