...
public class CodeAttribute extends ClassFileAttribute implements ICodeAttribute {
    ...
    CodeAttribute(byte[] classFileBytes, IConstantPool constantPool, int offset) throws ClassFormatException {
        ...
        for (int i = 0; i < this.attributesCount; i++) {
            ...
            if (equals(attributeName, IAttributeNamesConstants.LINE_NUMBER)) {
                ...
                this.attributes[attributesIndex++] = new LocalVariableTypeAttribute(classFileBytes, constantPool,
                                                                                    offset + readOffset);
            } else if (equals(attributeName, IAttributeNamesConstants.STACK_MAP_TABLE)) {
                this.attributes[attributesIndex++] = new StackMapTableAttribute(classFileBytes, constantPool,
                                                                                offset + readOffset);
            } else if (equals(attributeName, IAttributeNamesConstants.STACK_MAP)) {
                this.attributes[attributesIndex++] = new StackMapAttribute(classFileBytes, constantPool,
                                                                           offset + readOffset);
            } else {
                this.attributes[attributesIndex++] = new ClassFileAttribute(classFileBytes, constantPool,
                                                                            offset + readOffset);
            }
    ...
    public byte[] getBytecodes() {
        if (this.bytecodes == null) {
            System.arraycopy(this.classFileBytes, this.codeOffset, (this.bytecodes = new byte[(int)this.codeLength]), 0,
                             (int)this.codeLength);
        }
