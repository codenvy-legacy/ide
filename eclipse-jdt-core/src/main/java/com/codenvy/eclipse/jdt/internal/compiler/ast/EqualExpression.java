...
public class EqualExpression extends BinaryExpression
{
      ...
      FlowInfo initsWhenTrue, FlowInfo initsWhenFalse)
   {
      ...
      {
         checkVariableComparison(scope, flowContext, flowInfo, initsWhenTrue, initsWhenFalse, local, rightStatus,
            this.left);
      }
      local = this.right.localVariableBinding();
      if (local != null && (local.type.tagBits & TagBits.IsBaseType) == 0)
      {
         checkVariableComparison(scope, flowContext, flowInfo, initsWhenTrue, initsWhenFalse, local, leftStatus,
            this.right);
      }
   }
   ...
   public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo)
   {
      ...
      if (((this.bits & OperatorMASK) >> OperatorSHIFT) == EQUAL_EQUAL)
      {
         ...
         {
            result = this.right.analyseCode(currentScope, flowContext,
   ...
   public void generateNonBooleanEqual(BlockScope currentScope, CodeStream codeStream, boolean valueRequired)
   {
      ...
      if (valueRequired)
      {
         ...
         if (isEqualOperator)
         {
            switch ((this.left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4)
   ...
   public TypeBinding resolveType(BlockScope scope)
   {
      ...
      if (leftType.isBaseType() && rightType.isBaseType())
      {
         ...
         {
            if (leftTypeID != TypeIds.T_double && leftTypeID != TypeIds.T_float && (!(this.right instanceof Assignment))) // https://bugs.eclipse.org/bugs/show_bug.cgi?id=281776
            {
         ...
         else if (this.constant != Constant.NotAConstant)
         {
            ...
            if ((operator == EQUAL_EQUAL && this.constant == BooleanConstant.fromValue(
               true)) || (operator == NOT_EQUAL && this.constant == BooleanConstant.fromValue(false)))
            {
               scope.problemReporter().comparingIdenticalExpressions(this);
            }
         }
      ...
      if ((!leftType.isBaseType() || leftType == TypeBinding.NULL) // cannot compare: Object == (int)0
         && (!rightType.isBaseType() || rightType == TypeBinding.NULL) && (checkCastTypesCompatibility(scope, leftType,
         rightType, null) || checkCastTypesCompatibility(scope, rightType, leftType, null)))
      {
         ...
         if (unnecessaryLeftCast || unnecessaryRightCast)
         {
            ...
            if (checkCastTypesCompatibility(scope, alternateLeftType, alternateRightType,
               null) || checkCastTypesCompatibility(scope, alternateRightType, alternateLeftType, null))
            {
