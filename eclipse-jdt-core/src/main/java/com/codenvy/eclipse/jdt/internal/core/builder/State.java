...
public class State {
    ...
    void removePackage(IResourceDelta sourceDelta) {
        ...
        switch (resource.getType()) {
            ...
            case IResource.FILE:
                ...
                if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(typeLocatorPath.lastSegment())) {
    ...
    static State read(IProject project, DataInputStream in) throws IOException {
        ...
        if (VERSION != in.readByte()) {
            if (JavaBuilder.DEBUG) {
                System.out.println(
                        "Found non-compatible state version... answered null for " + project.getName()); //$NON-NLS-1$
            }
            return null;
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            ClasspathMultiDirectory md = (ClasspathMultiDirectory)ClasspathLocation.forSourceFolder(sourceFolder,
                                                                                                    outputFolder, readNames(in),
                                                                                                    readNames(in), in.readBoolean());
            if (in.readBoolean()) {
        ...
        for (int i = 0; i < length; i++) {
            switch (in.readByte()) {
                ...
                case BINARY_FOLDER:
                    ...
                    IContainer outputFolder = path.segmentCount() == 1 ? (IContainer)root.getProject(
                            path.toString()) : (IContainer)root.getFolder(path);
