...
public class NameLookup implements SuffixConstants
{
      ...
      ICompilationUnit[] workingCopies, Map rootToResolvedEntries)
   {
      ...
      if (VERBOSE)
      {
         ...
         Util.verbose(
            " -> pkg roots size: " + (packageFragmentRoots == null ? 0 : packageFragmentRoots.length));  //$NON-NLS-1$
               ...
               if (existing instanceof PackageFragmentRoot)
               {
                  ...
                  { // if not equal
                     this.packageFragments.put(pkgName,
                        exisitingPosition < rootPosition ? new IPackageFragmentRoot[]{(PackageFragmentRoot)existing, root} : new IPackageFragmentRoot[]{root, (PackageFragmentRoot)existing});
                  }
               }
               else
               {
   ...
   protected boolean acceptType(IType type, int acceptFlags, boolean isSourceType)
   {
      ...
      try
      {
         int kind = isSourceType ? TypeDeclaration.kind(
            ((SourceTypeElementInfo)((SourceType)type).getElementInfo()).getModifiers()) : TypeDeclaration.kind(
            ((IBinaryType)((BinaryType)type).getElementInfo()).getModifiers());
         switch (kind)
         {
 ...
 */
   ...
   public IPackageFragment[] findPackageFragments(String name, boolean partialMatch, boolean patternMatch)
   {
      ...
      if (partialMatch || hasPatternChars)
      {
         ...
         for (int i = 0, length = keys.length; i < length; i++)
         {
            ...
            if (pkgName != null)
            {
               boolean match = isStarPattern || (hasPatternChars ? CharOperation.match(lowercaseName,
                  Util.concatCompoundNameToCharArray(pkgName), false) : Util.startsWithIgnoreCase(pkgName, splittedName,
                  partialMatch));
               if (match)
               {
   ...
   {
      return findType(typeName, packageName, partialMatch, acceptFlags, true/* consider secondary types */, false/* do NOT wait for indexes */,
      ...
      { // reverse map always contains resolved CP entry
         ...
         if (accessRuleSet != null)
         {
            ...
            accessRestriction = accessRuleSet.getViolatedRestriction(
               CharOperation.concatWith(packageChars, typeChars, '/'));
         }
      }
   ...
   {
      return findType(name, partialMatch, acceptFlags, true/*consider secondary types*/, true/*wait for indexes*/,
         checkRestrictions, null);
   }
            ...
            for (int i = 0; i < length; i++)
            {
               ...
               if (elementName.regionMatches(true /*ignore case*/, 0, name, 0, matchLength))
               {
                  ...
                  { //not an anonymous type
                     if (nameMatches(unqualifiedName, type, true/*partial match*/) && acceptType(type, acceptFlags,
                        false/*not a source type*/))
                     {
                        requestor.acceptType(type);
                     }
                  }
