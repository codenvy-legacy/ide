 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.corext.dom.fragments;
...
public class ASTFragmentFactory {
     ...
     * the node to which the produced IASTFragment maps (see {@link  com.codenvy.eclipse.jdt.internal.corext.dom.fragments.IASTFragment
     * IASTFragment})
     ...
     * @param cu
     *         The compilation unit to which the source range applies, and to which the AST corresponds.
     ...
     */
    public static IASTFragment createFragmentForSourceRange(ISourceRange range, ASTNode scope,
    ...
    private static boolean isSingleNodeSelected(SelectionAnalyzer sa, ISourceRange range,
                                                ICompilationUnit cu) throws JavaModelException {
        return sa.getSelectedNodes().length == 1 && !rangeIncludesNonWhitespaceOutsideNode(range,
                                                                                           sa.getFirstSelectedNode(), cu);
    }

    private static boolean rangeIncludesNonWhitespaceOutsideNode(ISourceRange range, ASTNode node,
                                                                 ICompilationUnit cu) throws JavaModelException {
     ...
     * fragment.
     */
    private static IASTFragment createFragmentForSubPartBySourceRange(ASTNode node, ISourceRange range,
                                                                      ICompilationUnit cu) throws JavaModelException {
    ...
    private static class FragmentForSubPartBySourceRangeFactory extends FragmentFactory {
        ...
        public static IASTFragment createFragmentFor(ASTNode node, ISourceRange range,
                                                     ICompilationUnit cu) throws JavaModelException {
        ...
        private static IExpressionFragment createInfixExpressionSubPartFragmentBySourceRange(InfixExpression node,
                                                                                             ISourceRange range, ICompilationUnit cu)
                throws JavaModelException {
