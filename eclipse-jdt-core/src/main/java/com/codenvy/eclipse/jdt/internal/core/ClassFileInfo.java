...
/* package */ class ClassFileInfo extends OpenableElementInfo implements SuffixConstants
{
      ...
      IBinaryAnnotation annotationInfo, String memberValuePairName)
   {
      char[] typeName = com.codenvy.eclipse.jdt.core.Signature.toCharArray(
         CharOperation.replaceOnCopy(annotationInfo.getTypeName(), '/', '.'));
      ...
      HashMap newElements)
   {
      ...
      {
         generateStandardAnnotation(javaElement, TypeConstants.JAVA_LANG_DEPRECATED, Annotation.NO_MEMBER_VALUE_PAIRS,
            newElements);
      }
      ...
      {
         generateStandardAnnotation(javaElement, TypeConstants.JAVA_LANG_SAFEVARARGS, Annotation.NO_MEMBER_VALUE_PAIRS,
            newElements);
      }
   ...
   private IMemberValuePair[] getRetentionPolicy(long tagBits)
   {
      ...
      if ((tagBits & TagBits.AnnotationRuntimeRetention) == TagBits.AnnotationRuntimeRetention)
      {
         ...
         retention = new String(
            CharOperation.concatWith(TypeConstants.JAVA_LANG_ANNOTATION_RETENTIONPOLICY, '.')) + '.' + new String(
      ...
      for (int i = 0, methodCount = methods.length; i < methodCount; i++)
      {
         ...
         for (int j = 0; j < max; j++)
         {
            ...
            if (parameterAnnotations != null)
            {
               ...
               generateAnnotationsInfos(localVariable, argumentNames[j], parameterAnnotations, methodInfo.getTagBits(),
                  newElements);
            }
         }
   ...
   protected void readBinaryChildren(ClassFile classFile, HashMap newElements, IBinaryType typeInfo)
   {
      ...
      { //may not be a valid class file
         ...
         generateInnerClassHandles(type, typeInfo,
            childrenHandles); // Note inner class are separate openables that are not opened here: no need to pass in newElements
      }
