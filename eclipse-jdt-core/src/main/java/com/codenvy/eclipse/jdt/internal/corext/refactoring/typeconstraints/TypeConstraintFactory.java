...
public class TypeConstraintFactory implements ITypeConstraintFactory {
    ...
    private SimpleTypeConstraint createSimpleTypeConstraint(ConstraintVariable v1, ConstraintVariable v2,
    ...
    private SimpleTypeConstraint storeConstraint(ConstraintVariable v1, ConstraintVariable v2,
                                                 ConstraintOperator operator, Map<ConstraintOperator, SimpleTypeConstraint> m3) {
    ...
    public boolean filter(ConstraintVariable v1, ConstraintVariable v2, ConstraintOperator operator) {
        if ((v1.getBinding() != null && v1.getBinding().isPrimitive() &&
             v2.getBinding() != null && v2.getBinding().isPrimitive()) || v1 == v2) {
            if (PRINT_STATS) {
                fNrFiltered++;
            }
            if (PRINT_STATS) {
     ...
     * (non-Javadoc)
     *
     * @see org.eclipse.jdt.internal.corext.refactoring.typeconstraints.ITypeConstraintFactory#createCompositeOrTypeConstraint(org
     * .eclipse.jdt.internal.corext.refactoring.typeconstraints.ITypeConstraint[])
     */
    ...
    protected void dumpStats() {
        System.out.println(
                "Constraints: " + fNrCreated + " created, " + fNrRetrieved + " retrieved, " + fNrFiltered +
