...
import com.codenvy.eclipse.ltk.core.refactoring.IRefactoringStatusEntryComparator;
...
/**
 * Convenience wrapper for {@link com.codenvy.eclipse.jdt.core.search.SearchEngine} - performs searching and sorts the results by {@link com.codenvy.eclipse.core.resources.IResource}.
...
public class RefactoringSearchEngine
{
   ...
   public static ICompilationUnit[] findAffectedCompilationUnits(SearchPattern pattern, IJavaSearchScope scope,
      final IProgressMonitor pm, RefactoringStatus status,
      final boolean tolerateInAccurateMatches) throws JavaModelException
   {
   ...
   //TODO: throw CoreException
   public static ICompilationUnit[] findAffectedCompilationUnits(SearchPattern pattern, IJavaSearchScope scope,
      final IProgressMonitor pm, RefactoringStatus status) throws JavaModelException
   {
      return findAffectedCompilationUnits(pattern, scope, pm, status, false);
   }

   /**
    * Performs a search and groups the resulting {@link com.codenvy.eclipse.jdt.core.search.SearchMatch}es by
    ...
    *         has a different {@link com.codenvy.eclipse.jdt.core.search.SearchMatch#getResource() getResource()}s.
    * @throws com.codenvy.eclipse.jdt.core.JavaModelException
    *          when the search failed
    * @see com.codenvy.eclipse.jdt.core.search.SearchMatch
    */
   //TODO: throw CoreException
   ...
   private static SearchResultGroup[] internalSearch(SearchEngine searchEngine, SearchPattern pattern,
      IJavaSearchScope scope, CollectingSearchRequestor requestor, IProgressMonitor monitor,
      RefactoringStatus status) throws JavaModelException
   {
      try
      {
   ...
   public static SearchPattern createOrPattern(IJavaElement[] elements, int limitTo)
   {
      ...
      if (pattern == null) // check for bug 90138
      {
         throw new IllegalArgumentException(
            "Invalid java element: " + first.getHandleIdentifier() + "\n" + first.toString()); //$NON-NLS-1$ //$NON-NLS-2$
      }
      while (iter.hasNext())
      {
         ...
         {
            throw new IllegalArgumentException(
               "Invalid java element: " + each.getHandleIdentifier() + "\n" + each.toString()); //$NON-NLS-1$ //$NON-NLS-2$
         }
