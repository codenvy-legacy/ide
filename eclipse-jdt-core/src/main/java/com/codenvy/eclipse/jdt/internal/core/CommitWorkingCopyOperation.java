...
public class CommitWorkingCopyOperation extends JavaModelOperation
{
   ...
   protected void executeOperation() throws JavaModelException
   {
      try
      {
         ...
         if (isPrimary || (root.validateOnClasspath().isOK() && isIncluded && resource.isAccessible() && Util.isValidCompilationUnitName(
            workingCopy.getElementName(), project.getOption(JavaCore.COMPILER_SOURCE, true),
            ...
            try
            {
               ...
               if (resource.exists())
               {
                  resource.setContents(stream,
                     this.force ? IResource.FORCE | IResource.KEEP_HISTORY : IResource.KEEP_HISTORY, null);
               }
               else
               {
