...
import com.codenvy.eclipse.jdt.core.dom.InfixExpression.Operator;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.code.OperatorPrecedence;
...
public class NecessaryParenthesesChecker {
    ...
    private static boolean expressionTypeNeedsParentheses(Expression expression) {
        ...
        return type == ASTNode.INFIX_EXPRESSION || type == ASTNode.CONDITIONAL_EXPRESSION || type == ASTNode.PREFIX_EXPRESSION ||
               type == ASTNode.POSTFIX_EXPRESSION || type == ASTNode.CAST_EXPRESSION || type == ASTNode.INSTANCEOF_EXPRESSION ||
               type == ASTNode.ARRAY_CREATION || type == ASTNode.ASSIGNMENT;
    }
    ...
    private static boolean locationNeedsParentheses(StructuralPropertyDescriptor locationInParent) {
        ...
        if (locationInParent == VariableDeclarationFragment.INITIALIZER_PROPERTY ||
            locationInParent == SingleVariableDeclaration.INITIALIZER_PROPERTY || locationInParent == ReturnStatement.EXPRESSION_PROPERTY ||
            locationInParent == EnhancedForStatement.EXPRESSION_PROPERTY || locationInParent == ForStatement.EXPRESSION_PROPERTY ||
            locationInParent == WhileStatement.EXPRESSION_PROPERTY || locationInParent == DoStatement.EXPRESSION_PROPERTY ||
            locationInParent == AssertStatement.EXPRESSION_PROPERTY || locationInParent == AssertStatement.MESSAGE_PROPERTY ||
            locationInParent == IfStatement.EXPRESSION_PROPERTY || locationInParent == SwitchStatement.EXPRESSION_PROPERTY ||
            locationInParent == SwitchCase.EXPRESSION_PROPERTY || locationInParent == ArrayAccess.INDEX_PROPERTY ||
            locationInParent == ThrowStatement.EXPRESSION_PROPERTY || locationInParent == SynchronizedStatement.EXPRESSION_PROPERTY ||
    ...
    private static boolean isAssociative(InfixExpression expression) {
        ...
        if (operator == InfixExpression.Operator.PLUS) {
            return isExpressionStringType(expression) || isExpressionIntegerType(expression) && isAllOperandsHaveSameType(
                    expression);
        }
    ...
    private static boolean isIntegerNumber(String name) {
        return "int".equals(name) || "long".equals(name) || "byte".equals(name) || "char".equals(name) || "short".equals(
                name); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
    }
        ...
        if (locationInParent == InfixExpression.LEFT_OPERAND_PROPERTY) {
            ...
            if (expression instanceof InfixExpression) {
                ...
                if (isExpressionStringType(parentInfix)) {
                    if (parentInfix.getOperator() == InfixExpression.Operator.PLUS && operator == InfixExpression.Operator.PLUS &&
                        isExpressionStringType(
                                infixExpression)) {
                        // 1 + ("" + 2) == 1 + "" + 2
                        // 1 + (2 + "") != 1 + 2 + ""
                        // "" + (2 + "") == "" + 2 + ""
                        return !isExpressionStringType(infixExpression.getLeftOperand()) && !isExpressionStringType(
                                parentInfix.getLeftOperand());
                    }
                    //"" + (1 + 2), "" + (1 - 2) etc
                    return true;
                }
