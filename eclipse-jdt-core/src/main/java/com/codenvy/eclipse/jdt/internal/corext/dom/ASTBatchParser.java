...
public class ASTBatchParser {
     ...
     */
    public final void createASTs(ICompilationUnit[] compilationUnits, String[] bindingKeys, ASTRequestor requestor,
        ...
        try {
            ...
            for (int i = 0; i < splited.length; i++) {
                ...
                if (units.length <= MAX_AT_ONCE) {
                    createParser(units[0].getJavaProject()).createASTs(units, bindingKeys, requestor,
                    ...
                    while (cursor < units.length) {
                        ...
                        createParser(units[0].getJavaProject()).createASTs(
                                toParse.toArray(new ICompilationUnit[toParse.size()]), bindingKeys, requestor,
     ...
     *
     * @param project
     *         the project for which ASTs are been generated
    ...
    protected ASTParser createParser(IJavaProject project) {
        ASTParser result = ASTParser.newParser(AST.JLS4);
