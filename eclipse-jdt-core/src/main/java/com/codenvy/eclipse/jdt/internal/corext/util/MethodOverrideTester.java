...
public class MethodOverrideTester {
    ...
    /**
     * Finds the directly overridden method in a type and its super types. First the super class is examined and then the implemented
     * interfaces.
     * With generics it is possible that 2 methods in the same type are overidden at the same time. In that case,
     * the first overridden method
     * found is returned.
     *
     * @param type
    ...
    private void computeSubstitutions(IType instantiatedType, IType instantiatingType,
                                      String[] typeArguments) throws JavaModelException {
        ...
        if (instantiatingType == null) { // the focus type
            ...
            if (typeParameters.length == typeArguments.length) {
                for (int i = 0; i < typeParameters.length; i++) {
                    ITypeParameter curr = typeParameters[i];
                    String substString = getSubstitutedTypeName(typeArguments[i],
                                                                instantiatingType); // substitute in the context of the instantiatingType
                    String erasure = getErasedTypeName(typeArguments[i],
                                                       instantiatingType); // get the erasure from the type argument
     ...
     * @param context
     *         The context for the substitution
     * @return a type name
     * @throws com.codenvy.eclipse.jdt.core.JavaModelException
     *         if a problem occurs
     */
