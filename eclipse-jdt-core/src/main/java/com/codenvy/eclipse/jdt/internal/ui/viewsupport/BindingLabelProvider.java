...
public class BindingLabelProvider {
    ...
    //			if (type.isEnum() && (!Modifier.isPublic(modifiers) && !Modifier.isProtected(modifiers) && !Modifier.isPrivate(
    //            modifiers)) && ((IMethodBinding) binding).isConstructor())
    ...
    private static void getFieldLabel(IVariableBinding binding, long flags, StringBuffer buffer) {
        ...
        if ((flags & JavaElementLabels.F_FULLY_QUALIFIED) != 0) {
            ...
            if (declaringClass != null) { // test for array.length
                getTypeLabel(declaringClass, JavaElementLabels.T_FULLY_QUALIFIED | (flags & JavaElementLabels.P_COMPRESSED),
                             buffer);
                buffer.append('.');
            }
        }
    ...
    private static void getMethodLabel(IMethodBinding binding, long flags, StringBuffer buffer) {
        ...
        if ((flags & JavaElementLabels.M_FULLY_QUALIFIED) != 0) {
            getTypeLabel(binding.getDeclaringClass(),
    ...
    private static void getTypeLabel(ITypeBinding binding, long flags, StringBuffer buffer) {
        ...
        if ((flags & JavaElementLabels.T_POST_QUALIFIED) != 0) {
            ...
            if (declaringMethod != null) {
                buffer.append(JavaElementLabels.CONCAT_STRING);
                getMethodLabel(declaringMethod,
                               JavaElementLabels.T_FULLY_QUALIFIED | (flags & JavaElementLabels.P_COMPRESSED), buffer);
    ...
    /**
     * Returns the label for a Java element with the flags as defined by {@link com.codenvy.eclipse.jdt.ui.JavaElementLabels}.
     *
     * @param binding
     *         The binding to render.
     * @param flags
     *         The text flags as defined in {@link com.codenvy.eclipse.jdt.ui.JavaElementLabels}
    ...
    //			Point size= ((imageFlags & JavaElementImageProvider.SMALL_ICONS) != 0) ? JavaElementImageProvider.SMALL_SIZE :
    ...
    /**
     * @param textFlags
     *         Flags defined in {@link com.codenvy.eclipse.jdt.ui.JavaElementLabels}.
     */
