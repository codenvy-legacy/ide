                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (entry.getEntryKind() == IClasspathEntry.CPE_LIBRARY && entry.getPath().equals(projectPath)) {
                        ...
                        this.manager.indexLibrary(projectPath, this.project,
                                                  ((ClasspathEntry)entry).getLibraryIndexLocation());
                        return true;
                    }
                }
                                    ...
                                    case IResource.FILE:
                                        if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(proxy.getName())) {
                                            ...
                                            indexedFileNames.put(
                                                    Util.relativePath(file.getFullPath(), 1/*remove project segment*/), file);
                                        }
                                        return false;
                                    ...
                                    case IResource.FILE:
                                        if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(proxy.getName())) {
                                            ...
                                            indexedFileNames.put(relativePathString,
                                                                 ...
                                                                 //|| indexLastModified < EFS.getStore(location).fetchInfo()
                                                                 // .getLastModified()
                                                                 ? (Object)file : (Object)OK);
                                        }
                                        return false;
                                    case IResource.FOLDER:
                                        if (exclusionPatterns != null || inclusionPatterns != null) {
