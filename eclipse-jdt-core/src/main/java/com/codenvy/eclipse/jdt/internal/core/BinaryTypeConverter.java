...
import com.codenvy.eclipse.jdt.internal.compiler.parser.TypeConverter;
...
public class BinaryTypeConverter extends TypeConverter {
    ...
    public ImportReference[] buildImports(ClassFileReader reader) {
        ...
        for (int i = 0, length = set.length; i < length; i++) {
            char[][] typeName = set[i];
            if (typeName != null) {
                imports[index++] = new ImportReference(typeName, new long[typeName.length]/*dummy positions*/, false/*not on demand*/,
                                                       0);
            }
        }
        return imports;
    ...
    public TypeDeclaration buildTypeDeclaration(IType type,
                                                CompilationUnitDeclaration compilationUnit) throws JavaModelException {
    ...
    private AbstractMethodDeclaration convert(IMethod method, IType type) throws JavaModelException {
        ...
        com.codenvy.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams = null;

        // convert 1.5 specific constructs only if compliance is 1.5 or above
        if (this.has1_5Compliance) {         /* convert type parameters */
            ITypeParameter[] typeParameters = method.getTypeParameters();
            if (typeParameters != null && typeParameters.length > 0) {
                int parameterCount = typeParameters.length;
                typeParams = new com.codenvy.eclipse.jdt.internal.compiler.ast.TypeParameter[parameterCount];
        ...
        if (method.isConstructor()) {
            ...
            MethodDeclaration decl = type.isAnnotation() ? new AnnotationMethodDeclaration(
                    this.compilationResult) : new MethodDeclaration(this.compilationResult);
            /* convert return type */
    ...
    private TypeDeclaration convert(IType type, IType alreadyComputedMember,
        ...
        if (this.has1_5Compliance) {
            ...
            if (typeParameters != null && typeParameters.length > 0) {
                int parameterCount = typeParameters.length;
                com.codenvy.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams =
                        new com.codenvy.eclipse.jdt.internal.compiler.ast.TypeParameter[parameterCount];
                for (int i = 0; i < parameterCount; i++) {
                    ITypeParameter typeParameter = typeParameters[i];
                    typeParams[i] = createTypeParameter(typeParameter.getElementName().toCharArray(),
                                                        stringArrayToCharArray(typeParameter.getBounds()), 0, 0);
        ...
        for (int i = 0; i < memberTypeCount; i++) {
            if (alreadyComputedMember != null && alreadyComputedMember.getFullyQualifiedName().equals(
                    memberTypes[i].getFullyQualifiedName())) {
