...
public class CastExpression extends Expression {
    ...
    public static void checkNeedForArgumentCasts(BlockScope scope, int operator, int operatorSignature, Expression left,
        ...
        if (leftIsCast) {
            if ((left.bits & ASTNode.UnnecessaryCast) == 0 && left.resolvedType.isBaseType()) {
                ...
                if ((alternateLeftTypeId = alternateLeftType.id) == leftTypeId || scope.environment().computeBoxingType(
                        alternateLeftType).id == leftTypeId) { // obvious identity cast
        ...
        if (rightIsCast) {
            if ((right.bits & ASTNode.UnnecessaryCast) == 0 && right.resolvedType.isBaseType()) {
                ...
                if ((alternateRightTypeId = alternateRightType.id) == rightTypeId || scope.environment().computeBoxingType(
                        alternateRightType).id == rightTypeId) { // obvious identity cast
    ...
    private static void checkAlternateBinding(BlockScope scope, Expression receiver, TypeBinding receiverType,
        ...
        if (binding.isConstructor()) {
            ...
            bindingIfNoCast = receiver.isImplicitThis() ? scope.getImplicitMethod(binding.selector, alternateArgumentTypes,
                                                                                  fakeInvocationSite)
        ...
        if (bindingIfNoCast == binding) {
            ...
            if (binding.isVarargs()) {
                ...
                if (paramLength == argumentLength) {
                    ...
                    if (lastArgType.isCompatibleWith(varargsType.elementsType()) && lastArgType.isCompatibleWith(
                            varargsType)) {
                        return;
                    }
                }
                            ...
                            if (isNarrowing) {
                                ...
                                if (paramMatch.arguments == null || length > paramMatch.arguments.length) {
                                    ...
                                    for (int i = 0; i < length; i++) {
                                        ...
                                        System.arraycopy(paramCastType.arguments, 0, alternateArguments = new TypeBinding[length],
                                                         0, length);
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (needRuntimeCheckcast && this.expression.postConversionType(currentScope) !=
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.ast.Expression#localVariableBinding() */
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.ast.Expression#optimizedBooleanConstant() */
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.expression instanceof MessageSend) {
            ...
            if (methodBinding != null && methodBinding.isPolymorphic()) {
                messageSend.binding = scope.environment().updatePolymorphicMethodReturnType(
            ...
            if (expressionType != null) {
                ...
                if (isLegal) {
                    ...
                    if ((this.bits & ASTNode.UnsafeCast) != 0) { // unsafe cast
                        if (scope.compilerOptions().reportUnavoidableGenericTypeProblems || !this.expression.forcedToBeRaw(
                                scope.referenceContext())) {
                            scope.problemReporter().unsafeCast(this, scope);
                        }
                    } else {
                        if (castType.isRawType() && scope.compilerOptions().getSeverity(
    ...
    private boolean isIndirectlyUsed() {
        ...
        if (this.expectedType != null && this.resolvedType.isBaseType() && !this.resolvedType.isCompatibleWith(
                this.expectedType)) {
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.ast.Expression#tagAsNeedCheckCast() */
    public void tagAsNeedCheckCast() {
        this.bits |= ASTNode.GenerateCheckcast;
    }

    /** @see com.codenvy.eclipse.jdt.internal.compiler.ast.Expression#tagAsUnnecessaryCast(Scope, TypeBinding) */
