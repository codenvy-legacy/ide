...
public class SetVariablesOperation extends ChangeClasspathOperation {
    ...
    /*
     * Creates a new SetVariablesOperation for the given variable values (null path meaning removal),
    ...
    protected void executeOperation() throws JavaModelException {
        ...
        try {
            ...
            nextProject:
            for (int i = 0, projectLength = projects.length; i < projectLength; i++) {
                ...
                for (int j = 0, cpLength = classpath.length; j < cpLength; j++) {
                    ...
                    for (int k = 0; k < varLength; k++) {
                        ...
                        if (entry.getEntryKind() == IClasspathEntry.CPE_VARIABLE) {
                            ...
                            if (((sourcePath = entry.getSourceAttachmentPath()) != null && variableName.equals(
                                    sourcePath.segment(
                                            0))) || ((sourceRootPath = entry.getSourceAttachmentRootPath()) != null && variableName.equals(
                                    sourceRootPath.segment(0)))) {
    ...
    private void verbose_failure(String[] dbgVariableNames) {
        Util.verbose("CPVariable SET  - FAILED DUE TO EXCEPTION\n" + //$NON-NLS-1$
                     "	variables: " + com.codenvy.eclipse.jdt.internal.compiler.util.Util.toString(dbgVariableNames), //$NON-NLS-1$
                     System.err);
    }
    ...
    private void verbose_set_variables() {
        Util.verbose("CPVariable SET  - setting variables\n" + //$NON-NLS-1$
                     "	variables: " + com.codenvy.eclipse.jdt.internal.compiler.util.Util.toString(
                this.variableNames) + '\n' +//$NON-NLS-1$
                     "	values: " + com.codenvy.eclipse.jdt.internal.compiler.util.Util.toString(this.variablePaths)); //$NON-NLS-1$
    }
