 ...
 *     Guven Demir <guven.internet+eclipse@gmail.com> - [package explorer] Alternative package name shortening: abbreviation -
...
public class JavaElementLabelComposer {
    ...
    public void appendMethodLabel(IMethod method, long flags) {
        try {
            BindingKey resolvedKey = getFlag(flags,
                                             JavaElementLabels.USE_RESOLVED) && method.isResolved() ? new BindingKey(method.getKey())
                                                                                                    : null;
                    ...
                    if (isPolymorphic) {
                        ...
                        if (nParams != names.length) {
                            if (resolvedSig != null && types.length > names.length) {
                                ...
                                // Util.logErrorMessage("JavaElementLabels: Number of param types(" + nParams + ") != number of names(" +
                                // names.length + "): " + method.getElementName());   //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
     ...
     * @param typeSig
     *         a {@link com.codenvy.eclipse.jdt.core.Signature#CLASS_TYPE_SIGNATURE} or {@link com.codenvy.eclipse.jdt.core
    ...
    /**
     * Returns the string for rendering the {@link com.codenvy.eclipse.jdt.core.IJavaElement#getElementName() element name} of the given
     * element.
     *
     * @param element
     *         the element to render
