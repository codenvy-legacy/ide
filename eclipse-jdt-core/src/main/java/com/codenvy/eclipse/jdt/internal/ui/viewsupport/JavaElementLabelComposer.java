...
/**
 * Implementation of {@link com.codenvy.eclipse.jdt.ui.JavaElementLabels}.
 *
 * @since 3.5
 */
public class JavaElementLabelComposer
{
   ...
   public void appendMethodLabel(IMethod method, long flags)
   {
      try
      {
         BindingKey resolvedKey = getFlag(flags,
            JavaElementLabels.USE_RESOLVED) && method.isResolved() ? new BindingKey(method.getKey()) : null;
         ...
         if (getFlag(flags, JavaElementLabels.M_PARAMETER_TYPES | JavaElementLabels.M_PARAMETER_NAMES))
         {
            ...
            if (getFlag(flags, JavaElementLabels.M_PARAMETER_TYPES))
            {
               ...
               if (renderVarargs && resolvedSig != null && declaredParameterTypes.length == 1 && JavaModelUtil.isPolymorphicSignature(
                  method))
               {
                  renderVarargs = false;
                  isPolymorphic = true;
               ...
               { // types != null
                  if (nParams != names.length)
                  {
                     ...
                     else
                     {
                        ...
                        // Util.logErrorMessage("JavaElementLabels: Number of param types(" + nParams + ") != number of names(" + names.length + "): " + method.getElementName());   //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
      ...
      {
         Util.log(e); // NotExistsException will not reach this point
      }
   }
   ...
   private void appendAnnotationValue(IAnnotation annotation, Object value, int valueKind,
      long flags) throws JavaModelException
   {
   ...
   public void appendFieldLabel(IField field, long flags)
   {
      try
      {
         ...
         if (getFlag(flags, JavaElementLabels.F_PRE_TYPE_SIGNATURE) && field.exists() && !Flags.isEnum(
            field.getFlags()))
         {
         ...
         if (getFlag(flags, JavaElementLabels.F_APP_TYPE_SIGNATURE) && field.exists() && !Flags.isEnum(
            field.getFlags()))
         {
            int offset = fBuffer.length();
            fBuffer.append(JavaElementLabels.DECL_STRING);
   ...
   public void appendLocalVariableLabel(ILocalVariable localVariable, long flags)
   {
      ...
      if (getFlag(flags, JavaElementLabels.F_FULLY_QUALIFIED))
      {
         appendElementLabel(localVariable.getDeclaringMember(),
            JavaElementLabels.M_PARAMETER_TYPES | JavaElementLabels.M_FULLY_QUALIFIED | JavaElementLabels.T_FULLY_QUALIFIED | (flags & QUALIFIER_FLAGS));
         fBuffer.append('.');
      }
   ...
   private void appendTypeParameterWithBounds(ITypeParameter typeParameter, long flags) throws JavaModelException
   {
      ...
      if (typeParameter.exists())
      {
         ...
         if (bounds.length > 0 && !(bounds.length == 1 && "Ljava.lang.Object;".equals(bounds[0])))
         { //$NON-NLS-1$
    ...
    * @param typeSig          a {@link com.codenvy.eclipse.jdt.core.Signature#CLASS_TYPE_SIGNATURE} or {@link com.codenvy.eclipse.jdt.core.Signature#TYPE_VARIABLE_SIGNATURE}
   ...
   /**
    * Returns the string for rendering the {@link com.codenvy.eclipse.jdt.core.IJavaElement#getElementName() element name} of the given element.
    *
    * @param element the element to render
   ...
   public void appendDeclarationLabel(IJavaElement declaration, long flags)
   {
      if (getFlag(flags, JavaElementLabels.D_QUALIFIED))
      {
         ...
         if (openable != null)
         {
            appendElementLabel(openable,
               JavaElementLabels.CF_QUALIFIED | JavaElementLabels.CU_QUALIFIED | (flags & QUALIFIER_FLAGS));
            fBuffer.append('/');
         }
      }
   ...
   private void appendFolderLabel(IPackageFragmentRoot root, long flags)
   {
      ...
      boolean rootQualified = getFlag(flags, JavaElementLabels.ROOT_QUALIFIED);
      boolean referencedQualified = getFlag(flags, JavaElementLabels.REFERENCED_ROOT_POST_QUALIFIED) && isReferenced(
         root);
      if (rootQualified)
      {
         fBuffer.append(root.getPath().makeRelative().toString());
