...
public class ExternalFoldersManager
{
      ...
      IProgressMonitor monitor) throws CoreException
   {
      IProject externalFoldersProject = createExternalFoldersProject(
         monitor); // run outside synchronized as this can create a resource
      ...
      IProject externalFoldersProject, IProgressMonitor monitor) throws CoreException
   {
      ...
      {
         if (System.getProperty("jdt.bug.367669") != null)
         { //$NON-NLS-1$
            System.out.println(
               "============================================================================================================"); //$NON-NLS-1$
            System.out.println(
               "The following logs are created for troubleshooting bug: https://bugs.eclipse.org/bugs/show_bug.cgi?id=367669"); //$NON-NLS-1$
            ...
            System.out.println(
               externalFile.exists() ? "External folder exists" : "ERROR: External folder DOES NOT exist"); //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(
               result.exists() ? "Link for folder exists" : "ERROR: Link for folder does not exist");  //$NON-NLS-1$//$NON-NLS-2$
            ...
            if (externalProjectFile.exists())
            {
               ...
               {
                  System.out.println(
                     "Error reading external folder project file: Here is the stack trace:"); //$NON-NLS-1$
                  e1.printStackTrace();
               }
            }
            else
            ...
            System.out.println(
               "========================================== Debug information ends =========================================="); //$NON-NLS-1$
         }
         throw e;
      }
      return result;
   ...
   private ArrayList getFoldersToCleanUp(IProgressMonitor monitor) throws CoreException
   {
      ...
      synchronized (knownFolders)
      {
         ...
         while (iterator.hasNext())
         {
            ...
            if ((roots != null && !roots.containsKey(
               path)) && (sourceAttachments != null && !sourceAttachments.containsKey(path)))
            {
      ...
      if (this.folders == null)
      {
         ...
         try
         {
            ...
            for (int i = 0, length = members.length; i < length; i++)
            {
               ...
               if (member.getType() == IResource.FOLDER && member.isLinked() && member.getName().startsWith(
                  LINKED_FOLDER_NAME))
               {
   ...
   public void refreshReferences(final IProject[] sourceProjects, IProgressMonitor monitor)
   {
      ...
      try
      {
         ...
         for (int index = 0; index < sourceProjects.length; index++)
         {
            ...
            HashSet foldersInProject = getExternalFolders(
