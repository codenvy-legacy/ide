...
public class Javadoc extends ASTNode
{
   ...
   public void resolve(MethodScope methScope)
   {
      ...
      boolean overriding = methDecl == null /* field declaration */ || methDecl.binding == null /* compiler error */ ? false : !methDecl.binding.isStatic() && ((methDecl.binding.modifiers & (ExtraCompilerModifiers.AccImplementing | ExtraCompilerModifiers.AccOverriding)) != 0);
   ...
   private void resolveReference(Expression reference, Scope scope)
   {
      ...
      if (reference instanceof JavadocMessageSend)
      {
         ...
         // tag value
         if (source15 && msgSend.tagValue == JavadocTagConstants.TAG_VALUE_VALUE)
         { // cannot refer to method for @value tag
            if (scopeModifiers == -1)
            {
               scopeModifiers = scope.getDeclarationModifiers();
      ...
      else if (reference instanceof JavadocAllocationExpression)
      {
         ...
         // tag value
         if (source15 && alloc.tagValue == JavadocTagConstants.TAG_VALUE_VALUE)
         { // cannot refer to method for @value tag
            if (scopeModifiers == -1)
            {
               scopeModifiers = scope.getDeclarationModifiers();
