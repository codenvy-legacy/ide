...
public class Javadoc extends ASTNode {
    ...
    public ASTNode getNodeStartingAt(int start) {
        ...
        if (this.seeReferences != null) {
            ...
            for (int i = 0; i < length; i++) {
                com.codenvy.eclipse.jdt.internal.compiler.ast.Expression expression = this.seeReferences[i];
    ...
    public void resolve(MethodScope methScope) {
        ...
        boolean overriding = methDecl == null /* field declaration */ || methDecl.binding == null /* compiler error */ ? false :
                             !methDecl.binding.isStatic() && ((methDecl.binding.modifiers & (ExtraCompilerModifiers.AccImplementing |
                ...
                if (!methDecl.isConstructor()) {
                    if (overriding && this.seeReferences[i] instanceof JavadocMessageSend) {
                        ...
                        if (messageSend.binding != null && messageSend.binding.isValidBinding() &&
                            ...
                            TypeBinding superType = methDecl.binding.declaringClass.findSuperTypeOriginatingFrom(
                                    methodReceiverType);
                            if (superType != null && superType.original() != methDecl.binding.declaringClass && CharOperation.equals(
                                    messageSend.selector, methDecl.selector)) {
                    ...
                    if (allocationExpr.binding != null && allocationExpr.binding.isValidBinding()) {
                        ...
                        ReferenceBinding superType = (ReferenceBinding)methDecl.binding.declaringClass.findSuperTypeOriginatingFrom(
                                allocType);
                        if (superType != null && superType.original() != methDecl.binding.declaringClass) {
                            MethodBinding superConstructor = methScope.getConstructor(superType, methDecl.binding.parameters,
                                                                                      allocationExpr);
        ...
        boolean reportMissing = methDecl == null || !((overriding && this.inheritedPositions != null) || superRef ||
                                                      (methDecl.binding.declaringClass != null &&
                                                       methDecl.binding.declaringClass.isLocalType()));
    ...
    private void resolveReference(Expression reference, Scope scope) {
        ...
        // Verify type references
        if (!hasProblems && (reference instanceof JavadocSingleTypeReference || reference instanceof JavadocQualifiedTypeReference) &&
        ...
        if (parameters != null) {
            ...
            if (paramTypeParamLength == 0) {
                ...
                for (int i = 0; i < paramTypeParamLength; i++) {
                    if (bindings[i] != null) {
                        JavadocSingleTypeReference param = this.paramTypeParameters[i];
                        scope.problemReporter().javadocUndeclaredParamTagName(param.token, param.sourceStart, param.sourceEnd,
                                                                              modifiers);
                    }
                }
            }
        ...
        if (resolvedType.isValidBinding()) {
            ...
            if (reference != typeReference) {
                if (!canBeSeen(scope.problemReporter().options.reportInvalidJavadocTagsVisibility, resolvedType.modifiers)) {
                    scope.problemReporter().javadocHiddenReference(typeReference.sourceStart, typeReference.sourceEnd, scope,
            ...
            if (resolvedType.isMemberType()) {
                ...
                if (topLevelScope.parent.kind != Scope.COMPILATION_UNIT_SCOPE || !CharOperation.equals(
                        topLevelType.sourceName, topLevelScope.referenceContext.name)) {
