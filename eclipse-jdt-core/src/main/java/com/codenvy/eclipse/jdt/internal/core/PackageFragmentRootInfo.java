...
class PackageFragmentRootInfo extends OpenableElementInfo
{
      ...
      char[][] inclusionPatterns, char[][] exclusionPatterns) throws JavaModelException
   {
      ...
      try
      {
            ...
            for (int i = 0; i < length; i++)
            {
               ...
               switch (member.getType())
               {
                  ...
                  case IResource.FOLDER:
                     ...
                     if (Util.isValidFolderNameForPackage(member.getName(), sourceLevel,
                        complianceLevel) && (!Util.isExcluded(member, inclusionPatterns,
                        exclusionPatterns) || isClasspathEntry(member.getFullPath(), classpath)))
                     {
                        continue nextResource;
                     }
                     break;
               ...
               {
                  // resize
                  System.arraycopy(nonJavaResources, 0, (nonJavaResources = new IResource[nonJavaResourcesCounter * 2]),
         ...
         {
            System.arraycopy(nonJavaResources, 0, (nonJavaResources = new IResource[nonJavaResourcesCounter]), 0,
   ...
   private Object[] computeNonJavaResources(IResource underlyingResource, PackageFragmentRoot handle)
   {
      ...
      try
      {
         ...
         {
            resources = computeFolderNonJavaResources(handle, (IContainer)underlyingResource,
               handle.fullInclusionPatternChars(), handle.fullExclusionPatternChars());
         }
      }
