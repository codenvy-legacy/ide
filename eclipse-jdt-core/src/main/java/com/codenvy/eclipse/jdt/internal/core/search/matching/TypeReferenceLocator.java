...
import com.codenvy.eclipse.jdt.internal.compiler.lookup.ProblemBinding;
            ...
            switch (qNameRef.bits & ASTNode.RestrictiveFlagMASK) {
                ...
                case Binding.TYPE | Binding.VARIABLE:
                    if (binding instanceof ProblemFieldBinding) {
                        ...
                        maxType = CharOperation.occurencesOf('.', partialQualifiedName) -
                                  1; // index of last bound token is one before the pb token
            ...
            if (!knownTypes.includes(type)) {
                if (isBinary) {
                    ...
                    if (scope != null) {
                        ...
                        this.match = new TypeDeclarationMatch(((JavaElement)type).resolved(typeBinding), SearchMatch.A_ACCURATE, offset,
                                                              typeDecl.sourceEnd - offset + 1, locator.getParticipant(), resource);
                        locator.report(this.match);
                    }
                }
                knownTypes.add(type);
