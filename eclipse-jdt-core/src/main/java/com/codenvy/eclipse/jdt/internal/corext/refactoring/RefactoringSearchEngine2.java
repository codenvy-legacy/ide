...
public final class RefactoringSearchEngine2 {
    ...
    private class RefactoringCompilationUnitCollector extends RefactoringSearchCollector {
        ...
        public final void acceptSearchMatch(final SearchMatch match) throws CoreException {
            ...
            if (accepted != null) {
                ...
                if (fInaccurate && accepted.getAccuracy() == SearchMatch.A_INACCURATE && !fInaccurateMatches.contains(
                        accepted)) {
                    fStatus.addEntry(fSeverity,
                                     Messages.format(RefactoringCoreMessages.RefactoringSearchEngine_inaccurate_match,
     ...
     * <ul>
     * <li>If the search granularity is {@link #GRANULARITY_COMPILATION_UNIT}, the results are elements of type {@link
     * com.codenvy.eclipse.jdt.core.ICompilationUnit}.</li>
     * <li>If grouping by resource is enabled, the results are elements of type {@link SearchResultGroup},
     * otherwise the elements are of type
     * {@link com.codenvy.eclipse.jdt.core.search.SearchMatch}.</li>
     * </ul>
     *
    ...
    public final void searchPattern(IProgressMonitor monitor) throws JavaModelException {
        ...
        try {
            ...
            try {
                ...
                engine.search(fPattern, SearchUtils.getDefaultSearchParticipants(), fScope, getCollector(),
     ...
     *
     * @param element
     *         the java element whose referenced fields have to be found
     * @param monitor
     *         the progress monitor, or <code>null</code>
     * @throws com.codenvy.eclipse.jdt.core.JavaModelException
     *         if an error occurs during search
     */
    public final void searchReferencedFields(final IJavaElement element,
        ...
        try {
            ...
            try {
                ...
                engine.searchDeclarationsOfAccessedFields(element, getCollector(),
                                                          new SubProgressMonitor(monitor, 1, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
            } catch (CoreException exception) {
                throw new JavaModelException(exception);
            }
     ...
     *
     * @param element
     *         the java element whose referenced methods have to be found
     * @param monitor
     *         the progress monitor, or <code>null</code>
     * @throws com.codenvy.eclipse.jdt.core.JavaModelException
     *         if an error occurs during search
     */
    public final void searchReferencedMethods(final IJavaElement element,
                                              IProgressMonitor monitor) throws JavaModelException {
        Assert.isNotNull(element);
        if (monitor == null) {
            monitor = new NullProgressMonitor();
        ...
        try {
            ...
            try {
                ...
                engine.searchDeclarationsOfSentMessages(element, getCollector(),
                                                        new SubProgressMonitor(monitor, 1, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
            } catch (CoreException exception) {
                throw new JavaModelException(exception);
            }
     ...
     *         if an error occurs during search
     */
    public final void searchReferencedTypes(final IJavaElement element,
                                            IProgressMonitor monitor) throws JavaModelException {
        Assert.isNotNull(element);
        if (monitor == null) {
            monitor = new NullProgressMonitor();
        ...
        try {
            ...
            try {
                ...
                engine.searchDeclarationsOfReferencedTypes(element, getCollector(),
                                                           new SubProgressMonitor(monitor, 1, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
            } catch (CoreException exception) {
                throw new JavaModelException(exception);
            }
     ...
     *
     * @param inaccurate
     *         <code>true</code> to filter inaccurate matches, <code>false</code> otherwise
     * @param binary
     ...
     *
     * @param grouping
     *         <code>true</code> to group matches by their containing resource, <code>false</code> otherwise
     */
     ...
     * <p/>
     * This method must be called before {@link RefactoringSearchEngine2#searchPattern(com.codenvy.eclipse.core.runtime.IProgressMonitor)}
     *
     * @param first
     *         the first search pattern to set
     * @param second
     *         the second search pattern to set
     */
     ...
     *
     * @param owner
     *         the working copy owner to use, or <code>null</code> to use none
     */
     ...
     *         the set of elements
     * @param limitTo
     *         determines the nature of the expected matches. This is a combination of {@link com.codenvy.eclipse.jdt.core.search
    ...
    public final void setPattern(final IJavaElement[] elements, final int limitTo) {
        ...
        for (int index = 1; index < elements.length; index++) {
            ...
            pattern = SearchPattern.createOrPattern(pattern,
                                                    SearchPattern
                                                            .createPattern(element, limitTo, SearchUtils.GENERICS_AGNOSTIC_MATCH_RULE));
        }
        setPattern(pattern);
    }
    ...
    public final void setSeverity(final int severity) {
        Assert.isTrue(
                severity == RefactoringStatus.WARNING || severity == RefactoringStatus.INFO || severity == RefactoringStatus.FATAL ||
