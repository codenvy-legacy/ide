...
public final class RefactoringSearchEngine2
{
         ...
         if (accepted != null)
         {
            ...
            if (!resource.equals(fLastResource))
            {
                     ...
                     if (!fGrouping)
                     {
                        fStatus.addEntry(fSeverity,
                           Messages.format(RefactoringCoreMessages.RefactoringSearchEngine_binary_match_ungrouped,
                              BasicElementLabels.getResourceName(project)), null, null, RefactoringStatusEntry.NO_CODE);
                     }
                  ...
                  accepted))
               {
                  fStatus.addEntry(fSeverity,
                     Messages.format(RefactoringCoreMessages.RefactoringSearchEngine_inaccurate_match,
                        BasicElementLabels.getResourceName(resource)), null, null, RefactoringStatusEntry.NO_CODE);
                  fInaccurateMatches.add(accepted);
               }
            }
         }
    ...
    * <ul>
    * <li>If the search granularity is {@link #GRANULARITY_COMPILATION_UNIT}, the results are elements of type {@link com.codenvy.eclipse.jdt.core.ICompilationUnit}.</li>
    * <li>If grouping by resource is enabled, the results are elements of type {@link SearchResultGroup}, otherwise the elements are of type {@link com.codenvy.eclipse.jdt.core.search.SearchMatch}.</li>
    * </ul>
    *
    ...
    * @param limitTo  determines the nature of the expected matches. This is a combination of {@link com.codenvy.eclipse.jdt.core.search.IJavaSearchConstants}.
    */
   public final void setPattern(final IJavaElement[] elements, final int limitTo)
   {
      ...
      for (int index = 1; index < elements.length; index++)
      {
         ...
         pattern = SearchPattern.createOrPattern(pattern,
            SearchPattern.createPattern(element, limitTo, SearchUtils.GENERICS_AGNOSTIC_MATCH_RULE));
      }
      setPattern(pattern);
   }
   ...
   {
      Assert.isTrue(
         severity == RefactoringStatus.WARNING || severity == RefactoringStatus.INFO || severity == RefactoringStatus.FATAL || severity == RefactoringStatus.ERROR);
      fSeverity = severity;
   }
