...
public abstract class JavaElement extends PlatformObject implements IJavaElement
{
    ...
    * @throws IllegalArgumentException if the type is not one of the valid
    ...
    *
    * @throws JavaModelException if the element is not present or not accessible
    */
               ...
               if (start <= position && position <= end)
               {
                  if (child instanceof IField)
                  {
                     ...
                     do
                     {
                        ...
                        {
                           return candidate == null ? child.getSourceElementAt(position) : candidate.getSourceElementAt(
                              position);
                        }
   ...
   public JavaModelException newJavaModelException(IStatus status)
   {
      ...
      else
      {
         return new JavaModelException(
            new JavaModelStatus(status.getSeverity(), status.getCode(), status.getMessage()));
      }
   }
   ...
   protected String getURLContents(String docUrlValue) throws JavaModelException
   {
      ...
      try
      {
         ...
         byte[] contents = com.codenvy.eclipse.jdt.internal.compiler.util.Util.getInputStreamAsByteArray(stream,
                  ...
                  if (index2 != -1)
                  {
                     ...
                     if (charsetIndex != -1)
                     {
                        ...
                        encoding = new String(contents, start, index2 - start,
                           com.codenvy.eclipse.jdt.internal.compiler.util.Util.UTF_8);
                     }
                  }
               }
            }
