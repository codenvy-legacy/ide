 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.corext.codemanipulation;
...
import com.codenvy.eclipse.jdt.core.NamingConventions;
...
import com.codenvy.eclipse.jdt.core.dom.StructuralPropertyDescriptor;
...
import com.codenvy.eclipse.jdt.ui.CodeStyleConfiguration;
...
public class StubUtility {
    ...
    //	 */
    //	public static String getMethodBodyContent(boolean isConstructor, IJavaProject project, String destTypeName, String methodName,
    ...
    //	 * Don't use this method directly, use CodeGeneration.
    //	 */
    //	public static String getGetterMethodBodyContent(IJavaProject project, String destTypeName, String methodName, String fieldName,
    ...
    //	 * Don't use this method directly, use CodeGeneration.
    //	 */
    //	public static String getSetterMethodBodyContent(IJavaProject project, String destTypeName, String methodName, String fieldName,
    ...
    //	}
    //
    //	public static String getCatchBodyContent(ICompilationUnit cu, String exceptionType, String variableName, ASTNode locationInAST,
    // String lineDelimiter) throws CoreException
    //   {
    ...
    //
    //
    //	public static String getCatchBodyContent(ICompilationUnit cu, String exceptionType, String variableName, String enclosingType,
    ...
    //	 */
    //	public static String getCompilationUnitContent(ICompilationUnit cu, String fileComment, String typeComment, String typeContent,
    // String lineDelimiter) throws CoreException
    //   {
    ...
    //	 * @throws org.eclipse.core.runtime.CoreException thrown if the template could not be evaluated
    //	 * @see org.eclipse.jdt.ui.CodeGeneration#getTypeBody(String, org.eclipse.jdt.core.ICompilationUnit, String, String)
    //	 */
    ...
    //	/*
    //	 * Don't use this method directly, use CodeGeneration.
    //	 * @see org.eclipse.jdt.ui.CodeGeneration#getMethodComment(ICompilationUnit, String, String, String[], String[], String, String[],
    // IMethod, String)
    //	 */
    //	public static String getMethodComment(ICompilationUnit cu, String typeName, String methodName, String[] paramNames,
    // String[] excTypeSig, String retTypeSig, String[] typeParameterNames,
    ...
    //		for (int i= tagOffsets.length - 1; i >= 0; i--) { // from last to first
    //			try {
    //				insertTag(document, tagOffsets[i], position.getLength(), paramNames, exceptionNames, returnType, typeParameterNames, false,
    // lineDelimiter);
    //			} catch (BadLocationException e) {
    //				throw new CoreException(JavaUIStatus.createError(IStatus.ERROR, e));
    //			}
    ...
    //	 * Don't use this method directly, use CodeGeneration.
    //	 */
    //	public static String getFieldComment(ICompilationUnit cu, String typeName, String fieldName,
    // String lineDelimiter) throws CoreException
    //   {
    ...
    //	 */
    //	public static String getSetterComment(ICompilationUnit cu, String typeName, String methodName, String fieldName, String fieldType,
    ...
    //	 */
    //	public static String getGetterComment(ICompilationUnit cu, String typeName, String methodName, String fieldName, String fieldType,
    ...
    // String[] fullLineVariables) throws CoreException
    //   {
    //		TemplateBuffer buffer;
    //		try {
    //			buffer= context.evaluate(template);
    ...
    //		List<TypeParameter> typeParams= shouldGenerateMethodTypeParameterTags(cu.getJavaProject()) ? decl.typeParameters() : Collections
    // .emptyList();
    ...
    //		for (int i= tagOffsets.length - 1; i >= 0; i--) { // from last to first
    //			try {
    //				insertTag(textBuffer, tagOffsets[i], position.getLength(), paramNames, exceptionNames, returnType, typeParamNames, isDeprecated,
    // lineDelimiter);
    //			} catch (BadLocationException e) {
    //				throw new CoreException(JavaUIStatus.createError(IStatus.ERROR, e));
    //			}
    ...
    //	 * @throws org.eclipse.jdt.core.JavaModelException thrown if the element could not be accessed
    //	 */
    ...
    public static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, ITypeBinding expectedType,
        ...
        if (assignedExpression != null) {
            ...
            if (nameFromExpression != null) {
                add(getVariableNameSuggestions(variableKind, project, nameFromExpression, 0, excluded, false),
            ...
            if (nameFromParent != null) {
                add(getVariableNameSuggestions(variableKind, project, nameFromParent, 0, excluded, false),
                    res); // pass 0 as dimension, base name already contains plural.
            }
        }
    ...
    //			String nameFromExpression= getBaseNameFromExpression(project, assignedExpression, variableKind);
    //			if (nameFromExpression != null) {
    //				add(getVariableNameSuggestions(variableKind, project, nameFromExpression, 0, excluded, false), res); // pass 0 as dimension,
    // base name already contains plural.
    //			}
    //
    //			String nameFromParent= getBaseNameFromLocationInParent(assignedExpression);
    //			if (nameFromParent != null) {
    //				add(getVariableNameSuggestions(variableKind, project, nameFromParent, 0, excluded, false), res); // pass 0 as dimension,
    // base name already contains plural.
    //			}
    //		}
    //		if (expectedType != null) {
     ...
     *
     * @param variableKind
     *         specifies what type the variable is: {@link com.codenvy.eclipse.jdt.core.NamingConventions#VK_LOCAL},
     *         {@link com.codenvy.eclipse.jdt.core.NamingConventions#VK_PARAMETER}, {@link com.codenvy.eclipse.jdt.core
     *         .NamingConventions#VK_STATIC_FIELD},
     *         {@link com.codenvy.eclipse.jdt.core.NamingConventions#VK_INSTANCE_FIELD}, or
     *         {@link com.codenvy.eclipse.jdt.core.NamingConventions#VK_STATIC_FINAL_FIELD}.
     * @param project
     *         the current project
     * @param baseName
     *         the base name to make a suggestion on. The base name is expected to be a name
     ...
     * @param excluded
     *         a collection containing all excluded names or <code>null</code> if no names
     *         are excluded
     * @param evaluateDefault
     *         if set, the result is guaranteed to contain at least one result. If
    ...
    public static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, String baseName,
        ...
        return NamingConventions.suggestVariableNames(variableKind, NamingConventions.BK_TYPE_NAME,
                                                      removeTypeArguments(baseName), project, dimensions, getExcludedArray(excluded),
                                                      evaluateDefault);
    }
    ...
    private static String getBaseNameFromExpression(IJavaProject project, Expression assignedExpression,
        ...
        if (assignedExpression instanceof Name) {
            ...
            String string = assignedExpression instanceof StringLiteral ? ((StringLiteral)assignedExpression).getLiteralValue()
    ...
    //		return getVariableNameSuggestions(NamingConventions.VK_PARAMETER, type.getJavaProject(), type.getElementName(), 0,
    ...
    //		return getVariableNameSuggestions(NamingConventions.VK_PARAMETER, project, baseName, dimensions, new ExcludedCollection(excluded),
    // true);
    //	}
    //
    ...
    public static boolean useThisForFieldAccess(IJavaProject project) {
        return true; //Boolean.valueOf(PreferenceConstants.getPreference(PreferenceConstants.CODEGEN_KEYWORD_THIS, project)).booleanValue();
    }

    public static boolean useIsForBooleanGetters(IJavaProject project) {
        return true; //Boolean.valueOf(PreferenceConstants.getPreference(PreferenceConstants.CODEGEN_IS_FOR_GETTERS,
     ...
     * <p>
     * This method sets {@link com.codenvy.eclipse.jdt.core.dom.rewrite.ImportRewrite#setUseContextToFilterImplicitImports(boolean)} to
     * <code>true</code>
