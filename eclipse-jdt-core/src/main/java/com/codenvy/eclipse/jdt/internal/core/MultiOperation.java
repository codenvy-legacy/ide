...
public abstract class MultiOperation extends JavaModelOperation {
     ...
     *         if one or several errors occured during the operation.
     ...
     *
     * @throws JavaModelException
     *         if one or several operation(s) was unable to
     *         be completed.
     */
    ...
    protected void verifyRenaming(IJavaElement element) throws JavaModelException {
        ...
        switch (element.getElementType()) {
            case IJavaElement.PACKAGE_FRAGMENT:
                ...
                isValid = JavaConventions.validatePackageName(newName, sourceLevel,
                                                              complianceLevel).getSeverity() != IStatus.ERROR;
                break;
            case IJavaElement.COMPILATION_UNIT:
                isValid = JavaConventions.validateCompilationUnitName(newName, sourceLevel,
                                                                      complianceLevel).getSeverity() != IStatus.ERROR;
                break;
            case IJavaElement.INITIALIZER:
                ...
                break;
            default:
                isValid = JavaConventions.validateIdentifier(newName, sourceLevel,
                                                             complianceLevel).getSeverity() != IStatus.ERROR;
                break;
        }
