...
import com.codenvy.eclipse.jdt.core.ITypeHierarchyChangedListener;
...
import com.codenvy.eclipse.jdt.internal.core.TypeVector;
...
public class TypeHierarchy implements ITypeHierarchy, IElementChangedListener {
    ...
    public TypeHierarchy(IType type, ICompilationUnit[] workingCopies, IJavaSearchScope scope, boolean computeSubtypes) {
        this.focusType = type == null ? null : (IType)((JavaElement)type)
                .unresolved(); // unsure the focus type is unresolved (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=92357)
    ...
    public void fireChange() {
        ArrayList listeners =
                getClonedChangeListeners(); // clone so that a listener cannot have a side-effect on this list when being notified
        if (listeners == null) {
            return;
        }
        if (DEBUG) {
            ...
            if (this.focusType != null) {
                System.out.println(
                        "    for hierarchy focused on " + ((JavaElement)this.focusType).toStringWithAncestors()); //$NON-NLS-1$
            }
        }
            ...
            case IJavaElementDelta.ADDED:
                try {
                    ...
                    for (int i = 0; i < classpath.length; i++) {
                        if (classpath[i].getEntryKind() == IClasspathEntry.CPE_PROJECT && classpath[i].getPath().equals(
                                element.getPath())) {
                            return true;
                        }
                    }
                ...
                case IJavaElementDelta.CHANGED:
                    ...
                    for (int i = 0, length = children.length; i < length; i++) {
                        ...
                        if (childElement instanceof IType) {
                            ...
                            if ((hasVisibilityChange && hasSupertype(
                                    type.getElementName())) || (hasSupertypeChange && includesTypeOrSupertype(type))) {
                                return true;
                            }
                        }
                    }
    ...
    protected static byte[] readUntil(InputStream input, byte separator,
                                      int offset) throws IOException, JavaModelException {
        int length = 0;
    ...
    public synchronized void refresh(IProgressMonitor monitor) throws JavaModelException {
        try {
            ...
            if (monitor != null) {
                monitor.beginTask(this.focusType != null ? Messages.bind(Messages.hierarchy_creatingOnType,
                                                                         this.focusType.getFullyQualifiedName())
                                                         : Messages.hierarchy_creating, 100);
            }
            long start = -1;
            if (DEBUG) {
                ...
                if (this.computeSubtypes) {
                    System.out.println(
                            "CREATING TYPE HIERARCHY [" + Thread.currentThread() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
                } else {
                    System.out.println(
                            "CREATING SUPER TYPE HIERARCHY [" + Thread.currentThread() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
                }
                if (this.focusType != null) {
            ...
            if (DEBUG) {
                if (this.computeSubtypes) {
                    System.out.println(
                            "CREATED TYPE HIERARCHY in " + (System.currentTimeMillis() - start) + "ms"); //$NON-NLS-1$ //$NON-NLS-2$
                } else {
                    System.out.println(
                            "CREATED SUPER TYPE HIERARCHY in " + (System.currentTimeMillis() - start) + "ms"); //$NON-NLS-1$ //$NON-NLS-2$
                }
    ...
    boolean subtypesIncludeSupertypeOf(IType type) {
        ...
        int dot = -1;
        String simpleSuper = (dot = superclassName.lastIndexOf('.')) > -1 ? superclassName.substring(
                dot + 1) : superclassName;
        ...
        for (int i = 0, length = interfaceNames.length; i < length; i++) {
            ...
            String simpleInterface = (dot = interfaceName.lastIndexOf('.')) > -1 ? interfaceName.substring(
                    dot) : interfaceName;
