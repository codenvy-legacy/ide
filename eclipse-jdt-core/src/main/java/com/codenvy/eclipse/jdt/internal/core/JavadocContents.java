...
public class JavadocContents {
            ...
            if (CharOperation.prefixEquals(anchor, this.content, false, anchorEndStart)) {
                ...
                if (this.tempAnchorIndexes.length == this.tempAnchorIndexesCount) {
                    System.arraycopy(this.tempAnchorIndexes, 0,
                                     this.tempAnchorIndexes = new int[this.tempAnchorIndexesCount + 20], 0, this.tempAnchorIndexesCount);
                }
    ...
    private int[] computeChildRange(int anchorEndStart, char[] anchor, int indexOfBottom) {
        ...
        if (indexOfBottom != -1) {
            ...
            if (indexOfEndLink != -1) {
                ...
                int indexOfNextElement = CharOperation.indexOf(JavadocConstants.ANCHOR_PREFIX_START, this.content, false,
                                                               indexOfEndLink);
    ...
    private void computeChildrenSections() {
        ...
        this.indexOfConstructorDetails = CharOperation.indexOf(JavadocConstants.CONSTRUCTOR_DETAIL, this.content, false,
                                                               lastIndex);
        ...
        this.indexOfEndOfClassData = CharOperation.indexOf(JavadocConstants.END_OF_CLASS_DATA, this.content, false,
                                                           lastIndex);

        // try to find the field detail end
        this.indexOfFieldsBottom = this.indexOfConstructorDetails != -1 ? this.indexOfConstructorDetails
    ...
    private void computeTypeRange() throws JavaModelException {
        final int indexOfStartOfClassData = CharOperation.indexOf(JavadocConstants.START_OF_CLASS_DATA, this.content,
                                                                  false);
