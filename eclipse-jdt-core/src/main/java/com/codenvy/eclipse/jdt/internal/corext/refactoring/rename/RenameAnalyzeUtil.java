...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.base.JavaStringStatusContext;
...
import com.codenvy.eclipse.ltk.core.refactoring.TextEditChangeGroup;
...
class RenameAnalyzeUtil {
    ...
    private static class ProblemNodeFinder {
        ...
        public static SimpleName[] getProblemNodes(ASTNode methodNode, VariableDeclaration variableNode, TextEdit[] edits,
                                                   TextChange change) {
    ...
    static RefactoringStatus analyzeRenameChanges(TextChangeManager manager, SearchResultGroup[] oldOccurrences,
    ...
    static ICompilationUnit createNewWorkingCopy(ICompilationUnit cu, TextChangeManager manager, WorkingCopyOwner owner,
    ...
    private static boolean existsInNewOccurrences(SearchMatch searchResult, SearchResultGroup[] newOccurrences,
    ...
    static RefactoringStatus analyzeRenameChanges2(TextChangeManager manager, SearchResultGroup[] oldReferences,
                                                   SearchResultGroup[] newReferences, String newElementName) {
        RefactoringStatus result = new RefactoringStatus();

        HashMap<ICompilationUnit, SearchMatch[]> cuToNewResults = new HashMap<ICompilationUnit, SearchMatch[]>(
    ...
    private static void analyzeChanges(ICompilationUnit cu, TextChange change, SearchMatch[] oldMatches,
    ...
    private static void addReferenceShadowedError(ICompilationUnit cu, SearchMatch newMatch, String newElementName,
        ...
        String message = Messages.format(RefactoringCoreMessages.RenameAnalyzeUtil_reference_shadowed,
                                         new String[]{BasicElementLabels.getFileName(cu),
     ...
     *         the LocalAnalyzePackages containing the information about the local renames
     * @param cuChange
     *         the TextChange containing all local variable changes to be applied.
     * @param oldCUNode
     *         the fully (incl. bindings) resolved AST node of the original compilation unit
     * @param recovery
     *         whether statements and bindings recovery should be performed when parsing the changed CU
     ...
     *         thrown if there was an error greating the preview content of the change
     */
    public static RefactoringStatus analyzeLocalRenames(LocalAnalyzePackage[] analyzePackages, TextChange cuChange,
                                                        CompilationUnit oldCUNode, boolean recovery) throws CoreException {
        ...
        CompilationUnit newCUNode = new RefactoringASTParser(AST.JLS4).parse(newCuSource, compilationUnit, true, recovery,
                                                                             null);
        ...
        for (int i = 0; i < analyzePackages.length; i++) {
            ...
            SimpleName[] problemNodes = ProblemNodeFinder.getProblemNodes(enclosing, declaration,
    ...
    private static ASTNode getEnclosingBlockOrMethod(TextEdit declarationEdit, TextChange change,
    ...
    private static RefactoringStatus analyzeCompileErrors(String newCuSource, CompilationUnit newCUNode,
        ...
        for (int i = 0; i < newProblems.length; i++) {
            ...
            if (problem.isError()) {
                result.addEntry(
                        new RefactoringStatusEntry((problem.isError() ? RefactoringStatus.ERROR : RefactoringStatus.WARNING),
                                                   problem.getMessage(),
