...
import com.codenvy.eclipse.jdt.internal.corext.util.MethodOverrideTester;
...
public class MethodChecks {
    ...
    public static RefactoringStatus checkIfOverridesAnother(IMethod method,
        ...
        String message = Messages.format(RefactoringCoreMessages.MethodChecks_overrides,
                                         new String[]{JavaElementUtil.createMethodSignature(overrides), JavaElementLabels.getElementLabel(
                                                 overrides.getDeclaringType(), JavaElementLabels.ALL_FULLY_QUALIFIED)});
        return RefactoringStatus.createStatus(RefactoringStatus.FATAL, message, context, "IDE",
                                              RefactoringStatusCodes.OVERRIDES_ANOTHER_METHOD, overrides);
    }

    public static RefactoringStatus checkIfComesFromInterface(IMethod method, ITypeHierarchy hierarchy,
        ...
        String message = Messages.format(RefactoringCoreMessages.MethodChecks_implements,
                                         new String[]{JavaElementUtil.createMethodSignature(inInterface), JavaElementLabels.getElementLabel(
                                                 inInterface.getDeclaringType(), JavaElementLabels.ALL_FULLY_QUALIFIED)});
        return RefactoringStatus.createStatus(RefactoringStatus.FATAL, message, context, "IDE",
                                              RefactoringStatusCodes.METHOD_DECLARED_IN_INTERFACE, inInterface);
    }

    public static IMethod isDeclaredInInterface(IMethod method, ITypeHierarchy hierarchy,
                                                IProgressMonitor monitor) throws JavaModelException {
        ...
        try {
            ...
            for (int i = 0; i < classes.length; i++) {
                ...
                if (clazz.equals(hierarchy.getType())) {
                    ...
                    superinterfaces = clazz.newSupertypeHierarchy(
                            new SubProgressMonitor(subMonitor, 1)).getAllSuperInterfaces(clazz);
                }
    ...
    public static IMethod overridesAnotherMethod(IMethod method, ITypeHierarchy hierarchy) throws JavaModelException {
        ...
        boolean overrides = (found != null && !found.equals(method) && (!JdtFlags.isStatic(
                found)) && (!JdtFlags.isPrivate(found)));
        if (overrides) {
            return found;
        } else {
            return null;
     ...
     * @param typeHierarchy
     *         a ITypeHierarchy of the declaring type of the method. May be null
     * @param monitor
     *         an IProgressMonitor
     ...
     *
     */
    public static IMethod getTopmostMethod(IMethod method, ITypeHierarchy typeHierarchy,
                                           IProgressMonitor monitor) throws JavaModelException {
