...
public class ClasspathChange {
    ...
    private void addClasspathDeltas(JavaElementDelta delta, IPackageFragmentRoot[] roots, int flag) {
        for (int i = 0; i < roots.length; i++) {
            ...
            if ((flag & IJavaElementDelta.F_REMOVED_FROM_CLASSPATH) != 0 || (flag & IJavaElementDelta.F_SOURCEATTACHED) != 0 ||
    ...
    private int classpathContains(IClasspathEntry[] list, IClasspathEntry entry) {
        ...
        for (int i = 0; i < listLen; i++) {
            ...
            if (other.getContentKind() == entry.getContentKind() && other.getEntryKind() == entry.getEntryKind() &&
                other.isExported() == entry.isExported() && other.getPath().equals(
                    entry.getPath())) {
    ...
    public int generateDelta(JavaElementDelta delta, boolean addClasspathChange) {
        ...
        try {
            ...
            if (this.oldResolvedClasspath != null && JavaProject.areClasspathsEqual(this.oldResolvedClasspath,
                                                                                    newResolvedClasspath, this.oldOutputLocation,
                                                                                    newOutputLocation)) {
                return result;
            }
        ...
        state.addForRefresh(
                this.project); // ensure external jars are refreshed for this project (see https://bugs.eclipse.org/bugs/show_bug
                // .cgi?id=212769 )
