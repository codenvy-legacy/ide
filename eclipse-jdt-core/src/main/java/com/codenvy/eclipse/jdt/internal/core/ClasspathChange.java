...
import com.codenvy.eclipse.jdt.internal.core.DeltaProcessor.RootInfo;
...
public class ClasspathChange
{
   ...
   private int classpathContains(IClasspathEntry[] list, IClasspathEntry entry)
   {
      ...
      for (int i = 0; i < listLen; i++)
      {
         ...
         if (other.getContentKind() == entry.getContentKind() && other.getEntryKind() == entry.getEntryKind() && other.isExported() == entry.isExported() && other.getPath().equals(
            entry.getPath()))
         {
   ...
   private ArrayList determineAffectedPackageFragments(IPath location) throws JavaModelException
   {
      ...
      if (resource != null && resource.getType() == IResource.FOLDER)
      {
         ...
         for (int i = 0; i < classpath.length; i++)
         {
            ...
            if (entry.getEntryKind() != IClasspathEntry.CPE_PROJECT && path.isPrefixOf(location) && !path.equals(
               location))
            {
   ...
   public int generateDelta(JavaElementDelta delta, boolean addClasspathChange)
   {
      ...
      try
      {
         ...
         if (this.oldRawClasspath != null && !JavaProject.areClasspathsEqual(this.oldRawClasspath, newRawClasspath,
            this.oldOutputLocation, newOutputLocation))
         {
         ...
         if (this.oldResolvedClasspath != null && JavaProject.areClasspathsEqual(this.oldResolvedClasspath,
            newResolvedClasspath, this.oldOutputLocation, newOutputLocation))
         {
            return result;
         }
      ...
      state.addForRefresh(
         this.project); // ensure external jars are refreshed for this project (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=212769 )
      ...
      for (int i = 0; i < oldLength; i++)
      {
         ...
         if (index == -1)
         {
            ...
            {
               PackageFragmentRoot oldRoot = (PackageFragmentRoot)removedRoots.get(
                  this.oldResolvedClasspath[i].getPath());
               if (oldRoot != null)
            ...
            if (pkgFragmentRoots == null)
            {
               try
               {
                  ...
                  this.project.computePackageFragmentRoots(this.oldResolvedClasspath[i], accumulatedRoots, rootIDs,
         ...
         else
         {
            ...
            { //reordering of the classpath
               addClasspathDeltas(delta, this.project.computePackageFragmentRoots(this.oldResolvedClasspath[i]),
                  IJavaElementDelta.F_REORDER);
            }

            // check source attachment
            IPath newSourcePath = newResolvedClasspath[index].getSourceAttachmentPath();
            int sourceAttachmentFlags = getSourceAttachmentDeltaFlag(
               this.oldResolvedClasspath[i].getSourceAttachmentPath(), newSourcePath);
            ...
            else
            {
               if (oldRootPath == null && newRootPath == null)
               {
                  ...
                  IPackageFragmentRoot[] computedRoots = this.project.computePackageFragmentRoots(
      ...
      for (int i = 0; i < newLength; i++)
      {
         ...
         if (index == -1)
         {
            ...
            addClasspathDeltas(delta, this.project.computePackageFragmentRoots(newResolvedClasspath[i]),
      ...
      if ((newOutputLocation == null && this.oldOutputLocation != null) || (newOutputLocation != null && !newOutputLocation.equals(
