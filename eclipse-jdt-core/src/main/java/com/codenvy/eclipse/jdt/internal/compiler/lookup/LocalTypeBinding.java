...
public final class LocalTypeBinding extends NestedTypeBinding {
    ...
    */
    public void addInnerEmulationDependent(BlockScope dependentScope, boolean wasEnclosingInstanceSupplied) {
        ...
        //  System.out.println("Adding dependency: "+ new String(scope.enclosingType().readableName()) + " --> " + new String(this
        // .readableName()));
    }
    ...
    protected void checkRedundantNullnessDefaultRecurse(ASTNode location, Annotation[] annotations,
        ...
        long outerDefault = this.enclosingMethod != null ? this.enclosingMethod.tagBits & ((TagBits.AnnotationNonNullByDefault |
                                                                                            TagBits.AnnotationNullUnspecifiedByDefault))
                                                         : 0;
        if (outerDefault != 0) {
    ...
    public char[] readableName() /*java.lang.Object,  p.X<T> */ {
        char[] readableName;
        if (isAnonymousType()) {
            readableName = CharOperation.concat(TypeConstants.ANONYM_PREFIX, anonymousOriginalSuperType().readableName(),
