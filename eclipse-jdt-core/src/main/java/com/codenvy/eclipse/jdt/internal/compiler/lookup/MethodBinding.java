   ...
   MethodBinding asRawMethod(LookupEnvironment env)
   {
      ...
      for (int i = 0; i < length; i++)
      {
         ...
         else
         {
            ...
            arguments[i] = env.createWildcard(null, 0, rawFirstBound, rawOtherBounds,
               com.codenvy.eclipse.jdt.internal.compiler.ast.Wildcard.EXTENDS);
         }
      }
...
*/
   ...
   */
      ...
      if (isPrivate())
      {
         ...
         receiverCheck:
         {
            if (receiverType != this.declaringClass)
            {
               ...
               if (scope.compilerOptions().complianceLevel <= ClassFileConstants.JDK1_6 && receiverType.isTypeVariable() && ((TypeVariableBinding)receiverType).isErasureBoundTo(
                  this.declaringClass.erasure()))
               {
                  break receiverCheck;
               }
               return false;
   ...
   public char[] computeUniqueKey(boolean isLeaf)
   {
      ...
      boolean addThrownExceptions = thrownExceptionsLength > 0 && (!isGeneric || CharOperation.lastIndexOf('^',
         sig) < 0);
   ...
   */
   ...
   protected void fillInDefaultNonNullness()
   {
      ...
      if (this.returnType != null && !this.returnType.isBaseType() && (this.tagBits & (TagBits.AnnotationNonNull | TagBits.AnnotationNullable)) == 0)
      {
      ...
      if ((originalMethod.tagBits & TagBits.AnnotationResolved) == 0 && originalMethod.declaringClass instanceof SourceTypeBinding)
      {
         ...
         if (scope != null)
         {
            ...
            if (nullDefaultBits != 0 && this.declaringClass instanceof SourceTypeBinding)
            {
               ...
               if (declaringSourceType.checkRedundantNullnessDefaultOne(methodDecl, methodDecl.annotations,
                  nullDefaultBits))
               {
                  declaringSourceType.checkRedundantNullnessDefaultRecurse(methodDecl, methodDecl.annotations,
                     nullDefaultBits);
               }
            }
         }
   ...
   */
   ...
   {
      if (this.selector.length == 4 && CharOperation.equals(this.selector,
         TypeConstants.MAIN) && ((this.modifiers & (ClassFileConstants.AccPublic | ClassFileConstants.AccStatic)) != 0) && TypeBinding.VOID == this.returnType && this.parameters.length == 1)
      {
      ...
      if (this.parameters != null)
      {
         if (this.parameters != Binding.NO_PARAMETERS)
         {
            for (int i = 0, length = this.parameters.length; i < length; i++)
            {
               ...
               output.append(
                  this.parameters[i] != null ? this.parameters[i].debugName() : "<no argument type>"); //$NON-NLS-1$
            }
         }
      }
      else
      ...
      if (this.thrownExceptions != null)
      {
         if (this.thrownExceptions != Binding.NO_EXCEPTIONS)
         {
            ...
            for (int i = 0, length = this.thrownExceptions.length; i < length; i++)
            {
               ...
               output.append(
                  (this.thrownExceptions[i] != null) ? this.thrownExceptions[i].debugName() : "<no exception type>"); //$NON-NLS-1$
            }
         }
      }
      else
