...
*/
        ...
        if (isPrivate()) {
            ...
            receiverCheck:
            {
                if (receiverType != this.declaringClass) {
                    // special tolerance for type variable direct bounds, but only if compliance <= 1.6,
                    // see: https://bugs.eclipse.org/bugs/show_bug.cgi?id=334622
                    if (scope.compilerOptions().complianceLevel <= ClassFileConstants.JDK1_6 && receiverType.isTypeVariable() &&
                        ((TypeVariableBinding)receiverType).isErasureBoundTo(
    ...
    */
    ...
    protected void fillInDefaultNonNullness() {
        ...
        if (this.returnType != null && !this.returnType.isBaseType() &&
        ...
        if (this.thrownExceptions != null) {
            if (this.thrownExceptions != Binding.NO_EXCEPTIONS) {
                ...
                for (int i = 0, length = this.thrownExceptions.length; i < length; i++) {
                    ...
                    output.append(
                            (this.thrownExceptions[i] != null) ? this.thrownExceptions[i].debugName()
