 ...
 *
 * @see com.codenvy.eclipse.jdt.internal.core.JarPackageFragment
 */
public class SourceMapper extends ReferenceInfoAdapter implements ISourceElementRequestor, SuffixConstants
{
   ...
   private synchronized void computeAllRootPaths(IType type)
   {
      ...
      boolean containsJavaSource = !pkgFragmentRootPath.equals(
         this.sourcePath); // used to optimize zip file reading only if source path and root path are equals, otherwise assume that attachment contains Java source
         ...
         try
         {
            ...
            for (Enumeration entries = zip.entries(); entries.hasMoreElements(); )
            {
               ...
               if (!entry.isDirectory())
               {
                  if (Util.isClassFileName(entryName))
                  {
                     ...
                     if (index != -1)
                     {
                        ...
                        if (!firstLevelPackageNames.contains(firstLevelPackageName))
                        {
                           ...
                           IStatus status = JavaConventions.validatePackageName(firstLevelPackageName, sourceLevel,
                              complianceLevel);
                  ...
                  else if (!containsJavaSource && com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(entryName))
                  {
            ...
            if (resource instanceof IContainer)
            {
               try
               {
                  ...
                  for (int i = 0, max = members.length; i < max; i++)
                  {
                     ...
                     else if (!containsJavaSource && com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(
                        resourceName))
                     {
                        containsJavaSource = true;
                     }
         ...
         else
         {
            ...
            try
            {
               ...
               for (Enumeration entries = zip.entries(); entries.hasMoreElements(); )
               {
                  ZipEntry entry = (ZipEntry)entries.nextElement();
                  String entryName;
                  if (!entry.isDirectory() && com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(
      ...
      try
      {
         ...
         for (int i = 0, max = resources.length; i < max; i++)
         {
            ...
            if (i == max - 1 && hasDefaultPackage)
            {
               ...
               for (int j = 0; j < max; j++)
               {
                  if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resources[i].getName()))
                  {
   ...
   public void enterType(TypeInfo typeInfo)
   {
      ...
      { // need to grow
         ...
         System.arraycopy(this.memberDeclarationStart, 0, this.memberDeclarationStart = new int[this.typeDepth * 2], 0,
            this.typeDepth);
         System.arraycopy(this.memberNameRange, 0, this.memberNameRange = new SourceRange[this.typeDepth * 2], 0,
            this.typeDepth);
         System.arraycopy(this.methodParameterTypes, 0, this.methodParameterTypes = new char[this.typeDepth * 2][][], 0,
            this.typeDepth);
         System.arraycopy(this.methodParameterNames, 0, this.methodParameterNames = new char[this.typeDepth * 2][][], 0,
            this.typeDepth);
      ...
      if (typeInfo.typeParameters != null)
      {
         for (int i = 0, length = typeInfo.typeParameters.length; i < length; i++)
         {
            ...
            setSourceRange(typeParameter, new SourceRange(typeParameterInfo.declarationStart,
      ...
      if (this.typeDepth >= 0)
      {
         ...
         if (methodInfo.parameterInfos != null)
         {
            for (int i = 0, length = methodInfo.parameterInfos.length; i < length; i++)
            {
               ...
               SourceRange[] allRanges = new SourceRange[]{new SourceRange(parameterInfo.declarationStart,
                  parameterInfo.declarationEnd - parameterInfo.declarationStart + 1), new SourceRange(
                  parameterInfo.nameSourceStart, parameterInfo.nameSourceEnd - parameterInfo.nameSourceStart + 1)};
   ...
   public void exitType(int declarationEnd)
   {
      ...
      {
         IType currentType = this.types[this.typeDepth];
         setSourceRange(currentType, new SourceRange(this.typeDeclarationStarts[this.typeDepth],
            declarationEnd - this.typeDeclarationStarts[this.typeDepth] + 1), this.typeNameRanges[this.typeDepth]);
         this.typeDepth--;
      }
   }
      ...
      if (this.typeDepth >= 0)
      {
         ...
         setSourceRange(currentType.getField(this.memberName[this.typeDepth]),
            new SourceRange(this.memberDeclarationStart[this.typeDepth],
               declarationEnd - this.memberDeclarationStart[this.typeDepth] + 1), this.memberNameRange[this.typeDepth]);
      }
   }
   ...
   public char[] findSource(IType type, String simpleSourceFileName)
   {
      ...
      String name = com.codenvy.eclipse.jdt.internal.core.util.Util.concatWith(pkgFrag.names, simpleSourceFileName, '/');
      ...
      if (target instanceof IContainer)
      {
         ...
         if (res instanceof IFile)
         {
            try
            {
               ...
               source = com.codenvy.eclipse.jdt.internal.core.util.Util.getResourceContentsAsCharArray((IFile)res,
   ...
   public synchronized ISourceRange mapSource(IType type, char[] contents, IBinaryType info, IJavaElement elementToFind)
   {
      ...
      try
      {
         ...
         parser.parseCompilationUnit(
            new BasicCompilationUnit(contents, null, this.binaryType.sourceFileName(info), javaElement), doFullParse,
            null/*no progress*/);
         if (elementToFind != null)
         {
