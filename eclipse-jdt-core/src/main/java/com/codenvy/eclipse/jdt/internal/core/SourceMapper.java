 ...
 *     Kelly Campbell <kellyc@google.com> - Hangs in SourceMapper during java proposals - https://bugs.eclipse.org/bugs/show_bug
 *     .cgi?id=281575
 *******************************************************************************/
...
public class SourceMapper extends ReferenceInfoAdapter implements ISourceElementRequestor, SuffixConstants {
    ...
    private synchronized void computeAllRootPaths(IType type) {
        ...
        boolean containsJavaSource = !pkgFragmentRootPath.equals(
                this.sourcePath); // used to optimize zip file reading only if source path and root path are equals,
                // otherwise assume that attachment contains Java source
        ...
        if (this.typeDepth >= 0) {
            ...
            if (methodInfo.typeParameters != null) {
                for (int i = 0, length = methodInfo.typeParameters.length; i < length; i++) {
                    ...
                    setSourceRange(typeParameter, new SourceRange(typeParameterInfo.declarationStart,
                                                                  typeParameterInfo.declarationEnd - typeParameterInfo.declarationStart +
                                                                  1),
                                   new SourceRange(typeParameterInfo.nameSourceStart,
                                                   typeParameterInfo.nameSourceEnd - typeParameterInfo.nameSourceStart + 1));
                }
            ...
            if (methodInfo.parameterInfos != null) {
                for (int i = 0, length = methodInfo.parameterInfos.length; i < length; i++) {
                    ...
                    SourceRange[] allRanges = new SourceRange[]{new SourceRange(parameterInfo.declarationStart,
                                                                                parameterInfo.declarationEnd -
                                                                                parameterInfo.declarationStart + 1), new SourceRange(
