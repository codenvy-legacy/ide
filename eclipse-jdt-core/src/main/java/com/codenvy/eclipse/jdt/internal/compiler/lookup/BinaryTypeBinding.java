...
import com.codenvy.eclipse.jdt.internal.compiler.env.IBinaryElementValuePair;
import com.codenvy.eclipse.jdt.internal.compiler.env.IBinaryField;
import com.codenvy.eclipse.jdt.internal.compiler.env.IBinaryMethod;
import com.codenvy.eclipse.jdt.internal.compiler.env.IBinaryNestedType;
import com.codenvy.eclipse.jdt.internal.compiler.env.IBinaryType;
import com.codenvy.eclipse.jdt.internal.compiler.impl.BooleanConstant;
...
public class BinaryTypeBinding extends ReferenceBinding {
	 ...
	 */
        ...
        this.typeVariables = typeSignature != null && typeSignature.length > 0 && typeSignature[0] == Util.C_GENERIC_START ? null
                             // is initialized in cachePartsFrom (called from LookupEnvironment.createBinaryTypeFrom())... must set to
                             // null so isGenericType() answers true
                                                                                                                           : Binding
                                     .NO_TYPE_VARIABLES;
        ...
        if (enclosingTypeName != null) {
            ...
            this.enclosingType = environment.getTypeFromConstantPoolName(enclosingTypeName, 0, -1, true,
                                                                         null /* could not be missing */); // pretend parameterized to
                                                                         // avoid raw
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.lookup.ReferenceBinding#availableMethods() */
            ...
            if (memberTypeStructures != null) {
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        this.memberTypes[i] = this.environment.getTypeFromConstantPoolName(memberTypeStructures[i].getName(),
        ...
        if (iFields != null) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    TypeBinding type = fieldSignature == null ? this.environment.getTypeFromSignature(
                            binaryField.getTypeName(), 0, -1, false, this,
                            missingTypeNames) : this.environment.getTypeFromTypeSignature(new SignatureWrapper(fieldSignature),
                                                                                          Binding.NO_TYPE_VARIABLES, this,
                                                                                          missingTypeNames);
                    FieldBinding field = new FieldBinding(binaryField.getName(), type,
                                                          binaryField.getModifiers() | ExtraCompilerModifiers.AccUnresolved, this,
	 ...
	 */
        ...
        if (methodSignature == null) { // no generics
                    ...
                    if (i >= startIndex) {   // skip the synthetic arg if necessary
                        ...
                        if (paramAnnotations != null) {
                            paramAnnotations[i - startIndex] = createAnnotations(
                                    method.getParameterAnnotations(i - startIndex), this.environment, missingTypeNames);
                        }
                    }
                    index = end + 1;
                }
            ...
            if (!method.isConstructor()) {
                returnType = this.environment.getTypeFromSignature(methodDescriptor, index + 1, -1, false, this,
                                                                   missingTypeNames);   // index is currently pointing at the ')'
            }
        } else {
        ...
        MethodBinding result = method.isConstructor() ? new MethodBinding(methodModifiers, parameters, exceptions,
                                                                          this)
                                                      : new MethodBinding(methodModifiers, method.getSelector(), returnType, parameters,
                                                                          exceptions, this);
        if (this.environment.globalOptions.storeAnnotations) {
            result.setAnnotations(createAnnotations(method.getAnnotations(), this.environment, missingTypeNames),
                                  paramAnnotations, isAnnotationType() ? convertMemberValue(method.getDefaultValue(), this.environment,
                                                                                            missingTypeNames) : null, this.environment);
        }
    ...
    */
    ...
    private void initializeTypeVariable(TypeVariableBinding variable, TypeVariableBinding[] existingVariables,
        ...
        if (wrapper.signature[wrapper.start] == Util.C_COLON) {
            ...
            TypeBinding typeFromTypeSignature = this.environment.getTypeFromTypeSignature(wrapper, existingVariables, this,
                                                                                          missingTypeNames);
    ...
    MethodBinding resolveTypesFor(MethodBinding method) {
        ...
        for (int i = method.thrownExceptions.length; --i >= 0; ) {
            ReferenceBinding resolvedType = (ReferenceBinding)resolveType(method.thrownExceptions[i], this.environment,
                                                                          true /* raw conversion */);
    ...
    void scanMethodForNullAnnotation(IBinaryMethod method, MethodBinding methodBinding) {
        ...
        if (annotations != null) {
            for (int i = 0; i < annotations.length; i++) {
                ...
                char[][] typeName = CharOperation.splitOn('/', annotationTypeName, 1,
                                                          annotationTypeName.length - 1); // cut of leading 'L' and trailing ';'
        ...
        if (!explicitNullness && (methodBinding.returnType != null && !methodBinding.returnType.isBaseType()) &&
            currentDefault == NONNULL_BY_DEFAULT) {
            methodBinding.tagBits |= TagBits.AnnotationNonNull;
        }
    ...
    void scanTypeForNullDefaultAnnotation(IBinaryType binaryType, PackageBinding packageBinding,
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (CharOperation.equals(typeName, nonNullByDefaultAnnotationName)) {
                    ...
                    if (elementValuePairs != null && elementValuePairs.length == 1) {
                        ...
                        if (value instanceof BooleanConstant && !((BooleanConstant)value).booleanValue()) {
