...
import com.codenvy.eclipse.jdt.internal.core.builder.ReferenceCollection;
...
public class IndexSelector {
    ...
    private static boolean canSeeFocus(IJavaElement[] focuses, JavaProject javaProject, char[][][] focusQualifiedNames) {
        ...
        for (int i = 0; i < length; i++) {
            if (canSeeFocus(focuses[i], javaProject, focusQualifiedNames)) {
                return true;
            }
        }
        return false;
            ...
            for (int i = 0, length = entries.length; i < length; i++) {
                ...
                if (entry.getEntryKind() == IClasspathEntry.CPE_PROJECT && entry.getPath().equals(focusPath)) {
                    if (focusQualifiedNames !=
                        ...
                        State projectState = (State)JavaModelManager.getJavaModelManager().getLastBuiltState(
            ...
            for (int i = 0; i < length; i++) {
                ...
                for (int m = 0; m < mLength; m++) {
                    if (parameterCount == methods[m].getNumberOfParameters() && methods[m].getElementName().equals(selector)) {
                        IPackageFragmentRoot root = (IPackageFragmentRoot)allTypes[i].getAncestor(
    ...
    private void initializeIndexLocations() {
        ...
        if (focus == null) {
            ...
            try {
                ...
                // find the projects from projectsAndJars that see the focus then walk those projects looking for the jars from
                // projectsAndJars
                ...
                // jar files can be included in the search scope without including one of the projects that references them,
                // so scan all projects that have not been visited
        ...
        //   locations.add(manager.computeIndexLocation())
    ...
    private char[][][] getQualifiedNames(ObjectVector types) {
        ...
        return focusQualifiedNames.length == 0 ? null : ReferenceCollection.internQualifiedNames(focusQualifiedNames,
                                                                                                 true);
    }
}
