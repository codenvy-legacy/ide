 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.compiler.lookup;
...
public abstract class Scope {
   ...
   *
	...
	*/
        ...
        if ((originalType.tagBits & TagBits.HasTypeVariable) != 0) {
            switch (originalType.kind()) {
                case Binding.ARRAY_TYPE:
                    ...
                    TypeBinding substitute = convertEliminatingTypeVariables(originalLeafComponentType, genericType, rank,
                                                                             eliminatedVariables); // substitute could itself be array type
                ...
                case Binding.GENERIC_TYPE:
                    ...
                    for (int i = 0, length = originalArguments == null ? 0 : originalArguments.length; i < length; i++) {
                        TypeBinding originalArgument = originalArguments[i];
                        TypeBinding substitutedArgument = convertEliminatingTypeVariables(originalArgument, currentType, i,
                                                                                          eliminatedVariables);
                        if (substitutedArgument != originalArgument) {
                            if (substitutedArguments == originalArguments) {
                                System.arraycopy(originalArguments, 0, substitutedArguments = new TypeBinding[length], 0, i);
                ...
                case Binding.INTERSECTION_TYPE:
                    ...
                    for (int i = 0, length = originalOtherBounds == null ? 0 : originalOtherBounds.length; i < length; i++) {
                        ...
                        TypeBinding substitutedOtherBound = convertEliminatingTypeVariables(originalOtherBound, genericType,
                                                                                            rank, eliminatedVariables);
                        if (substitutedOtherBound != originalOtherBound) {
                            if (substitutedOtherBounds == originalOtherBounds) {
                                System.arraycopy(originalOtherBounds, 0, substitutedOtherBounds = new TypeBinding[length], 0,
                                                 i);
                            }
        ...
        if (length > 2 && length < 8) {
            switch (name[0]) {
                ...
                case 'v':
                    ...
                    break;
                case 'b':
                    if (length == 7 && name[1] == 'o' && name[2] == 'o' && name[3] == 'l' && name[4] == 'e' && name[5] == 'a' &&
                        name[6] == 'n') {
    ...
    public static TypeBinding substitute(Substitution substitution, TypeBinding originalType) {
        ...
        switch (originalType.kind()) {
            ...
            case Binding.ARRAY_TYPE:
                ArrayBinding originalArrayType = (ArrayBinding)originalType;
                TypeBinding originalLeafComponentType = originalArrayType.leafComponentType;
                TypeBinding substitute = substitute(substitution,
                                                    originalLeafComponentType); // substitute could itself be array type
                if (substitute != originalLeafComponentType) {
                    return originalArrayType.environment.createArrayType(substitute.leafComponentType(),
            ...
            case Binding.TYPE:
                ...
                if (substitutedEnclosing != originalEnclosing) {
                    return substitution.isRawSubstitution() ? substitution.environment().createRawType(originalReferenceType,
                                                                                                       substitutedEnclosing)
                                                            : substitution.environment().createParameterizedType(originalReferenceType,
                                                                                                                 null,
                                                                                                                 substitutedEnclosing);
                }
                break;
     ...
     */
    protected final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments,
                                                          InvocationSite invocationSite, boolean tiebreakingVarargsMethods) {
        TypeBinding[] genericTypeArguments = invocationSite.genericTypeArguments();
        TypeBinding[] parameters = method.parameters;
        TypeVariableBinding[] typeVariables = method.typeVariables;
        if (parameters == arguments && (method.returnType.tagBits & TagBits.HasTypeVariable) == 0 && genericTypeArguments == null &&
        ...
        if ((compatibilityLevel = parameterCompatibilityLevel(method, arguments,
                                                              tiebreakingVarargsMethods)) > NOT_COMPATIBLE) {
    ...
    protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions) {
        ...
        for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++) {
            ...
            TypeBinding superType =
                    this.kind == METHOD_SCOPE ? typeRef.resolveType((BlockScope)this, false/*no bound check*/) : typeRef.resolveType(
                            (ClassScope)this);
            if (superType == null) {
            ...
            if (boundRefs != null) {
                ...
                for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
                    typeRef = boundRefs[j];
                    superType = this.kind == METHOD_SCOPE ? typeRef.resolveType((BlockScope)this,
                                                                                false) : typeRef.resolveType((ClassScope)this);
                    if (superType == null) {
                        typeVariable.tagBits |= TagBits.HierarchyHasProblems;
                        continue nextBound;
                    } else {
    ...
    public TypeVariableBinding[] createTypeVariables(TypeParameter[] typeParameters, Binding declaringElement) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // detect duplicates, but keep each variable to reduce secondary errors with instantiating this generic type (assume number
            ...
            for (int i = startFoundSize; i < foundSize; i++) {
                ...
                if (compatibleMethod != null) {
                    if (compatibleMethod.isValidBinding()) {
                        if (concreteMatch != null && environment().methodVerifier().areMethodsCompatible(concreteMatch,
                                                                                                         compatibleMethod)) {
    ...
    public MethodBinding findExactMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        ...
        if (exactMethod != null && exactMethod.typeVariables == Binding.NO_TYPE_VARIABLES && !exactMethod.isBridge()) {
            ...
            if (receiverType.isInterface() || exactMethod.canBeSeenBy(receiverType, invocationSite, this)) {
                if (argumentTypes == Binding.NO_PARAMETERS && CharOperation.equals(selector,
                                                                                   TypeConstants.GETCLASS) &&
	...
	*/
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite,
        ...
        if (field != null) {
            ...
            if (invocationSite == null || insideTypeAnnotations ? field.canBeSeenBy(
                    getCurrentPackage()) : field.canBeSeenBy(currentType, invocationSite, this)) {
                return field;
            }
            ...
            for (int i = 0; i < nextPosition; i++) {
                ...
                if ((field = anInterface.getField(fieldName, true /*resolve*/)) != null) {
                    ...
                    if (visibleField == null) {
                        ...
                        ambiguous = new ProblemFieldBinding(visibleField /* closest match*/, visibleField.declaringClass,
                                                            fieldName, ProblemReasons.Ambiguous);
                        break done;
                    }
                } else {
    ...
    public ReferenceBinding findMemberType(char[] typeName, ReferenceBinding enclosingType) {
        ...
        if (memberType != null) {
            ...
            if (enclosingSourceType == null ||
                (this.parent == unitScope && (enclosingSourceType.tagBits & TagBits.TypeVariablesAreConnected) == 0) ? memberType
                        .canBeSeenBy(
                                currentPackage) : memberType.canBeSeenBy(enclosingType, enclosingSourceType)) {
                return memberType;
            }
            return new ProblemReferenceBinding(new char[][]{typeName}, memberType, ProblemReasons.NotVisible);
        }
    ...
    public MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        ...
        if (candidatesCount == 0) {
            ...
            MethodBinding bestGuess = (MethodBinding)found.elementAt(
                    0); // if no good match so just use the first one found
                    ...
                    for (int j = i + 1; j < visiblesCount; j++) {
                        ...
                        if (otherCandidate.hasSubstitutedParameters()) {
                            if (otherCandidate == candidate ||
                                (candidate.declaringClass == otherCandidate.declaringClass && candidate.areParametersEqual(
                                        otherCandidate))) {
                                return new ProblemMethodBinding(candidates[i], candidates[i].selector, candidates[i].parameters,
                                                                ProblemReasons.Ambiguous);
                            }
                        }
                    }
            ...
            if (argumentTypes == Binding.NO_PARAMETERS) {
                switch (selector[0]) {
                    case 'c':
                        ...
                        break;
                    case 'g':
                        if (CharOperation.equals(selector,
                                                 TypeConstants.GETCLASS) && methodBinding.returnType.isParameterizedType()/*1.5*/) {
                                ...
                                if (fieldBinding != null) { // skip it if we did not find anything
                                    ...
                                    if (fieldBinding.isValidBinding()) {
                                        ...
                                        if (receiverType == fieldBinding.declaringClass ||
                                            ...
                                            // type (that is, not inherited)
                                            // but only if "valid field" was inherited in the first place.
                                            {
                                                if (foundField.declaringClass != fieldBinding.declaringClass && foundField.declaringClass !=
                        ...
                        for (int i = 0, length = imports.length; i < length; i++) {
                            ...
                            if (importBinding.isStatic() && importBinding.onDemand) {
                                ...
                                if (resolvedImport instanceof ReferenceBinding) {
                                    FieldBinding temp = findField((ReferenceBinding)resolvedImport, name, invocationSite,
                                                                  needResolve);
                                    if (temp != null) {
                        ...
                        if (methodBinding != null) { // skip it if we did not find anything
                            if (foundMethod == null) {
                                if (methodBinding.isValidBinding()) {
                                    ...
                                    if (inheritedHasPrecedence || receiverType == methodBinding.declaringClass || (receiverType.getMethods(
                                            selector)) != Binding.NO_METHODS) {
                                        // found a valid method in the 'immediate' scope (i.e. not inherited)
                                        // OR in 1.4 mode (inherited visible shadows enclosing)
                                        // OR the receiverType implemented a method with the correct name
                                        // return the methodBinding if it is not declared in a superclass of the scope's binding (that
                                        // is, inherited)
                                    ...
                                    if (foundProblem == null) {
                                        foundProblem =
                                                methodBinding; // hold onto the first not visible/found error and keep the second not
                                ...
                                // otherwise if a method was found, complain when another is found in an 'immediate' enclosing type (that
                                // is, not inherited)
                        ...
                        if (importBinding.onDemand) {
                            ...
                            if (resolvedImport instanceof MethodBinding) {
                                ...
                                if (CharOperation.equals(staticField.name, selector)) {
                                    ...
                                    // answers closest approximation, may not check argumentTypes or visibility
                                    {
                                        possible = findMethod((ReferenceBinding)referencedType, selector, argumentTypes,
                                                              invocationSite, true);
                                    }
                                }
                            }
        ...
        while (currentIndex < length) {
            ...
            if (binding == null) {
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex),
                                                   null /* no closest match since search for pkg*/,
                                                   ProblemReasons.NotFound);
            }
            if (!binding.isValidBinding()) {
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex),
    ...
    public final TypeBinding getType(char[] name, PackageBinding packageBinding) {
        ...
        if (!binding.isValidBinding()) {
            return new ProblemReferenceBinding(
                    binding instanceof ReferenceBinding ? ((ReferenceBinding)binding).compoundName : CharOperation.arrayConcat(
                            packageBinding.compoundName, name),
	...
	*/
                            ...
                            if (memberType != null) { // skip it if we did not find anything
                                ...
                                if (memberType.isValidBinding()) {
                                    if (sourceType == memberType.enclosingType() || inheritedHasPrecedence) {
                                        ...
                                        // if a valid type was found, complain when another is found in an 'immediate' enclosing type (i
                                        // .e. not inherited)
                ...
                // replace it in the cache with the type
                    ...
                    if (cachedBinding instanceof ImportConflictBinding) {
                        typeOrPackageCache.put(name,
                                               cachedBinding =
                                                       ((ImportConflictBinding)cachedBinding).conflictingTypeBinding); // already know
                                                       // its visible
                    } else {
                        typeOrPackageCache.put(name,
                            ...
                            if (temp != null && temp.isStatic() && temp != type) {
                                if (temp.isValidBinding()) {
                                    if (!temp.canBeSeenBy(unitScope.fPackage)) {
                                        ...
                                        foundType = new ProblemReferenceBinding(new char[][]{name}, type,
                                                                                ProblemReasons.NotVisible);
                                    } else {
                                        ImportReference importReference = importBinding.reference;
                                        if (importReference != null) {
                ...
                for (int i = 0, length = imports.length; i < length; i++) {
                    ...
                    if (someImport.onDemand) {
                        ...
                        if (resolvedImport instanceof PackageBinding) {
                            ...
                            temp = findMemberType(name,
                                                  (ReferenceBinding)resolvedImport); // static imports are allowed to see inherited
                                                  // member types
    ...
    public final Binding getTypeOrPackage(char[][] compoundName) {
        ...
        ReferenceBinding qualifiedType =
                (ReferenceBinding)environment().convertToRawType(typeBinding, false /*do not force conversion of enclosing types*/);
        ...
        while (currentIndex < nameLength) {
            ...
            if (typeBinding.isGenericType()) {
                ...
                qualifiedType =
                        (qualifiedType != null && (qualifiedType.isRawType() || qualifiedType.isParameterizedType())) ? environment()
                                .createParameterizedType(
                                        typeBinding, null, qualifiedType) : typeBinding;
            }
        }
        ...
        if (mecs != null) {
            ...
            for (int k = 0, max = mecs.length; k < max; k++) {
                ...
                if (value instanceof TypeBinding[]) {
                    ...
                    problemReporter().superinterfacesCollide(invalidInvocations[0].erasure(), typeRef, invalidInvocations[0],
    ...
    protected boolean isAcceptableMethod(MethodBinding one, MethodBinding two) {
        ...
        if (one.isVarargs() && two.isVarargs()) {
            ...
            if (parameterCompatibilityLevel(one, twoParams, true) == NOT_COMPATIBLE && parameterCompatibilityLevel(two,
                                                                                                                   oneParams, true) ==
                                                                                       VARARGS_COMPATIBLE) {
                return true;
            }
                ...
                case Binding.PARAMETERIZED_TYPE:
                    ...
                    for (int j = 0; j < argLength; j++) {
                        TypeBinding bestArgument = leastContainingTypeArgument(bestArguments[j],
                                                                               parameterizedType.arguments[j], (ReferenceBinding)mec, j,
                                                                               lubStack);
                        if (bestArgument == null) {
                            return null;
                        }
                    ...
                    break;
                case Binding.RAW_TYPE:
                    return dim == 0 ? invocation : environment().createArrayType(invocation,
                                                                                 dim); // raw type is taking precedence
            }
        }
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (currentType.isCapture()) {
                ...
                if (firstBound != null && firstBound.isArrayType()) {
                    TypeBinding superType = dim == 0 ? firstBound : (TypeBinding)environment().createArrayType(firstBound,
                                                                                                               dim); // recreate array if
                                                                                                               // needed
                    if (!typesToVisit.contains(superType)) {
                        typesToVisit.add(superType);
                        max++;
                        TypeBinding superTypeErasure =
                                (firstBound.isTypeVariable() || firstBound.isWildcard() /*&& !itsInterface.isCapture()*/) ? superType
                                                                                                                          : superType
                                        .erasure();
            ...
            if (itsInterfaces !=
                ...
                for (int j = 0, count = itsInterfaces.length; j < count; j++) {
                    ...
                    TypeBinding superType = dim == 0 ? itsInterface : (TypeBinding)environment().createArrayType(
                            itsInterface, dim); // recreate array if needed
                    if (!typesToVisit.contains(superType)) {
                        typesToVisit.add(superType);
                        max++;
                        TypeBinding superTypeErasure =
                                (itsInterface.isTypeVariable() || itsInterface.isWildcard() /*&& !itsInterface.isCapture()*/) ? superType
                                                                                                                              : superType
                                        .erasure();
                        if (superTypeErasure != superType) {
                            allInvocations.put(superTypeErasure, superType);
            ...
            if (itsSuperclass != null) {
                TypeBinding superType = dim == 0 ? itsSuperclass : (TypeBinding)environment().createArrayType(itsSuperclass,
                                                                                                              dim); // recreate array if
                                                                                                              // needed
                if (!typesToVisit.contains(superType)) {
                    typesToVisit.add(superType);
                    max++;
                    TypeBinding superTypeErasure =
                            (itsSuperclass.isTypeVariable() || itsSuperclass.isWildcard() /*&& !itsSuperclass.isCapture()*/) ? superType
                                                                                                                             : superType
                                    .erasure();
                    if (superTypeErasure != superType) {
                        allInvocations.put(superTypeErasure, superType);
                ...
                for (int j = 0; j < superLength; j++) {
                    ...
                    if (erasedSuperType instanceof ReferenceBinding) {
                        ...
                        if (otherType.isArrayType() // keep Object[...] for an interface array (same dimensions)
                            ...
                            otherType.dimensions() == erasedSuperType.dimensions() && erasedSuperType.leafComponentType().isInterface()) {
                            continue nextOtherType;
                        }
                        if (erasedSuperType.findSuperTypeOriginatingFrom(otherType) != null) {
                            erasedSuperTypes[j] = null; // discard non minimal supertype
	...
	*/
    ...
    protected final MethodBinding mostSpecificMethodBinding(MethodBinding[] visible, int visibleSize,
        ...
        for (int level = 0, max = VARARGS_COMPATIBLE; level <= max; level++) {
            ...
            for (int i = 0; i < visibleSize; i++) {
                ...
                for (int j = 0; j < visibleSize; j++) {
                    ...
                    // pick a concrete method over a bridge method when parameters are equal since the return type of the concrete method
                    // is more specific
        ...
        for (int i = 0; i < visibleSize; i++) {
            ...
            if (current != null) {
                ...
                MethodBinding original = current.original();
                boolean shouldIntersectExceptions = original.declaringClass.isAbstract() && original.thrownExceptions !=
                                                                                            Binding.NO_EXCEPTIONS; // only needed when
                                                                                            // selecting from interface methods
                for (int j = 0; j < visibleSize; j++) {
                    ...
                    if (!original.isAbstract()) {
                        ...
                        if (original.returnType != original2.returnType) {
                            if (next.original().typeVariables != Binding.NO_TYPE_VARIABLES) {
                                if (original.returnType.erasure().findSuperTypeOriginatingFrom(
                                        original2.returnType.erasure()) == null) {
    ...
    public int parameterCompatibilityLevel(MethodBinding method, TypeBinding[] arguments,
            ...
            if (paramLength == argLength) { // accept X or X[] but not X[][]
                ...
                if (paramLength < argLength) { // all remaining argument types must be compatible with the elementsType of varArgType
                    ...
                    for (int i = lastIndex; i < argLength; i++) {
                        TypeBinding arg =
                                (tiebreakingVarargsMethods && (i == (argLength - 1))) ? ((ArrayBinding)arguments[i]).elementsType()
                                                                                      : arguments[i];
                        if (param != arg && parameterCompatibilityLevel(arg, param, env,
                                                                        tiebreakingVarargsMethods) == NOT_COMPATIBLE) {
                            return NOT_COMPATIBLE;
                        }
                    }
        ...
        for (int i = 0; i < lastIndex; i++) {
            TypeBinding param = parameters[i];
            TypeBinding arg =
                    (tiebreakingVarargsMethods && (i == (argLength - 1))) ? ((ArrayBinding)arguments[i]).elementsType() : arguments[i];
            if (arg != param) {
    ...
    private int parameterCompatibilityLevel(TypeBinding arg, TypeBinding param, LookupEnvironment env,
        ...
        if (tieBreakingVarargsMethods && (this.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_7 ||
                                          !CompilerOptions.tolerateIllegalAmbiguousVarargsInvocation)) {
    ...
    public MethodBinding getStaticFactory(ReferenceBinding allocationType, ReferenceBinding originalEnclosingType,
        ...
        for (int i = 0, length = methods.length; i < length; i++) {
            ...
            MethodBinding staticFactory = new MethodBinding(method.modifiers | ClassFileConstants.AccStatic,
                                                            TypeConstants.SYNTHETIC_STATIC_FACTORY, null, null, null,
            ...
            for (int j = 0; j < classTypeVariablesArity; j++) {
                map.put(classTypeVariables[j], staticFactory.typeVariables[j] = new TypeVariableBinding(
            ...
            for (int j = classTypeVariablesArity, max = classTypeVariablesArity + methodTypeVariablesArity; j < max; j++) {
                map.put(methodTypeVariables[j - classTypeVariablesArity],
                        (staticFactory.typeVariables[j] = new TypeVariableBinding(
                                CharOperation.concat(methodTypeVariables[j - classTypeVariablesArity].sourceName, "''".toCharArray()),
                                //$NON-NLS-1$
                                staticFactory, j, environment)));
            }
            ...
            for (int j = 0, max = classTypeVariablesArity + methodTypeVariablesArity; j < max; j++) {
                ...
                if (substitutedType instanceof TypeVariableBinding) {
                    ...
                    if (originalVariable.firstBound != null) {
                        substitutedVariable.firstBound = originalVariable.firstBound == originalVariable.superclass ? substitutedSuperclass
                        ...
                        default:
                            if (substitutedSuperclass.isInterface()) {
                                ...
                                substitutedVariable.superclass =
                                        (ReferenceBinding)substitutedSuperclass; // typeVar was extending other typeVar which got substituted with interface
                                substitutedVariable.superInterfaces = substitutedInterfaces;
                            }
                    }
                }
    ...
    public void validateNullAnnotation(long tagBits, TypeReference typeRef, Annotation[] annotations) {
        ...
        if (nullAnnotationTagBit != 0) {
            ...
            if (type != null && type.isBaseType()) {
                char[][] annotationName = (nullAnnotationTagBit == TagBits.AnnotationNonNull) ? environment().getNonNullAnnotationName()
