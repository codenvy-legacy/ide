   ...
   *
         ...
         case Binding.TYPE:
            ...
            if (substitutedEnclosing != originalEnclosing)
            {
               return substitution.isRawSubstitution() ? substitution.environment().createRawType(originalReferenceType,
                  substitutedEnclosing) : substitution.environment().createParameterizedType(originalReferenceType,
   ...
   protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions)
   {
      ...
      for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++)
      {
         ...
         TypeBinding superType = this.kind == METHOD_SCOPE ? typeRef.resolveType((BlockScope)this, false/*no bound check*/) : typeRef.resolveType(
            (ClassScope)this);
         if (superType == null)
         {
      ...
      if (exactMethod != null && exactMethod.typeVariables == Binding.NO_TYPE_VARIABLES && !exactMethod.isBridge())
      {
         ...
         if (receiverType.isInterface() || exactMethod.canBeSeenBy(receiverType, invocationSite, this))
         {
            if (argumentTypes == Binding.NO_PARAMETERS && CharOperation.equals(selector,
               TypeConstants.GETCLASS) && exactMethod.returnType.isParameterizedType()/*1.5*/)
            {
   ...
   public ReferenceBinding findMemberType(char[] typeName, ReferenceBinding enclosingType)
   {
      ...
      {
         unitScope.recordTypeReference(memberType);
         if (enclosingSourceType == null || (this.parent == unitScope && (enclosingSourceType.tagBits & TagBits.TypeVariablesAreConnected) == 0) ? memberType.canBeSeenBy(
      ...
      InvocationSite invocationSite, boolean inStaticContext)
   {
      ...
      if (complianceLevel <= ClassFileConstants.JDK1_3)
      {
         ...
         return !declaringClass.isInterface() ? mostSpecificClassMethodBinding(candidates, visiblesCount,
            invocationSite) : mostSpecificInterfaceMethodBinding(candidates, visiblesCount, invocationSite);
      }
         ...
         for (int i = 0; i < visiblesCount; i++)
         {
            ...
            if (candidate.hasSubstitutedParameters())
            {
               for (int j = i + 1; j < visiblesCount; j++)
               {
                  ...
                  {
                     if (otherCandidate == candidate || (candidate.declaringClass == otherCandidate.declaringClass && candidate.areParametersEqual(
                        otherCandidate)))
                     {
                        ...
                        { // skip it if we did not find anything
                           ...
                           if (fieldBinding.isValidBinding())
                           {
                              ...
                              if (receiverType == fieldBinding.declaringClass || compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4)
                              {
                                 ...
                                 {
                                    if (foundField.declaringClass != fieldBinding.declaringClass && foundField.declaringClass != foundActualReceiverType) // https://bugs.eclipse.org/bugs/show_bug.cgi?id=316956
                     ...
                     if (importBinding.isStatic() && !importBinding.onDemand)
                     {
                           ...
                           name))
                        {
                           if (unitScope.resolveSingleImport(importBinding,
                              Binding.TYPE | Binding.FIELD | Binding.METHOD) != null && importBinding.resolvedImport instanceof FieldBinding)
                           {
               ...
               if (!insideTypeAnnotation)
               {
                  ...
                  { // skip it if we did not find anything
                        ...
                        if (methodBinding.isValidBinding())
                        {
                           if (!methodBinding.isStatic() && (insideConstructorCall || insideStaticContext))
                           {
                              ...
                              return new ProblemMethodBinding(methodBinding, // closest match
                                 ...
                                 insideConstructorCall ? ProblemReasons.NonStaticReferenceInConstructorInvocation : ProblemReasons.NonStaticReferenceInStaticContext);
                           }
                     ...
                     { // found a valid method so check to see if this is a hiding case
                        if (methodBinding.problemId() == ProblemReasons.Ambiguous || (foundMethod.declaringClass != methodBinding.declaringClass && (receiverType == methodBinding.declaringClass || receiverType.getMethods(
                           selector) != Binding.NO_METHODS)))
   ...
   public final Binding getPackage(char[][] compoundName)
   {
      ...
      while (currentIndex < length)
      {
         ...
         if (binding == null)
         {
            return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), null /* no closest match since search for pkg*/,
               ProblemReasons.NotFound);
         }
         if (!binding.isValidBinding())
         {
               ...
               if (someImport.onDemand)
               {
                  ...
                  else if (someImport.isStatic())
                  {
                     temp = findMemberType(name,
                        (ReferenceBinding)resolvedImport); // static imports are allowed to see inherited member types
   ...
   public final Binding getTypeOrPackage(char[][] compoundName)
   {
      ...
      while (currentIndex < nameLength)
      {
         ...
         {
            qualifiedType = (qualifiedType != null && (qualifiedType.isRawType() || qualifiedType.isParameterizedType())) ? environment().createParameterizedType(
   ...
   protected boolean isAcceptableMethod(MethodBinding one, MethodBinding two)
   {
      ...
      if (one.isVarargs() && two.isVarargs())
      {
         ...
         if (parameterCompatibilityLevel(one, twoParams, true) == NOT_COMPATIBLE && parameterCompatibilityLevel(two,
            oneParams, true) == VARARGS_COMPATIBLE)
         {
            return true;
         }
      }
            ...
            for (int j = 0; j < superLength; j++)
            {
               ...
               else if (erasedSuperType.isArrayType())
               {
                  if (otherType.isArrayType() // keep Object[...] for an interface array (same dimensions)
                     && otherType.leafComponentType().id == TypeIds.T_JavaLangObject && otherType.dimensions() == erasedSuperType.dimensions() && erasedSuperType.leafComponentType().isInterface())
                  {
                     continue nextOtherType;
                  }
                  if (erasedSuperType.findSuperTypeOriginatingFrom(otherType) != null)
      ...
      for (int i = 0; i < visibleSize; i++)
      {
         ...
         if (current != null)
         {
            ...
            for (int j = 0; j < visibleSize; j++)
            {
               ...
               { // should not be null if original isAbstract, but be safe
   ...
   public void validateNullAnnotation(long tagBits, TypeReference typeRef, Annotation[] annotations)
   {
      ...
      if (nullAnnotationTagBit != 0)
      {
         ...
         {
            char[][] annotationName = (nullAnnotationTagBit == TagBits.AnnotationNonNull) ? environment().getNonNullAnnotationName() : environment().getNullableAnnotationName();
