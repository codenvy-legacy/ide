...
/* package */ class BinaryMethod extends BinaryMember implements IMethod {
    ...
    public boolean equals(Object o) {
        ...
        return super.equals(o) && Util.equalArraysOrNull(getErasedParameterTypes(),
                                                         ((BinaryMethod)o).getErasedParameterTypes());
    }
    ...
    public String[] getParameterNames() throws JavaModelException {
        ...
        if (paramCount != 0) {
            ...
            if (javadocContents == null) {
                ...
                try {
                    String option = getJavaProject().getOption(JavaCore.TIMEOUT_FOR_PARAMETER_NAME_FROM_ATTACHED_JAVADOC,
                                                               true);
                    if (option != null) {
    ...
    private char[][] splitParameters(char[] parametersSource, int paramCount) {
        ...
        while (index < length) {
            switch (parametersSource[index]) {
                ...
                case '&':
                    if ((index + 4) < length) {
                        if (parametersSource[index + 1] == 'l' && parametersSource[index + 2] == 't' &&
                            parametersSource[index + 3] == ';') {
                            balance++;
                            index += 4;
                        } else if (parametersSource[index + 1] == 'g' && parametersSource[index + 2] == 't' &&
                                   parametersSource[index + 3] == ';') {
                            balance--;
                            index += 4;
                        } else {
    ...
    /** @see com.codenvy.eclipse.jdt.internal.core.JavaElement#hashCode() */
    public int hashCode() {
    ...
    public JavaElement resolved(Binding binding) {
        SourceRefElement resolvedHandle = new ResolvedBinaryMethod(this.parent, this.name, this.parameterTypes,
