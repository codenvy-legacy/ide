...
public final class NamingConventions {
    static class NamingRequestor implements INamingRequestor {
        ...
        public void acceptNameWithPrefixAndSuffix(char[] name, boolean isFirstPrefix, boolean isFirstSuffix,
            ...
            if (isFirstPrefix && isFirstSuffix) {
                ...
                if (length == this.prefixAndSuffixResultsCount) {
                    System.arraycopy(this.prefixAndSuffixResults, 0, this.prefixAndSuffixResults = new char[length * 2][], 0,
                                     length);
                }
        ...
        public char[][] getResults() {
            int count = this.firstPrefixAndFirstSuffixResultsCount + this.firstPrefixAndSuffixResultsCount +
                        this.prefixAndFirstSuffixResultsCount + this.prefixAndSuffixResultsCount + this.firstPrefixResultsCount +
                        this.prefixResultsCount + this.firstSuffixResultsCount + this.suffixResultsCount + this.otherResultsCount;
    ...
    /**
     * Returns a base name which could be used to generate the given variable name with {@link #suggestVariableNames(int, int, String,
     * IJavaProject, int, String[], boolean)}.
     * <p>
     * e.g.<br>
     ...
     * If the variable is a {@link #VK_STATIC_FINAL_FIELD} and the variable name is <code>VARIABLE_NAME</code> then the base name will be
     * <code>variableName</code>.<br>
     * </p>
     * <p>
     ...
     * <li>{@link #VK_STATIC_FIELD}: {@link JavaCore#CODEASSIST_STATIC_FIELD_PREFIXES} and {@link
     * JavaCore#CODEASSIST_STATIC_FIELD_SUFFIXES}</li>
     * <li>{@link #VK_STATIC_FINAL_FIELD}: {@link JavaCore#CODEASSIST_STATIC_FINAL_FIELD_PREFIXES} and {@link
     * JavaCore#CODEASSIST_STATIC_FINAL_FIELD_SUFFIXES}</li>
     * </ul>
     * </p>
     * <p>
     * e.g.<br>
     * If the variable is a {@link #VK_LOCAL}, the variable name is <code>preVariableNamesuf</code>, a possible prefix is <code>pre</code>
     * and a possible suffix is <code>suf</code>
    ...
    public static String[] suggestArgumentNames(IJavaProject javaProject, String packageName, String qualifiedTypeName,
        ...
        return convertCharsToString(
                suggestArgumentNames(javaProject, packageName.toCharArray(), qualifiedTypeName.toCharArray(), dim,
    ...
    public static String[] suggestFieldNames(IJavaProject javaProject, String packageName, String qualifiedTypeName,
        ...
        return convertCharsToString(
                suggestFieldNames(javaProject, packageName.toCharArray(), qualifiedTypeName.toCharArray(), dim, modifiers,
                                  convertStringToChars(excludedNames)));
    }
    ...
    public static char[] suggestGetterName(IJavaProject project, char[] fieldName, int modifiers, boolean isBoolean,
        ...
        if (isBoolean) {
            ...
            if (CharOperation.prefixEquals(GETTER_BOOL_NAME, name) && name.length > prefixLen && ScannerHelper.isUpperCase(
                    name[prefixLen])) {
     ...
     * @see JavaCore#setOptions(java.util.Hashtable)
     * @see JavaCore#getDefaultOptions()
     * @deprecated Use {@link #suggestVariableNames(int, int, String, IJavaProject, int, String[], boolean)} instead with {@link #VK_LOCAL}
     *             as variable kind.
     */
    ...
    public static String[] suggestLocalVariableNames(IJavaProject javaProject, String packageName,
        ...
        return convertCharsToString(
                suggestLocalVariableNames(javaProject, packageName.toCharArray(), qualifiedTypeName.toCharArray(), dim,
                                          convertStringToChars(excludedNames)));
    }
     ...
     * <ul>
     * <li>{@link #BK_NAME}: the base name is a Java name and the whole base name is considered to compute the variable names. A prefix
     * and a
     * suffix can be added.<br>
     ...
     * </ul></li>
     * <li>{@link #BK_TYPE_NAME}: the base name is a Java simple type name (e.g. <code>HashMap</code>) and all the words of the base name
     * are
     * considered to compute the variable names. A prefix and a suffix can be added to these names.<br>
     * There is a heuristic by variable kind.
     * <ul>
     * <li>{@link #VK_PARAMETER}, {@link #VK_LOCAL}, {@link #VK_INSTANCE_FIELD} and {@link #VK_STATIC_FIELD}:<br>
     ...
     * <li>{@link #VK_STATIC_FINAL_FIELD} :<br>
     * In this case a variable name will contain some words of the base name, all letters of the name will be converted to upper case and
     * segments will be separated by a underscore (<code>"_"</code>).<br>
     ...
     * To programmatically change these options, see {@link JavaCore#setOptions(java.util.Hashtable)} and {@link
     * IJavaProject#setOptions(java.util.Map)}
     * </p>
     * <p>
     ...
     *         if <code>true</code>, the result is guaranteed to contain at least one result. If <code>false</code>, the result can be an
     *         empty array.
