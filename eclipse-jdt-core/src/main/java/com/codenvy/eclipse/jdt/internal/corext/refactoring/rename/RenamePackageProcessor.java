...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.changes.RenamePackageChange;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.rename.RenamePackageProcessor.ImportsManager.ImportChange;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.util.CommentAnalyzer;
...
public class RenamePackageProcessor extends JavaRenameProcessor
        implements IReferenceUpdating, ITextUpdating, IQualifiedNameUpdating, IResourceMapper, IJavaElementMapper {
     ...
     *
     * @param fragment
     *         the package fragment, or <code>null</code> if invoked by scripting
     */
        ...
        try {
            ...
            if (fRenameSubpackages) {
                ...
                for (int i = 0; i < allSubpackages.length; i++) {
                    new PackageRenamer(allSubpackages[i], this, fChangeManager, fImportsManager).doRename(
            ...
            if (fUpdateTextualMatches) {
                ...
                TextMatchUpdater.perform(new SubProgressMonitor(pm, 10), RefactoringScopeFactory.create(fPackage), this,
                                         fChangeManager, new SearchResultGroup[0]);
            }
        ...
        if (fRenameSubpackages) {
            ...
            for (int i = 0; i < packsToRename.length; i++) {
                ...
                if (!isAncestorPackage(currentName, newPack) && !isPackageNameOkInRoot(newPack, getPackageFragmentRoot())) {
                    String msg = Messages.format(RefactoringCoreMessages.RenamePackageProcessor_subpackage_collides,
    ...
    private RefactoringStatus checkTypeNameConflicts(IPackageFragmentRoot root, String newName,
                                                     Set<String> topLevelTypeNames) throws CoreException {
    ...
    private RefactoringStatus checkTypeNameConflicts(ICompilationUnit iCompilationUnit,
                                                     Set<String> topLevelTypeNames) throws CoreException {
        RefactoringStatus result = new RefactoringStatus();
        IType[] types = iCompilationUnit.getTypes();
        ...
        try {
            ...
            final DynamicValidationRefactoringChange result = new DynamicValidationRefactoringChange(descriptor,
                                                                                                     RefactoringCoreMessages
                                                                                                             ...
                                                                                                             .RenamePackageRefactoring_change_name);
    ...
    private RenameJavaElementDescriptor createRefactoringDescriptor() {
        ...
        final int flags = JavaRefactoringDescriptor.JAR_MIGRATION | JavaRefactoringDescriptor.JAR_REFACTORING |
                          RefactoringDescriptor.STRUCTURAL_CHANGE | RefactoringDescriptor.MULTI_CHANGE;
        final String description = Messages.format(
                RefactoringCoreMessages.RenamePackageProcessor_descriptor_description_short, getElementLabel(fPackage));
    ...
    private static class PackageRenamer {
        ...
        public PackageRenamer(IPackageFragment pack, RenamePackageProcessor processor,
                              TextChangeManager textChangeManager, ImportsManager importsManager) {
            fPackage = pack;
        ...
        void doRename(IProgressMonitor pm, RefactoringStatus result) throws CoreException {
            ...
            if (fProcessor.getUpdateReferences()) {
                ...
                fReferencesToTypesInPackage = getReferencesToTypesInPackage(new SubProgressMonitor(pm, 4), binaryRefs,
                                                                            result);
        ...
        private SearchResultGroup[] getReferences(IProgressMonitor pm, ReferencesInBinaryContext binaryRefs,
        ...
        private void addReferenceUpdates(IProgressMonitor pm) throws CoreException {
            pm.beginTask("",
                         fOccurrences.length + fReferencesToTypesInPackage.size() + fReferencesToTypesInNamesakes.size()); //$NON-NLS-1$
            for (int i = 0; i < fOccurrences.length; i++) {
                ...
                for (int j = 0; j < results.length; j++) {
                    ...
                    if (enclosingElement instanceof IImportDeclaration) {
                        ...
                        TextChangeCompatibility.addTextEdit(fTextChangeManager.get(cu),
         ...
         * @param cu
         *         the cu
         * @param searchResultGroups
         *         List of SearchResultGroup
        ...
        private List<SearchResultGroup> getReferencesToTypesInNamesakes(IProgressMonitor pm,
                                                                        RefactoringStatus status) throws CoreException {
            ...
            IPackageFragment[] namesakePackages = getNamesakePackages(namesakePackagesScope,
            ...
            SearchPattern pattern = RefactoringSearchEngine.createOrPattern(typesToSearch,
        ...
        private List<SearchResultGroup> getReferencesToTypesInPackage(IProgressMonitor pm,
                                                                      ReferencesInBinaryContext binaryRefs, RefactoringStatus status)
                throws CoreException {
            pm.beginTask("", 2); //$NON-NLS-1$
            IJavaSearchScope referencedFromNamesakesScope = RefactoringScopeFactory.create(fPackage, true, false);
            IPackageFragment[] namesakePackages = getNamesakePackages(referencedFromNamesakesScope,
                                                                      new SubProgressMonitor(pm, 1));
            if (namesakePackages.length == 0) {
                pm.done();
                return new ArrayList<SearchResultGroup>(0);
            ...
            SearchPattern pattern = RefactoringSearchEngine.createOrPattern(typesToSearch,
                                                                            IJavaSearchConstants.REFERENCES);
            CollectingSearchRequestor requestor = new CuCollectingSearchRequestor(binaryRefs);
            SearchResultGroup[] results = RefactoringSearchEngine.search(pattern, scope, requestor,
        ...
        private IPackageFragment[] getNamesakePackages(IJavaSearchScope scope, IProgressMonitor pm) throws CoreException {
            SearchPattern pattern = SearchPattern.createPattern(fPackage.getElementName(), IJavaSearchConstants.PACKAGE,
        ...
        private void updateImport(ICompilationUnit cu, IImportDeclaration importDeclaration,
                                  String updatedImport) throws JavaModelException {
        ...
        private void updateTypeImports(SearchResultGroup typeReferences) throws CoreException {
            ...
            for (int i = 0; i < searchResults.length; i++) {
                ...
                if (enclosingElement instanceof IImportDeclaration) {
                    IImportDeclaration importDeclaration = (IImportDeclaration)enclosingElement;
                    updateImport(typeReferences.getCompilationUnit(), importDeclaration,
