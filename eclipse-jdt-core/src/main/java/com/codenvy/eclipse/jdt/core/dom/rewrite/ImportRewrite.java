...
package com.codenvy.eclipse.jdt.core.dom.rewrite;
...
import com.codenvy.eclipse.jdt.internal.core.dom.rewrite.ImportRewriteAnalyzer;
 ...
 * <p>
 * The import rewrite is created on a compilation unit and collects references to types that are added or removed. When adding imports,
 * e.g.
 * using
 * {@link #addImport(String)}, the import rewrite evaluates if the type can be imported and returns the a reference to the type that can be
 * used in code.
 * This reference is either unqualified if the import could be added, or fully qualified if the import failed due to a conflict with
 * another
 ...
 * works on the import statements. It is possible to combine the result of an import rewrite with the result of a {@link
 * com.codenvy.eclipse.jdt.core.dom.rewrite.ASTRewrite}
 ...
 * <li>{@link #setOnDemandImportThreshold(int)} specifies the number of imports in a group needed for a on-demand import statement (star
 * import)</li>
 * <li>{@link #setStaticOnDemandImportThreshold(int)} specifies the number of static imports in a group needed for a on-demand import
 * statement (star import)</li>
 * </ul>
...
public final class ImportRewrite {
    ...
    /**
     * A {@link ImportRewrite.ImportRewriteContext} can optionally be used in e.g. {@link ImportRewrite#addImport(String,
     * ImportRewrite.ImportRewriteContext)} to
    ...
    final int findInImports(String qualifier, String name, int kind) {
        boolean allowAmbiguity = (kind == ImportRewriteContext.KIND_STATIC_METHOD) || (name.length() == 1 && name.charAt(
                0) == '*');
        ...
        if (this.filterImplicitImports && this.useContextToFilterImplicitImports) {
            ...
            if (kind == ImportRewriteContext.KIND_TYPE && (qualifier.equals(fPackageName) || fMainTypeName.equals(
                    Util.concatenateName(qualifier, name, '.')))) {
                return ImportRewriteContext.RES_NAME_FOUND;
            }
        }
        return ImportRewriteContext.RES_NAME_UNKNOWN;
     ...
     * <p>
     * No imports are added for types that are already known. If a import for a type is recorded to be removed,
     * this record is discarded instead.
     * </p>
     * <p>
    ...
    public String addImport(String qualifiedTypeName, ImportRewriteContext context) {
        ...
        if (angleBracketOffset != -1) {
            return internalAddImport(qualifiedTypeName.substring(0, angleBracketOffset),
                                     context) + qualifiedTypeName.substring(angleBracketOffset);
        }
        int bracketOffset = qualifiedTypeName.indexOf('[');
        if (bracketOffset != -1) {
            return internalAddImport(qualifiedTypeName.substring(0, bracketOffset), context) + qualifiedTypeName.substring(
                    bracketOffset);
        }
     ...
     * existing.
     * <p>
     * No imports are added for members that are already known. If a import for a type is recorded to be removed,
     * this record is discarded instead.
     * </p>
     * <p>
     * The content of the compilation unit itself is actually not modified
    ...
    public final TextEdit rewriteImports(IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            ImportRewriteAnalyzer computer = new ImportRewriteAnalyzer(this.compilationUnit, usedAstRoot, this.importOrder,
                                                                       this.importOnDemandThreshold, this.staticImportOnDemandThreshold,
