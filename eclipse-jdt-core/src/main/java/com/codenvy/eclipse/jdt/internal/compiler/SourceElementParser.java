         ...
         problemFactory)
      {
            ...
            boolean mandatoryError)
         {
            unitResult.record(problem, context,
               mandatoryError); // TODO (jerome) clients are trapping problems either through factory or requestor... is result storing needed?
   ...
   protected void classInstanceCreation(boolean alwaysQualified)
   {
      ...
      if (this.reportReferenceInfo)
      {
         ...
         this.requestor.acceptConstructorReference(
            typeRef instanceof SingleTypeReference ? ((SingleTypeReference)typeRef).token : CharOperation.concatWith(
               alloc.type.getParameterizedTypeName(), '.'), alloc.arguments == null ? 0 : alloc.arguments.length,
            alloc.sourceStart);
      }
   }
	 ...
	 */
      ...
      else
      {
            ...
            if (this.reportReferenceInfo)
            {
               ...
               else
               {
                  ...
                  this.requestor.acceptTypeReference(parameterizedQualifiedTypeReference.tokens,
                     parameterizedQualifiedTypeReference.sourceStart, parameterizedQualifiedTypeReference.sourceEnd);
               }
            }
            return ref;
         }
         else if (length == 1)
         {
            ...
            if (dim == 0)
            {
               SingleTypeReference ref = new SingleTypeReference(this.identifierStack[this.identifierPtr],
            ...
            {
               ArrayTypeReference ref = new ArrayTypeReference(this.identifierStack[this.identifierPtr], dim,
                  this.identifierPositionStack[this.identifierPtr--]);
               ref.sourceEnd = this.endPosition;
               if (this.reportReferenceInfo)
               {
