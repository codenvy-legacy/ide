...
public class BatchImageBuilder extends AbstractImageBuilder {
            ...
            for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
                ...
                if (sourceLocation.hasIndependentOutputFolder) {
                    ...
                    final char[][] exclusionPatterns = isOutputFolder ? sourceLocation.exclusionPatterns
                                                                      : null; // ignore exclusionPatterns if output folder == another
                                                                      // source folder... not this one
                    final char[][] inclusionPatterns = isOutputFolder ? sourceLocation.inclusionPatterns
                                                                      : null; // ignore inclusionPatterns if output folder == another
                                                                      // source folder... not this one
                    sourceLocation.binaryFolder.accept(new IResourceProxyVisitor() {
                        public boolean visit(IResourceProxy proxy) throws CoreException {
                            if (proxy.getType() == IResource.FILE) {
                                if (com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(proxy.getName())) {
        ...
        sourceLocation.sourceFolder.accept(new IResourceProxyVisitor() {
            public boolean visit(IResourceProxy proxy) throws CoreException {
                ...
                switch (proxy.getType()) {
                    case IResource.FILE:
                        if (com.codenvy.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(
                                proxy.getName()) || com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(proxy.getName())) {
                            return false;
                        }
    ...
    private void printStats() {
        ...
        System.out.println(
                ">   compiled " + lineCount + " lines in " + time + "ms:" + speed +
                "lines/s"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        System.out.print(
                ">   parse: " + compilerStats.parseTime + " ms (" + ((int)(compilerStats.parseTime * 1000.0 / time)) / 10.0 +
                "%)"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.print(
                ", resolve: " + compilerStats.resolveTime + " ms (" + ((int)(compilerStats.resolveTime * 1000.0 / time)) / 10.0 +
                "%)"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.print(
                ", analyze: " + compilerStats.analyzeTime + " ms (" + ((int)(compilerStats.analyzeTime * 1000.0 / time)) / 10.0 +
                "%)"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.println(
                ", generate: " + compilerStats.generateTime + " ms (" + ((int)(compilerStats.generateTime * 1000.0 / time)) / 10.0 +
                "%)"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
