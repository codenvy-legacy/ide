...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.base.RefactoringStatusCodes;
...
import com.codenvy.eclipse.jdt.internal.ui.text.correction.ASTResolving;
...
public class Checks
{
   ...
   public static RefactoringStatus checkIfConstructorName(IMethod method, String newMethodName, String newTypeName)
   {
      ...
      else
      {
         return RefactoringStatus.createWarningStatus(Messages.format(RefactoringCoreMessages.Checks_constructor_name,
            new Object[]{JavaElementUtil.createMethodSignature(method), JavaElementLabels.getElementLabel(
    ...
    * @param context an {@link com.codenvy.eclipse.jdt.core.IJavaElement} or <code>null</code>
   ...
   private static RefactoringStatus checkForMainAndNativeMethods(IMethod[] methods) throws JavaModelException
   {
      ...
      for (int i = 0; i < methods.length; i++)
      {
         ...
         {
            String typeName = JavaElementLabels.getElementLabel(methods[i].getDeclaringType(),
            ...
            result.addEntry(RefactoringStatus.ERROR, msg, JavaStatusContext.create(methods[i]), "IDE",
         ...
         if (methods[i].isMainMethod())
         {
            ...
            result.addEntry(RefactoringStatus.WARNING, msg, JavaStatusContext.create(methods[i]), "IDE",
   ...
   public static RefactoringStatus checkMethodInType(ITypeBinding type, String methodName, ITypeBinding[] parameters)
   {
      ...
      IMethodBinding method = com.codenvy.eclipse.jdt.internal.corext.dom.Bindings.findMethodInType(type, methodName,
         parameters);
      if (method != null)
      {
         result.addError(Messages.format(RefactoringCoreMessages.Checks_methodName_exists,
            new Object[]{BasicElementLabels.getJavaElementName(methodName), BasicElementLabels.getJavaElementName(
               type.getName())}), JavaStatusContext.create(method));
      }
      return result;
   }
    ...
    * @return the status
    */
   public static RefactoringStatus checkMethodInHierarchy(ITypeBinding type, String methodName, ITypeBinding returnType,
      ...
      if (method != null)
      {
         ...
         if (returnTypeClash)
         {
            result.addError(Messages.format(RefactoringCoreMessages.Checks_methodName_returnTypeClash,
               new Object[]{BasicElementLabels.getJavaElementName(methodName), BasicElementLabels.getJavaElementName(
                  dc.getName())}), JavaStatusContext.create(method));
         }
         else
         {
    ...
    *
    */
   public static IMethod findMethod(String name, int parameterCount, boolean isConstructor,
   ...
   public static IMethod findMethod(String name, int parameters, boolean isConstructor,
    ...
    *
    */
   public static SearchResultGroup[] excludeCompilationUnits(SearchResultGroup[] grouped,
   ...
   public static RefactoringStatus checkCompileErrorsInAffectedFiles(
      SearchResultGroup[] grouped) throws JavaModelException
   {
      RefactoringStatus result = new RefactoringStatus();
      for (int i = 0; i < grouped.length; i++)
      {
   ...
   public static void checkCompileErrorsInAffectedFile(RefactoringStatus result,
   ...
   private static boolean hasCompileErrors(IResource resource) throws JavaModelException
   {
      try
      {
         IMarker[] problemMarkers = resource.findMarkers(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER, true,
    ...
    * @throws com.codenvy.eclipse.jdt.core.JavaModelException
    *
    * @see com.codenvy.eclipse.jdt.core.IJavaElement#exists
    * @see com.codenvy.eclipse.jdt.core.IJavaElement#isReadOnly
    * @see com.codenvy.eclipse.jdt.core.IMember#isBinary
   ...
   public static RefactoringStatus checkAvailability(IJavaElement javaElement) throws JavaModelException
   {
      ...
      {
         result.addFatalError(
            Messages.format(RefactoringCoreMessages.Refactoring_unknown_structure, getJavaElementName(javaElement)));
      }
