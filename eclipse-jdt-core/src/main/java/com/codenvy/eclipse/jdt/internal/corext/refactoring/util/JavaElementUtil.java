...
public class JavaElementUtil {
    ...
    public static String createMethodSignature(IMethod method) {
        try {
            return BasicElementLabels.getJavaElementName(
                    Signature.toString(method.getSignature(), method.getElementName(), method.getParameterNames(), false,
    ...
    public static String createFieldSignature(IField field) {
        return BasicElementLabels.getJavaElementName(
                field.getDeclaringType().getFullyQualifiedName('.') + "." + field.getElementName()); //$NON-NLS-1$
    }
     ...
     *         if getting the raw classpath or all Java projects fails
     */
     ...
     * @throws com.codenvy.eclipse.jdt.core.JavaModelException
     *         if getting the all sibling packages fails
     */
