...
public class EvaluationContext implements EvaluationConstants, SuffixConstants
{
    ...
    * @param requestor   since the engine might produce answers of various forms, the engine
      ...
      IProgressMonitor monitor)
   {
      ...
      final CodeSnippetToCuMapper mapper = new CodeSnippetToCuMapper(codeSnippet, this.packageName, this.imports,
         className, this.installedVars == null ? null : this.installedVars.className, this.localVariableNames,
         this.localVariableTypeNames, this.localVariableModifiers, this.declaringTypeName, this.lineSeparator,
         complianceVersion);
            ...
            if (pkgName.length == 1 && pkgName[0] == '*')
            {
               ...
               if (!environment.isPackage(parentName, pkgName))
               {
                  ...
                  problems[0] = problemFactory.createProblem(importDeclaration, IProblem.ImportNotFound, arguments,
                     arguments, ProblemSeverities.Warning, 0, importDeclaration.length - 1, i, 0);
               }
            }
            else
            {
