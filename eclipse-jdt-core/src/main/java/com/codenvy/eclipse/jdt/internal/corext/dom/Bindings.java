...
import com.codenvy.eclipse.jdt.core.dom.IAnnotationBinding;
import com.codenvy.eclipse.jdt.core.dom.IBinding;
import com.codenvy.eclipse.jdt.core.dom.IMemberValuePairBinding;
import com.codenvy.eclipse.jdt.core.dom.IMethodBinding;
import com.codenvy.eclipse.jdt.core.dom.IPackageBinding;
...
public class Bindings {
     ...
     * @see com.codenvy.eclipse.jdt.internal.ui.viewsupport.BindingLabelProvider
     */
     ...
     * @param parameters
     *         The parameter types of the method to find. If <code>null</code> is passed, only
     ...
     *         The name of the method to find
     * @param parameters
     *         The parameter types of the method to find. If <code>null</code> is passed, only the name is matched and parameters are
     *         ignored.
     * @return the method binding representing the method
     */
    ...
    public static boolean isVisibleInHierarchy(IMethodBinding member, IPackageBinding pack) {
        ...
        if (Modifier.isPublic(otherflags) || Modifier.isProtected(
                otherflags) || (declaringType != null && declaringType.isInterface())) {
            return true;
     ...
     *
     * @param typeBinding
     *         the type binding to search for
     * @param project
     *         the project used as a scope
     * @return the compilation unit containing the type
     * @throws JavaModelException
     *         if an errors occurs in the Java model
     */
    public static ICompilationUnit findCompilationUnit(ITypeBinding typeBinding,
     ...
     * declaration signature, but <em>not</em> as type of a declaration (use {@link #normalizeForDeclarationUse(ITypeBinding, AST)} for
     * that).
     * <p>
     ...
     *             are in the same hierarchy (directly overrides each other), or {@link #findMethodInHierarchy(ITypeBinding, String,
     *             ITypeBinding[])}
     *             else.
     */
    ...
    private static boolean areSubTypeCompatible(IMethodBinding overridden, IMethodBinding overridable) {
        ...
        for (int index = 0; index < overriddenTypes.length; index++) {
            ...
            if (!overridableErasure.isSubTypeCompatible(overriddenErasure) || !overridableErasure.getKey().equals(
                    overriddenErasure.getKey())) {
                return false;
            }
        }
