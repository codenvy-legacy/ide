 ...
 * </p>
 *
 * @see JavaCore#create(com.codenvy.eclipse.core.resources.IProject)
 * @see IClasspathEntry
 */
public interface IJavaProject extends IParent, IJavaElement, IOpenable {
    ...
    /**
     * Helper method for returning one option value only. Equivalent to <code>(String)this.getOptions(inheritJavaCoreOptions).get
     * (optionName)</code>
     ...
     * @see #setOutputLocation(com.codenvy.eclipse.core.runtime.IPath, IProgressMonitor)
     ...
     * @since 3.0
     */
    ITypeHierarchy newTypeHierarchy(IType type, IRegion region, WorkingCopyOwner owner,
                                    IProgressMonitor monitor) throws JavaModelException;
     ...
     *         the name of an option
     * @param optionValue
     *         the value of the option to set. If <code>null</code>, then the option
     *         is removed from project preferences.
     * @throws NullPointerException
     *         if <code>optionName</code> is <code>null</code>
     ...
     * @param canModifyResources
     *         whether resources should be written to disk if needed
     * @param monitor
     *         the given progress monitor
     * @throws JavaModelException
     *         if the classpath could not be set. Reasons include:
     ...
     *         <li> The classpath failed the validation check as defined by {@link JavaConventions#validateClasspath(IJavaProject,
     *         IClasspathEntry[], IPath)}
     *         </ul>
     * @see IClasspathEntry
     * @since 3.2
     */
    void setRawClasspath(IClasspathEntry[] entries, IPath outputLocation, boolean canModifyResources,
                         IProgressMonitor monitor) throws JavaModelException;
     ...
     * @since 3.2
     */
    void setRawClasspath(IClasspathEntry[] entries, boolean canModifyResources,
                         IProgressMonitor monitor) throws JavaModelException;
     ...
     * @param entries
     *         a list of classpath entries
     * @param referencedEntries
     *         the list of referenced classpath entries to be persisted
     * @param outputLocation
     *         the default output location
     * @param monitor
     *         the given progress monitor
