...
public class SetContainerOperation extends ChangeClasspathOperation
{
      ...
      try
      {
         ...
         for (int i = 0; i < projectLength; i++)
         {
            ...
            if (JavaProject.hasJavaNature(affectedProject.getProject()))
            {
               ...
               for (int j = 0, cpLength = rawClasspath.length; j < cpLength; j++)
               {
                  ...
                  if (entry.getEntryKind() == IClasspathEntry.CPE_CONTAINER && entry.getPath().equals(
                     this.containerPath))
                  {
                     found = true;
                     break;
            ...
            if ((oldContainer != null && oldContainer.equals(
               this.respectiveContainers[i])) || (oldContainer == this.respectiveContainers[i])/*handle case where old and new containers are null (see bug 149043*/)
            {
         ...
         finally
         {
            for (int i = 0; i < projectLength; i++)
            {
               ...
               {
                  manager.containerPut(this.affectedProjects[i], this.containerPath,
   ...
   private void verbose_set_container()
   {
      Util.verbose("CPContainer SET  - setting container\n" + //$NON-NLS-1$
         ...
         "	projects: {" +//$NON-NLS-1$
         com.codenvy.eclipse.jdt.internal.compiler.util.Util.toString(this.affectedProjects,
         ...
         "}\n	values: {\n" +//$NON-NLS-1$
         com.codenvy.eclipse.jdt.internal.compiler.util.Util.toString(this.respectiveContainers,
            new com.codenvy.eclipse.jdt.internal.compiler.util.Util.Displayable()
            {
               public String displayString(Object o)
               {
