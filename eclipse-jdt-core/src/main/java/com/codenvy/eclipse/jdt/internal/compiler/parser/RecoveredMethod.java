...
public class RecoveredMethod extends RecoveredElement implements TerminalTokens {
     ...
     */
    public RecoveredElement add(Block nestedBlockDeclaration, int bracketBalanceValue) {   /* default behavior is to delegate recording
    to parent if any,
...
*/
                    ...
                    for (count = 0; count < argLength; count++) {
						...
						/* cannot be an argument if non final */
                            ...
                            if ((argument.modifiers & ~ClassFileConstants.AccFinal) != 0 ||
                                (argTypeName.length == 1 && CharOperation.equals(
                                        argTypeName[0], TypeBinding.VOID.sourceName()))) {
    ...
    public RecoveredElement addAnnotationName(int identifierPtr, int identifierLengthPtr, int annotationStart,
        ...
        RecoveredAnnotation element = new RecoveredAnnotation(identifierPtr, identifierLengthPtr, annotationStart, this,
                                                              bracketBalanceValue);
    ...
    void attach(TypeParameter[] parameters, int startPos) {
        ...
        if (Util.getLineNumber(this.methodDeclaration.declarationSourceStart, scanner.lineEnds, 0,
                               scanner.linePtr) != Util.getLineNumber(lastParameterEnd, scanner.lineEnds, 0, scanner.linePtr)) {
            return;
        }
