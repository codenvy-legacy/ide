...
public abstract class AbstractExceptionAnalyzer extends ASTVisitor
{
   ...
   public boolean visit(VariableDeclarationExpression node)
   {
      if (node.getAST().apiLevel() >= AST.JLS4 && node.getLocationInParent() == TryStatement.RESOURCES_PROPERTY)
      {
         ...
         {
            IMethodBinding methodBinding = Bindings.findMethodInHierarchy(resourceTypeBinding, "close",
               new ITypeBinding[0]); //$NON-NLS-1$
   ...
   private void removeCaughtExceptions(ITypeBinding catchTypeBinding)
   {
      ...
      for (Iterator<ITypeBinding> exceptions = new ArrayList<ITypeBinding>(
         fCurrentExceptions).iterator(); exceptions.hasNext(); )
      {
