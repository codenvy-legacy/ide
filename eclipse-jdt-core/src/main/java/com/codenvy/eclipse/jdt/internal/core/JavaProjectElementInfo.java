...
import com.codenvy.eclipse.jdt.internal.core.util.HashSetOfArray;
...
class JavaProjectElementInfo extends OpenableElementInfo
{
            ...
            for (int i = 0; i < length; i++)
            {
               ...
               switch (res.getType())
               {
                  case IResource.FILE:
                     ...
                     if (srcIsProject && Util.isValidCompilationUnitName(resName, sourceLevel,
                        complianceLevel) && !Util.isExcluded(res, inclusionPatterns, exclusionPatterns))
                     {
                        break;
                     }
                  ...
                  case IResource.FOLDER:
                     ...
                     if ((srcIsProject && !Util.isExcluded(res, inclusionPatterns,
                        exclusionPatterns) && Util.isValidFolderNameForPackage(res.getName(), sourceLevel,
                        complianceLevel)) || (isClasspathResolved && isClasspathEntryOrOutputLocation(resFullPath,
    ...
    */
   private boolean isClasspathEntryOrOutputLocation(IPath path, IPath location, IClasspathEntry[] resolvedClasspath,
      IPath projectOutput)
   {
