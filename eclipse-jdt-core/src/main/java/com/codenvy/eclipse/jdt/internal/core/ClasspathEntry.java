...
public class ClasspathEntry implements IClasspathEntry
{
      ...
      boolean combineAccessRules, IClasspathAttribute[] extraAttributes)
   {
      ...
      if (accessRules != null && (length = accessRules.length) > 0)
      {
         ...
         if (this.entryKind == CPE_PROJECT || this.entryKind == CPE_SOURCE)
      ...
      else
      {
         ...
         while (calledFilesIterator.hasNext())
         {
            ...
            if (!directoryPath.isValidPath(calledFileName))
            {
               ...
               {
                  Util.verbose(
                     "Invalid Class-Path entry " + calledFileName + " in manifest of jar file: " + jarPath.toOSString()); //$NON-NLS-1$ //$NON-NLS-2$
               }
            }
            else
            {
   ...
   public ClasspathEntry resolvedDotDot(IPath reference)
   {
      ...
      return new ClasspathEntry(getContentKind(), getEntryKind(), resolvedPath, this.inclusionPatterns,
         this.exclusionPatterns, getSourceAttachmentPath(), getSourceAttachmentRootPath(), getOutputLocation(),
         this.getReferencingEntry(), this.isExported, getAccessRules(), this.combineAccessRules, this.extraAttributes);
   }
      ...
      for (int i = 0; i < length; i++)
      {
            ...
            entryPath, false/*don't check existence*/) instanceof IContainer)))
         {
            for (int j = 0; j < classpath.length; j++)
            {
               ...
               if (entry != otherEntry && (otherKind == IClasspathEntry.CPE_SOURCE || (otherKind == IClasspathEntry.CPE_LIBRARY && (JavaModel.getTarget(
                           ...
                           if (!disableExclusionPatterns)
                           {
                              return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                 Messages.bind(Messages.classpath_cannotNestEntryInEntry,
                                    new String[]{entryPath.makeRelative().toString(), otherEntry.getPath().makeRelative().toString(), exclusionPattern}));
                           }
                           else
                           {
                              return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                        ...
                        else
                        {
                           return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                              Messages.bind(Messages.classpath_cannotNestEntryInLibrary,
                                 new String[]{entryPath.makeRelative().toString(), otherEntry.getPath().makeRelative().toString()}));
                        }
                     }
                  }
               }
      ...
      IClasspathContainer entryContainer, boolean checkSourceAttachment, boolean referredByContainer)
   {
      ...
      switch (entry.getEntryKind())
      {
         ...
         case IClasspathEntry.CPE_CONTAINER:
            ...
            {
               return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                  Messages.bind(Messages.classpath_illegalContainerPath, new String[]{entryPathMsg, projectName}));
            }
            break;
         ...
         case IClasspathEntry.CPE_VARIABLE:
            ...
            {
               return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                  Messages.bind(Messages.classpath_illegalVariablePath, new String[]{entryPathMsg, projectName}));
            }
         ...
         case IClasspathEntry.CPE_PROJECT:
            if (path.isAbsolute() && path.segmentCount() == 1)
            {
               ...
               try
               {
                  ...
                  {
                     return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                        Messages.bind(Messages.classpath_unboundProject, new String[]{path.segment(0), projectName}));
                  }
                        ...
                        return new JavaModelStatus(IJavaModelStatusConstants.INCOMPATIBLE_JDK_LEVEL, project, path,
                           Messages.bind(Messages.classpath_incompatibleLibraryJDKLevel,
                              new String[]{project.getElementName(), CompilerOptions.versionFromJdkLevel(
                                 projectTargetJDK), path.makeRelative().toString(), CompilerOptions.versionFromJdkLevel(
            ...
            {
               return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                  Messages.bind(Messages.classpath_illegalProjectPath, new String[]{path.toString(), projectName}));
            }
            break;

         // project source folder
         case IClasspathEntry.CPE_SOURCE:
            if (((entry.getInclusionPatterns() != null && entry.getInclusionPatterns().length > 0) || (entry.getExclusionPatterns() != null && entry.getExclusionPatterns().length > 0)) && JavaCore.DISABLED.equals(
            ...
            if (path.isAbsolute() && !path.isEmpty())
            {
               ...
               {
                  return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                     Messages.bind(Messages.classpath_unboundSourceFolder, new String[]{entryPathMsg, projectName}));
               }
            }
            else
            {
               return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                  Messages.bind(Messages.classpath_illegalSourceFolderPath, new String[]{entryPathMsg, projectName}));
            }
            break;
      }
      ...
      IPath sourceAttachment, String entryPathMsg)
   {
               ...
               if (container != null)
               {
                  return new JavaModelStatus(IJavaModelStatusConstants.INCOMPATIBLE_JDK_LEVEL, project, path,
                     Messages.bind(Messages.classpath_incompatibleLibraryJDKLevelInContainer,
                        new String[]{project.getElementName(), CompilerOptions.versionFromJdkLevel(
                           projectTargetJDK), path.makeRelative().toString(), container, CompilerOptions.versionFromJdkLevel(
                           libraryJDK)}));
               }
               else
               {
      ...
      else
      {
         ...
         {
            return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
               Messages.bind(Messages.classpath_illegalLibraryPathInContainer, new String[]{entryPathMsg, container}));
         }
         else
         {
            return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
