...
import com.codenvy.eclipse.jdt.internal.compiler.util.ManifestAnalyzer;
...
public class ClasspathEntry implements IClasspathEntry {
    ...
    public void elementEncode(XMLWriter writer, IPath projectPath, boolean indent, boolean newLine, Map unknownElements,
        ...
        // unknown attributes
        UnknownXmlElements unknownXmlElements = unknownElements == null ? null : (UnknownXmlElements)unknownElements.get(
                this.path);
    ...
    public static IClasspathEntry elementDecode(Element element, IJavaProject project, Map unknownElements) {
        ...
        IPath sourceAttachmentPath = element.hasAttribute(TAG_SOURCEPATH) ? new Path(
                removeAttribute(TAG_SOURCEPATH, attributes)) : null;
        if (kind != IClasspathEntry.CPE_VARIABLE && sourceAttachmentPath != null && !sourceAttachmentPath.isAbsolute()) {
            sourceAttachmentPath = projectPath.append(sourceAttachmentPath);
        }
        IPath sourceAttachmentRootPath = element.hasAttribute(TAG_ROOTPATH) ? new Path(
                removeAttribute(TAG_ROOTPATH, attributes)) : null;
        ...
        boolean combineAccessRestrictions = !removeAttribute(TAG_COMBINE_ACCESS_RULES, attributes).equals(
                "false"); //$NON-NLS-1$
        ...
        IPath outputLocation = element.hasAttribute(TAG_OUTPUT) ? projectPath.append(
                removeAttribute(TAG_OUTPUT, attributes)) : null;
            ...
            while (calledFilesIterator.hasNext()) {
                ...
                if (!directoryPath.isValidPath(calledFileName)) {
                    if (JavaModelManager.CP_RESOLVE_VERBOSE_FAILURE) {
                        Util.verbose(
                                "Invalid Class-Path entry " + calledFileName + " in manifest of jar file: " +
    ...
    private static List getCalledFileNames(IPath jarPath) {
        ...
        try {
            ...
            if (!success || analyzer.getClasspathSectionsCount() == 1 && calledFileNames == null) {
                ...
                if (JavaModelManager.CP_RESOLVE_VERBOSE_FAILURE) {
                    Util.verbose(
                            "Multiple Class-Path headers in manifest of jar file: " + jarPath.toOSString()); //$NON-NLS-1$
                }
                return null;
            }
            ...
            if (JavaModelManager.CP_RESOLVE_VERBOSE_FAILURE) {
                Util.verbose(
                        "Could not read Class-Path header in manifest of jar file: " + jarPath.toOSString()); //$NON-NLS-1$
                e.printStackTrace();
            }
        } catch (IOException e) {
            // not a zip file
    ...
    public boolean isOptional() {
        for (int i = 0, length = this.extraAttributes.length; i < length; i++) {
            IClasspathAttribute attribute = this.extraAttributes[i];
            if (IClasspathAttribute.OPTIONAL.equals(attribute.getName()) && "true".equals(
    ...
    public static IJavaModelStatus validateClasspath(IJavaProject javaProject, IClasspathEntry[] rawClasspath,
        ...
        for (int i = 0; i < rawLength; i++) {
            ...
            if (!pathes.add(entryPath)) {
                String entryPathMsg = projectName.equals(entryPath.segment(0)) ? entryPath.removeFirstSegments(
                        1).toString() : entryPath.makeRelative().toString();
        ...
        try {
            ...
            classpath = ((JavaProject)javaProject).resolveClasspath(rawClasspath, false/*don't use previous session*/,
        ...
        boolean disableExclusionPatterns = JavaCore.DISABLED.equals(
                javaProject.getOption(JavaCore.CORE_ENABLE_CLASSPATH_EXCLUSION_PATTERNS, true));
        boolean disableCustomOutputLocations = JavaCore.DISABLED.equals(
                javaProject.getOption(JavaCore.CORE_ENABLE_CLASSPATH_MULTIPLE_OUTPUT_LOCATIONS, true));
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (kind == IClasspathEntry.CPE_SOURCE || (kind == IClasspathEntry.CPE_LIBRARY && (JavaModel.getTarget(
                    entryPath, false/*don't check existence*/) instanceof IContainer))) {
                for (int j = 0; j < classpath.length; j++) {
                    ...
                    if (entry != otherEntry &&
                        (otherKind == IClasspathEntry.CPE_SOURCE || (otherKind == IClasspathEntry.CPE_LIBRARY && (JavaModel.getTarget(
                                otherPath, false/*don't check existence*/) instanceof IContainer)))) {
                        ...
                        if (otherPath.isPrefixOf(entryPath) && !otherPath.equals(entryPath) && !Util.isExcluded(
                                ...
                                exclusionPatterns = ((ClasspathEntry)otherEntry).fullExclusionPatternChars(), false)) { //$NON-NLS-1$
            ...
            for (int j = 0; j < outputCount; j++) {
                ...
                if (entryPath.isPrefixOf(currentOutput)) {
                    if (kind != IClasspathEntry.CPE_SOURCE || !Util.isExcluded(currentOutput, inclusionPatterns,
        ...
        for (int i = 0; i < length; i++) {
            ...
            String entryPathMsg = isProjectRelative ? entryPath.removeFirstSegments(
                    1).toString() : entryPath.makeRelative().toString();
            ...
            if (kind == IClasspathEntry.CPE_SOURCE) {
                ...
                if (output == null) {
                    output =
                            projectOutputLocation; // if no specific output, still need to check using default output (this line would
                ...
                for (int j = 0; j < length; j++) {
                    ...
                    switch (otherEntry.getEntryKind()) {
                        case IClasspathEntry.CPE_SOURCE:
                            ...
                            String option = javaProject.getOption(JavaCore.CORE_OUTPUT_LOCATION_OVERLAPPING_ANOTHER_SOURCE,
                                                                  true);
                            if (otherEntry.getPath().equals(output) && !JavaCore.IGNORE.equals(option)) {
                                ...
                                String otherPathMsg = opStartsWithProject ? otherEntry.getPath().removeFirstSegments(
                                        1).toString() : otherEntry.getPath().makeRelative().toString();
                                ...
                                if (cachedStatus == null) {
                                    ...
                                    cachedStatus = new JavaModelStatus(IStatus.OK,
                                                                       IJavaModelStatusConstants.OUTPUT_LOCATION_OVERLAPPING_ANOTHER_SOURCE,
                                                                       Messages.bind(
                                                                               Messages.classpath_cannotUseDistinctSourceFolderAsOutput,
                                                                               new String[]{entryPathMsg, otherPathMsg, projectName})) {
                                        public boolean isOK() {
                                            return true;
                                        }
     ...
     * @param referredByContainer
     *         flag indicating whether the given entry is referred by a classpath container
     * @return a java model status describing the problem related to this classpath entry if any, a status object with code
    ...
    private static IJavaModelStatus validateClasspathEntry(IJavaProject project, IClasspathEntry entry,
                                                           IClasspathContainer entryContainer, boolean checkSourceAttachment,
        ...
        String entryPathMsg = projectName.equals(path.segment(0)) ? path.removeFirstSegments(
        ...
        switch (entry.getEntryKind()) {
            ...
            case IClasspathEntry.CPE_CONTAINER:
                if (path.segmentCount() >= 1) {
                    try {
                        ...
                        IClasspathContainer container = JavaModelManager.getJavaModelManager().getClasspathContainer(path,
                                                                                                                     project);
                        ...
                        if (containerEntries != null) {
                            for (int i = 0, length = containerEntries.length; i < length; i++) {
                                ...
                                if (containerEntry == null || kind == IClasspathEntry.CPE_SOURCE || kind == IClasspathEntry.CPE_VARIABLE ||
                                    kind == IClasspathEntry.CPE_CONTAINER) {
                                    return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CP_CONTAINER_ENTRY, project,
                                                               path);
                                }
                                IJavaModelStatus containerEntryStatus = validateClasspathEntry(project, containerEntry,
                                                                                               container, checkSourceAttachment,
                    ...
                    return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                               Messages.bind(Messages.classpath_illegalContainerPath,
            ...
            case IClasspathEntry.CPE_VARIABLE:
                if (path.segmentCount() >= 1) {
                    ...
                    IJavaModelStatus status =
                            validateClasspathEntry(project, entry, null, checkSourceAttachment, false/*not referred by container*/);
                    if (!status.isOK()) {
                        return status;
                    }
                    ...
                    if (deprecatedMessage != null) {
                        return new JavaModelStatus(IStatus.WARNING, IJavaModelStatusConstants.DEPRECATED_VARIABLE, project,
                                                   path, deprecatedMessage);
                    }
                    return status;
                } else {
                    return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                               Messages.bind(Messages.classpath_illegalVariablePath,
                                                             new String[]{entryPathMsg, projectName}));
                }
                ...
                if (path.isAbsolute() && path.segmentCount() == 1) {
                    ...
                    try {
                        if (!prereqProjectRsc.exists() || !prereqProjectRsc.hasNature(JavaCore.NATURE_ID)) {
                            return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                                       Messages.bind(Messages.classpath_unboundProject,
                                ...
                                return new JavaModelStatus(IJavaModelStatusConstants.INCOMPATIBLE_JDK_LEVEL, project, path,
                                                           Messages.bind(Messages.classpath_incompatibleLibraryJDKLevel,
                                                                         new String[]{project.getElementName(),
                                                                                      CompilerOptions.versionFromJdkLevel(
                                                                                              projectTargetJDK),
                                                                                      path.makeRelative().toString(),
                                                                                      CompilerOptions.versionFromJdkLevel(
                                                                                              prereqProjectTargetJDK)}));
                            }
                        }
                    } catch (CoreException e) {
                        return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                    ...
                    return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                               Messages.bind(Messages.classpath_illegalProjectPath,
            ...
            case IClasspathEntry.CPE_SOURCE:
                if (((entry.getInclusionPatterns() != null && entry.getInclusionPatterns().length > 0) ||
                     (entry.getExclusionPatterns() != null && entry.getExclusionPatterns().length > 0)) && JavaCore.DISABLED.equals(
                ...
                if (entry.getOutputLocation() != null && JavaCore.DISABLED.equals(
                        project.getOption(JavaCore.CORE_ENABLE_CLASSPATH_MULTIPLE_OUTPUT_LOCATIONS, true))) {
                ...
                if (path.isAbsolute() && !path.isEmpty()) {
                    ...
                    if (!projectPath.isPrefixOf(path) || JavaModel.getTarget(path, true) == null) {
                        return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                                   Messages.bind(Messages.classpath_unboundSourceFolder,
                                                                 new String[]{entryPathMsg, projectName}));
                    }
                } else {
                    return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                                               Messages.bind(Messages.classpath_illegalSourceFolderPath,
                                                             new String[]{entryPathMsg, projectName}));
                }
                break;
        }
    ...
    private static IJavaModelStatus validateLibraryEntry(IPath path, IJavaProject project, String container,
                                                         IPath sourceAttachment, String entryPathMsg) {
        if (path.isAbsolute() && !path.isEmpty()) {
                    ...
                    if (container != null) {
                        return new JavaModelStatus(IJavaModelStatusConstants.INCOMPATIBLE_JDK_LEVEL, project, path,
                                                   Messages.bind(Messages.classpath_incompatibleLibraryJDKLevelInContainer,
                                                                 new String[]{project.getElementName(), CompilerOptions.versionFromJdkLevel(
                                                                         projectTargetJDK), path.makeRelative().toString(), container,
                                                                              CompilerOptions.versionFromJdkLevel(
                                                                                      libraryJDK)}));
                    } else {
                        return new JavaModelStatus(IJavaModelStatusConstants.INCOMPATIBLE_JDK_LEVEL, project, path,
                                                   Messages.bind(Messages.classpath_incompatibleLibraryJDKLevel,
                                                                 new String[]{project.getElementName(), CompilerOptions.versionFromJdkLevel(
                                                                         projectTargetJDK), path.makeRelative().toString(),
                                                                              CompilerOptions.versionFromJdkLevel(
                                                                                      libraryJDK)}));
                    }
                }
            ...
            if (target instanceof IResource) {
                ...
                switch (resolvedResource.getType()) {
                    case IResource.FILE:
                        if (sourceAttachment != null && !sourceAttachment.isEmpty() && JavaModel.getTarget(sourceAttachment,
                                                                                                           true) == null) {
                            if (container != null) {
                                return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                ...
                boolean isExternal = path.getDevice() != null || !ResourcesPlugin.getWorkspace().getRoot().getProject(
                        path.segment(0)).exists();
                if (isExternal) {
                    if (container != null) {
                        return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
                    ...
                    if (entryPathMsg == null) {
                        entryPathMsg = project.getElementName().equals(path.segment(0)) ? path.removeFirstSegments(
                                1).makeRelative().toString() : path.toString();
                    }
                    if (container != null) {
                        return new JavaModelStatus(IJavaModelStatusConstants.INVALID_CLASSPATH,
