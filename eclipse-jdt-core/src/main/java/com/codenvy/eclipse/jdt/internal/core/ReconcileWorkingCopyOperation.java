...
import com.codenvy.eclipse.jdt.core.compiler.ReconcileContext;
...
public class ReconcileWorkingCopyOperation extends JavaModelOperation {
    ...
    public com.codenvy.eclipse.jdt.core.dom.CompilationUnit ast;
     ...
     * Returns the AST.
     */
    public com.codenvy.eclipse.jdt.core.dom.CompilationUnit makeConsistent(CompilationUnit workingCopy) throws JavaModelException {
        ...
        try {
            ...
            if (JavaProject.hasJavaNature(
                    workingCopy.getJavaProject().getProject()) && (this.reconcileFlags & ICompilationUnit.FORCE_PROBLEM_DETECTION) != 0) {
                this.resolveBindings = this.requestorIsActive;
                if (this.problems == null) {
                    this.problems = new HashMap();
                }
                unit = CompilationUnitProblemFinder.process(source, this.workingCopyOwner, this.problems,
                                                            this.astLevel !=
                                                            ICompilationUnit.NO_AST/*creating AST if level is not NO_AST */,
                                                            this.reconcileFlags,
                                                            this.progressMonitor);
            ...
            if (this.astLevel != ICompilationUnit.NO_AST && unit !=
                                                            null/*unit is null if working copy is consistent && (problem detection not
                                                            forced || non-Java project) -> don't create AST as per API*/) {
    ...
    private void notifyParticipants(final CompilationUnit workingCopy) {
        ...
        CompilationParticipant[] participants = JavaModelManager.getJavaModelManager().compilationParticipants.getCompilationParticipants(
                javaProject);
