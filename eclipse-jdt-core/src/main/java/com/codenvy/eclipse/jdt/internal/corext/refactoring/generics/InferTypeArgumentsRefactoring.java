...
import com.codenvy.eclipse.jdt.core.BindingKey;
...
import com.codenvy.eclipse.jdt.core.dom.ASTRequestor;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.generics.InferTypeArgumentsUpdate.CuUpdate;
...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.typesets.EnumeratedTypeSet;
import com.codenvy.eclipse.jdt.internal.corext.refactoring.typeconstraints.typesets.TypeSet;
...
public class InferTypeArgumentsRefactoring extends Refactoring
{
   ...
   public RefactoringStatus checkFinalConditions(
      final IProgressMonitor pm) throws CoreException, OperationCanceledException
   {
                     ...
                     SafeRunner.run(new ISafeRunnable()
                     {
                        public void run() throws Exception
                        {
                           ...
                           for (int p = 0; p < problems.length; p++)
                           {
                              ...
                              {
                                 String cuName = JavaElementLabels.getElementLabel(source,
                                    JavaElementLabels.CU_QUALIFIED);
                                 String msg = Messages.format(
                                    RefactoringCoreMessages.InferTypeArgumentsRefactoring_error_in_cu_skipped,
                                    new Object[]{cuName});
                                 result.addError(msg,
                                    JavaStatusContext.create(source, SourceRangeFactory.create(problems[p])));
                                 return;
                              }
                           }
                        ...
                        public void handleException(Throwable exception)
                        {
                           ...
                           String msg = Messages.format(
                              RefactoringCoreMessages.InferTypeArgumentsRefactoring_internal_error,
                              new Object[]{cuName});
                           Util.log(new Status(IStatus.ERROR, "IDE", 10001, msg, null));
                           String msg2 = Messages.format(
                              RefactoringCoreMessages.InferTypeArgumentsRefactoring_error_skipped,
                              new Object[]{cuName});
   ...
   public static ParameterizedType[] inferArguments(SimpleType[] types, InferTypeArgumentsUpdate update,
      InferTypeArgumentsTCModel model, CompilationUnitRewrite rewrite)
   {
   ...
   private static ParameterizedType rewriteConstraintVariable(ConstraintVariable2 cv, CompilationUnitRewrite rewrite,
      InferTypeArgumentsTCModel tCModel, boolean leaveUnconstraindRaw, SimpleType[] types)
   {
   ...
   private static ParameterizedType rewriteTypeVariable(TypeVariable2 typeCv, CompilationUnitRewrite rewrite,
      InferTypeArgumentsTCModel tCModel, boolean leaveUnconstraindRaw, SimpleType[] types)
   {
      ...
      if (node instanceof Name && node.getParent() instanceof Type)
      {
         ...
         rewrite.getASTRewrite().replace(originalType, newType,
            rewrite.createGroupDescription(RefactoringCoreMessages.InferTypeArgumentsRefactoring_addTypeArguments));
         return newType;
      }
      else
    ...
    */
   private static Type[] getTypeArguments(Type baseType, ArrayList<CollectionElementVariable2> typeArgumentCvs,
      CompilationUnitRewrite rewrite, InferTypeArgumentsTCModel tCModel, boolean leaveUnconstraindRaw)
   {
      ...
      for (int i = 0; i < typeArgumentCvs.size(); i++)
      {
         ...
         if (chosenType != null)
         {
            ...
            typeArgument = rewrite.getImportRewrite().addImportFromSignature(bindingKey.toSignature(),
               rewrite.getAST());
            ArrayList<CollectionElementVariable2> nestedTypeArgumentCvs = getTypeArgumentCvs(elementCv, tCModel);
            Type[] nestedTypeArguments = getTypeArguments(typeArgument, nestedTypeArgumentCvs, rewrite, tCModel,
   ...
   private static ArrayList<CollectionElementVariable2> getTypeArgumentCvs(ConstraintVariable2 baseCv,
   ...
   private static ASTNode rewriteCastVariable(CastVariable2 castCv, CompilationUnitRewrite rewrite,
      InferTypeArgumentsTCModel tCModel)
   {//, List positionGroups) {
      ...
      rewrite.getASTRewrite().replace(nodeToReplace, newExpression,
         rewrite.createGroupDescription(RefactoringCoreMessages.InferTypeArgumentsRefactoring_removeCast));
   ...
   public Change createChange(IProgressMonitor pm) throws CoreException, OperationCanceledException
   {
      ...
      try
      {
         DynamicValidationStateChange result = new DynamicValidationStateChange(
            RefactoringCoreMessages.InferTypeArgumentsRefactoring_name, fChangeManager.getAllChanges())
         {
            ...
            public final ChangeDescriptor getDescriptor()
            {
               ...
               final String header = project != null ? Messages.format(
                  RefactoringCoreMessages.InferTypeArgumentsRefactoring_descriptor_description_project,
                  BasicElementLabels.getJavaElementName(
                     project.getElementName())) : RefactoringCoreMessages.InferTypeArgumentsRefactoring_descriptor_description;
               ...
               {
                  settings[index] = JavaElementLabels.getTextLabel(fElements[index],
               ...
               final InferTypeArgumentsDescriptor descriptor = RefactoringSignatureDescriptorFactory.createInferTypeArgumentsDescriptor(
                  name, description, comment.asString(), arguments,
   ...
   private RefactoringStatus initialize(JavaRefactoringArguments arguments)
   {
      ...
      while ((handle = arguments.getAttribute(attribute)) != null)
      {
         ...
         if (element == null || !element.exists())
         {
            return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, getName(),
               IJavaRefactorings.INFER_TYPE_ARGUMENTS);
         }
         else
         {
      ...
      if (elements.isEmpty())
      {
         return JavaRefactoringDescriptorUtil.createInputFatalStatus(null, getName(),
            IJavaRefactorings.INFER_TYPE_ARGUMENTS);
      }
      if (!status.isOK())
      {
