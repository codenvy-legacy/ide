...
public class InferTypeArgumentsRefactoring extends Refactoring {
     ...
     *
     * @param elements
     *         the elements to process, or <code>null</code> if invoked by scripting
     */
    ...
    private static ParameterizedType rewriteConstraintVariable(ConstraintVariable2 cv, CompilationUnitRewrite rewrite,
                                                               InferTypeArgumentsTCModel tCModel, boolean leaveUnconstraindRaw,
                                                               SimpleType[] types) {
     ...
     *         type argument constraint variables
     * @param rewrite
     *         the cu rewrite
     * @param tCModel
     *         the type constraints model
     * @param leaveUnconstraindRaw
     *         <code>true</code> to keep unconstrained type references raw,
    ...
    private static Type[] getTypeArguments(Type baseType, ArrayList<CollectionElementVariable2> typeArgumentCvs,
        ...
        for (int i = 0; i < typeArgumentCvs.size(); i++) {
            ...
            if (chosenType != null) {
                ...
                //					typeArgument= rewrite.getAST().newSimpleType(rewrite.getAST().newName(rewrite.getImportRewrite().addImport("java
                // .lang.Object"))); //$NON-NLS-1$
