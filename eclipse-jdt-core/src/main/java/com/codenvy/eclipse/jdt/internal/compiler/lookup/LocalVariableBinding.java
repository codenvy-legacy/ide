...
public class LocalVariableBinding extends VariableBinding {
    ...
    // note that the name of a variable should be chosen so as not to conflict with user ones (usually starting with a space char is all
    // needed)
    ...
    */
    ...
    public void recordInitializationStartPC(int pc) {
        ...
        if (this.initializationCount > 0) {
            ...
            if (previousEndPC == pc) {
                this.initializationPCs[((this.initializationCount - 1) << 1) + 1] =
                        -1; // reuse previous interval (its range will be augmented)
                return;
            }
        }
    ...
    public String toString() {
        ...
        if (this.initializationCount > 0) {
            ...
            for (int i = 0; i < this.initializationCount; i++) {
                ...
                s += String.valueOf(
                        this.initializationPCs[i << 1]) + "-" + ((this.initializationPCs[(i << 1) + 1] == -1) ? "?" : String.valueOf(
                        this.initializationPCs[(i << 1) + 1])); //$NON-NLS-2$ //$NON-NLS-1$
            }
            s += "]"; //$NON-NLS-1$
        }
        return s;
