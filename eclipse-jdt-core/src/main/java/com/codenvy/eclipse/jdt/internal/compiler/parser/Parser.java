 ...
 *******************************************************************************/
package com.codenvy.eclipse.jdt.internal.compiler.parser;
...
import com.codenvy.eclipse.jdt.internal.compiler.parser.diagnose.DiagnoseParser;
import com.codenvy.eclipse.jdt.internal.compiler.problem.AbortCompilation;
import com.codenvy.eclipse.jdt.internal.compiler.problem.AbortCompilationUnit;
...
public class Parser implements ParserBasicInformation, TerminalTokens, OperatorIds, TypeIds {
    ...
    private static void buildFilesForRecoveryTemplates(String indexFilename, String templatesFilename,
                                                       char[] newTerminalIndex, char[] newNonTerminalIndex, String[] newName, char[] newLhs,
                                                       String[] tokens) {
    ...
    private static void buildFilesForStatementsRecoveryFilter(String filename, char[] newNonTerminalIndex, char[] newLhs,
                                                              String[] tokens) {
    ...
    public final static void buildFilesFromLPG(String dataFilename, String dataFilename2) {
        ...
        char[] newTerminalIndex = buildFileOfIntFor(prefix + (++i) + ".rsc", "terminal_index",
                                                    tokens); //$NON-NLS-2$ //$NON-NLS-1$
        char[] newNonTerminalIndex = buildFileOfIntFor(prefix + (++i) + ".rsc", "non_terminal_index",
                                                       tokens); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        buildFileForReadableName(READABLE_NAMES_FILE + ".props", newLhs, newNonTerminalIndex, newName,
                                 tokens);//$NON-NLS-1$
    ...
    protected static long[] readLongTable(String filename) throws IOException {
        ...
        while (true) {
            longs[longIndex++] =
                    (((long)(bytes[i++] & 0xFF)) << 56) + (((long)(bytes[i++] & 0xFF)) << 48) + (((long)(bytes[i++] & 0xFF)) << 40) +
                    (((long)(bytes[i++] & 0xFF)) << 32) + (((long)(bytes[i++] & 0xFF)) << 24) + (((long)(bytes[i++] & 0xFF)) << 16) +
    ...
    public com.codenvy.eclipse.jdt.internal.compiler.ReadManager readManager;
 ...
 */
    ...
    * also rebuild bracket balance
                ...
                if (this.referenceContext instanceof TypeDeclaration) {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        if (field != null && field.getKind() == AbstractVariableDeclaration.INITIALIZER &&
                            ((Initializer)field).block != null && field.declarationSourceStart <= this.scanner.initialPosition &&
    ...
    protected void consumeAnnotationName() {
        if (this.currentElement != null) {
            ...
            if (this.annotationRecoveryActivated) {
                this.currentElement = this.currentElement.addAnnotationName(this.identifierPtr, this.identifierLengthPtr,
                                                                            start, 0);
            }
        }
    ...
    protected void consumeArrayAccess(boolean unspecifiedReference) {
        ...
        if (unspecifiedReference) {
            ...
            exp = this.expressionStack[this.expressionPtr] = new ArrayReference(this.expressionStack[this.expressionPtr],
    ...
    protected void consumeAssertStatement() {
        ...
        pushOnAstStack(
                new AssertStatement(this.expressionStack[this.expressionPtr--], this.expressionStack[this.expressionPtr--],
    ...
    protected void consumeAssignment() {
        ...
        this.expressionStack[this.expressionPtr] = (op != EQUAL) ? new CompoundAssignment(
                this.expressionStack[this.expressionPtr], expression, op, expression.sourceEnd) : new Assignment(
                this.expressionStack[this.expressionPtr], expression, expression.sourceEnd);
                    ...
                    if (expr1 instanceof StringLiteral) {
                        if (((expr1.bits & ASTNode.ParenthesizedMASK) >> ASTNode.ParenthesizedSHIFT) == 0) {
                            if (expr2 instanceof CharLiteral) { // string+char
                                this.expressionStack[this.expressionPtr] = ((StringLiteral)expr1).extendWith(
                                        (CharLiteral)expr2);
	...
	*/
    ...
    protected void consumeCastExpressionWithNameArray() {
        ...
        this.expressionStack[this.expressionPtr] = cast = new CastExpression(
                exp = this.expressionStack[this.expressionPtr], castType = getTypeReference(this.intStack[this.intPtr--]));
        castType.sourceEnd = end - 1;
        castType.sourceStart = (cast.sourceStart = this.intStack[this.intPtr--]) + 1;
        cast.sourceEnd = exp.sourceEnd;
    }
    ...
    protected void consumeClassHeaderName1() {
        ...
        if ((typeDecl.bits & ASTNode.IsMemberType) == 0 && (typeDecl.bits & ASTNode.IsLocalType) == 0) {
            if (this.compilationUnit != null && !CharOperation.equals(typeDecl.name,
                                                                      this.compilationUnit.getMainTypeName())) {
    ...
    protected void consumeClassInstanceCreationExpressionQualifiedWithTypeArguments() {
        ...
        // ClassInstanceCreationExpression ::= ClassInstanceCreationExpressionName 'new' TypeArguments SimpleName '(' ArgumentListopt ')'
        // ClassBodyopt
    ...
    protected void consumeConditionalExpressionWithName(int op) {
        ...
        this.expressionPtr -= 2;
        this.expressionLengthPtr -= 2;
        this.expressionStack[this.expressionPtr] = new ConditionalExpression(this.expressionStack[this.expressionPtr + 2],
                                                                             this.expressionStack[this.expressionPtr],
                                                                             this.expressionStack[this.expressionPtr + 1]);
    }
	...
	*/
        ...
        if (!(this.diet && this.dietInt == 0) && statements == null && (constructorCall == null || constructorCall.isImplicitSuper()) &&
            !containsComment(
                    cd.bodyStart, this.endPosition)) {
    ...
    protected void consumeEmptyStatement() {
        ...
        if (source[this.endStatementPosition] == ';') {
            ...
            if (source.length > 5) {
                ...
                if (source[pos] == '\\' &&
                    !((c1 = ScannerHelper.getHexadecimalValue(
                            source[this.endStatementPosition - 3])) > 15 || c1 < 0 || (c2 = ScannerHelper.getHexadecimalValue(
                            source[this.endStatementPosition - 2])) > 15 || c2 < 0 || (c3 = ScannerHelper.getHexadecimalValue(
                            source[this.endStatementPosition - 1])) > 15 || c3 < 0 || (c4 = ScannerHelper.getHexadecimalValue(
                            source[this.endStatementPosition])) > 15 || c4 < 0) &&
        ...
        if (this.currentElement != null) {
            if (!(this.currentElement instanceof RecoveredType) && (this.currentToken == TokenNameDOT
                                                                    ...
                                                                    (Util.getLineNumber(declaration.type.sourceStart, this.scanner.lineEnds,
                                                                     ...
                                                                     Util.getLineNumber((int)(namePosition >>> 32), this.scanner.lineEnds,
                                                                                        0,
                                                                                        this.scanner.linePtr)))) {
    ...
    protected void consumeEnumHeaderName() {
        ...
        if ((enumDeclaration.bits & ASTNode.IsMemberType) == 0 && (enumDeclaration.bits & ASTNode.IsLocalType) == 0) {
            if (this.compilationUnit != null && !CharOperation.equals(enumDeclaration.name,
                                                                      this.compilationUnit.getMainTypeName())) {
 ...
 */
	...
	*/
        ...
        } else if (!(this.diet && this.dietInt == 0) && statements == null && !containsComment(md.bodyStart,
                                                                                               this.endPosition)) {
    ...
    protected void consumeMethodHeader() {
        ...
        // AnnotationMethodHeader ::= AnnotationMethodHeaderName FormalParameterListopt MethodHeaderRightParen MethodHeaderExtendedDims
    ...
    protected void consumeOnlyTypeArguments() {
        if (!this.statementRecoveryActivated &&
            ...
            this.lastErrorEndPositionBeforeRecovery < this.scanner.currentPosition) {
            int length = this.genericsLengthStack[this.genericsLengthPtr];
            problemReporter().invalidUsageOfTypeArguments((TypeReference)this.genericsStack[this.genericsPtr - length + 1],
    ...
    protected void consumeReduceImports() {
        ...
        if ((length = this.astLengthStack[this.astLengthPtr--]) != 0) {
            this.astPtr -= length;
            System.arraycopy(this.astStack, this.astPtr + 1, this.compilationUnit.imports = new ImportReference[length], 0,
                             length);
        }
    }
    ...
    protected void consumeSingleMemberAnnotation() {
        ...
        if (this.currentElement != null) {
            ...
            if (this.currentElement instanceof RecoveredAnnotation) {
                this.currentElement = ((RecoveredAnnotation)this.currentElement).addAnnotation(singleMemberAnnotation,
                                                                                               oldIndex);
            }
        }
    ...
    protected void consumeStatementSynchronized() {
        ...
        if (this.astLengthStack[this.astLengthPtr] == 0) {
            ...
            this.astStack[this.astPtr] = new SynchronizedStatement(this.expressionStack[this.expressionPtr--],
                                                                   (Block)this.astStack[this.astPtr], this.intStack[this.intPtr--],
                                                                   this.endStatementPosition);
        }
        resetModifiers();
    }
	...
	/* try to minimize the number of build values */
        ...
        switch (type) {
            ...
            case TokenNameLongLiteral:
                pushOnExpressionStack(
                        LongLiteral.buildLongLiteral(this.scanner.getCurrentTokenSource(), this.scanner.startPosition,
                                                     this.scanner.currentPosition - 1));
                break;
            ...
            case TokenNameRPAREN:
                ...
                this.rParenPos =
                        this.scanner.currentPosition - 1; // position of the end of right parenthesis (in case of unicode \u0029) lex00101
                break;
    ...
    protected void consumeTypeHeaderNameWithTypeParameters() {
        ...
        int length = this.genericsLengthStack[this.genericsLengthPtr--];
        this.genericsPtr -= length;
        System.arraycopy(this.genericsStack, this.genericsPtr + 1, typeDecl.typeParameters = new TypeParameter[length], 0,
                         length);
        ...
        for (int end = 0; end <= length; end++) //<HERE> the plus one allows to
        {
            ...
            { //array copy
                switch (flagI) {
                    ...
                    case 3:
                        size3 += (length2 = end - start);
                        System.arraycopy(this.astStack, this.astPtr + start + 1, enumDeclaration.memberTypes, size3 - length2,
                                         length2);
                        break;
                }
                flagI = flag[start = end];
 ...
 */
    ...
    public void getMethodBodies(CompilationUnitDeclaration unit) {
        ...
        char[] contents = this.readManager != null ? this.readManager.getContents(
                compilationResult.compilationUnit) : compilationResult.compilationUnit.getContents();
    ...
    protected char getNextCharacter(char[] comment, int[] index) {
        ...
        switch (nextCharacter) {
            case '\\':
                ...
                if (!(((c1 = ScannerHelper.getHexadecimalValue(
                        comment[index[0]++])) > 15 || c1 < 0) || ((c2 = ScannerHelper.getHexadecimalValue(
                        comment[index[0]++])) > 15 || c2 < 0) || ((c3 = ScannerHelper.getHexadecimalValue(
                        comment[index[0]++])) > 15 || c3 < 0) || ((c4 = ScannerHelper.getHexadecimalValue(
                        comment[index[0]++])) > 15 || c4 < 0))) {
    ...
    public void initialize(boolean initializeNLS) {
        ...
        this.recordStringLiterals = true;
        final boolean checkNLS = this.options.getSeverity(
                CompilerOptions.NonExternalizedString) != ProblemSeverities.Ignore;
    ...
    public void initializeScanner() {
        this.scanner = new Scanner(false /*comment*/, false /*whitespace*/, false, /* will be set in initialize(boolean) */
    ...
    private void jumpOverType() {
        if (this.recoveredTypes != null && this.nextTypeStart > -1 && this.nextTypeStart < this.scanner.currentPosition) {
            ...
            if (!isAnonymous) {
                ...
                ((RecoveryScanner)this.scanner).setPendingTokens(
                        new int[]{TokenNameIdentifier, TokenNameEQUAL, TokenNameIdentifier});
            }
            ...
            if (++this.recoveredTypePtr < this.recoveredTypes.length) {
                ...
                this.nextTypeStart = nextTypeDeclaration.allocation == null ? nextTypeDeclaration.declarationSourceStart
...
called in order to remember (when needed) the consumed token */
    ...
    protected void parse() {
        ...
        for (; ; ) {
            ...
            if (act <= NUM_RULES) {
                ...
                if (DEBUG_AUTOMATON) {
                    System.out.print(
                            "Shift/Reduce - (" + name[terminal_index[this.currentToken]] + ") ");  //$NON-NLS-1$  //$NON-NLS-2$
                }
                ...
                if (act < ACCEPT_ACTION) { /* shift */
                    ...
                    if (DEBUG_AUTOMATON) {
                        System.out.println(
                                "Shift        - (" + name[terminal_index[this.currentToken]] + ")");  //$NON-NLS-1$  //$NON-NLS-2$
                    }
                    continue ProcessTerminals;
                }
                break ProcessTerminals;
    ...
    public void parseStatements(ReferenceContext rc, int start, int end, TypeDeclaration[] types,
        ...
        if (types != null && types.length > 0) {
            ...
            this.nextTypeStart = this.recoveredTypes[0].allocation == null ? this.recoveredTypes[0].declarationSourceStart
 ...
 */
	...
	identifierPtr points on the next top */
        ...
        if (++this.identifierPtr >= stackLength) {
            ...
            System.arraycopy(this.identifierPositionStack, 0, this.identifierPositionStack = new long[stackLength + 20], 0,
                             stackLength);
        }
	...
	genericsPtr points on the top*/
        ...
        if (++this.genericsPtr >= stackLength) {
            System.arraycopy(this.genericsStack, 0, this.genericsStack = new ASTNode[stackLength + GenericsStackIncrement],
                             0, stackLength);
        }
 ...
 */
            ...
            case TokenNameStringLiteral:
                if (this.recordStringLiterals &&
                    ...
                    StringLiteral stringLiteral = createStringLiteral(this.scanner.getCurrentTokenSourceString(),
                                                                      this.scanner.startPosition, this.scanner.currentPosition - 1,
                                                                      Util.getLineNumber(this.scanner.startPosition, this.scanner.lineEnds,
    ...
    protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
        ...
        if (isDietParse) {
            ...
            int[][] intervalToSkip = com.codenvy.eclipse.jdt.internal.compiler.parser.diagnose.RangeUtil.computeDietRange(types);
