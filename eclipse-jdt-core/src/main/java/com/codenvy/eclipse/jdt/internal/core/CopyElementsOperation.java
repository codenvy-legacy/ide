...
public class CopyElementsOperation extends MultiOperation implements SuffixConstants
{
      ...
      try
      {
         ...
         switch (element.getElementType())
         {
            ...
            case IJavaElement.TYPE:
               if (isRenamingMainType(element, dest))
               {
                  ...
                  return new RenameResourceElementsOperation(new IJavaElement[]{dest},
                     new IJavaElement[]{dest.getParent()}, new String[]{getNewNameFor(element) + '.' + extension},
                     this.force);
               }
               else
               {
                  String source = getSourceFor(element);
                  String lineSeparator = com.codenvy.eclipse.jdt.internal.core.util.Util.getLineSeparator(source,
            ...
            case IJavaElement.FIELD:
               ...
               lineSeparator = com.codenvy.eclipse.jdt.internal.core.util.Util.getLineSeparator(source,
                  element.getJavaProject());
   ...
   protected boolean isRenamingMainType(IJavaElement element, IJavaElement dest) throws JavaModelException
   {
      if ((isRename() || getNewNameFor(element) != null) && dest.getElementType() == IJavaElement.COMPILATION_UNIT)
      {
         ...
         typeName = com.codenvy.eclipse.jdt.internal.core.util.Util.getNameWithoutJavaLikeExtension(typeName);
         ...
         else if (isRename())
         {
            ...
            if (anchor != null)
            {
               ((CreateElementInCUOperation)op).setRelativePosition(anchor,
                  CreateElementInCUOperation.INSERT_AFTER); // insert after so that the anchor is found before when deleted below
            }
         }
