...
import com.codenvy.eclipse.jdt.internal.codeassist.impl.AssistOptions;
...
public class InternalNamingConventions {
    ...
    private static Scanner getNameScanner(CompilerOptions compilerOptions) {
        return new Scanner(false /*comment*/, false /*whitespace*/, false /*nls*/, compilerOptions.sourceLevel /*sourceLevel*/,
    ...
    private static char[][] computeNonBaseTypeNames(char[] sourceName, boolean isConstantField, boolean onlyLongest) {
        ...
        int previousCharKind = ScannerHelper.isLowerCase(c) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(
                c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;

        for (int i = length - 1; i >= 0; i--) {
            c = sourceName[i];

            int charKind = ScannerHelper.isLowerCase(c) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(
                    c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;
                ...
                case IS_UPPER_CASE:
                    if (previousCharKind == IS_LOWER_CASE) {
                        ...
                        if (i > 0) {
                            ...
                            previousCharKind = ScannerHelper.isLowerCase(pc) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(
                                    pc) ? IS_UPPER_CASE : pc == '_' ? IS_UNDERSCORE : IS_OTHER;
                        }
                        endIndex = i;
                    } else {
        ...
        if (prefixes != null) {
            ...
            for (int i = 0; i < prefixes.length; i++) {
                ...
                if (CharOperation.prefixEquals(prefix, name)) {
                    ...
                    if (!lastCharIsLetter || (lastCharIsLetter && name.length > currLen && ScannerHelper.isUpperCase(
                            name[currLen]))) {
    ...
    public static void suggestVariableNames(int variableKind, int baseNameKind, char[] baseName,
                                            IJavaProject javaProject, int dim, char[] internalPrefix, char[][] excluded,
        ...
        for (int i = 0; i < tempNames.length; i++) {
            ...
            if (!isConstantField) {
                ...
                for (int j = 0; j <= internalPrefix.length; j++) {
                    if (j == internalPrefix.length || CharOperation.prefixEquals(
                            CharOperation.subarray(internalPrefix, j, -1), unprefixedName, j != 0 /*do not check case when there is no
                            prefix*/)) {
                        matchingIndex = j;
                        break done;
                    }
