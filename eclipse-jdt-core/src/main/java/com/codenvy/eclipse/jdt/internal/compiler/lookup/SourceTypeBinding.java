...
public class SourceTypeBinding extends ReferenceBinding {
                        ...
                        for (int m = superMethods.length; --m >= 0; ) {
                            ...
                            if (defaultAbstractsCount == 0) {
                                ...
                                for (int k = 0; k < defaultAbstractsCount; k++) {
                                    ...
                                    if (CharOperation.equals(alreadyAdded.selector,
                                                             method.selector) && alreadyAdded.areParametersEqual(method)) {
                                        continue nextAbstractMethod;
                                    }
                                }
                            }
                            MethodBinding defaultAbstract = new MethodBinding(
                                    ...
                                    method.selector, method.returnType, method.parameters, method.thrownExceptions, this);
                            if (defaultAbstractsCount == defaultAbstracts.length) {
                                System.arraycopy(defaultAbstracts, 0,
                                                 defaultAbstracts = new MethodBinding[2 * defaultAbstractsCount], 0, defaultAbstractsCount);
                            }
    ...
    */
    public FieldBinding addSyntheticFieldForInnerclass(LocalVariableBinding actualOuterLocalVariable) {
        ...
        FieldBinding synthField = (FieldBinding)this.synthetics[SourceTypeBinding.FIELD_EMUL].get(
    ...
    */
    public FieldBinding addSyntheticFieldForInnerclass(ReferenceBinding enclosingType) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(CharOperation.concat(TypeConstants.SYNTHETIC_ENCLOSING_INSTANCE_PREFIX,
                                                                        String.valueOf(enclosingType.depth()).toCharArray()), enclosingType,
    ...
    */
    public FieldBinding addSyntheticFieldForClassLiteral(TypeBinding targetType, BlockScope blockScope) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(CharOperation.concat(TypeConstants.SYNTHETIC_CLASS,
                                                                        String.valueOf(this.synthetics[SourceTypeBinding.CLASS_LITERAL_EMUL]
                                                                                               .size()).toCharArray()),
                                                   blockScope.getJavaLangClass(),
                                                   ClassFileConstants.AccDefault | ClassFileConstants.AccStatic |
                                                   ClassFileConstants.AccSynthetic, this,
                                                   Constant.NotAConstant, this.synthetics[SourceTypeBinding.CLASS_LITERAL_EMUL].size());
    ...
    */
    public SyntheticMethodBinding addSyntheticMethod(FieldBinding targetField, boolean isReadAccess,
        ...
        SyntheticMethodBinding[] accessors = (SyntheticMethodBinding[])this.synthetics[SourceTypeBinding.METHOD_EMUL].get(
                targetField);
        if (accessors == null) {
    ...
    */
    ...
    public SyntheticMethodBinding addSyntheticBridgeMethod(MethodBinding inheritedMethodToBridge,
        ...
        if (inheritedMethodToBridge.returnType.erasure() == targetMethod.returnType.erasure() &&
            inheritedMethodToBridge.areParameterErasuresEqual(
                    targetMethod)) {
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.lookup.TypeBinding#genericTypeSignature() */
     ...
     * <T:LY<TT;>;U:Ljava/lang/Object;V::Ljava/lang/Runnable;:Ljava/lang/Cloneable;:Ljava/util/Map;>Ljava/lang/Exception;
     * Ljava/lang/Runnable;
     */
     ...
     *
     * @see com.codenvy.eclipse.jdt.internal.compiler.lookup.Binding#getAnnotationTagBits()
     */
            ...
            if ((range = ReferenceBinding.binarySearch(selector, this.methods)) >= 0) {
                ...
                for (int imethod = start; imethod <= end; imethod++) {
                    ...
                    if (resolveTypesFor(method) == null || method.returnType == null) {
                        ...
                        return getExactMethod(selector, argumentTypes,
                                              refScope); // try again since the problem methods have been removed
                    }
                }
                ...
                for (int i = start; i <= end; i++) {
                    ...
                    for (int j = end; j > i; j--) {
                        ...
                        boolean paramsMatch = isSource15 ? method1.areParameterErasuresEqual(
                                method2) : method1.areParametersEqual(method2);
                        if (paramsMatch) {
                            methods();
                            return getExactMethod(selector, argumentTypes,
                                                  refScope); // try again since the problem methods have been removed
    ...
    public MethodBinding[] getMethods(char[] selector) {
        ...
        for (int i = 0, length = result.length - 1; i < length; i++) {
            ...
            for (int j = length; j > i; j--) {
                boolean paramsMatch = isSource15 ? method.areParameterErasuresEqual(result[j]) : method.areParametersEqual(
                        result[j]);
                if (paramsMatch) {
                    methods();
    ...
    */
    public FieldBinding getSyntheticField(ReferenceBinding targetEnclosingType, boolean onlyExactMatch) {
        ...
        // class T { class M{}}
        // class S extends T { class N extends M {}} --> need to use S as a default enclosing instance for the super constructor call in
        // N().
        if (!onlyExactMatch) {
            Iterator accessFields = this.synthetics[SourceTypeBinding.FIELD_EMUL].values().iterator();
            while (accessFields.hasNext()) {
                field = (FieldBinding)accessFields.next();
                if (CharOperation.prefixEquals(TypeConstants.SYNTHETIC_ENCLOSING_INSTANCE_PREFIX,
                                               field.name) && field.type.findSuperTypeOriginatingFrom(targetEnclosingType) != null) {
                    return field;
                }
            }
        }
    ...
    /** @see com.codenvy.eclipse.jdt.internal.compiler.lookup.Binding#initializeDeprecatedAnnotationTagBits() */
        ...
        try {
            ...
            for (int i = 0, length = this.methods.length; i < length; i++) {
                ...
                for (int j = i + 1; j < length; j++) {
                                        ...
                                        for (; --index >= 0; ) {
                                            ...
                                            if (params1[index] == params2[index]) {
                                                ...
                                                if (type instanceof SourceTypeBinding &&
                                                    ...
                                                    index =
                                                            pLength; // handle comparing identical source types like X<T>... its erasure
                                                            // is itself BUT we need to answer false
                                                    break;
                                                }
                                            }
                                        }
                    ...
                    boolean isEnumSpecialMethod = isEnum() && (CharOperation.equals(selector,
                                                                                    TypeConstants.VALUEOF) ||
                                                               CharOperation.equals(selector, TypeConstants.VALUES));
                    // report duplicate
                    boolean removeMethod2 =
                            (severity == ProblemSeverities.Error) ? true : false; // do not remove if in 1.6 and just a warning given
                    if (methodDecl == null) {
                        methodDecl =
                                method.sourceMethod(); // cannot be retrieved after binding is lost & may still be null if method is special
    ...
    public FieldBinding resolveTypeFor(FieldBinding field) {
        ...
        for (int f = 0; f < length; f++) {
            ...
            MethodScope initializationScope =
                    field.isStatic() ? this.scope.referenceContext.staticInitializerScope : this.scope.referenceContext.initializerScope;
            FieldBinding previousField = initializationScope.initializedField;
            try {
                initializationScope.initializedField = field;
                FieldDeclaration fieldDecl = fieldDecls[f];
                TypeBinding fieldType = fieldDecl.getKind() == AbstractVariableDeclaration.ENUM_CONSTANT ? initializationScope.environment()
                                        ...
                                        // enum constant is implicitly of declaring enum type
                                                                                                         : fieldDecl.type.resolveType(
                                                                                                                 initializationScope, true /* check bounds*/);
    ...
    private void evaluateNullAnnotations(long annotationTagBits) {
        ...
        if (!isPackageInfo) {
            boolean isInNullnessAnnotationPackage =
                    pkg == this.scope.environment().nonnullAnnotationPackage || pkg == this.scope.environment().nullableAnnotationPackage ||
                    pkg == this.scope.environment().nonnullByDefaultAnnotationPackage;
            if (pkg.defaultNullness == NO_NULL_DEFAULT && !isInDefaultPkg && !isInNullnessAnnotationPackage &&
        ...
        if (newDefaultNullness != NO_NULL_DEFAULT) {
            if (isPackageInfo) {
                ...
                long nullDefaultBits =
                        annotationTagBits & (TagBits.AnnotationNullUnspecifiedByDefault | TagBits.AnnotationNonNullByDefault);
    ...
    protected void checkRedundantNullnessDefaultRecurse(ASTNode location, Annotation[] annotations,
                                                        long annotationTagBits) {
        if (this.fPackage.defaultNullness != NO_NULL_DEFAULT) {
            if ((this.fPackage.defaultNullness == NONNULL_BY_DEFAULT && ((annotationTagBits & TagBits.AnnotationNonNullByDefault) != 0))) {
    ...
    protected boolean checkRedundantNullnessDefaultOne(ASTNode location, Annotation[] annotations,
                                                       long annotationTagBits) {
     ...
     *
     * @param currentScope
     *         where to start search for lexically enclosing default
     * @param environment
     *         gateway to options
     */
