...
public abstract class Statement extends ASTNode
{
      ...
      LocalVariableBinding local, int nullStatus, Expression expression, TypeBinding providedType)
   {
      if (local != null)
      {
         if ((local.tagBits & TagBits.AnnotationNonNull) != 0 && nullStatus != FlowInfo.NON_NULL)
         {
            flowContext.recordNullityMismatch(currentScope, expression, providedType, local.type, nullStatus);
            return FlowInfo.NON_NULL;
         }
         else if ((local.tagBits & TagBits.AnnotationNullable) != 0 && nullStatus == FlowInfo.UNKNOWN)
         {   // provided a legacy type?
      ...
      Scope scope)
   {
      ...
      return expressionType.isBaseType()  // narrowing then boxing ? Only allowed for some target types see 362279
         && !targetType.isBaseType() && !targetType.isTypeVariable() && scope.compilerOptions().sourceLevel >= com.codenvy.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_5 // autoboxing
         && (targetType.id == TypeIds.T_JavaLangByte || targetType.id == TypeIds.T_JavaLangShort || targetType.id == TypeIds.T_JavaLangCharacter) && expression.isConstantValueOfTypeAssignableToType(
   ...
   /**
    * Implementation of {@link com.codenvy.eclipse.jdt.internal.compiler.lookup.InvocationSite#expectedType}
