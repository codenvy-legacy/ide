...
import com.codenvy.eclipse.jdt.internal.corext.refactoring.rename.RenameAnalyzeUtil.LocalAnalyzePackage;
...
public class RenameLocalVariableProcessor extends JavaRenameProcessor implements IReferenceUpdating
{
    ...
    */
   RenameLocalVariableProcessor(ILocalVariable localVariable, TextChangeManager manager, CompilationUnit node,
   ...
   public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException
   {
      ...
      if (!Checks.isDeclaredIn(fTempDeclarationNode, MethodDeclaration.class) && !Checks.isDeclaredIn(
         fTempDeclarationNode, Initializer.class))
      {
         return RefactoringStatus.createFatalErrorStatus(
      ...
      CheckConditionsContext context) throws CoreException, OperationCanceledException
   {
      try
      {
         ...
         if (!fIsComposite)
         {
            ...
            result.merge(
               RenameAnalyzeUtil.analyzeLocalRenames(localAnalyzePackages, fChange, fCompilationUnitNode, true));
         }
         return result;
      }
      finally
   ...
   public RefactoringStatus checkNewElementName(String newName) throws JavaModelException
   {
      ...
      if (!Checks.startsWithLowerCase(newName))
      {
         if (fIsComposite)
         {
            final String nameOfParent = JavaElementLabels.getElementLabel(fLocalVariable.getParent(),
               JavaElementLabels.ALL_DEFAULT);
            final String nameOfType = JavaElementLabels.getElementLabel(fLocalVariable.getAncestor(IJavaElement.TYPE),
               JavaElementLabels.ALL_DEFAULT);
      ...
      for (int i = 0; i < allRenameEdits.length; i++)
      {
         if (fIsComposite)
         {
            ...
            TextChangeCompatibility.addTextEdit(fChangeManager.get(fCu),
               RefactoringCoreMessages.RenameTempRefactoring_changeName, allRenameEdits[i].copy(), fCategorySet);
   ...
   private RenameJavaElementDescriptor createRefactoringDescriptor()
   {
      ...
      final String header = Messages.format(RefactoringCoreMessages.RenameLocalVariableProcessor_descriptor_description,
         new String[]{BasicElementLabels.getJavaElementName(fCurrentName), JavaElementLabels.getElementLabel(
            fLocalVariable.getParent(), JavaElementLabels.ALL_FULLY_QUALIFIED), BasicElementLabels.getJavaElementName(
            fNewName)});
