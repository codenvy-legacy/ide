...
public class QualifiedNameReference extends NameReference
{
      ...
      Assignment assignment, boolean isCompound)
   {
      ...
      if (lastFieldBinding.isFinal())
      {
         ...
         if (otherBindingsCount == 0 && this.indexOfFirstFieldBinding == 1 && lastFieldBinding.isBlankFinal() && !isCompound && currentScope.allowBlankFinalFieldAssignment(
            lastFieldBinding))
         {
            ...
            if (lastFieldBinding != null)
            {
               ...
               else
               {
                  if (needValue || (i > 0 && complyTo14) || lastGenericCast != null)
                  {
                     ...
                     {
                        TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(currentScope,
                           lastFieldBinding, lastReceiverType, i == 0 && this.indexOfFirstFieldBinding == 1);
                        if (lastFieldBinding.isStatic())
                        {
                           codeStream.fieldAccess(Opcodes.OPC_getstatic, lastFieldBinding, constantPoolDeclaringClass);
                        }
      ...
      while (index < length)
      {
         ...
         if (field.isValidBinding())
         {
            ...
            if (previousField != null)
            {
               ...
               { // record need for explicit cast at codegen
            ...
            if (field.isStatic())
            {
               ...
               { // enum constants are checked even when qualified)
      ...
      { // private access
         ...
         if (declaringClass != currentScope.enclosingSourceType())
         {
            setSyntheticAccessor(fieldBinding, index,
               ((SourceTypeBinding)declaringClass).addSyntheticMethod(codegenField, index >= 0 /*read-access?*/, false /*not super access*/));
            ...
            case Binding.TYPE | Binding.VARIABLE:
               if (this.binding instanceof LocalVariableBinding)
               {
                  ...
                  if (this.resolvedType != null && (this.resolvedType.tagBits & TagBits.HasMissingType) != 0)
                  {
                     ...
                     scope.problemReporter().invalidField(this,
                        new ProblemFieldBinding(lastField.declaringClass, lastField.name, ProblemReasons.NotFound),
               ...
               if (this.binding instanceof FieldBinding)
               {
                  ...
                  if (fieldBinding.isStatic())
                  {
                     ...
                     if (this.indexOfFirstFieldBinding > 1 && fieldBinding.declaringClass != this.actualReceiverType && fieldBinding.declaringClass.canBeSeenBy(
                        scope))
                     {
