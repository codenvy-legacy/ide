...
import com.codenvy.eclipse.jdt.internal.compiler.codegen.BranchLabel;
...
import com.codenvy.eclipse.jdt.internal.compiler.codegen.MultiCatchExceptionLabel;
import com.codenvy.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream;
import com.codenvy.eclipse.jdt.internal.compiler.flow.ExceptionHandlingFlowContext;
import com.codenvy.eclipse.jdt.internal.compiler.flow.FinallyFlowContext;
...
import com.codenvy.eclipse.jdt.internal.compiler.flow.InsideSubRoutineFlowContext;
import com.codenvy.eclipse.jdt.internal.compiler.flow.NullInfoRegistry;
...
public class TryStatement extends SubRoutineStatement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (this.subRoutineStartLabel == null) {
            ...
            ExceptionHandlingFlowContext handlingContext = new ExceptionHandlingFlowContext(flowContext, this,
                                                                                            ...
                                                                                            this.catchArguments, null, this.scope,
            ...
            for (int i = 0; i < resourcesLength; i++) {
                ...
                if (resourceBinding.closeTracker != null) {
                    ...
                    // keep the tracking variable in the resourceBinding in order to prevent creating a new one while analyzing the try
                    // block
                }
                TypeBinding type = resourceBinding.type;
                if (type != null && type.isValidBinding()) {
                    ReferenceBinding binding = (ReferenceBinding)type;
                    MethodBinding closeMethod = binding.getExactMethod(ConstantPool.Close, new TypeBinding[0],
                    ...
                    if (closeMethod == null) {
                        ...
                        closeMethod = this.scope.compilationUnitScope().findMethod(binding, ConstantPool.Close,
                                                                                   new TypeBinding[0], site, false);
                    }
                    if (closeMethod != null && closeMethod.isValidBinding() && closeMethod.returnType.id == TypeIds.T_void) {
                        ...
                        for (int j = 0, length = thrownExceptions.length; j < length; j++) {
                            handlingContext.checkExceptionHandlers(thrownExceptions[j], this.resources[i], tryInfo,
                                                                   currentScope, true);
                        }
                    }
                }
            ...
            if (this.catchArguments != null) {
                ...
                for (int i = 0; i < catchCount; i++) {
                    ...
                    if (isUncheckedCatchBlock(i)) {
                        catchInfo = handlingContext.initsOnFinally.mitigateNullInfoOf(flowInfo.unconditionalCopy().
                        ...
                        catchInfo = flowInfo.nullInfoLessUnconditionalCopy().addPotentialInitializationsFrom(
                                initsOnException).addNullInfoFrom(
                                initsOnException)   // null info only from here, this is the only way to enter the catch block
                                .addPotentialInitializationsFrom(
                                        tryInfo.nullInfoLessUnconditionalCopy()).addPotentialInitializationsFrom(
            ...
            subInfo = this.finallyBlock.analyseCode(currentScope,
            ...
            finallyContext.complainOnDeferredChecks(handlingContext.initsOnFinally.mitigateNullInfoOf(
                    (tryInfo.tagBits & FlowInfo.UNREACHABLE) == 0 ? flowInfo.unconditionalCopy().
                            addPotentialInitializationsFrom(tryInfo).
                                                                                    // lighten the influence of the try block,
                                                                                    // which may have
                                                                                            // exited at any point
                                                                                            addPotentialInitializationsFrom(
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        if (maxCatches > 0) {
            ...
            for (int i = 0; i < maxCatches; i++) {
                ...
                if ((argument.binding.tagBits & TagBits.MultiCatchParameter) != 0) {
                    MultiCatchExceptionLabel multiCatchExceptionLabel = new MultiCatchExceptionLabel(codeStream,
        ...
        try {
            ...
            if (resourceCount > 0) {
                for (int i = resourceCount; i >= 0; i--) {
                    ...
                    this.resourceExceptionLabels[i]
                            .placeEnd(); // outer handler if any is the one that should catch exceptions out of close()
		...
		*/
            ...
            ExceptionLabel naturalExitExceptionHandler = requiresNaturalExit && (finallyMode == FINALLY_SUBROUTINE) ? new ExceptionLabel(
                    codeStream, null) : null;
        ...
        if (targetLocation != null) {
                ...
                for (int i = 0, count = this.reusableJSRTargetsCount; i < count; i++) {
                    ...
                    differentTarget:
                    {
                        ...
                        if (targetLocation instanceof Constant && reusableJSRTarget instanceof Constant &&
                            ((Constant)targetLocation).hasSameValue(
                                    (Constant)reusableJSRTarget)) {
                            break differentTarget;
                        }
            ...
            if (reuseTargetLocation) {
                if (this.reusableJSRTargetsCount == this.reusableJSRTargets.length) {
                    System.arraycopy(this.reusableJSRTargets, 0,
                                     this.reusableJSRTargets = new Object[2 * this.reusableJSRTargetsCount], 0,
                                     this.reusableJSRTargetsCount);
                    System.arraycopy(this.reusableJSRSequenceStartLabels, 0,
                                     this.reusableJSRSequenceStartLabels = new BranchLabel[2 * this.reusableJSRTargetsCount], 0,
                                     this.reusableJSRTargetsCount);
                    System.arraycopy(this.reusableJSRStateIndexes, 0,
                                     this.reusableJSRStateIndexes = new int[2 * this.reusableJSRTargetsCount], 0,
                                     this.reusableJSRTargetsCount);
                }
    ...
    public void resolve(BlockScope upperScope) {
        ...
        if (resourceCount > 0) {
            ...
            this.primaryExceptionVariable = new LocalVariableBinding(TryStatement.SECRET_PRIMARY_EXCEPTION_VARIABLE_NAME,
                                                                     this.scope.getJavaLangThrowable(), ClassFileConstants.AccDefault,
                                                                     false);
            ...
            if (localVariableBinding != null && localVariableBinding.isValidBinding()) {
                ...
                if (resourceType instanceof ReferenceBinding) {
                    if (resourceType
                        ...
                        null && resourceType.isValidBinding()) {
                        upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType,
            ...
            if (this.finallyBlock.isEmptyBlock()) {
                ...
                if (!upperScope.compilerOptions().inlineJsrBytecode) {
                    this.returnAddressVariable = new LocalVariableBinding(TryStatement.SECRET_RETURN_ADDRESS_NAME,
                                                                          upperScope.getJavaLangObject(), ClassFileConstants.AccDefault,
                                                                          false);
