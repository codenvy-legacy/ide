   ...
   public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo)
   {
      ...
      if (this.subRoutineStartLabel == null)
      {
         ...
         ExceptionHandlingFlowContext handlingContext = new ExceptionHandlingFlowContext(flowContext, this,
            this.caughtExceptionTypes, this.caughtExceptionsCatchBlocks, this.catchArguments, null, this.scope,
         ...
         finallyContext.complainOnDeferredChecks(handlingContext.initsOnFinally.mitigateNullInfoOf(
            (tryInfo.tagBits & FlowInfo.UNREACHABLE) == 0 ? flowInfo.unconditionalCopy().
               ...
               // lighten the influence of the try block, which may have
                  // exited at any point
                  addPotentialInitializationsFrom(insideSubContext.initsOnReturn) : insideSubContext.initsOnReturn),
            currentScope);
         ...
         if (this.reusableJSRTargetsCount > 0)
         {
            ...
            for (int i = 0, count = this.reusableJSRTargetsCount; i < count; i++)
            {
               ...
               differentTarget:
               {
                  ...
                  if (targetLocation instanceof Constant && reusableJSRTarget instanceof Constant && ((Constant)targetLocation).hasSameValue(
   ...
   public void resolve(BlockScope upperScope)
   {
      ...
      for (int i = 0; i < resourceCount; i++)
      {
         ...
         if (localVariableBinding != null && localVariableBinding.isValidBinding())
         {
            ...
            { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=349862, avoid secondary error in problematic null case
