...
public class ParameterizedQualifiedTypeReference extends ArrayQualifiedTypeReference {
    ...
    /* (non-Javadoc)
      * @see org.eclipse.jdt.internal.compiler.ast.ArrayQualifiedTypeReference#getTypeBinding(org.eclipse.jdt.internal.compiler.lookup
      * .Scope)
      */
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length; i < max; i++) {
                ...
                if (typeIsConsistent && currentType.isStatic() &&
                    ...
                    scope.problemReporter().staticMemberOfParameterizedType(this,
                                                                            scope.environment().createParameterizedType(
                                                                                    (ReferenceBinding)currentType.erasure(), null,
                                                                                    qualifyingType), i);
            ...
            if (args != null) {
                ...
                if (typeVariables == Binding.NO_TYPE_VARIABLES) { // check generic
                    ...
                    this.resolvedType =
                            (qualifyingType != null && qualifyingType.isParameterizedType()) ? scope.environment().createParameterizedType(
