...
public class MethodVerifier {
    ...
    */
    MethodVerifier(LookupEnvironment environment) {
        ...
        this.allowCompatibleReturnTypes = environment.globalOptions.complianceLevel >= ClassFileConstants.JDK1_5 &&
    ...
    void checkAgainstInheritedMethods(MethodBinding currentMethod, MethodBinding[] methods, int length,
        ...
        for (int i = length; --i >= 0; ) {
            ...
            if (overriddenInheritedMethods == null || overriddenInheritedMethods[i] == 0) {
                ...
                if (!areReturnTypesCompatible(currentMethod,
                                              inheritedMethod) && (currentMethod.returnType.tagBits & TagBits.HasMissingType) == 0) {
        ...
        if (concreteMethod == null) {
            ...
            if (mustImplementAbstractMethod(bestAbstractMethod.declaringClass)) {
                ...
                if (superclassAbstractMethod == bestAbstractMethod || superclassAbstractMethod.declaringClass.isInterface()) {
                    if (typeDeclaration != null) {
                        MethodDeclaration missingAbstractMethod = typeDeclaration.addMissingAbstractMethodFor(
                                bestAbstractMethod);
    ...
    boolean checkInheritedReturnTypes(MethodBinding method, MethodBinding otherMethod) {
        ...
        if (!this.type.isInterface()) {
            if (method.declaringClass.isClass() || !this.type.implementsInterface(method.declaringClass, false)) {
                if (otherMethod.declaringClass.isClass() || !this.type.implementsInterface(otherMethod.declaringClass,
                                                                                           false)) {
    ...
    */
                ...
                for (int i = 0, length1 = current.length; i < length1; i++) {
                    ...
                    if (index >= 0) {
                        checkAgainstInheritedMethods(currentMethod, matchingInherited, index + 1,
                                                     inherited); // pass in the length of matching
                        while (index >= 0) {
    ...
    void computeInheritedMethods() {
        ReferenceBinding superclass =
                this.type.isInterface() ? this.type.scope.getJavaLangObject() // check interface methods against Object
    ...
    */
    void computeInheritedMethods(ReferenceBinding superclass, ReferenceBinding[] superInterfaces) {
        ...
        this.inheritedMethods = new HashtableOfObject(
                51); // maps method selectors to an array of methods... must search to match paramaters & return type
        ...
        ReferenceBinding superType = superclass;
        HashtableOfObject nonVisibleDefaultMethods = new HashtableOfObject(
                3); // maps method selectors to an array of methods
            ...
            for (int m = methods.length; --m >= 0; ) {
                ...
                if (existingMethods != null) {
                    ...
                    for (int i = 0, length = existingMethods.length; i < length; i++) {
                        ...
                        if (existingMethod.declaringClass != inheritedMethod.declaringClass && areMethodsCompatible(
                                existingMethod, inheritedMethod) && !canOverridingMethodDifferInErasure(existingMethod,
                                                                                                        inheritedMethod)) {
                            if (inheritedMethod.isDefault()) {
                                if (inheritedMethod.isAbstract()) {
                                    ...
                                    if (this.type.fPackage == inheritedMethod.declaringClass.fPackage && !areReturnTypesCompatible(
                                            inheritedMethod, existingMethod)) {
                ...
                if (!inheritedMethod.isDefault() || inheritedMethod.declaringClass.fPackage == this.type.fPackage) {
                    ...
                    if (inheritedMethod.isAbstract() &&
                        !this.type.isAbstract()) // non visible abstract methods cannot be overridden so the type must be defined abstract
                    {
                ...
                for (int m = methods.length; --m >= 0; ) { // Interface methods are all abstract public
                    ...
                    if (existingMethods == null) {
                        ...
                        for (int e = 0; e < length; e++) {
                            if (isInterfaceMethodImplemented(inheritedMethod, existingMethods[e],
                                                             superType) &&
    ...
    void computeMethods() {
        ...
        this.currentMethods = new HashtableOfObject(
                size == 0 ? 1 : size); // maps method selectors to an array of methods... must search to match paramaters & return type
        for (int m = size; --m >= 0; ) {
            MethodBinding method = methods[m];
    ...
    boolean isInterfaceMethodImplemented(MethodBinding inheritedMethod, MethodBinding existingMethod,
        ...
        return areParametersEqual(existingMethod, inheritedMethod) && existingMethod.declaringClass.implementsInterface(
                superType, true);
    }

    public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
        return com.codenvy.eclipse.jdt.core.compiler.CharOperation.equals(method.selector,
    ...
    boolean mustImplementAbstractMethod(ReferenceBinding declaringClass) {
        ...
        if (declaringClass.isClass()) {
            ...
            if (this.type.implementsInterface(declaringClass, false)) {
                if (!superclass.implementsInterface(declaringClass,
                                                    true)) // only if a superclass does not also implement the interface
                {
                    return true;
                }
            }
    ...
    ProblemReporter problemReporter(MethodBinding currentMethod) {
        ...
        if (currentMethod.declaringClass == this.type &&
            currentMethod.sourceMethod() != null)   // only report against the currentMethod if its implemented by the type
        {
