...
import com.codenvy.eclipse.jdt.internal.compiler.util.HashtableOfIntValues;
...
import com.codenvy.eclipse.jdt.internal.compiler.util.SimpleSetOfCharArray;
import com.codenvy.eclipse.jdt.internal.core.util.Messages;
import com.codenvy.eclipse.jdt.internal.core.util.SimpleWordSet;
...
public class DiskIndex {
    ...
    private HashtableOfObject addQueryResult(HashtableOfObject results, char[] word, Object docs,
                                             MemoryIndex memoryIndex, boolean prevResults) throws IOException {
        // must skip over documents which have been added/changed/deleted in the memory index
        if (results == null) {
    ...
    HashtableOfObject addQueryResults(char[][] categories, char[] key, int matchRule,
        ...
        if (key == null) {
            for (int i = 0, l = categories.length; i < l; i++) {
                HashtableOfObject wordsToDocNumbers = readCategoryTable(categories[i],
                                                                        true); // cache if key is null since its a definite match
    ...
    private String[] computeDocumentNames(String[] onDiskNames, int[] positions, SimpleLookupTable indexedDocuments,
        ...
        for (int i = 0, l = docNames.length; i < l; i++) {
            ...
            if (docName != null) {
                ...
                if (referenceTables[i] != null) {
                    indexedDocuments.put(docName,
                                         null); // remember each new document, skip deleted documents which were never saved
                }
            }
        }
            ...
            switch (positions[i]) {
                ...
                case RE_INDEXED:
                    ...
                    if (newName.equals(onDiskNames[i])) {
                        indexedDocuments.put(newName,
                                             new Integer(count)); // the reindexed docName that was at position i is now at position count
                        i++;
                    }
                    break;
                default:
            ...
            if (reuseExistingFile) {
                ...
                try {
                    ...
                    if (this.headerInfoOffset > 0) { // file is empty if its not set
                        stream.skip(
                                this.headerInfoOffset - this.bufferEnd); // assume that the header info offset is over current buffer end
                        this.bufferIndex = 0;
                        this.bufferEnd = stream.read(this.streamBuffer, 0, this.streamBuffer.length);
    ...
    private void mergeCategory(char[] categoryName, DiskIndex onDisk, int[] positions,
    ...
    DiskIndex mergeWith(MemoryIndex memoryIndex) throws IOException {
        ...
        DiskIndex newDiskIndex = new DiskIndex(
                new FileIndexLocation(new File(oldIndexFile.getPath() + ".tmp"))); //$NON-NLS-1$
        ...
        try {
            ...
            try {
                ...
                if (indexedDocuments.elementSize > 0) {
                    ...
                    for (int i = 0, l = names.length; i < l; i++) {
                        if (names[i] != null) {
                            newDiskIndex.copyQueryResults((HashtableOfObject)memoryIndex.docsToReferences.get(names[i]),
            ...
            if (newIndexFile.exists() && !newIndexFile.delete()) {
                if (DEBUG) {
                    System.out.println(
                            "mergeWith - Failed to delete temp index " + newDiskIndex.indexLocation); //$NON-NLS-1$
                }
            }
            throw e;
        }
    ...
    private synchronized HashtableOfObject readCategoryTable(char[] categoryName,
                                                             boolean readDocNumbers) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (arrayOffset <= 0) {
                    categoryTable.putUnsafely(word,
                                              new int[]{-arrayOffset}); // store 1 element array by negating documentNumber
                } else if (arrayOffset < largeArraySize) {
                    categoryTable.putUnsafely(word,
                                              readStreamDocumentArray(stream, arrayOffset)); // read in-lined array providing size
                } else {
     ...
     * @throws EOFException
     *         if this end of data input is reached while reading it.
     * @throws IOException
     *         if an I/O error occurs while reading data input.
    ...
    private void writeAllDocumentNames(String[] sortedDocNames, FileOutputStream stream) throws IOException {
        ...
        for (int i = 0; i < this.numberOfChunks; i++) {
            ...
            for (int j = 1; j < chunkSize; j++) {
                ...
                writeStreamChars(stream,
                                 (start < last ? CharOperation.subarray(next.toCharArray(), start, last) : CharOperation.NO_CHAR));
                current = next;
            }
        }
    ...
    private void writeCategoryTable(char[] categoryName, HashtableOfObject wordsToDocs,
                                    FileOutputStream stream) throws IOException {
            ...
            if (o != null) {
                ...
                if (o instanceof int[]) {
                    ...
                    if (documentNumbers.length == 1) {
                        writeStreamInt(stream,
                                       -documentNumbers[0]); // store an array of 1 element by negating the documentNumber (can be zero)
                    } else {
                        writeDocumentNumbers(documentNumbers, stream);
                    }
                } else {
     ...
     * @throws IOException
     *         if an I/O error occurs while writting
