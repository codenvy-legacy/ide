...
public class ConstructorPattern extends JavaSearchPattern {
    ...
    public ConstructorPattern(
        ...
        this.declaringSimpleName =
                (this.isCaseSensitive || this.isCamelCase) ? declaringSimpleName : CharOperation.toLowerCase(declaringSimpleName);
        ...
        switch (getMatchMode()) {
            ...
            case R_PATTERN_MATCH:
                if (this.parameterCount >= 0 && !this.varargs) {
                    key = CharOperation.concat(createIndexKey(this.declaringSimpleName == null ? ONE_STAR : this.declaringSimpleName,
                                                              this.parameterCount), ONE_STAR);
    ...
    protected StringBuffer print(StringBuffer output) {
        ...
        if (this.parameterSimpleNames == null) {
            ...
            for (int i = 0, max = this.parameterSimpleNames.length; i < max; i++) {
                ...
                if (this.parameterSimpleNames[i] == null) output.append('*');
                else output.append(this.parameterSimpleNames[i]);
            }
        }
        output.append(')');
