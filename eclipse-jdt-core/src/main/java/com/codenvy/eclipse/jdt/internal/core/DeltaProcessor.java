...
public class DeltaProcessor
{
   ...
   private void checkProjectsAndClasspathChanges(IResourceDelta delta)
   {
      ...
      switch (resource.getType())
      {
         ...
         case IResource.FOLDER:
            ...
            { // look for .jar file change to update classpath
               children = delta.getAffectedChildren();
            }
            break;
         case IResource.FILE:
      ...
      switch (elementType)
      {
         ...
         case IJavaElement.JAVA_PROJECT:
            ...
            if (resource instanceof IProject)
            {
               ...
               if (this.currentElement != null && this.currentElement.getElementType() == IJavaElement.JAVA_PROJECT && ((IJavaProject)this.currentElement).getProject().equals(
                  resource))
               {
      ...
      try
      {
         ...
         else
         {
            ...
            this.oldRoots.put(javaProject,
               javaProject.computePackageFragmentRoots(javaProject.getResolvedClasspath(), false, null /*no reverse map*/));
         }
   ...
   private void elementAdded(Openable element, IResourceDelta delta, RootInfo rootInfo)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            Openable movedFromElement = elementType != IJavaElement.JAVA_PROJECT && movedFromType == IJavaElement.JAVA_PROJECT ? null : // outside classpath
   ...
   private void elementRemoved(Openable element, IResourceDelta delta, RootInfo rootInfo)
   {
      ...
      else
      {
         ...
         Openable movedToElement = elementType != IJavaElement.JAVA_PROJECT && movedToType == IJavaElement.JAVA_PROJECT ? null : // outside classpath
                  ...
                  if (elementType == IJavaElement.JAVA_PROJECT && res instanceof IFile)
                  {
                     if (sourceLevel == null)
                     {
                        ...
                        javaProject = rootInfo == null ? (JavaProject)createElement(res.getProject(),
                           IJavaElement.JAVA_PROJECT, null) : rootInfo.project;
                        if (javaProject != null)
                        {
   ...
   private IJavaElementDelta processResourceDelta(IResourceDelta changes)
   {
      ...
      try
      {
         ...
         for (int i = 0; i < deltas.length; i++)
         {
            ...
            if (elementType == NON_JAVA_RESOURCE || (wasJavaProject != isJavaProject && (delta.getKind()) == IResourceDelta.CHANGED))
   ...
   private void traverseDelta(IResourceDelta delta, int elementType, RootInfo rootInfo, OutputsInfo outputsInfo)
   {
      ...
      if (processChildren)
      {
                  ...
                  { // if inside a package fragment root
                     ...
                     if (parent == null)
                     {
                        ...
                        if (this.currentElement == null || !rootInfo.project.equals(
                           this.currentElement.getJavaProject()))
         ...
         { // non-java resource directly under a project
            ...
            if (adoptiveProject != null && JavaProject.hasJavaNature(rscProject))
      ...
      switch (resource.getType())
      {
         ...
         case IResource.FILE:
            ...
            if (fileName.equals(JavaProject.CLASSPATH_FILENAME) || ((rootInfo = rootInfo(file.getFullPath(),
               delta.getKind())) != null && rootInfo.entryKind == IClasspathEntry.CPE_LIBRARY))
            {
