...
import com.codenvy.eclipse.jdt.core.ElementChangedEvent;
import com.codenvy.eclipse.jdt.core.IClasspathEntry;
import com.codenvy.eclipse.jdt.core.IElementChangedListener;
import com.codenvy.eclipse.jdt.core.IJavaElement;
import com.codenvy.eclipse.jdt.core.IJavaElementDelta;
...
public class DeltaProcessor {
        ...
        switch (resource.getType()) {
            ...
            case IResource.FILE:
               ...
               /* classpath file change */
                    ...
                    switch (kind) {
                        case IResourceDelta.CHANGED:
                            ...
                            if ((flags & IResourceDelta.CONTENT) == 0  // only consider content change
                                ...
                                && (flags & IResourceDelta.MOVED_FROM) ==
                                   0) {// and also move and overide scenario (see http://dev.eclipse.org/bugs/show_bug.cgi?id=21420)
                                break;
                            }
                            //$FALL-THROUGH$
                        case IResourceDelta.ADDED:
                    ...
                    this.state.rootsAreStale = true;
                } else if ((rootInfo = rootInfo(file.getFullPath(),
                                                kind)) != null && rootInfo.entryKind == IClasspathEntry.CPE_LIBRARY) {
                    javaProject = (JavaProject)JavaCore.create(file.getProject());
        ...
        switch (elementType) {
            ...
            case IJavaElement.JAVA_PROJECT:
                ...
                if (resource instanceof IProject) {
                    ...
                    if (this.currentElement != null && this.currentElement.getElementType() == IJavaElement.JAVA_PROJECT &&
                        ((IJavaProject)this.currentElement).getProject().equals(
                                resource)) {
            ...
            case IJavaElement.CLASS_FILE:
                ...
                if (this.currentElement == null) {
                    element = rootInfo == null ? JavaCore.create(resource) : JavaModelManager.create(resource,
                                                                                                     rootInfo.project);
                } else {
                    // find the package
    ...
    */
    private boolean createExternalArchiveDelta(HashSet refreshedElements, IProgressMonitor monitor) {
                ...
                if (entries[j].getEntryKind() == IClasspathEntry.CPE_LIBRARY) {
                    ...
                    if (status == null) {
                        ...
                        if (targetLibrary == null) { // missing JAR
                            if (this.state.getExternalLibTimeStamps().remove(
                                    entryPath) != null /* file was known*/ && this.state.roots.get(
                                    entryPath) != null /* and it was on the classpath*/) {
                            ...
                            if (oldTimestamp != null) {
                                ...
                                if (newTimeStamp == 0) { // file doesn't exist
                                    ...
                                    this.manager.indexManager.indexLibrary(entryPath, project.getProject(),
                                                                           ((ClasspathEntry)entries[j]).getLibraryIndexLocation());
                                } else {
                    ...
                    if (status != null) {
                        if (status == EXTERNAL_JAR_ADDED) {
                            PackageFragmentRoot root = (PackageFragmentRoot)javaProject.getPackageFragmentRoot(
                                    entryPath.toString());
                            if (VERBOSE) {
                                System.out.println(
                                        "- External JAR ADDED, affecting root: " + root.getElementName()); //$NON-NLS-1$
                            }
                            elementAdded(root, null, null);
                            deltaContainsModifiedJar = true;
                            this.state.addClasspathValidation(
                                    javaProject); // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=185733
                            hasDelta = true;
                            ...
                            if (VERBOSE) {
                                System.out.println(
                                        "- External JAR CHANGED, affecting root: " + root.getElementName()); //$NON-NLS-1$
                            }
                            contentChanged(root);
                            deltaContainsModifiedJar = true;
                            hasDelta = true;
                            ...
                            if (VERBOSE) {
                                System.out.println(
                                        "- External JAR REMOVED, affecting root: " + root.getElementName()); //$NON-NLS-1$
                            }
        ...
        // ensure the external file cache is reset so that if a .jar file is deleted but no longer on the classpath,
        // it won't appear as changed next time it is added
    ...
    private void deleting(IProject project) {
        ...
        try {
            ...
            if (javaProject.isOpen()) {
                ...
                this.oldRoots.put(javaProject,
                                  javaProject
                                          .computePackageFragmentRoots(javaProject.getResolvedClasspath(), false, null /*no reverse map*/));
            }
        ...
        if (elementType == IJavaElement.JAVA_PROJECT) {
            ...
            if (delta != null && JavaProject.hasJavaNature(project = (IProject)delta.getResource())) {
                ...
                if ((delta.getFlags() & IResourceDelta.MOVED_FROM) != 0) {
                    Openable movedFromElement = (Openable)element.getJavaModel().getJavaProject(
            ...
            if (delta == null || (delta.getFlags() & IResourceDelta.MOVED_FROM) == 0) {
                ...
                int movedFromType = elementType(movedFromRes, IResourceDelta.REMOVED, element.getParent().getElementType(),
                                                movedFromInfo);
                ...
                Openable movedFromElement =
                        elementType != IJavaElement.JAVA_PROJECT && movedFromType == IJavaElement.JAVA_PROJECT ? null : // outside classpath
    ...
    private void elementRemoved(Openable element, IResourceDelta delta, RootInfo rootInfo) {
        ...
        if (delta == null || (delta.getFlags() & IResourceDelta.MOVED_TO) == 0) {
            ...
            Openable movedToElement =
                    elementType != IJavaElement.JAVA_PROJECT && movedToType == IJavaElement.JAVA_PROJECT ? null : // outside classpath
        ...
        switch (parentType) {
            ...
            case IJavaElement.PACKAGE_FRAGMENT:
                ...
                if (res.getType() == IResource.FOLDER) {
                    if (parentType == NON_JAVA_RESOURCE && !Util.isExcluded(res.getParent(), rootInfo.inclusionPatterns,
                    ...
                    String sourceLevel = rootInfo.project == null ? null : rootInfo.project.getOption(
                            JavaCore.COMPILER_SOURCE, true);
                    String complianceLevel = rootInfo.project == null ? null : rootInfo.project.getOption(
                ...
                String sourceLevel = rootInfo.project == null ? null : rootInfo.project.getOption(JavaCore.COMPILER_SOURCE,
                                                                                                  true);
                String complianceLevel = rootInfo.project == null ? null : rootInfo.project.getOption(
                        JavaCore.COMPILER_COMPLIANCE, true);
                if (Util.isValidCompilationUnitName(fileName, sourceLevel, complianceLevel)) {
                    ...
                    if ((rootInfo = rootInfo(rootPath,
                                             kind)) != null && rootInfo.project.getProject().getFullPath().isPrefixOf(
                    ...
                    if (info.traverseModes[i] != IGNORE) {
                        ...
                        if (elementType == IJavaElement.JAVA_PROJECT && res instanceof IFile) {
                            if (sourceLevel == null) {
                                ...
                                javaProject = rootInfo == null ? (JavaProject)createElement(res.getProject(),
                                                                                            IJavaElement.JAVA_PROJECT, null)
                                                               : rootInfo.project;
                                if (javaProject != null) {
        ...
        if (element.isOpen()) {
            ...
            switch (element.getElementType()) {
                ...
                case IJavaElement.JAVA_PROJECT:
                    ...
                    PackageFragmentRoot projectRoot = (PackageFragmentRoot)project.getPackageFragmentRoot(
                            project.getProject());
    ...
    */
    ...
    private OutputsInfo outputsInfo(RootInfo rootInfo, IResource res) {
        try {
            JavaProject proj = rootInfo == null ? (JavaProject)createElement(res.getProject(), IJavaElement.JAVA_PROJECT,
                                                                             null) : rootInfo.project;
            if (proj != null) {
    ...
    private void popUntilPrefixOf(IPath path) {
        while (this.currentElement != null) {
            ...
            if (currentElementPath != null) {
                if (this.currentElement instanceof IPackageFragment && ((IPackageFragment)this.currentElement).isDefaultPackage() &&
    ...
    private IJavaElementDelta processResourceDelta(IResourceDelta changes) {
        ...
        try {
            ...
            IResourceDelta[] deltas = changes.getAffectedChildren(
                    IResourceDelta.ADDED | IResourceDelta.REMOVED | IResourceDelta.CHANGED, IContainer.INCLUDE_HIDDEN);
            for (int i = 0; i < deltas.length; i++) {
                ...
                if (elementType == NON_JAVA_RESOURCE || (wasJavaProject != isJavaProject && (delta.getKind()) ==
                                                                                            IResourceDelta.CHANGED)) { // project has changed nature (description or open/closed)
                    try {
            ...
            case IResourceChangeEvent.PRE_BUILD:
                ...
                if (folderChanges != null) {
                    for (int i = 0, length = folderChanges.length; i < length; i++) {
                        try {
                            folderChanges[i].updateExternalFoldersIfNecessary(false/*do not refresh since we are not in the thread that added the external folder to the classpath*/,
                                                                              null);
                        } catch (JavaModelException e) {
                            if (!e.isDoesNotExist()) {
    ...
    private void traverseDelta(IResourceDelta delta, int elementType, RootInfo rootInfo, OutputsInfo outputsInfo) {
        ...
        if (res instanceof IProject) {
            ...
            processChildren = updateCurrentDeltaAndIndex(delta,
                                                         elementType == IJavaElement.PACKAGE_FRAGMENT_ROOT ? IJavaElement.JAVA_PROJECT :
                                                         // case of prj=src
        ...
        if (processChildren) {
            ...
            for (int i = 0; i < length; i++) {
                    ...
                    if (childType == NON_JAVA_RESOURCE) {
                        if (rootInfo != null) { // if inside a package fragment root
                            ...
                            if (parent == null) {
                                ...
                                if (this.currentElement == null || !rootInfo.project.equals(
                                        this.currentElement
                                                .getJavaProject())) { // note if currentElement is the IJavaModel, getJavaProject() is null
                ...
                if ((rootList = otherRootsInfo(childPath, childKind)) != null) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        this.currentElement =
                                null; // ensure that 2 roots refering to the same resource don't share the current element (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=210746 )
                        traverseDelta(child, IJavaElement.PACKAGE_FRAGMENT_ROOT, originalChildRootInfo,
                                      null); // binary output of childRootInfo.project cannot be this root
                    }
                }
            ...
            if (orphanChildren != null && (oneChildOnClasspath // orphan children are siblings of a package fragment root
                ...
                if (adoptiveProject != null && JavaProject.hasJavaNature(rscProject)) { // delta iff Java project (18698)
                    for (int i = 0; i < length; i++) {
    ...
    private void validateClasspaths(IResourceDelta delta, HashSet affectedProjects) {
        ...
        switch (resource.getType()) {
            ...
            /* check classpath or prefs files change */
                ...
                if (fileName.equals(JavaProject.CLASSPATH_FILENAME) || ((rootInfo = rootInfo(file.getFullPath(),
                                                                                             delta.getKind())) != null &&
                                                                        rootInfo.entryKind == IClasspathEntry.CPE_LIBRARY)) {
                    ...
                    if ((flags & IResourceDelta.DESCRIPTION) != 0) {
                        ...
                        if (wasJavaProject != isJavaProject) {
                            ...
                            if (isJavaProject) {
                                ...
                                if (JavaBuilder.DEBUG) {
                                    System.out.println(
                                            "Clearing last state for project loosing Java nature: " + res); //$NON-NLS-1$
                                }
                    ...
                    case IResourceDelta.REMOVED:
                        ...
                        for (int i = 0, length = children.length; i < length; i++) {
                            ...
                            if (resource instanceof IFile) {
                                ...
                                } else if (com.codenvy.eclipse.jdt.internal.compiler.util.Util.isClassFileName(name)) {
