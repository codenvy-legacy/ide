   ...
   private void renameApplication()
   {
      ...
      try
      {
         AppfogClientService.getInstance().renameApplication(vfs.getId(), projectId, applicationName, null,
            newName, new AppfogAsyncRequestCallback<String>(null, renameAppLoggedInHandler, null)
         {
            @Override
            protected void onSuccess(String result)
            {
               closeView();
               IDE.fireEvent(new OutputEvent(AppfogExtension.LOCALIZATION_CONSTANT.renameApplicationSuccess(
