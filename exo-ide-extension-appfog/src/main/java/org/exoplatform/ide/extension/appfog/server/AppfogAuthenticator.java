 ...
 * @version $Id: $
 */
public class AppfogAuthenticator extends BaseAppfogAuthenticator
{
   ...
   public AppfogAuthenticator(VirtualFileSystemRegistry vfsRegistry, InitParams initParams)
   {
      this(vfsRegistry, readValueParam(initParams, "workspace"), readValueParam(initParams, "user-config"));
   }

   protected AppfogAuthenticator(VirtualFileSystemRegistry vfsRegistry, String workspace, String config)
   {
   ...
   public String getTarget() throws VirtualFileSystemException, IOException
   {
      ...
      String path = config + user + "/appfog/af_target";
   ...
   public AppfogCredentials readCredentials() throws VirtualFileSystemException, IOException
   {
      VirtualFileSystem vfs = vfsRegistry.getProvider(workspace).newInstance(null, null);
      String user = ConversationState.getCurrent().getIdentity().getUserId();
      String path = config + user + "/appfog/af_token";
   ...
   {
      VirtualFileSystem vfs = vfsRegistry.getProvider(workspace).newInstance(null, null);
      writeFile(vfs, getConfigParent(vfs), "af_target", target);
   }
   ...
   public void writeCredentials(AppfogCredentials credentials) throws VirtualFileSystemException, IOException
   {
      ...
      writeFile(vfs, getConfigParent(vfs), "af_token", w.toString());
   }

   private Folder getConfigParent(VirtualFileSystem vfs) throws VirtualFileSystemException
   {
      String user = ConversationState.getCurrent().getIdentity().getUserId();
      String appfogPath = config + user + "/appfog";
      VirtualFileSystemInfo info = vfs.getInfo();
      Folder appfog;
      try
      {
         Item item = vfs.getItemByPath(appfogPath, null, PropertyFilter.NONE_FILTER);
         if (ItemType.FOLDER != item.getItemType())
         {
            throw new RuntimeException("Item " + appfogPath + " is not a Folder. ");
         }
         appfog = (Folder)item;
      }
      catch (ItemNotFoundException e)
      {
         appfog = vfs.createFolder(info.getRoot().getId(), appfogPath.substring(1));
      }
      return appfog;
   }
