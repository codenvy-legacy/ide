...
public class AndroidRunnerService {
    ...
    public String run(@QueryParam("apk") URL apk) throws Exception {
        final ManymoApplication manymoApplication = uploadApplication(apk);
        return "{\"applicationUrl\":\"" + "https://www.manymo.com/apps/" + manymoApplication.getId() + "/emulators/83/connect" + "\"}";
    }

    private ManymoApplication uploadApplication(URL apk) throws Exception {
        HttpURLConnection http = null;
        java.io.File path = null;
        try {
            path = FileUtils.downloadFile(null, "app-", ".apk", apk);
            FileUtils.downloadFile(null, "app-", ".apk", apk);
            http = (HttpURLConnection)new URL("https://www.manymo.com/api/v1/apps").openConnection();
            http.setInstanceFollowRedirects(false);
            http.setRequestMethod("POST");
            http.setRequestProperty("Authorization", "OAuth token=" + OAUTH_TOKEN);
