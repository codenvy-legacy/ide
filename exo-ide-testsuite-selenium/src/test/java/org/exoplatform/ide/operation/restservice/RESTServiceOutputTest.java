...
public class RESTServiceOutputTest extends BaseTest {
	...
	private final static String PROJECT = RESTServiceOutputTest.class
			.getSimpleName();
	...
	public void testOutput() throws Exception {
		...
		checkFields("/overralTest/Inner/node/{paramList: .+}", "POST",
				"text/plain", "application/xml;charset=utf-8");
		...
		checkFields("/overralTest", "OPTIONS", "",
		...
		checkFields("/overralTest/Inner/node/param1", "POST", "text/plain",
		...
		assertTrue(mess
				.contains("Param List 1:param1; Test Query Parameter 1: ; Test-Header 1: test; Body:"));
		...
		IDE.REST_SERVICE
				.typeToBodyField("Ð¿Ñ€Ð¸ÐºÐ»Ð°Ð´ Ð¿Ð¾Ð²Ñ–Ð´Ð¾Ð¼Ð»ÐµÐ½Ð½Ñ�");
		IDE.REST_SERVICE.sendRequest();
		...
		IDE.REST_SERVICE.launchRestService();
		IDE.REST_SERVICE
				.typeToPathField("/overralTest/Inner/node/param1/param2/param3");
	...
	private void checkFields(String path, String method, String request,
			String response) {
	...
	private void checkQueryParameter(int parameterIndex, String name,
			String type, String defaultValue, String value) {
		assertEquals(name,
				IDE.REST_SERVICE.getQueryParameterName(parameterIndex));
		assertEquals(type,
				IDE.REST_SERVICE.getQueryParameterType(parameterIndex));
		assertEquals(defaultValue,
				IDE.REST_SERVICE.getQueryParameterDefaultValue(parameterIndex));
		assertEquals(value,
				IDE.REST_SERVICE.getQueryParameterValue(parameterIndex));
	}

	private void checkHeaderParameter(int parameterIndex, String name,
			String type, String defaultValue, String value) {
		assertEquals(name,
				IDE.REST_SERVICE.getHeaderParameterName(parameterIndex));
		assertEquals(type,
				IDE.REST_SERVICE.getHeaderParameterType(parameterIndex));
		assertEquals(defaultValue,
				IDE.REST_SERVICE.getHeaderParameterDefaultValue(parameterIndex));
		assertEquals(value,
				IDE.REST_SERVICE.getHeaderParameterValue(parameterIndex));
	}
