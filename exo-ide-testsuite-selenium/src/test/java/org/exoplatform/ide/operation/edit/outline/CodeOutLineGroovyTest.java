...
public class CodeOutLineGroovyTest extends CodeAssistantBaseTest
{
   ...
   public static void setUp()
   {
      try
      {
         createProject(CodeOutLineGroovyTest.class.getSimpleName());
         VirtualFileSystemUtils.createFileFromLocal(project.get(Link.REL_CREATE_FILE), FILE_NAME,
            MimeType.APPLICATION_GROOVY, "src/test/resources/org/exoplatform/ide/operation/edit/outline/" + FILE_NAME);
      }
      catch (Exception e)
      {
      }
   ...
   public void testCodeOutLineGroovy() throws Exception
   {
      ...
      outlineTreeHelper.addOutlineItem("g : String", 47, false, TokenType.PROPERTY); // false, because outline node is not
                                                                                     // highlighted from test, but highlighted
                                                                                     // when goto this line manually
      ...
      outlineTreeHelper.addOutlineItem("addVar1 : List<Tree>", 78, false, TokenType.VARIABLE); // false, because outline node is
                                                                                               // not highlighted from test, but
                                                                                               // highlighted when goto this line
                                                                                               // manually
