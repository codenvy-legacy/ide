...
//      builder.append("  }\n");
    ...
    private void emitSerializer(List<Method> methods, StringBuilder builder) {
        ...
        if (isCompactJson()) {
            builder.append("      Array result = new Array();\n");
            for (Method method : methods) {
    ...
    private void emitSerializeFieldForMethod(Method method, final StringBuilder builder) {
        if (method.getName().equals("getType")) {
            ...
            builder.append("      result.add(\"" + typeFieldName + "\", new JsonPrimitive(getType()));\n");
            return;
        }
     ...
     *         indentation string
     */
    private void emitSerializerImpl(List<Type> expandedTypes, int depth, StringBuilder builder, String inVar,
        ...
        if (isJsonArray(rawClass)) {
            ...
            builder.append(i).append("Array ").append(outVar).append(" = new Array();\n");
            if (depth == 0) {
            ...
            builder.append(i).append("for (Map.Entry<String, ").append(childInTypeName).append("> ").append(
            ...
            builder.append(i).append("JsonElement ").append(outVar).append(" = ").append(inVar).append(
                    " == null ? JsonNull.INSTANCE : ").append(inVar).append(".toJsonElement();\n");
    ...
    private void emitDeserializer(List<Method> methods, StringBuilder builder) {
        ...
        builder.append("      ").append(getImplClassName()).append(" dto = new ").append(getImplClassName()).append(
                "();\n");
        if (isCompactJson()) {
            builder.append("      Array json = jsonElem.getAsJsonArray();\n");
            for (Method method : methods) {
                if (method == null) {
                    continue;
                }
    ...
    private void emitDeserializerImpl(List<Type> expandedTypes, int depth, StringBuilder builder, String inVar,
        ...
        if (isJsonArray(rawClass)) {
            ...
            builder.append(i).append("if (").append(inVar).append(" != null && !").append(inVar).append(
                    ".isJsonNull()) {\n");
            builder.append(i).append("  ").append(outVar).append(" = new ").append(getImplName(type, false)).append(
                    "();\n");
            builder.append(i).append("  ").append(getImplName(Iterator.class, true)).append("<JsonElement> ").append(
                    inVarIterator).append(" = ").append(inVar).append(".getAsJsonArray().iterator();\n");
            builder.append(i).append("  while (").append(inVarIterator).append(".hasNext()) {\n");
            builder.append(i).append("    JsonElement ").append(childInVar).append(" = ").append(inVarIterator).append(
                    ".next();\n");
            ...
            builder.append(i).append("  ").append(outVar).append(" = new ").append(getImplName(type, false)).append(
                    "();\n");
            builder.append(i).append("  java.util.Set<Map.Entry<String, JsonElement>> ").append(entriesVar).append(
                    " = ").append(inVar).append(".getAsJsonObject().entrySet();\n");
            builder.append(i).append("  for (Map.Entry<String, JsonElement> ").append(entryVar).append(" : ").append(
                    entriesVar).append(") {\n");
            builder.append(i).append("    JsonElement ").append(childInVar).append(" = ").append(entryVar).append(
                    ".getValue();\n");
            ...
            builder.append(i).append(implClassName).append(" ").append(outVar).append(" = ").append(implClassName).append(
                    ".fromJsonElement(").append(inVar).append(");\n");
     ...
     * Array<T>, which is special in the server impl case, since it must be
    ...
    private String getImplName(Type type, boolean allowJreCollectionInterface) {
        ...
        if (allowJreCollectionInterface) {
            if (isJsonArray(rawClass)) {
                fqName = fqName.replaceFirst(ArrayList.class.getCanonicalName(), List.class.getCanonicalName());
            } else if (isJsonStringMap(rawClass)) {
                fqName = fqName.replaceFirst(HashMap.class.getCanonicalName(), Map.class.getCanonicalName());
            }
        }
