...
public class LineDimensionsUtils
{
    ...
    */
   final static String NEEDS_OFFSET_LINE_TAG = LineDimensionsCalculator.class.getName() + "NEEDS_OFFSET";
   ...
   static boolean hasSpecialCharactersMaybeWithExclusions(String lineText, boolean stripTab)
   {
      ...
      return RegExpUtils.resetAndTest(UnicodeUtils.regexpNonAsciiTabOrCarriageReturn,
         lineText.substring(index, endIndex));
   }
   ...
   public static void preTextIsOffsetNeededAndCache(Line line, int column, TextChange.Type type, String text)
   {
      ...
      else
      {
         ...
         if (hadNeedsOffset == null || !hadNeedsOffset)
         {
            ...
            line.putTag(NEEDS_OFFSET_LINE_TAG, hasSpecialCharactersMaybeWithExclusions(newText, true));
         }
         else
         {
            // We don't know the zoom level here and am only going to mark the cache
