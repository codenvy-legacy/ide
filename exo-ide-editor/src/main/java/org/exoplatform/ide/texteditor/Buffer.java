...
import org.exoplatform.ide.text.store.Document.LineCountListener;
import org.exoplatform.ide.text.store.Document.LineListener;
   ...
   CoordinateMap.DocumentSizeProvider
{
   ...
   public static class View extends CompositeView<ViewEvents>
   {
      ...
      private final EventListener mouseMoveListener = new EventListener()
      {
         ...
         public void handleEvent(Event event)
         {
            ...
            getDelegate()
               .onMouseMove(targetOffsetInBuffer.left + eventOffsetX, targetOffsetInBuffer.top + eventOffsetY);
         }
      };
      ...
      private Element createScrollableElement(BaseResources.Css baseCss)
      {
         ...
         MouseGestureListener.createAndAttach(scrollableElement, new MouseGestureListener.Callback()
         {
            ...
            public boolean onClick(int clickCount, MouseEvent event)
            {
               ...
               for (Element element = scrollableElement; element.getOffsetParent() != null; element =
               ...
               getDelegate().onMouseClick(clickCount, event.getClientX(), event.getClientY(), bufferClientLeft,
                  bufferClientTop, event.isShiftKey());
      ...
      void registerMouseMoveListener()
      {
         ...
         {
            mouseMoveListenerRemover = scrollableElement.addEventListener(Event.MOUSEMOVE, mouseMoveListener, false);
         }
      }
      ...
      void registerMouseOutListener()
      {
         ...
         {
            mouseOutListenerRemover = scrollableElement.addEventListener(Event.MOUSEOUT, mouseOutListener, false);
         }
      }
   ...
   {
      void onMouseClick(int clickCount, int clientX, int clientY, int bufferClientLeft, int bufferClientTop,
         boolean isShiftHeld);
   ...
   private final ListenerManager<HeightListener> heightListenerManager = ListenerManager.create();

   private int maxLineLength;

   private final ListenerManager<MouseClickListener> mouseClickListenerManager = ListenerManager.create();

   private final ListenerManager<MouseDragListener> mouseDragListenerManager = ListenerManager.create();

   private final ListenerManager<MouseMoveListener> mouseMoveListenerManager = ListenerManager
      .create(new ListenerManager.RegistrationListener<MouseMoveListener>()
      {
         @Override
   ...
   private final ListenerManager<MouseOutListener> mouseOutListenerManager = ListenerManager
      .create(new ListenerManager.RegistrationListener<MouseOutListener>()
      {
         @Override
   ...
   private static final Dispatcher<MouseOutListener> mouseOutListenerDispatcher = new Dispatcher<MouseOutListener>()
   {
      @Override
      ...
      Executor renderTimeExecutor)
   {
      ...
      view.setDelegate(new ViewEvents()
      {
         ...
         @Override
         public void onMouseClick(final int clickCount, int clientX, int clientY, int bufferClientLeft,
            int bufferClientTop, final boolean isShiftHeld)
         {
            ...
            mouseDragListenerManager.dispatch(new Dispatcher<MouseDragListener>()
            {
               ...
               {
                  listener.onMouseClick(Buffer.this, clickCount, bufferRelativeX, bufferRelativeY, isShiftHeld);
               }
            });
         }
   ...
   {
      final Spacer createdSpacer = coordinateMap.createSpacer(lineInfo, height, this, getView().css.spacer());
   ...
   public void removeSpacer(final Spacer spacer)
   {
      ...
      {
         updateBufferHeightAndMaybeScrollTop(convertLineNumberToY(spacerLineNumber), -spacer.getHeight());
         spacerListenerManager.dispatch(new Dispatcher<Buffer.SpacerListener>()
         {
            @Override
            public void dispatch(SpacerListener listener)
   ...
   {
      return LineUtils.rubberbandColumn(line, lineDimensions.convertXToColumn(line, x, roundingStrategy));
   }
   ...
   {
      return calculateColumnLeftIgnoringSpecialCharacters(column) + getView().css.scrollableLeftPadding();
   }
   ...
   public int getFlooredHeightInLines()
   {
       ...
       */
      return convertYToLineNumber(getScrollTop() + getHeight(), false) - convertYToLineNumber(getScrollTop(), false);
   }
   ...
   @Override
   public void onLineAdded(Document document, final int lineNumber, final JsonArray<Line> addedLines)
   {
   ...
   @Override
   public void onLineRemoved(final Document document, final int lineNumber, final JsonArray<Line> removedLines)
   {
      renderTimeExecutor.execute(new Runnable()
      {
         @Override
         public void run()
         {
             ...
             * clamp them
             */
            int safeLineNumber = Math.min(document.getLastLineNumber(), lineNumber);
            updateBufferHeightAndMaybeScrollTop(convertLineNumberToY(safeLineNumber), -removedLines.size()
               * getEditorLineHeight());
         }
      });
   }
   ...
   {
      final int totalBufferHeight = coordinateMap.getTotalSpacerHeight() + document.getLineCount() * editorLineHeight;
   ...
   private void updateColumnMarkerPosition()
   {
      getView().columnMarkerElement.getStyle().setLeft(
         calculateColumnLeftRelativeToScrollableIgnoringSpecialCharacters(MARKER_COLUMN), CSSStyleDeclaration.Unit.PX);
   }
   ...
   {
      CssUtils.setDisplayVisibility2(getView().scrollbarElement, getView().getScrollHeight() > getView().getHeight());
   }
