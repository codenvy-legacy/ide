 ...
 * {@link org.eclipse.jface.text.IPositionUpdater}s. Position updaters are managed as a list. The list defines the sequence in
 ...
 * listeners are informed. A document uses an <code>IDocumentPartitioner</code> to manage its partitions. A document may be
 ...
 * registered partitioning listeners are informed exactly once. The extension interface
 * {@link org.eclipse.jface.text.IDocumentExtension3} introduced in version 3.0 extends the concept of partitions and allows a
 * document to not only manage one but multiple partitioning. Each partitioning has an id which must be used to refer to a
 * particular partitioning.
 * </p>
 * <p>
 * An <code>IDocument</code> provides methods to map line numbers and character positions onto each other based on the document's
 ...
 * </p>
 * <p>
 * An <code>IDocument</code> does not care about mixed line delimiters. Clients who want to ensure a single line delimiter in
 ...
 * </p>
 * <p>
 * <code>IDocument</code> throws <code>BadLocationException</code> if the parameters of queries or manipulation requests are not
 * inside the bounds of the document. The purpose of this style of exception handling is
 * <ul>
 ...
 * </p>
 * <p>
 * A document support for searching has deprecated since version 3.0. The recommended way for searching is to use a
 * {@link org.eclipse.jface.text.FindReplaceDocumentAdapter}.
 * </p>
 * <p>
 * In order to provide backward compatibility for clients of <code>IDocument</code>, extension interfaces are used to provide a
 * means of evolution. The following extension interfaces exist:
 * <ul>
 * <li> {@link org.eclipse.jface.text.IDocumentExtension} since version 2.0 introducing the concept of post notification replaces
 * in order to allow document listeners to manipulate the document while receiving a document change notification</li>
 * <li> {@link org.eclipse.jface.text.IDocumentExtension2} since version 2.1 introducing configuration methods for post
 * notification replaces and document change notification.</li>
 * <li> {@link org.eclipse.jface.text.IDocumentExtension3} since version 3.0 replacing the original partitioning concept by
 * allowing multiple partitionings at the same time and introducing zero- length partitions in conjunction with the distinction
 * between open and closed partitions.</li>
 * <li> {@link org.eclipse.jface.text.IDocumentExtension4} since version 3.1 introducing the concept of rewrite sessions. A rewrite
 * session is a sequence of document replace operations that form a semantic unit. It also introduces a modification stamp and the
 * ability to set the initial line delimiter and to query the default line delimiter.</li>
 * </ul>
 * </p>
 * <p>
 * Clients may implement this interface and its extension interfaces or use the default implementation provided by
 * <code>AbstractDocument</code> and <code>Document</code>.
 * </p>
 * 
 */
public interface IDocument
{
    ...
    * </p>
    * This is only supported if the connected <code>IDocumentPartitioner</code> supports it, i.e. implements
    * <code>IDocumentPartitionerExtension2</code>. Otherwise, <code>preferOpenPartitions</code> is ignored. </p>
    * 
    * @param partitioning the partitioning
    * @param offset the document offset
    * @param preferOpenPartitions <code>true</code> if precedence should be given to a open partition ending at
    ...
    * </p>
    * This is only supported if the connected <code>IDocumentPartitioner</code> supports it, i.e. implements
    * <code>IDocumentPartitionerExtension2</code>. Otherwise, <code>includeZeroLengthPartitions</code> is ignored. </p>
    * 
    ...
    * @param includeZeroLengthPartitions <code>true</code> if zero-length partitions should be returned as part of the computed
    *           partitioning
    ...
    * <code>IDocumentListener</code>. This method is a convenience method for <code>replace(0, getLength(), text)</code>.
    * 
    ...
    * <p>
    * <strong>Note:</strong> The position is only updated on each change applied to the document if a {@link IPositionUpdater} has
    ...
    * <p>
    * Use {@link IDocumentExtension3#getLegalContentTypes(String)} when the document supports multiple partitionings. In that case
    ...
    * <p>
    * Use {@link IDocumentExtension3#getContentType(String, int, boolean)} when the document supports multiple partitionings. In
