...
(function () {
    ...
    CKEDITOR.plugins.pastefromword = {utils: {createListBulletMarker: function (h, i) {
        ...
        if ((k = h.removeAnyChildWithName('cke:listbullet')) && k.length && (k = k[0])) {
            h.name = 'cke:li';
            if (j.style)j.style = CKEDITOR.plugins.pastefromword.filters.stylesFilter([
                ['text-indent'],
                ['line-height'],
                [/^margin(:?-left)?$/, null, function (n) {
                    var o = n.split(' ');
                    n = o[3] || o[1] || o[0];
                    n = parseInt(n, 10);
                    if (!e && f && n > f)e = n - f;
            ...
            if ('cke:li' == j.name) {
                ...
                if (!p) {
                    ...
                    if (n > r) {
                        ...
                        var t = r - n, u = p.parent;
                        while (t-- && u)p = u.parent;
                        p.add(k);
                    } else p.add(k);
                    i.splice(s--, 1);
                }
        ...
        return{elementNames: [
            ...
            if (CKEDITOR.env.gecko) {
                ...
                if (E) {
                    E.replace(/[\n\r]/g, '').replace(/(.+?)\{(.+?)\}/g, function (G, H, I) {
                        H = H.split(',');
                        var J = H.length, K;
                        for (var L = 0; L < J; L++)CKEDITOR.tools.trim(H[L]).replace(/^(\w+)(\.[\w-]+)?$/g, function (M, N, O) {
                            N = N || '*';
                            O = O.substring(1, O.length);
                            if (O.match(/MsoNormal/))return;
                            if (!F[N])F[N] = {};
                            if (O)F[N][O] = I; else F[N] = I;
                        });
                    });
            ...
            if (C.getAncestor('thead'))C.name = 'th';
        }, ol: s, ul: s, dl: s, font: function (C) {
            ...
            if (!CKEDITOR.env.gecko && u(C)) {
                var D = C.firstChild(function (K) {
                    return K.value || K.name == 'img';
            ...
            if (I)H.style = n([
                ['line-height'],
                [/^font-family$/, null, !A ? p(k.font_style, 'family') : null],
                [/^font-size$/, null, !A ? p(k.fontSize_style, 'size') : null],
                [/^color$/, null, !A ? p(k.colorButton_foreStyle, 'color') : null],
                [/^background-color$/, null, !A ? p(k.colorButton_backStyle, 'color') : null]
            ])(I, C) || '';
            return null;
        }, b: o(k.coreStyles_bold), i: o(k.coreStyles_italic), u: o(k.coreStyles_underline), s: o(k.coreStyles_strike), sup: o(k.coreStyles_superscript), sub: o(k.coreStyles_subscript), a: function (C) {
            var D = C.attributes;
            if (D && !D.href && D.name)delete C.name;
        }, 'cke:listbullet': function (C) {
            if (C.getAncestor(/h\d/) && !k.pasteFromWordNumberedHeadingToList)delete C.name;
        }}, attributeNames: [
            [/^onmouse(:?out|over)/, ''],
            [/^onload$/, ''],
            [/(?:v|o):\w+/, ''],
            [/^lang/, '']
        ], attributes: {style: n(B ? [
            [/^margin$|^margin-(?!bottom|top)/, null, function (C, D, E) {
                if (D.name in {p: 1, div: 1}) {
                    ...
                    if (E == 'margin')C = y(E, C, [F])[F]; else if (E != F)return null;
            ...
            ['display', 'none'],
            A ? [/font-?/] : null
        ], B), width: function (C, D) {
            if (D.name in i.$tableContent)return false;
        }, border: function (C, D) {
            if (D.name in i.$tableContent)return false;
        }, 'class': m, bgcolor: m, valign: B ? m : function (C, D) {
