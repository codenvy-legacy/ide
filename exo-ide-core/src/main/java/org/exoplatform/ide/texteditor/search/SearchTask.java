...
package org.exoplatform.ide.texteditor.search;
...
import org.exoplatform.ide.texteditor.search.SearchProgressListener;
...
public class SearchTask
{
   ...
   private static class SearchDirectionHelper
   {
      ...
      public SearchDirectionHelper(ViewportModel viewport, DocumentModel document)
      {
      ...
      {
         boolean atEdge =
            isGoingDown() ? viewport.getTopLine() == document.getFirstLine() : viewport.getBottomLine() == document
               .getLastLine();
         return !atEdge;
      }
   }
   ...
   private static final AnchorType SEARCH_TASK_ANCHOR = AnchorType.create(SearchTask.class, "SearchAnchor");
   ...
   public SearchTask(DocumentModel document, ViewportModel viewport, IncrementalScheduler scheduler)
   {
      this.document = document;
      this.viewport = viewport;
   ...
   {
      searchDocumentStartingAtLine(executor, progressListener, SearchDirection.DOWN, DEFAULT_START_LINE);
   }
    ...
    * line.
    */
   public void searchDocument(SearchTaskExecutor executor, SearchProgressListener progressListener, LineInfo startLine)
   {
      searchDocumentStartingAtLine(executor, progressListener, SearchDirection.DOWN, startLine);
   }
    ...
    */
   public void searchDocumentStartingAtLine(SearchTaskExecutor executor, SearchProgressListener progressListener,
      SearchDirection direction, LineInfo startLine)
   {
      scheduler.cancel();
      ...
      if (startLine.number() >= viewport.getTopLineNumber() && startLine.number() <= viewport.getBottomLineNumber())
      {
   ...
   private void setupSearchTaskAnchors(LineInfo stopLine)
   {
      ...
      {
         stopLineAnchor =
            document.getAnchorManager().createAnchor(SEARCH_TASK_ANCHOR, stopLine.line(), stopLine.number(),
               AnchorManager.IGNORE_COLUMN);
         stopLineAnchor.setRemovalStrategy(RemovalStrategy.SHIFT);
      }
      else
      {
         document.getAnchorManager().moveAnchor(stopLineAnchor, stopLine.line(), stopLine.number(),
            AnchorManager.IGNORE_COLUMN);
      }
      ...
      {
         document.getAnchorManager().moveAnchor(searchTaskAnchor, startAnchorLine.line(), startAnchorLine.number(),
            AnchorManager.IGNORE_COLUMN);
      }
   }
   ...
   private class AsyncSearchTask implements IncrementalScheduler.Task
   {
      ...
      public boolean run(int workAmount)
      {
         ...
         document.getAnchorManager().moveAnchor(searchTaskAnchor, lineInfo.line(), lineInfo.number(),
            AnchorManager.IGNORE_COLUMN);
