 ...
 *     Anton Leherbauer <anton.leherbauer@windriver.com> - [projection] "Backspace" key deleting something else - http://bugs.eclipse
 *     .org/301023
 *******************************************************************************/
...
public class ProjectionDocument extends AbstractDocument {
     ...
     *         in case the fragment is invalid
     * @throws BadPositionCategoryException
     *         in case the segment category is invalid
     */
     ...
     *
     * @param offsetInMaster
     *         offset of the master document range
     * @param lengthInMaster
     *         length of the master document range
     * @param masterDocumentEvent
     *         the master document event that causes this
     *         projection change or <code>null</code> if none
     * @throws BadLocationException
     *         if the given range is invalid in the master
     *         document
     */
    private void internalAddMasterDocumentRange(int offsetInMaster, int lengthInMaster, DocumentEvent masterDocumentEvent)
        ...
        try {
            ...
            ProjectionDocumentEvent event =
                    new ProjectionDocumentEvent(this, offsetInSlave, 0, fMasterDocument.get(offsetInMaster, lengthInMaster), offsetInMaster,
     ...
     *
     * @param offsetInMaster
     *         the offset of the range in the master document
     * @param lengthInMaster
     *         the length of the range in the master document
     ...
     *         the length of the range in the master document
     * @throws BadLocationException
     *         if the given range is not valid in the
     *         master document
     * @throws IllegalArgumentException
     *         if the given range is not projected in
    ...
    private void internalRemoveMasterDocumentRange(int offsetInMaster, int lengthInMaster) throws BadLocationException {
        try {
            ...
            ProjectionDocumentEvent event =
                    new ProjectionDocumentEvent(this, imageRegion.getOffset(), imageRegion.getLength(), "", offsetInMaster,
                                                lengthInMaster); //$NON-NLS-1$
            super.fireDocumentAboutToBeChanged(event);
     ...
     *
     * @param offsetInMaster
     *         the range offset in the master document
     * @param lengthInMaster
     ...
     *         the length of the master document range
     * @param masterDocumentEvent
     *         the master document event which causes this
     *         projection change, or <code>null</code> if none
     * @throws BadLocationException
     *         in case the master event is not valid
     */
    ...
    private ProjectionDocumentEvent normalize(DocumentEvent masterEvent) throws BadLocationException {
        if (!isUpdating()) {
            ...
            if (imageRegion != null)
                return new ProjectionDocumentEvent(this, imageRegion.getOffset(), imageRegion.getLength(), masterEvent.getText(),
                                                   masterEvent);
            return null;
        }

        ProjectionDocumentEvent event = new ProjectionDocumentEvent(this, fOriginalEvent.getOffset(), fOriginalEvent.getLength(),
                                                                    fOriginalEvent.getText(), masterEvent);
        fOriginalEvent = null;
        return event;
    }
    ...
    protected final boolean adaptProjectionToMasterChange(DocumentEvent masterEvent) throws BadLocationException {
        if (!isUpdating() && fFragmentsUpdater.affectsPositions(masterEvent) || fIsAutoExpanding && masterEvent.getLength() > 0) {
            ...
            if (fragments.length == 0) {
                ...
                // need to bypass the usual infrastructure as the new segment/fragment would be of length 0 and thus the segmentation be
                // not well
                // formed
                try {
     ...
     *         <code>true</code> if auto-expanding
     */
    ...
    public void replaceMasterDocumentRanges(int offsetInMaster, int lengthInMaster) throws BadLocationException {
        try {
            ...
            ProjectionDocumentEvent event =
                    new ProjectionDocumentEvent(this, 0, fMapping.getImageLength(), fMasterDocument.get(offsetInMaster, lengthInMaster),
                                                offsetInMaster, lengthInMaster);
            super.fireDocumentAboutToBeChanged(event);
