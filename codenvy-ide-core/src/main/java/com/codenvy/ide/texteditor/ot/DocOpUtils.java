...
public final class DocOpUtils {
    ...
    public static void acceptComponent(DocOpComponent component, DocOpCursor visitor) {
        switch (component.getType()) {
            case DELETE:
                visitor.delete(((DocOpComponent.Delete)component).getText());
                break;

            case INSERT:
                visitor.insert(((DocOpComponent.Insert)component).getText());
                break;
            ...
            case RETAIN_LINE:
                visitor.retainLine(((DocOpComponent.RetainLine)component).getLineCount());
                break;
     ...
     *         doc ops factory
     * @param textChanges
     *         list of changes to convert to doc op
     * @return composed doc op, or {@code null} if text changes array is empty
     * @throws Composer.ComposeException
    ...
    public static String toString(DocOpComponent component, boolean verbose) {
        switch (component.getType()) {
            case DELETE:
                String deleteText = ((DocOpComponent.Delete)component).getText();
                return "D(" + toStringForComponentText(deleteText, verbose) + ")";

            case INSERT:
                String insertText = ((DocOpComponent.Insert)component).getText();
