...
class CoordinateMap implements DocumentModel.LineListener {
    ...
    private static class OffsetCache {
        ...
        private static final SortedList.Comparator<OffsetCache> COMPARATOR = new SortedList.Comparator<OffsetCache>() {
            @Override
    ...
    private static final AnchorType SPACER_ANCHOR_TYPE = AnchorType.create(CoordinateMap.class, "spacerAnchorType");

    private static final Spacer.Comparator SPACER_COMPARATOR = new Spacer.Comparator();

    private static final Spacer.OneWaySpacerComparator SPACER_ONE_WAY_COMPARATOR = new Spacer.OneWaySpacerComparator();
    ...
    int convertLineNumberToY(int lineNumber) {
        ...
        int precedingOffsetCacheBottom = precedingOffsetCache.offset + precedingOffsetCache.height;
        int offsetRelativeToOffsetCacheBottom = (lineNumber - precedingOffsetCache.lineNumber) * lineHeight;
     ...
     *
     * @param y
     *         the y, or {@link #IGNORE} if looking up by {@code lineNumber}
     * @param lineNumber
     *         the line number, or {@link #IGNORE} if looking up by
     *         {@code y}
     */
    private OffsetCache getPrecedingOffsetCache(int y, int lineNumber) {
        ...
        if (y != IGNORE) {
            previousOffsetCache = getCachedPrecedingOffsetCacheImpl(OffsetCache.Y_OFFSET_ONE_WAY_COMPARATOR, y);
        } else {
            previousOffsetCache =
       ...
       */
        ...
        if (offsetCacheSize > 0 && isTargetEarlierThanOffsetCache(y, lineNumber, offsetCache.get(offsetCacheSize - 1))) {
       ...
       */
        ...
        for (int n = spacers.size(); spacerPos < n; spacerPos++) {
            ...
            int simpleLinesHeight = (curSpacer.getLineNumber() - previousOffsetCache.lineNumber) * lineHeight;
            ...
            OffsetCache curOffsetCache =
                    createOffsetCache(previousOffsetCacheBottom + simpleLinesHeight, curSpacer.getLineNumber(),
                                      curSpacer.getHeight());
     ...
     */
    private OffsetCache getCachedPrecedingOffsetCacheImpl(OneWayIntComparator<OffsetCache> comparator, int value) {
    ...
    private boolean isTargetEarlierThanOffsetCache(int y, int lineNumber, OffsetCache offsetCache) {
        return ((y != IGNORE && y < offsetCache.offset) || (lineNumber != IGNORE && lineNumber < offsetCache.lineNumber));
    }
     ...
     *
     * @param lineInfo
     *         the line before which the spacer will be inserted
     * @param height
     *         the height in pixels of the spacer
     */
    ...
    void handleDocumentChange(DocumentModel document) {
        ...
        offsetCache = new SortedList<OffsetCache>(OffsetCache.COMPARATOR);
