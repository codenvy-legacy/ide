...
class ColumnOffsetCache {
     ...
     *
     * @param line
     *         The line for this ColumnOffsetCache
     * @param zoomId
     *         A unique parameter which identifies the current page zoom
    ...
    private static final String LINE_TAG_COLUMN_OFFSET_CACHE = ColumnOffsetCache.class.getName() + "COLUMN_OFFSET_CACHE";
     ...
     */
    static final ColumnOffset FULLY_MEASURED = new ColumnOffset(Integer.MIN_VALUE, Double.MIN_VALUE, Double.MIN_VALUE);
     ...
     *
     * @param defaultCharacterWidth
     *         The width that will be returned if the
    ...
    public Pair<ColumnOffset, Double> getColumnOffsetForX(double x, double defaultCharacterWidth) {
       ...
       */
        return Pair.of(offset, nextOffset.column == offset.column + 1 ? nextOffset.previousColumnWidth
     ...
     * @param findLastOffsetIfZeroWidth
     *         if true then
     ...
     */
    private int getColumnOffsetIndex(SortedList.OneWayComparator<ColumnOffset> comparator,
                                     boolean findLastOffsetIfZeroWidth) {
