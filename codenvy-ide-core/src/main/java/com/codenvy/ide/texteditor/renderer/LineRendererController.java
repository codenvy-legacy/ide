...
class LineRendererController {
    ...
    void renderLine(Line line, int lineNumber, Element targetElement, boolean isTargetElementEmpty) {
        ...
        for (int indexInLine = 0, lineSize = line.getText().length(); indexInLine < lineSize
                                                                      && ensureAllRenderersHaveARenderedNextChunk(); ) {
        ...
        if (line.getText().endsWith("\n")) {
            ...
            Assert.isTrue(lastChunk != null, "This line has no chunks!");
