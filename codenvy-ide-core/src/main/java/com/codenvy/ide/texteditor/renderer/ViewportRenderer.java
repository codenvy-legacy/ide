...
public class ViewportRenderer {
    ...
    private class AnimationController {
        private final TextEditorViewImpl.View editorView;
        ...
        AnimationController(TextEditorViewImpl.View editorView) {
        ...
        void onBeforeAnimationStarted() {
            ...
            animationFinishedTimer.schedule(TextEditorViewImpl.ANIMATION_DURATION);
        }
    }
     ...
     */
    private static final String LINE_TAG_LINE_NUMBER_CACHE_ANCHOR = "ViewportRenderer.lineNumberCacheAnchor";

    private static final AnchorType OLD_VIEWPORT_ANCHOR_TYPE = AnchorType.create(ViewportRenderer.class, "Old viewport");

    private static final AnchorType VIEWPORT_RENDERER_ANCHOR_TYPE = AnchorType.create(ViewportRenderer.class,
                                                                                      "viewport renderer");
    ...
    ViewportRenderer(DocumentModel document, Buffer buffer, ViewportModel viewport, TextEditorViewImpl.View editorView,
    ...
    private void placeOldViewportAnchors() {
        ...
        if (viewportOldTopAnchor == null) {
            viewportOldTopAnchor =
                    anchorManager.createAnchor(OLD_VIEWPORT_ANCHOR_TYPE, viewport.getTopLine(), viewport.getTopLineNumber(),
            ...
            anchorManager.moveAnchor(viewportOldTopAnchor, viewport.getTopLine(), viewport.getTopLineNumber(),
                                     AnchorManager.IGNORE_COLUMN);
            anchorManager.moveAnchor(viewportOldBottomAnchor, viewport.getBottomLine(), viewport.getBottomLineNumber(),
                                     AnchorManager.IGNORE_COLUMN);
        }
    }
    ...
    void renderDirtyLines(Array<Line> dirtyLines) {
        ...
        for (int i = 0, n = dirtyLines.size(); i < n; i++) {
            ...
            if (buffer.hasLineElement(lineElement)) {
                lineRendererController.renderLine(line, LineUtils.getCachedLineNumber(line), lineElement, false);
            }
    ...
    void renderViewportContentChange(int beginLineNumber, Array<Line> removedLines) {
       ...
       */
        ...
        if (beginLineNumber <= viewport.getBottomLineNumber()) {
            ...
            LineInfo beginLine = viewport.getDocument().getLineFinder().findLine(viewport.getBottom(), beginLineNumber);
    ...
    void renderViewportShift(boolean forceRerender) {
        ...
        LineInfo oldBottom = viewportOldBottomAnchor != null ? viewportOldBottomAnchor.getLineInfo() : null;

        LineInfo top = viewport.getTop();
        LineInfo bottom = viewport.getBottom();

        if (oldTop == null || oldBottom == null || bottom.number() < oldTop.number() || top.number() > oldBottom.number()
            || forceRerender) {
         /*
          ...
          */
            ...
            if (oldTop.number() < top.number()) {
                ...
                garbageCollectLines(top.line().getPreviousLine(), top.number() - 1, oldTop.line(), oldTop.number());
            } else if (oldTop.number() > top.number()) {
                // The viewport moved up, need to fill with lines
                fillOrUpdateLines(top.line(), top.number(), oldTop.line().getPreviousLine(), oldTop.number() - 1, 0);
            }

            if (oldBottom.number() < bottom.number()) {
                // The viewport moved down, need to fill with lines
                fillOrUpdateLines(oldBottom.line().getNextLine(), oldBottom.number() + 1, bottom.line(), bottom.number(), 0);
            } else if (oldBottom.number() > bottom.number()) {
                // The viewport moved up, need to GC the offscreen lines
                garbageCollectLines(bottom.line().getNextLine(), bottom.number() + 1, oldBottom.line(), oldBottom.number());
            }
        }
    }
     ...
     *         the line number to start filling from
     * @param endLineNumber
     *         the line number of last line (inclusive) to finish
     *         filling
     * @param createOffset
     *         the offset in pixels that this line would be at before
     *         this rendering pass, used to animate in from offscreen
     */
    public void fillOrUpdateLines(Line beginLine, int beginLineNumber, Line endLine, int endLineNumber, int createOffset) {
    ...
    private void handleLineEnteredViewport(Line line, int lineNumber, Element lineElement) {
        ...
        Anchor anchor =
                line.getDocument().getAnchorManager()
                    .createAnchor(VIEWPORT_RENDERER_ANCHOR_TYPE, line, lineNumber, AnchorManager.IGNORE_COLUMN);
