...
public final class PositionUtils
{
   ...
   {
      return LineUtils.comparePositions(a.getLineNumber(), a.getColumn(), b.getLineNumber(), b.getColumn());
   }
   ...
   {
      return relativeOffset < 0 ? getPositionBackwards(line, lineNumber, column, -relativeOffset) : getPositionForward(
         line, lineNumber, column, relativeOffset);
   }
   ...
   public static Position getPosition(Position position, int relativeOffset)
   {
      return relativeOffset < 0 ? getPositionBackwards(position.getLine(), position.getLineNumber(),
         ...
         position.getColumn(), relativeOffset);
   }
...
/*  public static Position getPosition(Position start, int relativeOffset) {
   ...
   public static void visit(LineVisitor lineVisitor, Position start, Position end)
   {
      ...
      {
         lineVisitor.accept(start.getLine(), start.getLineNumber(), start.getColumn(), end.getColumn());
         return;
      }
      ...
      if (iterateForward)
      {
         if (!lineVisitor.accept(start.getLine(), start.getLineNumber(), start.getColumn(), start.getLine().length()))
         {
            return;
         }
      }
      ...
      while (curLine.line() != end.getLine())
      {
         ...
         {
            throw new IllegalStateException("Could not find the requested ending line while visiting position range");
         }
      }
      ...
      {
         lineVisitor.accept(end.getLine(), end.getLineNumber(), end.getColumn(), end.getLine().length());
      }
   }
