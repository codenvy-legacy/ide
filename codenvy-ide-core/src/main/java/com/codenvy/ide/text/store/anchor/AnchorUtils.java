...
public final class AnchorUtils
{
   ...
   public static int compare(Anchor a, Anchor b)
   {
      ...
      assert ((a.getColumn() == AnchorManager.IGNORE_COLUMN) == (b.getColumn() == AnchorManager.IGNORE_COLUMN));
    ...
    * with {@code text}.
    */
   public static void setTextBetweenAnchors(String text, Anchor begin, Anchor end, TextStoreMutator documentMutator)
   {
      ...
      Assert.isTrue(begin.getLineNumber() <= end.getLineNumber(), "begin line below end line");
     ...
     */
      ...
      int deleteCount = LineUtils.getTextCount(topLine, 0, bottomLine,
         bottomLine.getText().length() - (deleteEndingNewline ? 0 : 1));

      documentMutator.insertText(topLine, topLineInfo.number(), 0, text, false);
      Position endOfInsertion = PositionUtils.getPosition(topLine, topLineInfo.number(), 0, text.length() - 1);
