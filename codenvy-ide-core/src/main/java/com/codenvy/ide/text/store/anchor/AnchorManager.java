...
public class AnchorManager
{
   ...
   */
   ...
   public Anchor findClosestAnchorWithLineNumber(int lineNumber)
   {
      ...
      int distanceFromPreviousAnchor = lineNumber - lineAnchors.get(insertionIndex - 1).getLineNumber();
      int distanceFromNextAnchor = lineAnchors.get(insertionIndex).getLineNumber() - lineNumber;
      return distanceFromNextAnchor < distanceFromPreviousAnchor ? lineAnchors.get(insertionIndex) : lineAnchors.get(
         insertionIndex - 1);
   }
   ...
   public void handleTextPredeletionForLine(Line line, int column, int deleteCountForLine,
      final JsonArray<Anchor> anchorsInDeletedRangeToRemove, final JsonArray<Anchor> anchorsInDeletedRangeToShift,
      boolean isFirstLine)
   {
     ...
     */
      int lastColumnToDelete = entireLineDeleted ? Integer.MAX_VALUE : column + deleteCountForLine - 1;
      for (int i = anchors.findInsertionIndex(column,
         Anchor.ID_FIRST_IN_COLUMN), n = anchors.size(); i < n && anchors.get(i).getColumn() <= lastColumnToDelete; i++)
      {
         categorizeAccordingToRemovalStrategy(anchors.get(i), anchorsInDeletedRangeToRemove,
            anchorsInDeletedRangeToShift);
      }
   ...
   // TODO: not public
   public void handleTextDeletionLastLineLeftover(JsonArray<Anchor> anchorsLeftoverFromLastLine, Line firstLine,
      Line lastLine, int lastLineFirstUntouchedColumn)
   {
      ...
      for (int i = anchors.findInsertionIndex(lastLineFirstUntouchedColumn,
         Anchor.ID_FIRST_IN_COLUMN), n = anchors.size(); i < n; i++)
      {
         anchorsLeftoverFromLastLine.add(anchors.get(i));
      }
   }
   ...
   public void handleTextDeletionFinished(JsonArray<Anchor> anchorsInDeletedRangeToRemove,
      JsonArray<Anchor> anchorsInDeletionRangeToShift, JsonArray<Anchor> anchorsLeftoverOnLastLine, Line firstLine,
      int firstLineNumber, int firstLineColumn, int numberOfLinesDeleted, int lastLineFirstUntouchedColumn)
   {
      ...
      for (int i = 0, n = anchorsInDeletionRangeToShift.size(); i < n; i++)
      {
         ...
         updateAnchorPositionObeyingExistingIgnoresWithoutDispatch(anchor, firstLine, firstLineNumber, firstLineColumn);
     ...
     */
      for (int i = 0, n = anchorsLeftoverOnLastLine.size(); i < n; i++)
      {
         ...
         int anchorFirstLineColumn = anchor.getColumn() - lastLineFirstUntouchedColumn + firstLineColumn;
       ...
       */
         shiftLineNumbersDeferDispatch(firstLineNumber + numberOfLinesDeleted + 1, -numberOfLinesDeleted, dispatcher);
      }

      dispatcher.dispatch();
   }

   private void categorizeAccordingToRemovalStrategy(Anchor anchor, JsonArray<Anchor> anchorsToRemove,
      JsonArray<Anchor> anchorsToShift)
   {
   ...
   // TODO: not public
   public void handleMultilineTextInsertion(Line oldLine, int oldLineNumber, int oldColumn, Line newLine,
      int newLineNumber, int newColumn)
   {
     ...
     */
      shiftLineNumbersDeferDispatch(oldLineNumber + 1, newLineNumber - oldLineNumber, dispatcher);
     ...
     */
      ...
      if (anchors != null)
      {
         ...
         for (int i = 0; i < anchors.size() && anchors.get(i).isLineAnchor(); )
         {
            ...
            {
               updateAnchorPositionWithoutDispatch(anchor, newLine, newLineNumber, AnchorManager.IGNORE_COLUMN);
               dispatcher.deferDispatchShifted(anchor);
            }
            else
            {
       ...
       */
         for (int i = anchors.findInsertionIndex(oldColumn, Anchor.ID_FIRST_IN_COLUMN); i < anchors.size(); )
         {
            Anchor anchor = anchors.get(i);

            if (anchor.getColumn() == oldColumn && !isInsertionPlacementStrategyLater(anchor, oldLine, oldColumn))
            {
          /*
           ...
           */
            ...
            updateAnchorPositionObeyingExistingIgnoresWithoutDispatch(anchor, newLine, newLineNumber, newAnchorColumn);
            dispatcher.deferDispatchShifted(anchor);
   ...
   {
      InsertionPlacementStrategy.Placement placement = anchor.getInsertionPlacementStrategy().determineForInsertion(
         anchor, line, column);
   ...
   private void updateAnchorPositionObeyingExistingIgnoresWithoutDispatch(Anchor anchor, Line newLine,
      int newLineNumber, int newColumn)
   {
      int anchorsNewColumn = anchor.getColumn() == IGNORE_COLUMN ? IGNORE_COLUMN : newColumn;
      int anchorsNewLineNumber = anchor.getLineNumber() == IGNORE_LINE_NUMBER ? IGNORE_LINE_NUMBER : newLineNumber;
    ...
    */
   private void updateAnchorPositionWithoutDispatch(final Anchor anchor, Line line, int lineNumber, int column)
   {

      Assert.isNotNull(anchor.isAttached(), "Cannot move detached anchor");

      // Ensure it's different
      if (anchor.getLine() == line && anchor.getLineNumber() == lineNumber && anchor.getColumn() == column)
      {
         return;
      }
      ...
      if (oldAnchors == null)
      {
         throw new IllegalStateException(
            "List of line's anchors should not be null\nLine anchors:\n" + dumpAnchors(lineAnchors));
      }
    ...
    */
   private void shiftColumnAnchors(Line line, int column, int shiftAmount, boolean consultPlacementStrategy)
   {
