...
public class NewProjectWizardPresenter implements WizardDialog, Wizard.UpdateDelegate, ProjectWizardView.ActionDelegate {
        ...
        builderServiceClient.getRegisteredServers(workspaceId, new AsyncRequestCallback<Array<BuilderDescriptor>>(
            ...
            protected void onSuccess(Array<BuilderDescriptor> results) {
                for (BuilderDescriptor builderDescriptor : results.asIterable()) {
                    if (builderDescriptor.getEnvironments() != null) {
                        for (BuilderEnvironment environment : builderDescriptor.getEnvironments().values()) {
                            if (environment.getId() != null && environment.getDisplayName() != null) {
                                builderDescriptionMap.put(environment.getId(), environment.getDisplayName());
                            }
                        }
                    }
                }
    ...
    private void recursiveRunnersDescriptions(RunnerEnvironmentTree tree) {
        if (tree == null) {
            return;
        }
        final List<RunnerEnvironmentTree> runnerEnvironmentTrees = tree.getNodes();
        for (RunnerEnvironmentTree runnerEnvironmentTree : runnerEnvironmentTrees) {
            if (runnerEnvironmentTree == null || runnerEnvironmentTree.getLeaves() == null) {
                break;
            } else {
                for (RunnerEnvironmentLeaf runnerEnvironmentLeaf : runnerEnvironmentTree.getLeaves()) {
                    final RunnerEnvironment runnerEnvironment = runnerEnvironmentLeaf.getEnvironment();
                    if (runnerEnvironment != null && runnerEnvironment.getDescription() != null) {
                        runnersDescriptionMap.put(runnerEnvironment.getId(), runnerEnvironment.getDescription());
                    }
                }
                recursiveRunnersDescriptions(runnerEnvironmentTree);
            }
        }
    }
    ...
    private void requestRunnersDescriptor() {
        runnerServiceClient.getRunners(new AsyncRequestCallback<RunnerEnvironmentTree>(
            ...
            protected void onSuccess(RunnerEnvironmentTree result) {
                recursiveRunnersDescriptions(result);
            }
