...
import com.codenvy.ide.text.TypedPosition;
 ...
 * {@link PartitionTokenScanner} to scan the document and to determine the
 ...
 * structure.
 * <p>
 * To reduce array creations in {@link Document#getPositions(String)}, the
 ...
 * </p>
 *
 * @see PartitionTokenScanner
 */
public class FastPartitioner implements DocumentPartitioner {

    /** The position category this partitioner uses to store the document's partitioning information. */
    private static final String CONTENT_TYPES_CATEGORY = "__content_types_category"; //$NON-NLS-1$
    /** The partitioner's scanner */
    protected final PartitionTokenScanner  fScanner;
     ...
     *         the legal content types of this partitioner
     */
    public FastPartitioner(PartitionTokenScanner scanner, String[] legalContentTypes) {
        fScanner = scanner;
    ...
    /*
     * @see org.eclipse.jface.text.DocumentPartitionerExtension2#getManagingPositionCategories()
     */
    ...
    /*
     * @see org.eclipse.jface.text.DocumentPartitioner#connect(org.eclipse.jface.text.Document)
     */
    public final void connect(Document document) {
     ...
     * </p>
     */
    public void connect(Document document, boolean delayInitialization) {
    ...
    /*
     * @see DocumentPartitioner#documentChanged(DocumentEvent)
     */
    public final boolean documentChanged(DocumentEvent e) {
        if (fIsInitialized) {
            Region region = documentChanged2(e);
            return (region != null);
        }
        return false;
    }
    ...
    private Region createRegion() {
        if (fDeleteOffset == -1) {
            if (fStartOffset == -1 || fEndOffset == -1)
                return null;
            return new RegionImpl(fStartOffset, fEndOffset - fStartOffset);
        } else if (fStartOffset == -1 || fEndOffset == -1) {
            return new RegionImpl(fDeleteOffset, 0);
        } else {
    ...
    public Region documentChanged2(DocumentEvent e) {
        ...
        try {
            ...
            Region line = fDocument.getLineInformationOfOffset(e.getOffset());
     ...
     * </p>
     */
    public TypedRegion getPartition(int offset) {
        checkInitialization();
        ...
        try {
            ...
            if (category == null || category.length == 0)
                return new TypedRegionImpl(0, fDocument.getLength(), Document.DEFAULT_CONTENT_TYPE);
            ...
            if (index < category.length) {
                ...
                if (index == 0)
                    return new TypedRegionImpl(0, next.offset, Document.DEFAULT_CONTENT_TYPE);
                ...
                return new TypedRegionImpl(endOffset, next.getOffset() - endOffset, Document.DEFAULT_CONTENT_TYPE);
            }
            ...
            int endOffset = previous.getOffset() + previous.getLength();
            return new TypedRegionImpl(endOffset, fDocument.getLength() - endOffset, Document.DEFAULT_CONTENT_TYPE);
    ...
    /*
     * @see DocumentPartitioner#computePartitioning(int, int)
     */
    public final TypedRegion[] computePartitioning(int offset, int length) {
     ...
     */
    protected String getTokenContentType(Token token) {
     ...
     * </p>
     */
    public TypedRegion getPartition(int offset, boolean preferOpenPartitions) {
        TypedRegion region = getPartition(offset);
        if (preferOpenPartitions) {
            if (region.getOffset() == offset && !region.getType().equals(Document.DEFAULT_CONTENT_TYPE)) {
                if (offset > 0) {
                    region = getPartition(offset - 1);
                    if (region.getType().equals(Document.DEFAULT_CONTENT_TYPE))
                        return region;
                }
                return new TypedRegionImpl(offset, 0, Document.DEFAULT_CONTENT_TYPE);
            }
        }
        return region;
    }
     ...
     * </p>
     */
    public TypedRegion[] computePartitioning(int offset, int length, boolean includeZeroLengthPartitions) {
        checkInitialization();
        ...
        try {
            ...
            for (int i = startIndex; i < endIndex; i++) {
                ...
                if ((includeZeroLengthPartitions && overlapsOrTouches(gap, offset, length)) ||
                    ...
                    list.add(new TypedRegionImpl(start, end - start, Document.DEFAULT_CONTENT_TYPE));
                }

                if (current.overlapsWith(offset, length)) {
                    start = Math.max(offset, current.getOffset());
                    end = Math.min(endOffset, current.getOffset() + current.getLength());
                    list.add(new TypedRegionImpl(start, end - start, current.getType()));
                }
            ...
            if (list.isEmpty())
                list.add(new TypedRegionImpl(offset, length, Document.DEFAULT_CONTENT_TYPE));
     ...
     *         if getting the positions from the
     *         document fails
     */
