...
public final class DefaultOperationHistory implements IOperationHistory {
    ...
    private Map<IUndoContext, Integer> limits = new HashMap<IUndoContext, Integer>();
    ...
    private IStatus doRedo(IUndoableOperation operation) throws ExecutionException {
        ...
        IStatus status = null;// getRedoApproval(operation, info);
        //      if (status.isOK())
        //      {
        notifyAboutToRedo(operation);
        try {
            status = operation.redo();
    ...
    private IStatus doUndo(IUndoableOperation operation) throws ExecutionException {
        IStatus status = null;//getUndoApproval(operation, info);
        //      if (status.isOK())
        //      {
        notifyAboutToUndo(operation);
        try {
            status = operation.undo();
        } catch (OperationCanceledException e) {
            status = Status.CANCEL_STATUS;
        } catch (ExecutionException e) {
            notifyNotOK(operation);
     ...
     */
    @Override
    public IStatus execute(IUndoableOperation operation) throws ExecutionException {
        Assert.isNotNull(operation);
        ...
        IStatus status = null;//getExecuteApproval(operation, info);
        //      if (!status.isOK())
        //      {
     ...
     */
    @Override
    public IStatus redo(IUndoContext context) throws ExecutionException {
        Assert.isNotNull(context);
        IUndoableOperation operation = getRedoOperation(context);
    ...
    */
    ...
    @Override
    public IStatus redoOperation(IUndoableOperation operation) throws ExecutionException {
        Assert.isNotNull(operation);
        IStatus status;
        if (operation.canRedo()) {
            status = doRedo(operation);
        } else {
    ...
    //	 * (non-Javadoc)
    //	 *
    //	 * @see org.eclipse.core.commands.operations.IOperationHistory#removeOperationApprover(org.eclipse.core.commands.operations
    // .IOperationApprover)
    //	 */
     ...
     * (non-Javadoc)
     *
     * @see org.eclipse.core.commands.operations.IOperationHistory#removeOperationHistoryListener(org.eclipse.core.commands.operations
     ...
     */
    @Override
    public IStatus undo(IUndoContext context) throws ExecutionException {
        Assert.isNotNull(context);
        IUndoableOperation operation = getUndoOperation(context);
     ...
     */
    @Override
    public IStatus undoOperation(IUndoableOperation operation) throws ExecutionException {
        Assert.isNotNull(operation);
        IStatus status;
        if (operation.canUndo()) {
            status = doUndo(operation);
        } else {
            status = IOperationHistory.OPERATION_INVALID_STATUS;
        }
        return status;
    ...
    //	 * (non-Javadoc)
    //	 *
    //	 * @see org.eclipse.core.commands.operations.IOperationHistory#openOperation(org.eclipse.core.commands.operations
    // .ICompositeOperation)
    //	 */
