...
set EXO_OPTS="-Djavasrc=%JAVA_HOME%\src.zip" "-Djre.lib=%JAVA_HOME%\jre\lib" "-Dorg.exoplatform.mimetypes=conf\mimetypes.properties" "-Dorg.exoplatform.ide.git.server=git" "-Dtenant.masterhost=localhost" "-Dorganization.application.server.url=http://localhost:${PORT}/userdb" "-Dmailsender.application.server.url=http://localhost:${PORT}/mail" "-Dbuilder.timeout=600" "-Dcodenvy.local.conf.dir=${CATALINA_HOME}/conf"
