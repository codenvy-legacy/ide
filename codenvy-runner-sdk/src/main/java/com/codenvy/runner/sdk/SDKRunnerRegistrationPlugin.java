...
public class SDKRunnerRegistrationPlugin implements RunnerRegistrationPlugin {
    ...
    private static Path calculateDirPath(Path parent, String workspaceId) {
        ...
        return Paths.get(parent.toString(), relPath);
    }

    private static Path getMountPath(String workspaceId) {
        final String vfsRootDir = System.getProperty("com.codenvy.vfs.rootdir", "../temp/fs-root");
        return calculateDirPath(Paths.get(vfsRootDir), workspaceId);
    }

    @Override
    public void registerTo(RunnerRegistry registry) {
        final Path fsMountPoint = getMountPath(WS_NAME);
        registry.add(new SDKRunner(fsMountPoint));
    }
}
