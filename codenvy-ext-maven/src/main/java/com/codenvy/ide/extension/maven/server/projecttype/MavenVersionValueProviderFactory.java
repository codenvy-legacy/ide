...
public class MavenVersionValueProviderFactory implements ValueProviderFactory {
    private static final Logger LOG = LoggerFactory.getLogger(MavenVersionValueProviderFactory.class);
                ...
                try {
                    ...
                    if (pomFile != null && pomFile.isFile()) {
                        try (InputStream input = ((FileEntry)pomFile).getInputStream()) {
                            list.add(MavenUtils.readModel(input).getVersion());
                        }
                    }
                    ...
                    if (pomFile != null) {
                        if (!pomFile.isFile()) {
                            throw new IllegalStateException(
                                    String.format("Unable to set Version. Path %s exists but is not a file.", pomFile.getPath()));
                        }
                        try (InputStream input = ((FileEntry)pomFile).getInputStream()) {
