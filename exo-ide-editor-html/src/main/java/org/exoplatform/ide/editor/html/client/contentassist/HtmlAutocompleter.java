  ...
  public AutocompleteProposals findAutocompletions(
    ...
    if (column == 0) {
      ...
      if (inTag) {
        ...
        return new HtmlAutocompleteProposals("", proposals, CompletionType.ATTRIBUTE);
      }
      return AutocompleteProposals.EMPTY;
    }
    ...
    if (TokenType.TAG == type) {
      ...
      return new HtmlAutocompleteProposals(
          value, htmlAttributes.searchTags(value.toLowerCase()), CompletionType.ELEMENT);
    }
    if (TokenType.WHITESPACE == type || TokenType.ATTRIBUTE == type) {
      ...
      return new HtmlAutocompleteProposals(value, proposals, CompletionType.ATTRIBUTE);
    }
