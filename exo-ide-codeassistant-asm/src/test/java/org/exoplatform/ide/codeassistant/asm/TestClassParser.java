   ...
   private void assertLoadedClasses(List<TypeInfo> classes)
   {
      ...
      for (TypeInfo current : classes)
      {
               ...
               for (MethodInfo method : methods)
               {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "b", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT, "()",
                        "()", "public abstract double " + PACKAGE + ".TestAnnotation.b()", new String[0], "double",
                        "double");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "c", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT, "()",
                        "()", "public abstract java.lang.String " + PACKAGE + ".TestAnnotation.c()", new String[0],
         ...
         else if (current.getName().equals("TestClass"))
         {
...

            {
               ...
               for (RoutineInfo constructor : constructors)
               {
                  ...
                  {
                     assertRoutine(constructor, "TestClass", typeInfo.getQualifiedName(), Modifier.PUBLIC,
                        "(int, String)", "(int, java.lang.String)", "public " + PACKAGE
                           + ".TestClass(int, java.lang.String)", new String[]{"java.lang.ClassNotFoundException"});
               ...
               for (MethodInfo method : methods)
               {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method3", typeInfo.getQualifiedName(), Modifier.PUBLIC,
                        "(double, int, char, float[][][], String[])",
                        "(double, int, char, float[][][], java.lang.String[])", "public int " + PACKAGE
                           + ".TestClass.method3(double, int, char, float[][][], java.lang.String[])", new String[0],
                        "int", "int");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method1", typeInfo.getQualifiedName(), Modifier.PUBLIC, "()", "()",
                        "public int " + PACKAGE + ".TestClass.method1()", new String[0], "int", "int");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
         ...
         else if (current.getName().equals("TestEnum"))
         {
            ...
            Assert.assertEquals(Integer.valueOf(Modifier.SYNCHRONIZED | Modifier.FINAL | TypeInfoBuilder.MODIFIER_ENUM
               | Modifier.PUBLIC), typeInfo.getModifiers());
...

            {
               ...
               for (RoutineInfo constructor : constructors)
               {
                  ...
                  {
                     assertRoutine(constructor, "TestEnum", typeInfo.getQualifiedName(), Modifier.PRIVATE,
                        "(String, int)", "(java.lang.String, int)", "private " + PACKAGE
                           + ".TestEnum(java.lang.String, int)", new String[0]);
                     visitedConstructors.add(constructor.getName());
                  }
                  else
                  {
               ...
               for (MethodInfo method : methods)
               {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "values", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.STATIC,
                        "()", "()", "public static " + PACKAGE + ".TestEnum[] " + PACKAGE + ".TestEnum.values()",
                        new String[0], "TestEnum[]", PACKAGE + ".TestEnum[]");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
               ...
               for (MethodInfo method : methods)
               {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method3", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT,
                        "(double, int, char, float[][][], String[])",
                        "(double, int, char, float[][][], java.lang.String[])", "public abstract int " + PACKAGE
                           + ".TestInterface2.method3(double, int, char, float[][][], java.lang.String[])",
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method4", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT,
                        "(String, Boolean, boolean, int[][][][][])",
                        "(java.lang.String, java.lang.Boolean, boolean, int[][][][][])", "public abstract void "
                           + PACKAGE
               ...
               for (MethodInfo method : methods)
               {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method1", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT,
                        "()", "()", "public abstract int " + PACKAGE + ".TestInterface.method1()", new String[0],
                        "int", "int");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                  else if (method.getName().equals("method2") && method.getParameterTypes().equals("(int)")
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method2", typeInfo.getQualifiedName(), Modifier.PUBLIC | Modifier.ABSTRACT,
                        "(int)", "(int)", "public abstract int " + PACKAGE + ".TestInterface.method2(int)",
                        new String[0], "int", "int");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                  else
...

            {
               ...
               for (MethodInfo method : methods)
               {
                  if (method.getName().equals("method0") && method.getParameterTypes().equals("(int)")
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method0", typeInfo.getQualifiedName(), Modifier.PROTECTED
                        | Modifier.ABSTRACT, "(int)", "(int)", "protected abstract void " + PACKAGE
                        + ".TestSuper.method0(int)", new String[0], "void", "void");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                  else if (method.getName().equals("method") && method.getParameterTypes().equals("()")
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method", typeInfo.getQualifiedName(), Modifier.PUBLIC, "()", "()",
                        "public void " + PACKAGE + ".TestSuper.method()", new String[0], "void", "void");
                     visitedMethods.add(method.getName() + method.getParameterTypes());
                  }
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     ...
                     && !visitedMethods.contains(method.getName() + method.getParameterTypes()))
                  {
                     assertMethod(method, "method", typeInfo.getQualifiedName(), Modifier.PUBLIC, "(double)",
                        "(double)", "public void " + PACKAGE + ".TestSuper.method(double)",
