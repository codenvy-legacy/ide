...
set EXO_CONFIG_OPTS="-Xshare:auto -Xms512m -Xmx1024m -XX:MaxPermSize=256m"
set EXO_OPTS="-Djavasrc=%JAVA_HOME%\src.zip" "-Djre.lib=%JAVA_HOME%\jre\lib" "-Dorg.exoplatform.mimetypes=conf\mimetypes.properties" "-Dorg.exoplatform.ide.git.server=git" "-Dcom.codenvy.vfs.rootdir=${CATALINA_HOME}/temp/fs-root" "-Dorg.exoplatform.ide.server.fs-root-path=${CATALINA_HOME}/temp/fs-root" "-Dtenant.masterhost=localhost" "-Dorganization.application.server.url=http://localhost:8080/userdb" "-Dmailsender.application.server.url=http://localhost:8080/mail" "-Dbuilder.timeout=600" "-Dcodenvy.local.conf.dir=${CATALINA_HOME}/conf"
set JAVA_OPTS=%LOG_OPTS% %SECURITY_OPTS% %EXO_CONFIG_OPTS% %EXO_OPTS% %IDE_OPTS%
