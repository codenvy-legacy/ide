...
EXO_OPTS="-Djavasrc=$JAVA_HOME/src.zip -Djre.lib=$JAVA_HOME/jre/lib -Dorg.exoplatform.mimetypes=conf/mimetypes.properties -Dorg.exoplatform.ide.git.server=git -Dcom.codenvy.vfs.rootdir=${CATALINA_HOME}/temp/fs-root -Dtenant.masterhost=localhost -Dorganization.application.server.url=http://localhost:8080/userdb/ -Dmailsender.application.server.url=http://localhoat:8080/mail/ -Dcodenvy.local.conf.dir=${CATALINA_HOME}/conf"
