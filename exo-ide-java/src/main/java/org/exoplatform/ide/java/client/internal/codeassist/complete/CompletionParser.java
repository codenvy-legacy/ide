...
import org.exoplatform.ide.java.client.internal.compiler.parser.JavadocParser;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredBlock;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredElement;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredField;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredInitializer;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredLocalVariable;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredMethod;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredType;
import org.exoplatform.ide.java.client.internal.compiler.parser.RecoveredUnit;
...
import org.exoplatform.ide.java.client.internal.compiler.util.HashtableOfObjectToInt;
...
public class CompletionParser extends AssistParser
{
         ...
         if (detector.containsCompletionNode())
         {
            ...
            if (expression == this.assistNode
               || (expression instanceof Assignment // https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
                  && ((Assignment)expression).expression == this.assistNode && ((this.expressionPtr > 0 && stackHasInstanceOfExpression(
               ...
               || (expression instanceof AND_AND_Expression && (this.elementPtr >= 0 && this.elementObjectInfoStack[this.elementPtr] instanceof InstanceOfExpression)))
            {
   ...
   {
      if (this.identifierLengthPtr >= 1 && this.previousToken == TokenNameDOT)
   ...
   private boolean checkInvocation()
   {
      ...
      if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER
         ...
         || (isEmptyAssistIdentifier =
            this.indexOfAssistIdentifier() >= 0 && this.identifierStack[this.identifierPtr].length == 0)))
      { // e.g. it is something like "this.fred(1 [cursor]"
      ...
      if (isInsideMethod() || isInsideFieldInitialization())
      {
         ...
         CompletionOnBranchStatementLabel statementLabel =
            new CompletionOnBranchStatementLabel(kind == K_INSIDE_BREAK_STATEMENT
               ? CompletionOnBranchStatementLabel.BREAK : CompletionOnBranchStatementLabel.CONTINUE,
               this.identifierStack[this.identifierPtr--], (int)(position >>> 32), (int)position, labels);
   ...
   protected void consumeConstructorHeaderName()
   {
      ...
      if (indexOfAssistIdentifier() < 0)
      {
         ...
         if (this.currentElement != null && this.lastIgnoredToken == TokenNamenew)
         ...
         if (this.sourceEnds != null && this.astPtr > currentAstPtr)
               ...
               && !(this.currentElement instanceof RecoveredUnit && ((RecoveredUnit)this.currentElement).typeCount == 0))
            {
               ...
               if (!(this.currentElement instanceof RecoveredType)
                  && (this.currentToken == TokenNameDOT || (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds,
                     0, this.scanner.linePtr) != Util.getLineNumber(nameSourceStart, this.scanner.lineEnds, 0,
                     this.scanner.linePtr))))
               {
         ...
         switch (token)
         {
            case TokenNameDOT :
               switch (previous)
               {
                  ...
                  case TokenNameIdentifier : // e.g. bar[.]fred()
                     if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) != K_BETWEEN_NEW_AND_LEFT_BRACKET)
                     {
                        if (this.identifierPtr != prevIdentifierPtr)
                        { // if identifier has been consumed, e.g. this.x[.]fred()
                           this.invocationType = EXPLICIT_RECEIVER;
                        }
                        else
                        {
            ...
            case TokenNameIdentifier :
               ...
               { // e.g. foo().[fred]()
                  ...
                  { // e.g. not fred().new foo.[Bar]()

                     this.invocationType = EXPLICIT_RECEIVER;
                     this.qualifier = this.expressionPtr;
                  }
               }
               if (previous == TokenNameGREATER)
               { // e.g. foo().<X>[fred]()
   ...
   protected void popUntilCompletedAnnotationIfNecessary()
   {
      ...
      int i = this.elementPtr;
      while (i > -1
         && (this.elementKindStack[i] != K_BETWEEN_ANNOTATION_NAME_AND_RPAREN || (this.elementInfoStack[i] & ANNOTATION_NAME_COMPLETION) == 0))
      {
         i--;
      }
