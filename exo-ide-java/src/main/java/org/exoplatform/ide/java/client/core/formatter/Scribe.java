...
import org.exoplatform.ide.java.client.core.formatter.comment.CommentFormatterUtil;
import org.exoplatform.ide.java.client.core.formatter.comment.IJavaDocTagConstants;
...
import org.exoplatform.ide.java.client.internal.core.util.RecordedParsingInformation;
...
import org.exoplatform.ide.text.edits.MultiTextEdit;
...
public class Scribe implements IJavaDocTagConstants
{
      ...
      int continuationIndent, boolean adjust)
   {
      ...
      if ((this.currentAlignment == null && this.formatter.expressionsDepth >= 0)
         || (this.currentAlignment != null && this.currentAlignment.kind == Alignment.BINARY_EXPRESSION && (this.formatter.expressionsPos & CodeFormatterVisitor.EXPRESSIONS_POS_MASK) == CodeFormatterVisitor.EXPRESSIONS_POS_BETWEEN_TWO))
      {
         switch (kind)
         {
                  ...
                  else
                  {
                     ...
                     else if (hasComment)
                     {
                        ...
                        && (!this.formatter.preferences.join_wrapped_lines
                           || this.formatter.preferences.number_of_empty_lines_to_preserve != 0 || this.blank_lines_between_import_groups > 0))
                     {
                        addReplaceEdit(tokenStartPosition, whitespacesEndPosition,
   ...
   private void printLineComment()
   {
      ...
      {
         if (onFirstColumn
            && ((includesLineComments && !this.formatter.preferences.comment_format_line_comment_starting_on_first_column) || this.formatter.preferences.never_indent_line_comments_on_first_column))
         {
            commentIndentationLevel = this.column - 1;
         }
         else
   ...
   private int printJavadocHtmlTag(FormatJavadocText text, FormatJavadocBlock block, boolean textOnNewLine)
   {
      ...
      this.needSpace = !closingTag && max > 0 // not a single or not closed tag (e.g. <br>)
         && (previousEnd + 1) < nextStart; // There's no space between text and html tag or inline block => do not insert space a
               ...
               case TerminalTokens.TokenNameWHITESPACE :
                  if (tokensBufferLength > 0)
                  {
                     boolean shouldSplit = (this.column + tokensBufferLength) > maxColumn // the max length is reached
                        && !isHtmlTag && (insertSpace || tokensBufferLength > 1) // allow to split at the beginning only when
