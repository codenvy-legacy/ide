 ...
 *******************************************************************************/
package org.exoplatform.ide.java.client.core.formatter.comment;
...
import org.exoplatform.ide.text.DefaultPositionUpdater;
   ...
   public static TextEdit format2(int kind, String string, int indentationLevel, String lineSeparator, Map options)
   {
      ...
      if (0 < 0 || length < 0 || 0 + length > string.length())
      {
         throw new IllegalArgumentException(
            "offset or length outside of string. offset: " + 0 + ", length: " + length + ", string size: " + string.length()); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
      }
      ...
      try
      {
         if (positions != null)
         {
            ...
            for (int i = 0; i < positions.length; i++)
            {
               ...
               {
                  throw new IllegalArgumentException(
                     "Position outside of string. offset: " + positions[i].offset + ", length: " + positions[i].length + ", string size: " + content.length()); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
               }
            }
         }
      }
