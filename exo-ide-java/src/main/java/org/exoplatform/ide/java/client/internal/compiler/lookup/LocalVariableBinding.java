...
public class LocalVariableBinding extends VariableBinding
{
   ...
   public char[] computeUniqueKey(boolean isLeaf)
   {
      ...
      if (scope != null)
      {
         ...
         { // use linear search assuming the number of locals per scope is low
         ...
         for (int i = 0; i < this.initializationCount; i++)
         {
            ...
            s +=
               String.valueOf(this.initializationPCs[i << 1])
                  + "-" + ((this.initializationPCs[(i << 1) + 1] == -1) ? "?" : String.valueOf(this.initializationPCs[(i << 1) + 1])); //$NON-NLS-2$ //$NON-NLS-1$
         }
         s += "]"; //$NON-NLS-1$
      }
      return s;
