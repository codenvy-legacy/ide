...
public class CssPartialParser
{
   ...
   private static final RegExp REGEXP_RGB_BYTES = compileRegExp("rgb\\(" + BYTE_PATTERN + "\\,\\s*" + BYTE_PATTERN
      + "\\,\\s*" + BYTE_PATTERN + "\\)");

   private static final RegExp REGEXP_RGB_PERCENTS = compileRegExp("rgb\\(" + PERCENTAGE_PATTERN + "\\,\\s*"
      + PERCENTAGE_PATTERN + "\\,\\s*" + PERCENTAGE_PATTERN + "\\)");
   ...
   private static final RegExp REGEXP_RECT = compileRegExp("rect\\(\\s*(" + LENGTH_PATTERN + "|auto)\\s*\\,\\s*("
      + LENGTH_PATTERN + "|auto)\\s*\\,\\s*(" + LENGTH_PATTERN + "|auto)\\s*\\,\\s*(" + LENGTH_PATTERN
      + "|auto)\\s*\\)");
   ...
   {
      return (((maybeSpecialValue.charAt(0) == '"') && (maybeSpecialValue.charAt(maybeSpecialValue.length() - 1) == '"')) || ((maybeSpecialValue
         .charAt(0) == '\'') && (maybeSpecialValue.charAt(maybeSpecialValue.length() - 1) == '\'')));
   }
   ...
   {
      return maybeSpecialValue.equals("aqua") || maybeSpecialValue.equals("black") || maybeSpecialValue.equals("blue")
         || maybeSpecialValue.equals("fuchsia") || maybeSpecialValue.equals("gray")
         || maybeSpecialValue.equals("green") || maybeSpecialValue.equals("lime") || maybeSpecialValue.equals("maroon")
         || maybeSpecialValue.equals("navy") || maybeSpecialValue.equals("olive") || maybeSpecialValue.equals("orange")
         || maybeSpecialValue.equals("purple") || maybeSpecialValue.equals("red") || maybeSpecialValue.equals("silver")
         || maybeSpecialValue.equals("teal") || maybeSpecialValue.equals("white") || maybeSpecialValue.equals("yellow")
         || REGEXP_3_HEX.test(maybeSpecialValue) || REGEXP_6_HEX.test(maybeSpecialValue)
         || REGEXP_RGB_BYTES.test(maybeSpecialValue) || REGEXP_RGB_PERCENTS.test(maybeSpecialValue);
   }
      ...
      String incomplete, JsArrayString valuesAfter)
   {
      ...
      if (valuesForSlotInQuestion != null)
      {
         ...
         for (int keyCt = 0; keyCt < keySet.length(); keyCt++)
         {
            ...
            JavaScriptObject triggers = valuesForSlotInQuestion.<Jso> cast().getJsObjectField(currentValue).cast();
               ...
               if (currentValue.charAt(0) == '<')
               {
                  ...
                  if (valueProposals != null && valueProposals.length() != 0)
                  {
                     ...
                     for (int i = 0; i < valueProposals.length(); i++)
                     {
                        ...
                        {
                           proposals.add(new CssCompletionProposal(valueProposals.get(i), CompletionType.VALUE));
                        }
                     }
                  }
               }
               ...
               for (int keyTriggerCt = 0; keyTriggerCt < keyTriggerSet.length(); keyTriggerCt++)
               {
                  ...
                  if (triggerValue.charAt(0) == '<')
                  {
                     JsArrayString specialValueProposalsAfterCheck =
                        checkIfSpecialValueAndGetSpecialValueProposals(valuesBefore.get(triggerSlot), triggerValue);
                     if (specialValueProposalsAfterCheck.length() != 0)
                     {
                        ...
                        for (int i = 0; i < specialValueProposalsAfterCheck.length(); i++)
                        {
                           if (specialValueProposalsAfterCheck.get(i).startsWith(incomplete)
                              && (isRepeatingProperty || !inExistingValues(triggerValue, valuesAndSpecialValuesBefore,
                                 valuesAndSpecialValuesAfter)))
                           {
                              proposals.add(new CssCompletionProposal(specialValueProposalsAfterCheck.get(i),
   ...
   private native JavaScriptObject setupSpecialValueProposals() /*-{
		return {
			'<angle>' : [ '<number>deg', '<number>grad', '<number>rad' ],
			'<family-name>' : [ '<family-name>' ],
			'<number>' : [ '<number>' ],
			'<integer>' : [ '<integer>' ],
			'<color>' : [ 'black', 'blue', 'fuchsia', 'gray', 'green', 'lime',
			...
			'<length>' : [ '<number>em', '<number>ex', '<number>in',
					'<number>cm', '<number>mm', '<number>pt', '<number>pc',
					'<number>px' ],
			'<percentage>' : [ '<number>%' ],
