 ...
 */
public abstract class PlainFileSystemTest extends TestCase
{
   ...
   protected PlainFileSystemContext fsContext;
   ...
   protected java.io.File testRoot;
   ...
   protected void setUp() throws Exception
   {
      ...
      fsContext = new PlainFileSystemContext(testRoot);

      DependencySupplierImpl dependencies = new DependencySupplierImpl();
      virtualFileSystemRegistry.registerProvider("fs", new PlainFileSystemProvider("fs", fsContext));
   ...
   {
      virtualFileSystemRegistry.unregisterProvider("fs");
      if (!FileUtils.deleteRecursive(testRoot))
      {
   ...
   // XXX : Copied from PlainFileSystemContext and adopted for tests.
   protected String pathToId(String path)
   {
      if ("/".equals(path))
      {
         return PlainFileSystemContext.ROOT_ID;
      }
      try
      {
         return '/' == java.io.File.separatorChar
            ? Base64.encodeBase64URLSafeString(path.getBytes("UTF-8"))
            : Base64.encodeBase64URLSafeString(path.replace(java.io.File.separatorChar, '/').getBytes("UTF-8"));
      }
   ...
   protected byte[] readFile(java.io.File f) throws IOException
   {
