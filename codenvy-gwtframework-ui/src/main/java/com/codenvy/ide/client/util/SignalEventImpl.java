...
import com.codenvy.ide.client.util.SignalKeyLogic.OperatingSystem;
import com.codenvy.ide.client.util.SignalKeyLogic.UserAgentType;
...
public class SignalEventImpl implements SignalEvent
{
   ...
   private static final UserAgentType currentUserAgent = (UserAgent.isWebkit() ? UserAgentType.WEBKIT : (UserAgent.isFirefox() ? UserAgentType.GECKO : UserAgentType.IE));

   private static final OperatingSystem currentOs = (UserAgent.isWin() ? OperatingSystem.WINDOWS : (UserAgent.isMac() ? OperatingSystem.MAC : OperatingSystem.LINUX));
