...
public class CollectionUtils {
    ...
    private static final class StringMapAdapter<V> implements StringMap<V> {
        ...
        public void putAll(Map<String, V> sourceMap) {
            Preconditions.checkArgument(!sourceMap.containsKey(null), "Source map must not contain a null key");
        ...
        public void filter(EntryFilter<? super V> filter) {
            for (Iterator<Map.Entry<String, V>> iterator = backend.entrySet().iterator(); iterator.hasNext(); ) {
    ...
    private static final class NumberMapAdapter<V> implements NumberMap<V> {
        ...
        public void filter(EntryFilter<V> filter) {
            for (Iterator<Map.Entry<Double, V>> iterator = backend.entrySet().iterator(); iterator.hasNext(); ) {
    ...
    private static final class IntMapAdapter<V> implements IntMap<V> {
        ...
        @Override
        public void filter(EntryFilter<V> filter) {
            for (Iterator<Map.Entry<Integer, V>> iterator = backend.entrySet().iterator(); iterator.hasNext(); ) {
    ...
    private static class IdentityHashMapAdapter<K, V> implements IdentityMap<K, V> {
        ...
        public void put(K key, V value) {
            ...
            // IdentitySetAdapter.
            if (key instanceof String || key instanceof Integer || key instanceof Double || key instanceof Long || key instanceof Boolean) {
                throw new UnsupportedOperationException("Should NOT use boxed primitives as key with identity map");
            }
            backend.put(key, value);
        }
