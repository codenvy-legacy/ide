...
import com.codenvy.ide.client.util.CopyOnWriteSet;
import com.codenvy.ide.client.util.IdentityMap;
import com.codenvy.ide.client.util.IdentityMap.ProcV;
...
public class BrowserBackedScheduler implements Scheduler {
    ...
    private TaskInfo createDelayedTask(Priority priority, Schedulable job, double startTime, double interval) {
     ...
     *
     * @param interval
     *         if -1, then not repeating
     */
    private void scheduleDelayedJob(Priority priority, Schedulable job, int minimumTime, int interval) {
        if (controller.isSuppressed(priority, job) && priority != Priority.INTERNAL_SUPPRESS) {
     ...
     * @param priority
     * @param maxMillis
     *         the max number of millisec we are allowed to execute one task before
     *         reporting it for a task that's too slow.
     * @param endTime
     *         if we exceeded this time, then we should stop and return false.
     ...
     *
     * @param when
     *         System time in millis when a slice should run
     */
    ...
    public String toString() {
        return "Scheduler[ids:" + tasks() + ", jobs:" + jobs.toString() + ", delayed: " + delayedJobs.toString() + "]";
    }
