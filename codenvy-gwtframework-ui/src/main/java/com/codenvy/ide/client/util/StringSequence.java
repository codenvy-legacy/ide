...
public final class StringSequence implements Sequence<String> {
    ...
    public static StringSequence create(String serializedSequence) {
        Preconditions.checkArgument(serializedSequence.startsWith(DELIMITER) && serializedSequence.endsWith(DELIMITER));
     ...
     *
     * @param term
     *         (coded) term to search for
     * @param forwardFirst
     *         if true, the search is performed with a forward search
    ...
    private int find(String term, boolean forwardFirst) {
        ...
        throw new IllegalArgumentException(
                "Item not found: " + CODEC.decode(term.substring(DELIMITER.length(), term.length() - DELIMITER.length())));
    }
    ...
    public String getLast() {
        ...
        return recentIndex == 0 ? null // \u2620
                                : CODEC
                       .decode(data.substring(data.lastIndexOf(DELIMITER, recentIndex - 1) + DELIMITER.length(), recentIndex));
    }
     ...
     *
     * @param ref
     *         reference value (or {@code null} for append)
     * @param x
     *         value to insert
     * @throws IllegalArgumentException
     *         if {@code x} is null, or {@code ref} is
    ...
    public void insertBefore(String ref, String x) {
        ...
        if (ref == null) {
            ...
            data = concat(data.substring(0, refIndex), DELIMITER, CODEC.encode(x), data.substring(refIndex));
        }
    }
     ...
     *
     * @param ref
     *         reference value (or {@code null} for prepend)
     * @param x
     *         value to insert
     * @throws IllegalArgumentException
     *         if {@code x} is null, or {@code ref} is
    ...
    public void insertAfter(String ref, String x) {
        ...
        if (ref == null) {
            ...
            data = concat(data.substring(0, refIndex), CODEC.encode(x), DELIMITER, data.substring(refIndex));
        }
    }
     ...
     * @param x
     * @throws IllegalArgumentException
     *         if {@code x} is null or not in this
     *         sequence.
     */
