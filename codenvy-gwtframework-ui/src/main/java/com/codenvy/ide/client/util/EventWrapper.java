...
public class EventWrapper {
    ...
    public static KeyCombo getKeyCombo(char keyCode, int modifiers) {
        int gwtCode =
                keyCode + (((modifiers & KeyCodes.KEY_CTRL) != 0) ? CTRL : 0) + (((modifiers & KeyCodes.KEY_SHIFT) != 0) ? SHIFT : 0) +
    ...
    public static String modifiers(Event event) {
        ...
        return (event.getAltKey() ? " alt" : "") + (event.getShiftKey() ? " shift" : "") + (event.getCtrlKey() ? " ctrl" : "") +
               (event.getMetaKey() ? " meta" : "") + ((event.getTypeInt() == Event.ONKEYDOWN) && event.getRepeat() ? " repeat" : "");
    }
