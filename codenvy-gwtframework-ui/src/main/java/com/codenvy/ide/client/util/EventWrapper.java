...
public class EventWrapper
{
    ...
    */
   private final static Map<Integer, KeyCombo> keyMap = new HashMap<Integer, KeyCombo>()
   {
      {
         // Tab
         put(9, KeyCombo.TAB);
   ...
   public static EventWrapper getCurrentEvent()
   {
      ...
      return current != null ? new EventWrapper(DOM.eventGetCurrentEvent()) : null;
   }
   ...
   {
      return getKeyCombo(signal.getKeyCode(), signal.getCtrlKey(), signal.getShiftKey(), signal.getAltKey(),
         signal.getMetaKey());
   }
   ...
   {
      return getKeyCombo(getKeyCode(evt), DOM.eventGetCtrlKey(evt), DOM.eventGetShiftKey(evt), DOM.eventGetAltKey(evt),
         DOM.eventGetMetaKey(evt));
   }

   private static KeyCombo getKeyCombo(int keyCode, boolean ctrl, boolean shift, boolean alt, boolean meta)
   {
      int gwtCode = keyCode;
      gwtCode += (ctrl ? CTRL : 0) + (shift ? SHIFT : 0) + (alt ? ALT : 0) +
         +(meta ? META : 0);
   ...
   {
      int gwtCode = keyCode + (((modifiers & KeyCodes.KEY_CTRL) != 0) ? CTRL : 0) + (((modifiers & KeyCodes.KEY_SHIFT) != 0) ? SHIFT : 0) + (((modifiers & KeyCodes.KEY_ALT) != 0) ? ALT : 0);
      return keyMap.containsKey(gwtCode) ? keyMap.get(gwtCode) : KeyCombo.OTHER;
   }
   ...
   public static String modifiers(Event event)
   {
      ...
      return (event.getAltKey() ? " alt" : "") + (event.getShiftKey() ? " shift" : "") + (event.getCtrlKey() ? " ctrl" : "") + (event.getMetaKey() ? " meta" : "") + ((event.getTypeInt() == Event.ONKEYDOWN) && event.getRepeat() ? " repeat" : "");
   }
   ...
   private static String mouseButtons(Event event)
   {
      ...
      {
         return ((event.getButton() & Event.BUTTON_LEFT) != 0 ? " left" : "") + ((event.getButton() & Event.BUTTON_MIDDLE) != 0 ? " middle" : "") + ((event.getButton() & Event.BUTTON_RIGHT) != 0 ? " right" : "");
      }
   }
