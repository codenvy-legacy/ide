...
public class PhpParser extends CodeMirrorParserImpl {
     ...
     *
     * @param onlyScalarDataType
     *         if true, then will be analyze only scalar data (boolean, integer, float, string, null)
     * @param nodeStack
     *         non-safe
     * @return
     */
    ...
    private boolean isInteger(Node node) {
        return "php-atom".startsWith(node.getType()) && (node.getContent().toLowerCase().matches("^[-+]?[0-9]+$") // integer like
                                                         // "-1"
                                                         || node.getContent().toLowerCase()
                                                                .matches("^0x[0-9a-f]+$") // hexadecimal integer like "0xa34"
        );
    }
