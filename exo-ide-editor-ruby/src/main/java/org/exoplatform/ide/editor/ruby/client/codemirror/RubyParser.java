...
public class RubyParser extends CodeMirrorParserImpl
{
      ...
      boolean hasParentParser)
   {
      ...
      else if (isEndNode(nodeStack.lastElement()))
      {
         if (!enclosers.empty())
         {
            ...
            if (!enclosers.lastElement().equals(TokenType.BLOCK) && currentToken.getParentToken() != null)
            {
   ...
   private void addToken(TokenBeenImpl newToken, int lineNumber, TokenBeenImpl currentToken)
   {
      ...
      switch (newToken.getType())
      {
         ...
         case CLASS_VARIABLE :
            ...
            if (TokenType.METHOD.equals(currentToken.getType()) && currentToken.getParentToken() != null
   ...
   private boolean isFirstVariableOccurance(String variableName, TokenType variableType, TokenBeenImpl currentToken)
   {
      switch (variableType)
      {
         ...
         case GLOBAL_VARIABLE :
            ...
            if (TokenType.METHOD.equals(currentToken.getType()) || globalVariables.contains(variableName))
            {
               return false;
            }
            break;
         ...
         case CLASS_VARIABLE :
            ...
            // find variable in the class's local variable list
            else if (TokenType.METHOD.equals(currentToken.getType()) || TokenType.CLASS.equals(currentToken.getType()))
            {
   ...
   {
      TokenBeenImpl newToken =
         new TokenBeenImpl(nodeStack.lastElement().getContent(), tokenType, lineNumber, MimeType.APPLICATION_RUBY);
      currentToken.addSubToken(newToken);
   ...
   {
      if (nodeStack.size() > 1 && isClassNode(nodeStack.get(nodeStack.size() - 2))
   ...
   {
      if (nodeStack.size() > 1 && isModuleNode(nodeStack.get(nodeStack.size() - 2))
         && (isMethodNode(nodeStack.lastElement().getType()) || isConstant(nodeStack.lastElement().getType())))
      {
         return true;
      }
   ...
   {
      if (nodeStack.size() > 1 && isDefNode(nodeStack.get(nodeStack.size() - 2))
   ...
   private boolean isBlockNode(Node node)
   {
      return isKeyword(node.getType())
         && ("case".equals(node.getContent()) || "if".equals(node.getContent()) || "unless".equals(node.getContent())
   ...
   /**
    * Recognize ruby keywords 'begin', 'class', 'ensure', 'nil', 'self', 'when', 'end', 'def', 'false', 'not', 'super', 'while',
    * 'alias', 'defined', 'for', 'or', 'then', 'yield', 'and', 'do', 'if', 'redo', 'true', 'begin', 'else', 'in', 'rescue',
    * 'undef', 'break', 'elsif', 'module', 'retry', 'unless', 'case', 'end', 'next', 'return', 'until'
    * 
    * @param nodeType
    * @return
    */
   ...
   private TokenBeenImpl isVariableWithAssigmentValue(Stack<Node> nodeStack, TokenBeenImpl currentToken)
   {
      if (nodeStack.size() > 2)
      {
         ...
         else if (isHexNumber(lastNode.getType()) || isBinaryNumber(lastNode.getType()))
         {
         ...
         if (cloneNodeStack.size() > 1)
         {
            ...
            if (isEqualSign(cloneNodeStack.get(cloneNodeStack.size() - 1))
               ...
               && isFirstVariableOccurance(cloneNodeStack.get(cloneNodeStack.size() - 2).getContent(), variableType,
                  currentToken))
            {

               newToken =
                  new TokenBeenImpl(cloneNodeStack.get(cloneNodeStack.size() - 2).getContent(), variableType, 0,
   ...
   /**
    * b = ClassName.new b = ClassName.new() b = ClassName.new v, t
    * 
    * @param nodeStack
    * @return ClassName
    */
   ...
   private TokenBeenImpl isVariableWithoutAssignment(Stack<Node> nodeStack, TokenBeenImpl currentToken)
   {
      if (nodeStack.size() > 1)
      {
         ...
         if (nodeStack.get(nodeStack.size() - 1).isLineBreak()
            ...
            && (TokenType.GLOBAL_VARIABLE.equals(variableType) || TokenType.INSTANCE_VARIABLE.equals(variableType))
            && isFirstVariableOccurance(nodeStack.get(nodeStack.size() - 2).getContent(), variableType, currentToken))
         {
            return new TokenBeenImpl(nodeStack.get(nodeStack.size() - 2).getContent(), variableType, 0,
               MimeType.APPLICATION_RUBY);
         }
      }
