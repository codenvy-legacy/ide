...
import org.exoplatform.ide.vfs.server.impl.memory.MemoryFileSystem;
...
final class FileEditSessionImpl implements FileEditSession {
  ...
  final VirtualFileSystemInfo vfsInfo;
...
//  }
  ...
  public void save(String currentPath) throws IOException {
    ...
    if (currentPath == null) {
      System.out.println(String.format("We do do not know the path for edit session [%s]!", this)); // TODO : error
      return;
    }
    ...
    System.out.println(String.format("Saved file [%s]", this)); // TODO : debug
  }
  ...
  private void saveChanges(String path, String text) throws IOException {
     ...
     */
     System.out.println(String.format("Saving file [%s]", path)); // TODO :  debug

     // String id, MediaType mediaType, InputStream newcontent, String lockToken
     Item file;
     try {
       file = vfs.getItemByPath(path, null, PropertyFilter.NONE_FILTER);
     } catch (VirtualFileSystemException e) {
       throw new IOException(e);// TODO
     }
     try {
       vfs.updateContent(file.getId(), MediaType.valueOf(file.getMimeType()), new ByteArrayInputStream(text.getBytes()), null);
     } catch (VirtualFileSystemException e) {
       throw new IOException(e);// TODO
     }
  }
